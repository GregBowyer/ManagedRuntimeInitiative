...
#define VM_ATTACH_CLOSE(a)proxy_invoke_remote_VMAttachClose(a)
#define VM_ATTACH_SHUTDOWN(a, b) proxy_invoke_remote_VMAttachShutdown(a, b);
#else // !AZ_PROXIED
#define VM_ATTACH_CLOSE(a)       ::close(a)
#define VM_ATTACH_SHUTDOWN(a, b) ::shutdown(a, b);
#endif // !AZ_PROXIED

// Azul - we mirror the linux implementation and use UNIX domain sockets on
// all proxies
//
...
LinuxAttachOperation* LinuxAttachListener::read_request(int s) {
  ...
  do {
    int n;
#ifdef AZ_PROXIED
RESTARTABLE(proxy_invoke_remote_VMAttachRead(s,buf+off,left),n);
#else // !AZ_PROXIED
...
#ifdef AZ_PROXIED
  int n = proxy_invoke_remote_VMAttachWrite(s, buf, len);
  if (n >= 0) {
    return 0;
  } else {
    return -1;
