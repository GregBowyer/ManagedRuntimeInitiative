...
parse_err(SYNERR,"Instructions specify emit, not encode\n");
    }
else if(!strcmp(ident,"emit"))instr->_emit=emit_parse();
...
else if(!strcmp(ident,"format")){
parse_err(SYNERR,"Format is Dead!  Please use only emit()\n");
    }
...
//------------------------------frame_pointer_parse----------------------------
void ADLParser::frame_pointer_parse(FrameForm*frame){
  char *frame_pointer = parse_one_arg("frame pointer entry");
  // Assign value into frame form
frame->_frame_pointer=frame_pointer;
}

//------------------------------cisc_spilling_operand_parse---------------------
void ADLParser::cisc_spilling_operand_name_parse(FrameForm*frame){
...
//------------------------------return_addr_parse------------------------------
void ADLParser::return_addr_parse(FrameForm*frame){
...
frame->_return_addr_loc=in_register;
...
const char*ADLParser::preserve_stack_parse(){
  if(_curchar == '(') {
const char*token=get_paren_expr("preserve_stack_slots");
if(_curchar!=';')//check for semi-colon
...
encoding=get_expr("encoding",")");
  ...
  // Debug Stuff
  if (_AD._adl_debug > 1) {
fprintf(stderr,"Register Definition: %s ( %s )\n",rname,
            (callconv ? callconv : ""));
  }

  // Record new register definition.
_AD._register->addRegDef(rname,callconv,idealtype,encoding);
  return;
}
...
//------------------------------emit_parse------------------------------------
EmitRule* ADLParser::emit_parse(void) {
  skipws();                       // Skip whitespace
if(_curchar!='%'||*(_ptr+1)!='{'){
...
next_char();//Move past the '{'
  ...
  EmitRule *emit = new EmitRule(_ptr);
  while( _ptr[0] != '%' || _ptr[1] != '}' ) next_char_or_line();
  char *s = _ptr-1;
  while( *s <= ' ' ) s--;       // Skip whitespace backwards
  s[1] = 0;                     // null terminate string
_ptr+=2;//skip %}
_curchar=*_ptr;
  return emit;
}
