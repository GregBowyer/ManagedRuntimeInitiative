...
void frame_pointer_parse(FrameForm*frame);
void cisc_spilling_operand_name_parse(FrameForm*frame);
  void stack_alignment_parse(FrameForm *frame);
void return_addr_parse(FrameForm*frame);
  const char *preserve_stack_parse();
  ...
  EmitRule      *emit_parse(void);       // Parse emit block
