...
void ArchDesc::inspectInstructions() {
  ...
  for( ; (instr = (InstructForm*)_instructions.iter()) != NULL; ) {
    ...
    while (attr != NULL) {
if(strcmp(attr->_ident,"ins_alignment")==0&&
          attr->int_val(*this) != 0) {
...
void ArchDesc::machineDependentIncludes(ADLFILE &adlfile) {
  // Build #include lines
  fprintf(adlfile._fp, "\n");
fprintf(adlfile._fp,"#include \"ad_pd.hpp\"\n");
  fprintf(adlfile._fp, "\n");
}
...
||strcmp(idealName,"CompareAndSwapP")==0
||strcmp(idealName,"CompareAndSwapL")==0
||strcmp(idealName,"CompareAndSwapI")==0
         ) {
