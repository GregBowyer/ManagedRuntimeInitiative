...
if(_matrule->find_type("FastLock",index))return true;
if(_matrule->find_type("FastUnlock",index))return true;
...
// Return 'true' if this instruction matches an ideal 'Prefetch' node
bool InstructForm::is_ideal_pref()const{
  if( _matrule == NULL ) return false;

return _matrule->is_ideal_pref();
}
...
Form::CallType InstructForm::is_ideal_call() const {
  ...
  idx = 0;
if(_matrule->find_type("Unlock",idx))return Form::JAVA_LEAF;
  idx = 0;
  ...
  idx = 0;
if(_matrule->find_type("Allocate",idx))return Form::JAVA_NEW;
  idx = 0;
if(_matrule->find_type("AllocateArray",idx))return Form::JAVA_NEW;
  idx = 0;
...
if(strcmp(_matrule->_opType,"MemBarCPUOrder")==0)return true;
...
!strcmp(_matrule->_rChild->_opType,"KID2Klass")||
        !strcmp(_matrule->_rChild->_opType,"GetKID") ||
        !strcmp(_matrule->_rChild->_opType,"FastAlloc") ||
        0)
      ) 
    return true;
...
(strcmp(_matrule->_rChild->_opType,"StrComp")==0||
        strcmp(_matrule->_rChild->_opType,"StrEquals" )==0 ||
        strcmp(_matrule->_rChild->_opType,"StrIndexOf")==0 )) {
        // String.(compareTo/equals/indexOf) take 1 control and 2 memory edges.
return 3;
  }
...
else if(is_ideal_call()==Form::JAVA_NEW){
return"MachAllocateNode";
  }
...
Form::InterfaceType OpClassForm::interface_type(FormDict &globals) const {
  ...
  Form::InterfaceType  interface = Form::no_interface;
  bool  first = true;
...
if(strcmp(name,"RegD")==0)size=1;
if(strcmp(name,"RegL")==0)size=1;
if(strcmp(name,"RegP")==0)size=1;
...
fprintf(fp,"      C2OUT->print(\"%cs\",reg_str);\n",'%');
    fprintf(fp,"    }\n");
...
fprintf(fp,"C2OUT->print(\"No format defined for %s\n\");\n",_ident);
    fflush(fp);
...
void OperandForm::format_constant(FILE *fp, uint const_index, uint const_type) {
  switch(const_type) {
case Form::idealI:fprintf(fp,"C2OUT->print(\"%%d\", _c%d);\n",const_index);break;
case Form::idealP:fprintf(fp,"_c%d->dump_on(C2OUT);\n",const_index);break;
case Form::idealL:fprintf(fp,"C2OUT->print(\"%%lld\", _c%d);\n",const_index);break;
case Form::idealF:fprintf(fp,"C2OUT->print(\"%%f\", _c%d);\n",const_index);break;
case Form::idealD:fprintf(fp,"C2OUT->print(\"%%f\", _c%d);\n",const_index);break;
  default:
    assert( false, "ShouldNotReachHere()");
  }
}
...
int MatchRule::is_ideal_copy() const {
  if( _rChild ) {
    ...
    if( strcmp(opType,"CastII")==0 )
      return 1;
    // The register allocator can coalesce this
    // into a single LRG.
...
bool MatchRule::is_ideal_if() const {
  ...
  return
    !strcmp(_opType,"If"            ) ||
!strcmp(_opType,"FastLock")||
!strcmp(_opType,"FastUnlock")||
    !strcmp(_opType,"CountedLoopEnd");
}

bool MatchRule::is_ideal_pref()const{
  if( !_opType ) return false;
  return !strcmp(_opType,"Prefetch");
}
