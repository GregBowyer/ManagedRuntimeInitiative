...
public:
  ...
  EmitRule      *_emit;            // Emit for assembly generation
...
virtual bool is_ideal_call_leaf()const;//node matches ideal 'CallLeaf', CallClearArray
  virtual bool        is_ideal_if()   const;    // node matches ideal 'If'
  virtual bool        is_ideal_pref() const;    // node matches ideal 'Prefetch'
...
int unique_opnds_idx(int idx){
if(_uniq_idx!=NULL&&idx>0){
                          assert(idx < (int)_uniq_idx_length, "out of bounds");
...
public:
  ...
  bool       is_ideal_pref() const;    // node matches ideal 'Prefetch'
