...
fprintf(fp,"const Register Matcher::_regEncode[REG_COUNT] = {\n");
...
fprintf(fp_cpp,"    C2OUT->print(\"#   functional_unit_latency: start == %%d, this->exclusively == 0x%%03x, pred->exclusively == 0x%%03x\\n\", start, resourcesUsedExclusively(), pred->resourcesUsedExclusively());\n");
  fprintf(fp_cpp, "  }\n");
  fprintf(fp_cpp, "#endif\n\n");
#endif
  ...
  fprintf(fp_cpp, "\n#ifndef PRODUCT\n");
  fprintf(fp_cpp, "  if (TraceOptoOutput) {\n");
fprintf(fp_cpp,"    C2OUT->print(\"#   functional_unit_latency: mask == 0x%%x\\n\", mask);\n");
  fprintf(fp_cpp, "  }\n");
  fprintf(fp_cpp, "#endif\n\n");
#endif
  ...
  fprintf(fp_cpp, "#ifndef PRODUCT\n");
  fprintf(fp_cpp, "  if (TraceOptoOutput) {\n");
fprintf(fp_cpp,"    C2OUT->print(\"#   operand_latency(%%d), _read_stage_count = %%d\\n\", opnd, _read_stage_count);\n");
  fprintf(fp_cpp, "  }\n");
  fprintf(fp_cpp, "#endif\n\n");
#endif
  ...
  fprintf(fp_cpp, "\n#ifndef PRODUCT\n");
  fprintf(fp_cpp, "  if (TraceOptoOutput) {\n");
fprintf(fp_cpp,"    C2OUT->print(\"#   operand_latency: writeStage=%%s readStage=%%s, opnd=%%d\\n\", stageName(writeStage), stageName(readStage), opnd);\n");
  fprintf(fp_cpp, "  }\n");
  fprintf(fp_cpp, "#endif\n\n");
#endif
  fprintf(fp_cpp, "\n");
  ...
  fprintf(fp_cpp, "\n#ifndef PRODUCT\n");
  fprintf(fp_cpp, "  if (TraceOptoOutput) {\n");
fprintf(fp_cpp,"    C2OUT->print(\"# operand_latency: delta=%%d\\n\", delta);\n");
  fprintf(fp_cpp, "  }\n");
  fprintf(fp_cpp, "#endif\n\n");
#endif
...
fprintf(fp_cpp,"static const Pipeline pipeline_class_Zero_Instructions   (0, 0, true, 0, 0, false, false, false, NULL, NULL, NULL, Pipeline_Use(0, 0, 0, NULL));\n\n");
fprintf(fp_cpp,"static const Pipeline pipeline_class_Unknown_Instructions(0, 0, true, 0, 0, false,  true,  true, NULL, NULL, NULL, Pipeline_Use(0, 0, 0, NULL));\n\n");
...
fprintf(fp_cpp,", %d, %s, %d, %d, %s, %s, %s,\n",
  ...
  if (_pipeline) {
    ...
    fprintf(fp_cpp, "#ifndef PRODUCT\n");
    fprintf(fp_cpp, " if (TraceOptoOutput) {\n");
fprintf(fp_cpp,"    C2OUT->print(\"# %%4d->latency(%%d)\\n\", _idx, i);\n");
    fprintf(fp_cpp, " }\n");
    fprintf(fp_cpp, "#endif\n");
#endif
...
fprintf(fp_cpp,"    C2OUT->print(\"%%s%%d instr%%s\", needs_comma ? \", \" : \"\", instr_count(), instr_count() != 1 ? \"s\" : \"\");\n");
  fprintf(fp_cpp, "    needs_comma = true;\n");
  fprintf(fp_cpp, "  };\n");
  fprintf(fp_cpp, "  uint r = resources_used();\n");
  fprintf(fp_cpp, "  if (r) {\n");
fprintf(fp_cpp,"    C2OUT->print(\"%%sresource%%s:\", needs_comma ? \", \" : \"\", (r & (r-1)) != 0 ? \"s\" : \"\");\n");
  fprintf(fp_cpp, "    for (uint i = 0; i < %d; i++)\n", _pipeline->_rescount);
  fprintf(fp_cpp, "      if ((r & (1 << i)) != 0)\n");
fprintf(fp_cpp,"        C2OUT->print(\" %%s\", resource_names[i]);\n");
  fprintf(fp_cpp, "    needs_comma = true;\n");
  fprintf(fp_cpp, "  };\n");
fprintf(fp_cpp,"  C2OUT->print(\"\\n\");\n");
  fprintf(fp_cpp, "}\n");
  fprintf(fp_cpp, "#endif\n");
}
...
fprintf(fp,"        ra_->set     (root->_idx, ra_->get_reg(inst%d));\n",inst_num);
...
fprintf(fp,"  idx0--; // Adjust base because memory edge hasn't been inserted yet\n");
    ...
    for(expand->reset_instructions();
      ...
      if ( expand_instruction->_matrule == NULL ) 
        // no Match rule?  Then no MachNodeGenerator
        fprintf(fp,"\n  %sNode *n%d = new (C) %sNode();\n", new_id, cnt, new_id);
      else
        // yes Match rule?  Then use MachNodeGenerator, as it fills in other fields
        fprintf(fp,"\n  %sNode *n%d = (%sNode*)state->MachNodeGenerator(%s_rule,C);\n", new_id, cnt, new_id, new_id);
      // Add control edge for this node
...
if((int)j!=node->unique_opnds_idx(j)){
...
fprintf(fp,"void %sNode::emit(PhaseRegAlloc *ra_) const {\n",inst->_ident);
...
fprintf(fp,"}\n\n");
  ...
  // Print the index of the first input edge for each operand
  inst->index_temps(fp,_globalNames,"","");
  fprintf(fp,"  ");

  // Scan for replacement variables while printing.
  for( char *s = inst->_emit->_temp; *s; s++ ) {
    const char c = *s;
if(c!='$'){
      fputc(c,fp);
      continue;
    }
    // Found a replacement var
    const char *var = ++s;
    while( *s != '$' ) s++;
    *s = '\0';
    const Form  *local = inst->_localNames[var];
    if( !local ) { fprintf(stderr,"Unknown replacement var %s\n",var); exit(-1); }
    uint idx = inst->operand_position_format(var);
    assert( idx < inst->num_opnds(), "too many operands?");
    *s = '$';                   // restore string

OpClassForm*opc=local->is_opclass();
    if( opc && opc->interface_type(_globalNames) == Form::memory_interface ) {
const char*tok=NULL;
      if( s[2] == 'b' ) tok = "$base";
      if( s[2] == 'd' ) tok = "$disp";
      if( s[2] == 'i' ) tok = "$index";
      if( s[2] == 's' ) tok = "$scale";
      if( !tok || strncmp(s+1,tok,strlen(tok)) ) 
        { fprintf(stderr,"Memory operand %s followed by %10.10s not by one of $base,$disp,$index,$scale\n ",var,s+1); exit(-1); }
s+=strlen(tok);
      fprintf(fp,"_opnds[%d]->%s(",idx,tok+1);
      if( idx == 0 ) {
fprintf(fp,"/*stack slot*/ra_,this,0)");
      } else {
        if( strcmp(tok,"$scale") ) fprintf(fp,"ra_,this,idx%d",idx);
        fprintf(fp,")");
      }
    ...
    if( opc && opc->interface_type(_globalNames) == Form::constant_interface ) {
if(strcmp(opc->_ident,"immP")==0){
fprintf(fp,"_opnds[%d]->type()",idx);
      } else {
        const char *s = "";
        if( strncmp(opc->_ident, "immF",4) == 0 ) s="F";
        if( strncmp(opc->_ident, "immD",4) == 0 ) s="D";
fprintf(fp,"_opnds[%d]->constant%s()",idx,s);
      }
      continue;
    }

    if( opc && opc->interface_type(_globalNames) == Form::register_interface ) {
fprintf(fp,"_opnds[%d]->reg(ra_,this",idx);
if(idx)fprintf(fp,",idx%d",idx);
fprintf(fp,")");
      continue;
    } 

    if( opc && opc->interface_type(_globalNames) == Form::conditional_interface ) {
fprintf(fp,"_opnds[%d]->ccode()",idx);
      continue;
    } 

    if( opc && idx == (uint)inst->label_position() ) {
fprintf(fp,"(*_opnds[%d]->label())",idx);
      continue;
    }

    if( opc && idx == (uint)inst->method_position() ) {
fprintf(fp,"_entry_point");
      continue;
    }

    { fprintf(stderr,"Var %s is not a known operand type\n",var); exit(-1); }
...
fprintf(fp,"  virtual Register %s",name);
...
fprintf(fp,"  virtual int %s",name);
  ...
  if( *encoding == '$' ) {
    ...
    if ( (op->_matrule && op->_matrule->is_base_register(globals)) ) {
      ...
      position = idx_offset;
fprintf(fp,"    return ra_->get_encode(node->in(idx");
...
fprintf(fp,"      return ra_->reg2offset(ra_->get_reg(node->in(idx)));/* sReg */\n");
      fprintf(fp,"    }\n");
      fprintf(fp,"    // Access register number from myself\n");
fprintf(fp,"    return ra_->reg2offset(ra_->get_reg(node));/* sReg */\n");
...
fprintf(fp,"    assert(is_int32(");
      oper.access_constant(fp, globals, (uint)idx /* , const_type */);
fprintf(fp,"), \"expected int size constant\");\n");
    ...
    // Hex value
fprintf(fp,"return Register(%s);",encoding);
  } else {
...
void ArchDesc::defineClasses(FILE *fp) {
  ...
  for( ; (oper = (OperandForm*)_operands.iter()) != NULL; ) {
    ...
    if ( strcmp(oper->_ident,"label") == 0 ) {
      ...
      fprintf(fp,"MachOper  *%sOper::clone(Compile* C) const {\n", oper->_ident);
fprintf(fp,"  return  new (C) %sOper(_label);\n",oper->_ident);
      fprintf(fp,"}\n");
...
instr->num_opnds()!=instr->num_unique_opnds()))
  ...
  while( (instr = (InstructForm*)_instructions.iter()) != NULL ) {
    ...
    if( label_position != -1 ) {
      // Set the label
fprintf(fp,"void %sNode::label_set( Label* label ) {\n",instr->_ident);
fprintf(fp,"  ((labelOper*)(opnd_array(%d)))->_label = label;\n",label_position);
      fprintf(fp,"}\n");
    }
  }
  ...
  for( ; (instr = (InstructForm*)_instructions.iter()) != NULL; ) {
    ...
    if (instr->_emit) defineEmit(fp, instr);
  }
...
void ArchDesc::buildMachNode(FILE *fp_cpp, InstructForm *inst, const char *indent) {
  ...
  if ( (inst->num_post_match_opnds() != 0) ) {
    ...
    comp_list.reset();
while((comp=comp_list.post_match_iter())!=0){
...
fprintf(fp_cpp,"%s node->_msg = _leaf->as_Halt()->_msg;\n",indent);
  }
...
fprintf(fp_cpp,"uint Compile:: in_preserve_stack_slots() { return %s; }\n\n",_frame->_in_preserve_slots);
  // Top Of Stack Slot Preservation, for both Java and C
  fprintf(fp_cpp,"uint Compile::out_preserve_stack_slots() ");
fprintf(fp_cpp,"{ return 0; }\n\n");
  // Java Argument Position
fprintf(fp_cpp,"void Matcher::calling_convention(BasicType *sig_bt, VReg::VR *regs, uint length, bool is_outgoing) {\n");
  fprintf(fp_cpp,"%s\n", _frame->_calling_convention);
  fprintf(fp_cpp,"}\n\n");
  // Java Return Value Location
fprintf(fp_cpp,"VReg::VR Matcher::return_value(int ideal_reg, bool is_outgoing) {\n");
