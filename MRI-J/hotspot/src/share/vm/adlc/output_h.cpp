...
fprintf(fp,"    C2OUT->print(\"+%%lld\", _c%d);\n",i);
    ++i;
  }
  else if (!strcmp(ideal_type, "ConF")) {
fprintf(fp,"    C2OUT->print(\"%%f\", _c%d);\n",i);
    ++i;
  }
...
fprintf(fp,"  virtual int ccode() const { return _c0; }\n");
    }
    ...
    // Support setting a spill offset into a constant operand.  We only
    // support setting an 'int' offset, while spill offsets are added with an
    // AddP which requires a long constant.  Thus we don't support spilling in
    // frames larger than 4Gig.
    if( oper->has_conI(_globalNames) ||
oper->has_conL(_globalNames)){
      if( oper->num_consts(_globalNames) == 1 )
...
fprintf(fp,"  virtual int reg_slot(PhaseRegAlloc *ra_, const Node *node) const {\n");
fprintf(fp,"    return (int)OptoReg::reg2stack(ra_->get_reg(node));/* sReg */\n");
      fprintf(fp,"  }\n");
fprintf(fp,"  virtual int reg_slot(PhaseRegAlloc *ra_, const Node *node, int idx) const {\n");
fprintf(fp,"    return (int)OptoReg::reg2stack(ra_->get_reg(node->in(idx)));/* sReg */\n");
      fprintf(fp,"  }\n");
    }
    ...
    // (2)    C2OUT->print("%d", _c#);      // Constant != ConP
    //  OR    _c#->dump_on();               // Type ConP
    //  ...
    // (3)  }
    ...
    if( num_consts > 0 ) {
      ...
      // generate format string for C2OUT->print
...
fprintf(fp,"  virtual void label_set( Label* label );\n");
    }
...
if(strcmp(attr->_ident,"ins_cost")){
    ...
    // virtual functions for emit
if(instr->_emit){
fprintf(fp,"  virtual void emit(PhaseRegAlloc *ra_) const;\n");
    }
...
fprintf(fp,"  const Type            *bottom_type() const { const Type *t = in(oper_input_base()+%d)->bottom_type(); return (req() <= oper_input_base()+%d) ? t : t->meet(in(oper_input_base()+%d)->bottom_type()); } // CMoveN\n",
    ...
    if( instr->_matrule && instr->_matrule->_rChild ) {
      if( !strcmp(instr->_matrule->_rChild->_opType,"StoreP" ) ) 
        fprintf(fp, 
"  virtual int            base_derived_idx() const { \n"
"    return (req() > MemNode::ValueIn && \n"
"            in(MemNode::Address)->bottom_type()->isa_oopptr() != NULL &&\n"
"            in(MemNode::ValueIn)->bottom_type()->isa_oopptr() != NULL )\n"
"            ? MemNode::ValueIn+1 : 0;\n"
"  }\n");
      else if( !strcmp(instr->_matrule->_rChild->_opType,"CompareAndSwapP" ) ) 
fprintf(fp,"  virtual int            base_derived_idx() const { return LoadStoreNode::ExpectedIn+1; } \n");
      else if ( instr->captures_bottom_type() )
fprintf(fp,"  virtual int            base_derived_idx() const { return 0; } \n");
    }
