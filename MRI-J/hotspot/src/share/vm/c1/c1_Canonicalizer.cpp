  ...
  if (lt->is_constant() && rt->is_constant()) {
    ...
    if (l->as_CompareOp() != NULL) {
      ...
      if (lss_sux == eql_sux && eql_sux == gtr_sux) {
        ...
        If::Condition cond=If::eql;  // TODO: this should have an "illegal" value.
...
}else if(rt==ThreadLocals->_objectNull&&(l->as_NewInstance()||l->as_NewArray())){
...
if(shift->x()->type()!=ThreadLocals->_intType)return false;
    *index = shift->x();
...
if((*index)->type()!=ThreadLocals->_intType)return false;
      // Well-known type and value?
      IntConstant* val = con->type()->as_IntConstant();
      if (val == NULL) return false;
      switch (val->value()) {
...
if(convert->op()==Bytecodes::_i2l&&convert->value()->type()==ThreadLocals->_intType){
...
void Canonicalizer::do_IncrementCount(IncrementCount*x){}
