...
#include "c1_Canonicalizer.hpp"
#include "c1_Defs.hpp"
#include "c1_GraphBuilder.hpp"
...
assert(is_power_of_2((unsigned)_loop_map.at(block->block_id())),"exactly one bit must be set");
  }
}
...
case objectTag:return con->type()==ThreadLocals->_objectNull;
...
if(h->handler_bci()!=InvocationEntryBci){
...
ValueType*t=ThreadLocals->_illegalType;
...
load_local(ThreadLocals->_intType,index);
  ipush(append(new Constant(new IntConstant(delta))));
arithmetic_op(ThreadLocals->_intType,Bytecodes::_iadd);
store_local(ThreadLocals->_intType,index);
}


void GraphBuilder::_goto(int from_bci, int to_bci) {
BlockBegin*target=block_at(to_bci);
  if( UseC2 && ProfileBackedges && from_bci >= to_bci ) {
    int beoff = CodeProfile::backedge_count_offset_in_bytes() + in_bytes(InvocationCounter::counter_offset());
    // Use IncrementCountAndTrapIfHot so a LARGE backedge kicks us back to
    // the interpreter which will bounce us back into a C2 OSR
    append(new IncrementCount(beoff, InvocationEntryBci, 1<<C1_LogBailOutToC2, state()));
  }
  append(new Goto(target, to_bci <= from_bci));
}


void GraphBuilder::if_node(Value x, If::Condition cond, Value y, ValueStack* state_before) {
  const int curbci = bci();
  const int tbci = stream()->get_dest();
  const int fbci = stream()->next_bci();
  const bool t_is_bb = tbci <= curbci;
  const bool f_is_bb = fbci <= curbci;
BlockBegin*tsux=block_at(tbci);
BlockBegin*fsux=block_at(fbci);
  const bool is_bb =  t_is_bb || f_is_bb;
...
if_node->set_profiled_bci(curbci);
    if_node->set_should_profile(true);
  }
  if (UseC2 && ProfileBlocks && if_node) {
_block->set_end(if_node);
if_node->set_state(state());
    int cpdoff = find_cpdoff();
    int beoff = CodeProfile::backedge_count_offset_in_bytes() + in_bytes(InvocationCounter::counter_offset());
    debug_only( assert( ((class CPData*)((address)(compilation()->codeprofile())+cpdoff))->is_Branch(), "Not a valid cpdata structure!") );
    BlockBegin *tPP = _block->insert_block_between(tsux);
    BlockBegin *fPP = _block->insert_block_between(fsux);
tPP->set(BlockBegin::was_visited_flag);
fPP->set(BlockBegin::was_visited_flag);
    tPP->set_depth_first_number(_block->depth_first_number()+1);
    fPP->set_depth_first_number(_block->depth_first_number()+1);
    Instruction* tI = tPP->set_next(new IncrementCount(cpdoff + CPData_Branch::   taken_offset(), cpdoff), curbci);
    Instruction* fI = fPP->set_next(new IncrementCount(cpdoff + CPData_Branch::nottaken_offset(), cpdoff), curbci);
    if( ProfileBackedges ) {
      if( t_is_bb ) tI = tI->set_next(new IncrementCount(beoff, -1), curbci);
      if( f_is_bb ) fI = fI->set_next(new IncrementCount(beoff, -1), curbci);
    }
    tI->set_next(tPP->end(),curbci);
    fI->set_next(fPP->end(),curbci);
  }
}


void GraphBuilder::if_zero(ValueType* type, If::Condition cond) {
Value y=append(new Constant(ThreadLocals->_intZero));
  ValueStack* state_before = state()->copy();
  Value x = ipop();
...
Value y=append(new Constant(ThreadLocals->_objectNull));
  ValueStack* state_before = state()->copy();
  Value x = apop();
  if_node(x, cond, y, state_before);
}
...
void GraphBuilder::jsr(int dest) {
  ...
  push(ThreadLocals->_addressType, append(new Constant(new AddressConstant(next_bci()))));
...
void GraphBuilder::call_register_finalizer() {
  ...
  if (exact_type == NULL &&
    ...
    ciInstanceKlass* uk = ik->unique_concrete_subklass(compilation()->masm());
    if( uk ) {                  // test class is leaf class
exact_type=uk;
    } else {
      declared_type = ik;
    }
...
if(!ik->has_finalizable_subclass(compilation()->masm())){
      needs_check = false;
    }
  }
...
load_local(ThreadLocals->_objectType,0);
append_split(new Intrinsic(ThreadLocals->_voidType,vmIntrinsics::_Object_init,
...
monitorexit(state()->lock_at(i),InvocationEntryBci);
    }
...
receiver=append(new Constant(new InstanceConstant(method()->holder()->java_mirror())));
    }
    ...
    if (will_link && !target->is_static()) {
      ...
      if (type == NULL) {
        ...
        if (type != NULL && type->is_loaded() &&
            type->is_instance_klass() && !type->as_instance_klass()->is_interface()) {
          receiver_klass = (ciInstanceKlass*) type;
if(receiver_klass->is_leaf_type(compilation()->masm())&&!receiver_klass->is_final()){
            // Insert a dependency on this type since
            // find_monomorphic_target may assume it's already done.
compilation()->masm()->assert_leaf_type(receiver_klass);
            type_is_exact = true;
          }
        }
      }
    ...
    // Force promotion of actual_recv if possible
    ciInstanceKlass *impl = actual_recv->unique_concrete_subklass(compilation()->masm());
    if ((intptr_t)impl > 0) {
      ciMethod* cim = impl->find_method(target->name(), target->signature()->as_symbol());
      if (cim && actual_recv != impl) {
actual_recv=impl;
      }
    }
...
ciInstanceKlass*singleton=target->holder()->unique_concrete_subklass(compilation()->masm());
if((intptr_t)singleton>0){
cha_monomorphic_target=target->find_monomorphic_target(calling_klass,singleton,singleton);
...
actual_recv=singleton;

          // insert a check it's really the expected class.
CheckCast*c=new CheckCast(klass,receiver,NULL,find_cpdoff()+CPData_Null::bitdata_offset());
  ...
  if (cha_monomorphic_target != NULL) {
    if (!(target->is_final_method())) {         
      ...
      ciInstanceKlass *impl = actual_recv->unique_concrete_subklass(compilation()->masm());
      if ((intptr_t)impl > 0) {
        ciMethod* cim = impl->find_method(cha_monomorphic_target->name(), cha_monomorphic_target->signature()->as_symbol());
        if (cim && actual_recv != impl) {
actual_recv=impl;
        }
      }
...
compilation()->masm()->assert_unique_concrete_method(actual_recv,cha_monomorphic_target);
    }
    code = Bytecodes::_invokespecial;
  }
...
int offset=find_cpdoff();
  if (UseC2 && ProfileCallsite) {
    debug_only( assert( ((class CPData*)((address)(compilation()->codeprofile())+offset))->is_Invoke(), "Not a valid cpdata structure!") );
    append(new IncrementCount(offset + in_bytes(CPData_Invoke::invoke_count_offset()),offset));
  }

  Invoke* result = new Invoke(code, offset, result_type, recv, args, vtable_index, target);
  // push result
  append_split(result);

  if (result_type != ThreadLocals->_voidType) {
  ...
  if (k->is_loaded() && k->is_instance_klass() && !UseSlowPath) {
    ...
    if (ik->is_final()) {
      ...
      if (DeoptC1 && UseCHA && !(ik->has_subklass() || ik->is_interface())) {
        ...
        CommonAsm *casm = compilation()->masm();
        if( ik->nof_implementors(casm) == 1 ) casm->assert_leaf_type(ik);
        else                                  casm->assert_no_implementation(ik);
        return true;
      }
    }
  }
...
void GraphBuilder::check_cast(int klass_index) {
  ...
  ciKlass* klass = stream()->get_klass(will_link);
  ValueStack* state_before = !klass->is_loaded() || PatchALot ? state()->copy() : NULL;
CheckCast*c=new CheckCast(klass,apop(),state_before,find_cpdoff()+CPData_Null::bitdata_offset());
...
void GraphBuilder::instance_of(int klass_index) {
  ...
  ciKlass* klass = stream()->get_klass(will_link);
  ValueStack* state_before = !klass->is_loaded() || PatchALot ? state()->copy() : NULL;
InstanceOf*i=new InstanceOf(klass,apop(),state_before,find_cpdoff()+CPData_Null::bitdata_offset());
...
assert(i2->bci()!=InvocationEntryBci,"should already be linked");
      return i2;
    }
...
assert(cur_bci==InvocationEntryBci||cur_bci==cur_scope_data->stream()->cur_bci(),"invalid bci");
...
assert(entry->bci()==InvocationEntryBci||entry==cur_scope_data->block_at(entry->bci()),"blocks must correspond");
...
C1OUT->cr();
    InstructionPrinter ip;
ip.print_instr(_block);C1OUT->cr();
ip.print_stack(_block->state());C1OUT->cr();
    ip.print_inline_level(_block);
    ip.print_head();
C1OUT->print_cr("locals size: %d stack size: %d",state()->locals_size(),state()->stack_size());
  }
#endif
  _skip_block = false;
...
case Bytecodes::_aconst_null:apush(append(new Constant(ThreadLocals->_objectNull)));break;
      case Bytecodes::_iconst_m1      : ipush(append(new Constant(new IntConstant   (-1)))); break;
case Bytecodes::_iconst_0:ipush(append(new Constant(ThreadLocals->_intZero)));break;
case Bytecodes::_iconst_1:ipush(append(new Constant(ThreadLocals->_intOne)));break;
...
case Bytecodes::_iload:load_local(ThreadLocals->_intType,s.get_index());break;
case Bytecodes::_lload:load_local(ThreadLocals->_longType,s.get_index());break;
case Bytecodes::_fload:load_local(ThreadLocals->_floatType,s.get_index());break;
case Bytecodes::_dload:load_local(ThreadLocals->_doubleType,s.get_index());break;
case Bytecodes::_aload:load_local(ThreadLocals->_instanceType,s.get_index());break;
case Bytecodes::_iload_0:load_local(ThreadLocals->_intType,0);break;
case Bytecodes::_iload_1:load_local(ThreadLocals->_intType,1);break;
case Bytecodes::_iload_2:load_local(ThreadLocals->_intType,2);break;
case Bytecodes::_iload_3:load_local(ThreadLocals->_intType,3);break;
case Bytecodes::_lload_0:load_local(ThreadLocals->_longType,0);break;
case Bytecodes::_lload_1:load_local(ThreadLocals->_longType,1);break;
case Bytecodes::_lload_2:load_local(ThreadLocals->_longType,2);break;
case Bytecodes::_lload_3:load_local(ThreadLocals->_longType,3);break;
case Bytecodes::_fload_0:load_local(ThreadLocals->_floatType,0);break;
case Bytecodes::_fload_1:load_local(ThreadLocals->_floatType,1);break;
case Bytecodes::_fload_2:load_local(ThreadLocals->_floatType,2);break;
case Bytecodes::_fload_3:load_local(ThreadLocals->_floatType,3);break;
case Bytecodes::_dload_0:load_local(ThreadLocals->_doubleType,0);break;
case Bytecodes::_dload_1:load_local(ThreadLocals->_doubleType,1);break;
case Bytecodes::_dload_2:load_local(ThreadLocals->_doubleType,2);break;
case Bytecodes::_dload_3:load_local(ThreadLocals->_doubleType,3);break;
case Bytecodes::_aload_0:load_local(ThreadLocals->_objectType,0);break;
case Bytecodes::_aload_1:load_local(ThreadLocals->_objectType,1);break;
case Bytecodes::_aload_2:load_local(ThreadLocals->_objectType,2);break;
case Bytecodes::_aload_3:load_local(ThreadLocals->_objectType,3);break;
...
case Bytecodes::_istore:store_local(ThreadLocals->_intType,s.get_index());break;
case Bytecodes::_lstore:store_local(ThreadLocals->_longType,s.get_index());break;
case Bytecodes::_fstore:store_local(ThreadLocals->_floatType,s.get_index());break;
case Bytecodes::_dstore:store_local(ThreadLocals->_doubleType,s.get_index());break;
case Bytecodes::_astore:store_local(ThreadLocals->_objectType,s.get_index());break;
case Bytecodes::_istore_0:store_local(ThreadLocals->_intType,0);break;
case Bytecodes::_istore_1:store_local(ThreadLocals->_intType,1);break;
case Bytecodes::_istore_2:store_local(ThreadLocals->_intType,2);break;
case Bytecodes::_istore_3:store_local(ThreadLocals->_intType,3);break;
case Bytecodes::_lstore_0:store_local(ThreadLocals->_longType,0);break;
case Bytecodes::_lstore_1:store_local(ThreadLocals->_longType,1);break;
case Bytecodes::_lstore_2:store_local(ThreadLocals->_longType,2);break;
case Bytecodes::_lstore_3:store_local(ThreadLocals->_longType,3);break;
case Bytecodes::_fstore_0:store_local(ThreadLocals->_floatType,0);break;
case Bytecodes::_fstore_1:store_local(ThreadLocals->_floatType,1);break;
case Bytecodes::_fstore_2:store_local(ThreadLocals->_floatType,2);break;
case Bytecodes::_fstore_3:store_local(ThreadLocals->_floatType,3);break;
case Bytecodes::_dstore_0:store_local(ThreadLocals->_doubleType,0);break;
case Bytecodes::_dstore_1:store_local(ThreadLocals->_doubleType,1);break;
case Bytecodes::_dstore_2:store_local(ThreadLocals->_doubleType,2);break;
case Bytecodes::_dstore_3:store_local(ThreadLocals->_doubleType,3);break;
case Bytecodes::_astore_0:store_local(ThreadLocals->_objectType,0);break;
case Bytecodes::_astore_1:store_local(ThreadLocals->_objectType,1);break;
case Bytecodes::_astore_2:store_local(ThreadLocals->_objectType,2);break;
case Bytecodes::_astore_3:store_local(ThreadLocals->_objectType,3);break;
...
case Bytecodes::_iadd:arithmetic_op(ThreadLocals->_intType,code);break;
case Bytecodes::_ladd:arithmetic_op(ThreadLocals->_longType,code);break;
case Bytecodes::_fadd:arithmetic_op(ThreadLocals->_floatType,code);break;
case Bytecodes::_dadd:arithmetic_op(ThreadLocals->_doubleType,code);break;
case Bytecodes::_isub:arithmetic_op(ThreadLocals->_intType,code);break;
case Bytecodes::_lsub:arithmetic_op(ThreadLocals->_longType,code);break;
case Bytecodes::_fsub:arithmetic_op(ThreadLocals->_floatType,code);break;
case Bytecodes::_dsub:arithmetic_op(ThreadLocals->_doubleType,code);break;
case Bytecodes::_imul:arithmetic_op(ThreadLocals->_intType,code);break;
case Bytecodes::_lmul:arithmetic_op(ThreadLocals->_longType,code);break;
case Bytecodes::_fmul:arithmetic_op(ThreadLocals->_floatType,code);break;
case Bytecodes::_dmul:arithmetic_op(ThreadLocals->_doubleType,code);break;
case Bytecodes::_idiv:arithmetic_op(ThreadLocals->_intType,code,lock_stack());break;
case Bytecodes::_ldiv:arithmetic_op(ThreadLocals->_longType,code,lock_stack());break;
case Bytecodes::_fdiv:arithmetic_op(ThreadLocals->_floatType,code);break;
case Bytecodes::_ddiv:arithmetic_op(ThreadLocals->_doubleType,code);break;
case Bytecodes::_irem:arithmetic_op(ThreadLocals->_intType,code,lock_stack());break;
case Bytecodes::_lrem:arithmetic_op(ThreadLocals->_longType,code,lock_stack());break;
case Bytecodes::_frem:arithmetic_op(ThreadLocals->_floatType,code);break;
case Bytecodes::_drem:arithmetic_op(ThreadLocals->_doubleType,code);break;
case Bytecodes::_ineg:negate_op(ThreadLocals->_intType);break;
case Bytecodes::_lneg:negate_op(ThreadLocals->_longType);break;
case Bytecodes::_fneg:negate_op(ThreadLocals->_floatType);break;
case Bytecodes::_dneg:negate_op(ThreadLocals->_doubleType);break;
case Bytecodes::_ishl:shift_op(ThreadLocals->_intType,code);break;
case Bytecodes::_lshl:shift_op(ThreadLocals->_longType,code);break;
case Bytecodes::_ishr:shift_op(ThreadLocals->_intType,code);break;
case Bytecodes::_lshr:shift_op(ThreadLocals->_longType,code);break;
case Bytecodes::_iushr:shift_op(ThreadLocals->_intType,code);break;
case Bytecodes::_lushr:shift_op(ThreadLocals->_longType,code);break;
case Bytecodes::_iand:logic_op(ThreadLocals->_intType,code);break;
case Bytecodes::_land:logic_op(ThreadLocals->_longType,code);break;
case Bytecodes::_ior:logic_op(ThreadLocals->_intType,code);break;
case Bytecodes::_lor:logic_op(ThreadLocals->_longType,code);break;
case Bytecodes::_ixor:logic_op(ThreadLocals->_intType,code);break;
case Bytecodes::_lxor:logic_op(ThreadLocals->_longType,code);break;
...
case Bytecodes::_lcmp:compare_op(ThreadLocals->_longType,code);break;
case Bytecodes::_fcmpl:compare_op(ThreadLocals->_floatType,code);break;
case Bytecodes::_fcmpg:compare_op(ThreadLocals->_floatType,code);break;
case Bytecodes::_dcmpl:compare_op(ThreadLocals->_doubleType,code);break;
case Bytecodes::_dcmpg:compare_op(ThreadLocals->_doubleType,code);break;
case Bytecodes::_ifeq:if_zero(ThreadLocals->_intType,If::eql);break;
case Bytecodes::_ifne:if_zero(ThreadLocals->_intType,If::neq);break;
case Bytecodes::_iflt:if_zero(ThreadLocals->_intType,If::lss);break;
case Bytecodes::_ifge:if_zero(ThreadLocals->_intType,If::geq);break;
case Bytecodes::_ifgt:if_zero(ThreadLocals->_intType,If::gtr);break;
case Bytecodes::_ifle:if_zero(ThreadLocals->_intType,If::leq);break;
case Bytecodes::_if_icmpeq:if_same(ThreadLocals->_intType,If::eql);break;
case Bytecodes::_if_icmpne:if_same(ThreadLocals->_intType,If::neq);break;
case Bytecodes::_if_icmplt:if_same(ThreadLocals->_intType,If::lss);break;
case Bytecodes::_if_icmpge:if_same(ThreadLocals->_intType,If::geq);break;
case Bytecodes::_if_icmpgt:if_same(ThreadLocals->_intType,If::gtr);break;
case Bytecodes::_if_icmple:if_same(ThreadLocals->_intType,If::leq);break;
case Bytecodes::_if_acmpeq:if_same(ThreadLocals->_objectType,If::eql);break;
case Bytecodes::_if_acmpne:if_same(ThreadLocals->_objectType,If::neq);break;
...
case Bytecodes::_ifnull:if_null(ThreadLocals->_objectType,If::eql);break;
case Bytecodes::_ifnonnull:if_null(ThreadLocals->_objectType,If::neq);break;
  ...
  for (int i = end->number_of_sux() - 1; i >= 0; i--) {
    ...
    // Profiling is done on CFG edges, so critical blocks get split to
    // hold increments, but these blocks do not correspond to any BCI.
    if( sux->is_set(BlockBegin::critical_edge_split_flag) )
      sux = sux->sux_at(0);
    // be careful, bailout if bytecodes are strange
    if (!sux->try_merge(state())) BAILOUT_("block join failed", NULL);
scope_data()->add_to_work_list(sux);
  }
...
state->store_local(idx,new Local(ThreadLocals->_objectType,idx));
    idx = 1;
  }
...
sync_handler=new BlockBegin(InvocationEntryBci);
    sync_handler->set(BlockBegin::exception_entry_flag);
    sync_handler->set(BlockBegin::is_on_work_list_flag);
    sync_handler->set(BlockBegin::default_exception_handler_flag);
    
    ciExceptionHandler* desc = new ciExceptionHandler(method()->holder(), 0, method()->code_size(), InvocationEntryBci, 0);
  ...
  case vmIntrinsics::_dlog10        : // fall through
    {
      ...
      _last  = start_block;
load_local(ThreadLocals->_doubleType,0);
...
fill_sync_handler(lock,sync_handler,true,0xdeadbeef);
  }
...
#ifndef PRODUCT
if(PrintCompilation&&Verbose)C1OUT->print_cr("Created %d Instructions",_instruction_count);
#endif
}
...
bool GraphBuilder::try_inline(ciMethod* callee, bool holder_known) {
  ...
  } else if (!callee->is_c1_compilable() || callee->number_of_breakpoints()) {
...
if(result_type!=ThreadLocals->_voidType)push(result_type,value);
...
void GraphBuilder::inline_sync_entry(Value lock, BlockBegin* sync_handler) {
  ...
  set_exception_state(state()->copy());
monitorenter(lock,InvocationEntryBci);
...
// instructions here.
void GraphBuilder::fill_sync_handler(Value lock,BlockBegin*sync_handler,bool default_handler,int old_cpdata_offset){
...
Value exception=append_with_bci(new ExceptionObject(),InvocationEntryBci);
...
int bci=InvocationEntryBci;
  if (lock) {
    ...
    if (lock->bci() == -99) {
lock=append_with_bci(lock,InvocationEntryBci);
    }

    // exit the monitor in the context of the synchronized method
monitorexit(lock,InvocationEntryBci);
    
    // exit the context of the synchronized method
    if (!default_handler) {
      pop_scope(old_cpdata_offset);
      _state = _state->copy();
...
bool GraphBuilder::try_inline_full(ciMethod* callee, bool holder_known) {
  ...
  // Insert invoke profiler before callee scope is in effect
  if (UseC2 && ProfileCallsite) {
    int offset = compilation()->get_cpdata() + sizeof(CodeProfile) + method()->bci2cpd_map()->bci_to_cpdoff(bci());
    debug_only( assert( ((class CPData*)((address)(compilation()->codeprofile())+offset))->is_Invoke(), "Not a valid cpdata structure!") );
    int invoff = in_bytes(CPData_Invoke::invoke_count_offset());
    append(new IncrementCount(offset+invoff, offset));
  }
  ...
  int caller_bci = bci();       // Must get before pushing scope
ciMethod*caller=method();
  // Push callee scope
  push_scope(callee, cont);
  // Get a copy of the old code profile in case inlining fails
  CodeProfile* oldcp = compilation()->codeprofile()->clone();
  // Push offsets into the CodeProfile structure
  int old_cpdata_offset = compilation()->inline_and_push_cpdata(caller_bci,callee, caller);
...
sync_handler=new BlockBegin(InvocationEntryBci);
  ...
  // If we bailed out during parsing, return immediately (this is bad news)
if(bailed_out()){
    compilation()->pop_cpdata(old_cpdata_offset);
    compilation()->revert_codeprofile(oldcp);
    return false;
  }
...
fill_sync_handler(lock,sync_handler,false,old_cpdata_offset);
  } else {
    pop_scope(old_cpdata_offset);
  }
  ...
  oldcp->free(CodeProfile::_temporary_freed);
  return true;
}
...
IRScope*callee_scope=new IRScope(compilation(),scope(),bci(),callee,InvocationEntryBci,false);
  scope()->add_callee(callee_scope);

  BlockListBuilder blb(compilation(), callee_scope, InvocationEntryBci);
  CHECK_BAILOUT();
...
void GraphBuilder::pop_scope(int old_cpdata_offset) {
  compilation()->pop_cpdata(old_cpdata_offset);
...
C1OUT->print("     %c%c%c ",sync_char,exception_char,monitors_char);
for(int i=0;i<scope()->level();i++)C1OUT->print("  ");
  if (res) {
C1OUT->print("  ");
  } else {
C1OUT->print("- ");
...
C1OUT->print(" (%d bytes)",callee->code_size());
  if (_inline_bailout_msg) {
C1OUT->print("  %s",_inline_bailout_msg);
  }
C1OUT->cr();

  if (res && CIPrintMethodCodes) {
callee->print_codes(C1OUT,NULL);
  }
}
