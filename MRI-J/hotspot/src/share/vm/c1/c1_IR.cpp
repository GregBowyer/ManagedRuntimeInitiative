...
Unimplemented();//how to test that unbound labels refer to the same address?
...
{
  _caller             = caller;
_caller_bci=caller==NULL?InvocationEntryBci:caller_bci;
...
assert(_bci==InvocationEntryBci||Bytecodes::is_defined(scope()->method()->java_code_at_bci(_bci)),"make sure bci points at a real bytecode");
}
...
IR::IR(Compilation* compilation, ciMethod* method, int osr_bci) :
  ...
  // setup IR fields
  _compilation = compilation;
_top_scope=new IRScope(compilation,NULL,InvocationEntryBci,method,osr_bci,true);
  _code        = NULL;
}
...
#ifndef PRODUCT
if(PrintCFG||PrintCFG1){C1OUT->print_cr("CFG after CEE");print(true);}
if(PrintIR||PrintIR1){C1OUT->print_cr("IR after CEE");print(false);}
#endif
  }
  if (EliminateBlocks) {
    opt.eliminate_blocks();
#ifndef PRODUCT
if(PrintCFG||PrintCFG1){C1OUT->print_cr("CFG after block elimination");print(true);}
if(PrintIR||PrintIR1){C1OUT->print_cr("IR after block elimination");print(false);}
#endif
  }
  if (EliminateNullChecks) {
    opt.eliminate_null_checks();
#ifndef PRODUCT
if(PrintCFG||PrintCFG1){C1OUT->print_cr("CFG after null check elimination");print(true);}
if(PrintIR||PrintIR1){C1OUT->print_cr("IR after null check elimination");print(false);}
#endif
  }
}
...
C1OUT->print_cr("Split critical edge B%d -> B%d (new block B%d)",
