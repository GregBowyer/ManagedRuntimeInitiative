...
IRScope* StateSplit::scope() const {
  return _state ? _state->scope() : NULL;
}
...
Invoke::Invoke(Bytecodes::Code code, int cpdoff, ValueType* result_type, Value recv, Values* args,
...
ip.print_instr(this);C1OUT->cr();
ip.print_stack(this->state());C1OUT->cr();
