...
#ifndef C1_INSTRUCTION_HPP
#define C1_INSTRUCTION_HPP
...
class InstructionVisitor: public StackObj {
 public:
  ...
  virtual void do_IncrementCount (IncrementCount*  x) = 0;
};
...
class Instruction: public CompilationResourceObj {
 ...
 protected:
void set_bci(int bci){assert0(bci==InvocationEntryBci||bci>=0);_bci=bci;}
 ...
 public:
  ...
  // initialization
  static void initialize()                       { ThreadLocals->_next_instruction_id = 0; }
  static int number_of_instructions()            { return ThreadLocals->_next_instruction_id; }

  // creation
  Instruction(ValueType* type, bool type_is_constant = false, bool create_hi = true)
  : _id(ThreadLocals->_next_instruction_id++)
  , _bci(-99)
  , _use_count(0)
  , _pin_state(0)
  , _type(type)
...
subst->type()->base()==ThreadLocals->_illegalType,"type can't change");
    _subst = subst;
  }
...
:Instruction(ThreadLocals->_illegalType,false,false),
      _lo_word(lo_word) {
...
void make_illegal(){set_type(ThreadLocals->_illegalType);}
...
LEAF(Phi, Instruction)
 ...
 public:
  ...
  void   make_illegal() {
    set(cannot_simplify);
set_type(ThreadLocals->_illegalType);
  }
...
:AccessArray(ThreadLocals->_intType,array,lock_stack)
  , _explicit_null_check(NULL) {}
...
:Op2(ThreadLocals->_intType,op,x,y)
  , _state_before(state_before)
  {}
...
LEAF(Invoke, StateSplit)
 ...
 public:
  // creation
Invoke(Bytecodes::Code code,int cpdoff,ValueType*result_type,Value recv,Values*args,
...
LEAF(NewInstance, StateSplit)
 ...
 public:
  // creation
NewInstance(ciInstanceKlass*klass):StateSplit(ThreadLocals->_instanceType),_klass(klass){}
...
BASE(NewArray, StateSplit)
 ...
 public:
  // creation
NewArray(Value length,ValueStack*state_before):StateSplit(ThreadLocals->_objectType),_length(length),_state_before(state_before){
...
BASE(TypeCheck, StateSplit)
 ...
 public:
  // creation
TypeCheck(ciKlass*klass,Value obj,ValueType*type,ValueStack*state_before,int cpdoff):StateSplit(type),_klass(klass),_obj(obj),_state_before(state_before),_cpdoff(cpdoff){
    ASSERT_VALUES
...
LEAF(CheckCast, TypeCheck)
 ...
 public:
  // creation
CheckCast(ciKlass*klass,Value obj,ValueStack*state_before,int cpdoff)
:TypeCheck(klass,obj,ThreadLocals->_objectType,state_before,cpdoff)
...
LEAF(InstanceOf, TypeCheck)
 public:
  // creation
InstanceOf(ciKlass*klass,Value obj,ValueStack*state_before,int cpdoff):TypeCheck(klass,obj,ThreadLocals->_intType,state_before,cpdoff){}
};
...
:StateSplit(ThreadLocals->_illegalType)
  , _obj(obj)
...
LEAF(BlockBegin, StateSplit)
 ...
 public:
  ...
  static void initialize()                       { ThreadLocals->_BB_next_block_id = 0; }
  static int  number_of_blocks()                 { return ThreadLocals->_BB_next_block_id; }

  // creation
  BlockBegin(int bci)
:StateSplit(ThreadLocals->_illegalType)
  , _block_id(ThreadLocals->_BB_next_block_id++)
...
LEAF(Goto, BlockEnd)
 public:
  // creation
Goto(BlockBegin*sux,ValueStack*state_before,bool is_safepoint=false):BlockEnd(ThreadLocals->_illegalType,state_before,is_safepoint){
    BlockList* s = new BlockList(1);
    s->append(sux);
    set_sux(s);
  }

  Goto(BlockBegin* sux, bool is_safepoint) : BlockEnd(ThreadLocals->_illegalType, NULL, is_safepoint) {
    BlockList* s = new BlockList(1);
    s->append(sux);
    set_sux(s);
  }
...
:BlockEnd(ThreadLocals->_illegalType,state_before,is_safepoint)
  , _x(x)
  , _cond(cond)
  , _y(y)
  , _profiled_method(NULL)
...
:BlockEnd(ThreadLocals->_illegalType,NULL,false)//temporary set to false
  , _klass(klass)
  , _obj(obj)
...
LEAF(Return, BlockEnd)
 ...
 public:
  // creation
  Return(Value result) :
BlockEnd(result==NULL?ThreadLocals->_voidType:result->type()->base(),NULL,true),
    _result(result) {}
...
LEAF(Throw, BlockEnd)
 ...
 public:
  // creation
Throw(Value exception,ValueStack*state_before):BlockEnd(ThreadLocals->_illegalType,state_before,true),_exception(exception){
    ASSERT_VALUES
  }
...
LEAF(Base, BlockEnd)
 public:
  // creation
Base(BlockBegin*std_entry,BlockBegin*osr_entry):BlockEnd(ThreadLocals->_illegalType,NULL,false){
...
LEAF(OsrEntry, Instruction)
 public:
  // creation
OsrEntry():Instruction(ThreadLocals->_longType,false){pin();}
...
LEAF(ExceptionObject, Instruction)
 public:
  // creation
ExceptionObject():Instruction(ThreadLocals->_objectType,false){
    pin();
  }
...
:Instruction(is_put?ThreadLocals->_voidType:as_ValueType(basic_type))
...
:Instruction(ThreadLocals->_voidType)
    , _method(method)
    , _bci_of_invoke(bci)
    , _recv(recv)
...
//
// Simple node representing a counter update generally used for updating CodeProfile's
//
LEAF(IncrementCount,Instruction)
  int _count_offset_from_CP; // (in bytes)
  int _cpd_offset_from_CP; // (in bytes)
  unsigned int _deopt_threshold;
  ValueStack* _state;

 public:
  // cpdoffset provides a path to the top of the CPData structure so C1 can
  // generate runtime checks for CPData magic.  A value of -1 means we're
  // incrementing a value in the CodeProfile structure (not in a cpdata
  // structure). If this IncrementCount is incrementing a non-cpdata
  // structure, don't specify cpdoffset.  Deopt_count, if non-0, will force a
  // deoptimization of C1 if the count is reached.
  IncrementCount(int count_offset_from_CP, int cpdoffset) : Instruction(ThreadLocals->_voidType), _count_offset_from_CP(count_offset_from_CP), _cpd_offset_from_CP(cpdoffset), _deopt_threshold(0), _state(NULL) {
    pin();
  }

  IncrementCount(int count_offset_from_CP, int cpdoffset, unsigned int deopt_threshold, ValueStack* state) : Instruction(ThreadLocals->_voidType), _count_offset_from_CP(count_offset_from_CP), _cpd_offset_from_CP(cpdoffset), _deopt_threshold(deopt_threshold), _state(state) {
    pin();
  }
  ...
  int count_offset_from_CP() const { return _count_offset_from_CP; }
  int cpd_offset_from_CP() const { return _cpd_offset_from_CP; }

  unsigned int deopt_threshold() const { return _deopt_threshold; }
