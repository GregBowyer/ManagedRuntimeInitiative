...
output()->print_cr("inlining depth %d",block->scope()?block->scope()->level():0);
}
...
void InstructionPrinter::print_line(Instruction* instr) {
  ...
  if (instr->exception_handlers() != NULL) instr->exception_handlers()->print(output());
  output()->cr();
...
void InstructionPrinter::do_IncrementCount(IncrementCount*x){
  output()->print("inc profile +%d",x->_cpd_offset_from_CP);
}
