 ...
 , _cached_blocks(*ir->linear_scan_order())
{
  ...
  assert(nof_regs == (int)LIR_OprDesc::vreg_base, "check vreg numbering is consistent");
...
LinearScan::Name LinearScan::reg_num(LIR_Opr opr) {
  ...
  if (opr->is_virtual_register()) {
    ...
    return (LinearScan::Name)opr->vreg_number();
  } else if (opr->is_single_cpu()) {
    return (LinearScan::Name)opr->cpu_regnr();
  } else if (opr->is_double_cpu()) {
    return (LinearScan::Name)opr->cpu_regnrLo();
#ifdef AZ_X86
  } else if (opr->is_single_xmm()) {
    return (LinearScan::Name)(opr->xmm_regnr() + pd_first_xmm_reg);
...
LinearScan::Name LinearScan::reg_numHi(LIR_Opr opr) {
  assert(opr->is_register(), "should not call this otherwise");
  return any_reg;
}
...
return i->reg_num()>=(int)LIR_OprDesc::vreg_base;
}
...
return i->reg_num()>=(int)LIR_OprDesc::vreg_base&&(i->type()!=T_FLOAT&&i->type()!=T_DOUBLE);
}

bool LinearScan::is_precolored_fpu_interval(const Interval* i) {
return(i->reg_num()>=nof_cpu_regs)&&(i->reg_num()<(nof_cpu_regs+LinearScan::nof_xmm_regs));
}
...
LinearScan::Name LinearScan::allocate_spill_slot(bool double_word){
  int spill_slot;
  if (FrameMap::spill_slot_size_in_bytes == 4) {
    if (double_word) {
      if ((_max_spills & 1) == 1) {
        // alignment of double-word values
        // the hole because of the alignment is filled with the next single-word value
assert(_unused_spill_slot==any_reg,"wasting a spill slot");
...
assert(_unused_spill_slot==-1,"never expect an unused hole for 8byte spill slots");
spill_slot=_max_spills;
    _max_spills++;
  }

  Name result = (Name)(spill_slot + LinearScan::nof_regs + frame_map()->argcount());
...
Name spill=allocate_spill_slot(type2spill_size[it->type()]==2);
...
Interval*LinearScan::create_interval(Name reg_num){
...
if(reg_num<(int)LIR_OprDesc::vreg_base){
...
it->set_reg_num((Name)_intervals.length());
...
#if 0 // AZUL: both of these flags only have meaning on non-Azulian platforms
...
assert(from_opr->is_fixed_cpu()||from_opr->is_xmm_register(),"from operand must be a register");
...
BasicType type=opr->type_register();
  if (opr->is_virtual_register()) {
    assert(reg_num(opr) == opr->vreg_number() && !is_valid_reg_num(reg_numHi(opr)), "invalid optimization below");
add_def((Name)opr->vreg_number(),def_pos,use_kind,type);

  } else {
Name reg=reg_num(opr);
    if (is_processed_reg_num(reg)) {
add_def((Name)reg,def_pos,use_kind,type);
    }
    reg = reg_numHi(opr);
    if (is_valid_reg_num(reg) && is_processed_reg_num(reg)) {
add_def((Name)reg,def_pos,use_kind,type);
...
BasicType type=opr->type_register();
  if (opr->is_virtual_register()) {
    assert(reg_num(opr) == opr->vreg_number() && !is_valid_reg_num(reg_numHi(opr)), "invalid optimization below");
add_use((Name)opr->vreg_number(),from,to,use_kind,type);

  } else {
Name reg=reg_num(opr);
    if (is_processed_reg_num(reg)) {
add_use((Name)reg,from,to,use_kind,type);
    }
    reg = reg_numHi(opr);
    if (is_valid_reg_num(reg) && is_processed_reg_num(reg)) {
add_use((Name)reg,from,to,use_kind,type);
...
BasicType type=opr->type_register();
  if (opr->is_virtual_register()) {
    assert(reg_num(opr) == opr->vreg_number() && !is_valid_reg_num(reg_numHi(opr)), "invalid optimization below");
add_temp((Name)opr->vreg_number(),temp_pos,use_kind,type);

  } else {
Name reg=reg_num(opr);
    if (is_processed_reg_num(reg)) {
add_temp((Name)reg,temp_pos,use_kind,type);
    }
    reg = reg_numHi(opr);
    if (is_valid_reg_num(reg) && is_processed_reg_num(reg)) {
add_temp((Name)reg,temp_pos,use_kind,type);
...
void LinearScan::add_def(Name reg_num, int def_pos, IntervalUseKind use_kind, BasicType type) {
...
interval->set_has_non_temp_def();
}

void LinearScan::add_use(Name reg_num, int from, int to, IntervalUseKind use_kind, BasicType type) {
  Interval* interval = interval_at(reg_num);
  if (interval == NULL) {
    interval = create_interval(reg_num);
  }
...
void LinearScan::add_temp(Name reg_num, int temp_pos, IntervalUseKind use_kind, BasicType type) {
  Interval* interval = interval_at(reg_num);
  if (interval == NULL) {
    interval = create_interval(reg_num);
  }
...
void LinearScan::handle_method_arguments(LIR_Op* op) {
  ...
  if (op->code() == lir_move) {
    ...
    if (move->in_opr()->is_stack()) {
      ...
      Name stack_slot = (Name)(LinearScan::nof_regs + (move->in_opr()->is_single_stack() ? move->in_opr()->single_stack_ix() : move->in_opr()->double_stack_ix()));
      assert(stack_slot >= LinearScan::nof_regs && stack_slot < LinearScan::nof_regs+frame_map()->argcount(), "out of range");
...
Name caller_save_registers[LinearScan::nof_regs];
  ...
  for (i = block_count() - 1; i >= 0; i--) {
    ...
    for (int number = live.get_next_one_offset(0, size); number < size; number = live.get_next_one_offset(number + 1, size)) {
      ...
      add_use((Name)number, block_from, block_to + 2, noUse, T_ILLEGAL);
    ...
    for (int j = instructions->length() - 1; j >= 1; j--) {
      ...
      // visit definitions (temp and output operands)
      int k, n;
      n = visitor.opr_count(LIR_OpVisitState::outputMode);
      for (k = 0; k < n; k++) {
        LIR_Opr opr = visitor.opr_at(LIR_OpVisitState::outputMode, k);
      ...
      for (k = 0; k < n; k++) {
        ...
        if( opr->is_destroyed() ) add_temp(opr, op_id, shouldHaveRegister);
      }
  ...
  for (int n = 0; n < LinearScan::nof_regs; n++) {
    ...
    if (interval != NULL) {
      ...
      if( !LinearScan::is_caller_save(n) ) {
        frame_map()->record_callee_save_allocation(n);
      }
    }
  }
}
...
int LinearScan::interval_cmp(Interval** a, Interval** b) {
  if (*a != NULL) {
    if (*b != NULL) {
      // AZUL: NB we could prefer objects and arrays at the same address over
      // primitive types, the effect on compressing the oop map is negligible
...
result=new Interval((Name)LIR_OprDesc::vreg_base);
result->assign_reg((Name)0);
...
Interval* LinearScan::interval_at_op_id(Name reg_num, int op_id) {
  assert(LinearScan::nof_regs <= reg_num && reg_num < num_virtual_regs(), "register number out of bounds");
  assert(interval_at(reg_num) != NULL, "no interval found");
...
Name reg=interval->assigned_reg();
  Name regHi = interval->assigned_regHi();
...
void LinearScan::resolve_exception_edge(XHandler* handler, int throwing_op_id, Name reg_num, Phi* phi, MoveResolver &move_resolver) {
  if (interval_at(reg_num) == NULL) {
    // if a phi function is never used, no interval is created -> ignore this
    return;
  }
...
Interval*from_interval=interval_at_op_id((Name)from_value->operand()->vreg_number(),throwing_op_id);
      move_resolver.add_mapping(from_interval, to_interval);
    }
...
void LinearScan::resolve_exception_edge(XHandler* handler, int throwing_op_id, MoveResolver &move_resolver) {
  ...
  int size = live_set_size();
  for (int r = block->live_in().get_next_one_offset(0, size); r < size; r = block->live_in().get_next_one_offset(r + 1, size)) {
resolve_exception_edge(handler,throwing_op_id,(Name)r,NULL,move_resolver);
  }

  // the live_in bits are not set for phi functions of the xhandler entry, so iterate them separately
  for_each_phi_fun(block, phi,
resolve_exception_edge(handler,throwing_op_id,(Name)phi->operand()->vreg_number(),phi,move_resolver)
  );
...
VOopReg::VR LinearScan::vm_reg_for_interval(Interval* interval) {
  VOopReg::VR reg = interval->cached_vm_reg();
  if (!VOopReg::is_valid(reg) ) {
...
VOopReg::VR LinearScan::vm_reg_for_operand(LIR_Opr opr) {
  assert(opr->is_oop(), "currently only implemented for oop operands");
return frame_map()->oopregname(opr);
}
...
LIR_Opr LinearScan::operand_for_interval(Interval* interval) {
  ...
  assert(opr == calc_operand_for_interval(interval) ||
         opr == calc_operand_for_interval(interval)->set_destroyed()
,"wrong cached value");
  return opr;
}
...
LIR_Opr LinearScan::color_lir_opr(LIR_Opr opr, int op_id, LIR_OpVisitState::OprMode mode) {
  ...
  // AZUL: no callee_saved vreg flag
  ...
  if (res->is_register() && opr->is_destroyed() && !res->is_destroyed() ) {
res=res->set_destroyed();
interval->set_cached_opr(res);
  }
  return res;
}
...
if(info->bci()!=InvocationEntryBci&&!info->scope()->method()->is_native()){
...
OopMap2*LinearScan::compute_oop_map(IntervalWalker*iw,LIR_Op*op,CodeEmitInfo*info,bool is_call_site){
...
assert(interval->reg_num()>=(int)LIR_OprDesc::vreg_base,"fixed interval found");
    ...
    if (is_patch_info || op->id() < interval->current_to()) {
      ...
      if( operand_for_interval(interval)->is_register() && operand_for_interval(interval)->is_destroyed() ) {
        // don't place destroyed operands in debug map
        continue;
      }
      VOopReg::VR name = vm_reg_for_interval(interval);
map->add(name);
      ...
      if (interval->always_in_memory() && 
        ...
        VOopReg::VR vreg = frame_map()->slot_oopregname(interval->canonical_spill_slot() - LinearScan::nof_regs);
        map->add(vreg);
      }
    }
  }
  ...
  for (int i = 0; i < locks_count; i++) {
    VOopReg::VR vreg = frame_map()->monitor_object_oopregname(i);
    map->add(vreg);
  }
...
OopMap2*first_oop_map=compute_oop_map(iw,op,first_info,visitor.has_call());

  for (int i = 0; i < visitor.info_count(); i++) {
    CodeEmitInfo* info = visitor.info_at(i);
OopMap2*oop_map=first_oop_map;
...
assert(info->_oop_map->equals(oop_map),"same CodeEmitInfo used for multiple LIR instructions");
    }
  }
}
...
VReg::VR LinearScan::location_for_monitor_index(int monitor_index) {
  return frame_map()->location_for_monitor_object(monitor_index);
}
...
int LinearScan::append_scope_value_for_constant(LIR_Opr opr, DebugScopeBuilder *dbg, DebugInfoBuilder::JVM_Part part, int idx ) {
...
if(o==NULL){
        dbg->add_const_int( part, idx, 0 );
      } else {
        int oop_index = ciEnv::get_OopTable_index(o);
    ...
    case T_FLOAT: {
jint value=c->as_jint_bits();
      dbg->add_const_int( part, idx, value );
      return 1;
    }
    case T_LONG: // fall through
    case T_DOUBLE: {
      jlong value = c->as_jlong_bits();
      dbg->add_const_long( part, idx, value );
      return 2;
    }
    default:
      ShouldNotReachHere();
...
int LinearScan::append_scope_value_for_operand(LIR_Opr opr, DebugScopeBuilder *dbg, DebugInfoBuilder::JVM_Part part, int idx ) {
  VReg::VR rname;
if(opr->is_single_word()){
if(opr->is_stack()){
      int stack_idx = opr->single_stack_ix();
      rname = frame_map()->slot_dbgregname(stack_idx);
    } else {
      assert0 (opr->is_cpu_register() || opr->is_xmm_register());
      rname = frame_map()->dbgregname(opr);
    }
    dbg->add_vreg( part, idx, rname, opr->is_oop() );
    return 1;
  } else {
if(opr->is_stack()){
int stack_idx=opr->double_stack_ix();
      rname = frame_map()->slot_dbgregname(stack_idx);
    } else {
      assert0 (opr->is_cpu_register() || opr->is_xmm_register());
      rname = frame_map()->dbgregname(opr);
    }
    dbg->add_vreg_long( part, idx, rname );
    return 2;
  }
}


int LinearScan::append_scope_value(int op_id, Value value, DebugScopeBuilder *dbg, DebugInfoBuilder::JVM_Part part, int idx ) {
  if (value == NULL) {
    // Add empty value for optimized away values
    dbg->add_empty(part, idx);
    return 1;
  }
  LIR_Opr opr = value->operand();
  Constant* con = value->as_Constant();
  ...
  if (opr->is_virtual()) {
    ...
    assert(!has_call(op_id) || opr->is_stack() || is_caller_save(reg_num(opr)) || !opr->is_oop_register(), "No oops in callee-save registers across calls");
    
    // Append to ScopeValue array
return append_scope_value_for_operand(opr,dbg,part,idx);
    ...
    return append_scope_value_for_constant(opr, dbg, part, idx );
  }
}


DebugScopeBuilder*LinearScan::compute_debug_info_for_scope(int op_id,IRScope*cur_scope,ValueStack*cur_state,ValueStack*innermost_state,int cur_bci,int stack_end,int locks_end){

DebugScopeBuilder*caller_debug_info=NULL;
  int stack_begin = 0, locks_begin = 0;
  ...
  DebugScopeBuilder *dbg = new DebugScopeBuilder( caller_debug_info, cur_scope->method()->objectId(), nof_locals, cur_scope->method()->max_stack(), nof_locks, cur_bci );

  // describe local variable values
  for( int i=0; i<nof_locals; ) {
    assert0(i < cur_state->locals_size());
Value local=cur_state->local_at(i);
    i += append_scope_value(op_id, local, dbg, DebugInfoBuilder::JLocal, i);
  }
  assert0( dbg->size_jlocals() == nof_locals );
  assert0( nof_locals == cur_state->locals_size());
  ...
  for( int i=0; i<nof_stack; ) {
    Value expression = innermost_state->stack_at(i+stack_begin);
    i += append_scope_value(op_id, expression, dbg, DebugInfoBuilder::JStack, i);
  }

  // describe monitors
for(int i=0;i<nof_locks;i++){
    dbg->add_vreg(DebugInfoBuilder::JLock, i, location_for_monitor_index(i+locks_begin), true);
  }
...
void LinearScan::compute_debug_info(CodeEmitInfo* info, int op_id) {
  ...
  if (info->_scope_debug_info == NULL) {
    ...
    const int cs = frame_map()->callee_saves();
    if( cs ) {
      int csnum = 0;
for(int i=0;i<LinearScan::nof_cpu_regs;i++){
        if( (cs & (1<<i)) ) {
          const int sp_offset = in_bytes(frame_map()->sp_offset_for_callee_save(csnum++));
          info->_scope_debug_info->add_callee_save_pair(VReg::VR(i),VReg::stk2reg(sp_offset));
        }
      }
    }
  } else {
    // debug information already set. Check that it is correct from the current point of view
DEBUG_ONLY(info->_scope_debug_info->equals(compute_debug_info_for_scope(op_id,innermost_scope,innermost_state,innermost_state,info->bci(),stack_end,locks_end)));
  }
}
...
void LinearScan::verify_intervals() {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (i1->reg_num() >= (int)LIR_OprDesc::vreg_base && i1->type() == T_ILLEGAL) {
...
void MoveResolver::resolve_mappings() {
  ...
  while (_mapping_from.length() > 0) {
    ...
    if (!processed_interval) {
      ...
      Interval* spill_interval = new Interval(LinearScan::any_reg);
...
LinearScan::Name spill_slot=from_interval->canonical_spill_slot();
      if (spill_slot < 0) {
...
// initialize sentinel
Range Range::_end(max_jint,max_jint,NULL);
...
assert(r1!=end()&&r2!=end(),"empty ranges not allowed");
  ...
  do {
    if (r1->from() < r2->from()) {
      if (r1->to() <= r2->from()) {
r1=r1->next();if(r1==end())return-1;
      } else {
        return r2->from();
      }
    } else if (r2->from() < r1->from()) {
      if (r2->to() <= r1->from()) {
r2=r2->next();if(r2==end())return-1;
      } else {
        return r1->from();
      }
...
void Interval::initialize() {
  Range::initialize();
  ThreadLocals->_interval_end = new Interval(LinearScan::any_reg);
}

Interval::Interval(LinearScan::Name reg_num) :
  _reg_num(reg_num),
  _type(T_ILLEGAL),
  _first(Range::end()),
_use_pos_and_kinds(reg_num==-1?0:12),
  _current(Range::end()),
  _next(reg_num == -1 ? NULL : end()),
  _state(invalidState),
  _has_non_temp_def(false),
  _assigned_reg(LinearScan::any_reg),
  _assigned_regHi(LinearScan::any_reg),
  _cached_to(-1),
  _cached_opr(LIR_OprFact::illegalOpr),
_cached_vm_reg(VOopReg::Bad),
  _split_children(0),
_canonical_spill_slot(LinearScan::any_reg),
...
void Interval::add_use_pos(int pos, IntervalUseKind use_kind) {
  ...
  // they are never used
if(use_kind!=noUse&&reg_num()>=(int)LIR_OprDesc::vreg_base){
#ifdef ASSERT
...
Interval* Interval::new_split_child() {
  ...
  Interval* result = new Interval(LinearScan::any_reg);
...
assert(noinline::lt(hole_from,hole_to),"check");
...
if(assigned_reg()>=(int)pd_first_cpu_reg&&assigned_reg()<=(int)pd_last_cpu_reg){
      opr = LIR_OprFact::single_cpu(assigned_reg());
#ifdef AZ_X86
}else if(assigned_reg()>=(int)pd_first_xmm_reg&&assigned_reg()<=(int)pd_last_xmm_reg){
...
assert(i->assigned_reg()!=LinearScan::any_reg,"interval has no register assigned");
...
LinearScan::Name LinearScanWalker::find_free_reg(BasicType type, int reg_needed_until, int interval_to, int hint_reg, int ignore_reg, bool is_temp, bool* need_split) {
  // Best register available for full interval
  LinearScan::Name min_full_reg = LinearScan::any_reg;
  // Best register available for part of the interval
  LinearScan::Name max_partial_reg = LinearScan::any_reg;
  // Does the interval span a call?
  const bool spans_call = _allocator->has_call_in_range(reg_needed_until, interval_to+1);
  TRACE_LINEAR_SCAN(9, tty->print_cr("      Range [%d, %d[ does%s span a call", reg_needed_until, interval_to, spans_call?"":"n't"));
  // Can we allocate to a callee save register to avoid a spill?
  const bool prefer_callee_save_allocation = !is_temp && spans_call && (type!=T_OBJECT && type!=T_ARRAY);
  // Must we avoid callee save allocation?
  const bool avoid_callee_save_allocation  = spans_call && (type==T_OBJECT || type==T_ARRAY);
#ifdef AZ_X86
  // Should we search for registers forward or backwards, by placing 64bit values
  // in high registers we may avoid unnecessary REX prefix bytes on <64bit operations
  // NB this only makes sense for non-temp values, as temp values hold a range of types
  const bool backward = !is_temp && (type==T_OBJECT || type==T_ARRAY || type == T_LONG);
#else
  const bool backward = false;
#endif
  // Attempt allocation to hint register
  // For something like a float to double we will hint that the double should be
  // placed in the same physical register as the float
  if (hint_reg != LinearScan::any_reg) {
    assert (LinearScan::is_processed_reg_num(hint_reg), "must be");
    // check that the hint register is applicable for this interval (ie don't
    // allocate a cpu register for an xmm)
    if (hint_reg >= _first_reg && hint_reg <= _last_reg &&
        // AZUL: dont allocate a callee save register for an oop if avoid_callee_save_allocation
        (!avoid_callee_save_allocation || LinearScan::is_caller_save(hint_reg))) {
      int use_pos = _use_pos[hint_reg];
      if (use_pos >= interval_to) {
        min_full_reg = (LinearScan::Name)hint_reg;
      }
    }
  }
  // Do 2 phases of allocation:
  // - phase 1 we attempt to allocate to a full interval
  // - phase 2 we attempt allocation to a partial interval
  for( int phase=1; (min_full_reg == LinearScan::any_reg) && (phase <= 2); phase++ ) {
    bool partial_allocation = (phase != 1);
    // Do 2 rounds of allocation, depending on preference caller or callee saves
    // then the opposite
    for (int round=1; (min_full_reg == LinearScan::any_reg) && (round <= 2); round++ ) {
      // do we allocate caller save or callee saves this round?
      bool caller_save_round;
      if (round == 1) {
        caller_save_round = !prefer_callee_save_allocation;
      } else {
        caller_save_round = prefer_callee_save_allocation;
      }
      if (!caller_save_round && avoid_callee_save_allocation) {
        continue;
      }
      // Iterate over physical (fixed) registers
      const int start = backward ? _last_reg  : _first_reg;
      const int end   = backward ? _first_reg : _last_reg;
      const int step  = backward ? -1         : 1;
      for (int i=start; (backward ? i >= end : i <= end); i+= step) {
        if (!LinearScan::is_processed_reg_num(i)) {
          // AZUL: don't allocate this register as it is a deliberate hole in the
          // register numbers (e.g. for the stack pointer). This is deliberately
          // different to JavaSoft's implementation where they renumber registers
          // to avoid holes.
          continue;
        } else if (LinearScan::is_caller_save(i) != caller_save_round) {
          // AZUL: don't allocate in this round
          continue;
}else if(i==ignore_reg){
          // this register must be ignored
          continue;
        } else {
          int use_pos = _use_pos[i];
          if (!partial_allocation) {
            if (use_pos >= interval_to) {
              // the register is available for the full interval, do we prefer it to our current choice?
              if (min_full_reg == LinearScan::any_reg || use_pos < _use_pos[min_full_reg]) {
                min_full_reg = (LinearScan::Name)i;
              }
            }
          } else {
            if (use_pos > reg_needed_until) {
              // this register is at least partially free until reg_needed_until
              if (max_partial_reg == LinearScan::any_reg || i == hint_reg || (_use_pos[i] > _use_pos[max_partial_reg] && max_partial_reg != hint_reg)) {
                max_partial_reg = (LinearScan::Name)i;
              }
            }
          }
        }
  ...
  // Record any allocation, necessary splits and return
  if (min_full_reg != LinearScan::any_reg) {
    assert (min_full_reg >= _first_reg && min_full_reg <= _last_reg, "must allocate to a valid register");
    if (!LinearScan::is_caller_save(min_full_reg)) {
      allocator()->frame_map()->record_callee_save_allocation(min_full_reg);
    }
    return min_full_reg;
}else if(max_partial_reg!=LinearScan::any_reg){
    assert (max_partial_reg >= _first_reg && max_partial_reg <= _last_reg, "must allocate to a valid register");
    if (!LinearScan::is_caller_save(max_partial_reg)) {
      allocator()->frame_map()->record_callee_save_allocation(max_partial_reg);
    }
    *need_split = true;
    return max_partial_reg;
  } else {
    return LinearScan::any_reg;
  }
}

LinearScan::Name LinearScanWalker::find_free_double_reg(int reg_needed_until, int interval_to, int hint_reg, bool* need_split) {
  assert((_last_reg - _first_reg + 1) % 2 == 0, "adjust algorithm");

  Untested(); // do we need this for x86 64?
  LinearScan::Name min_full_reg = LinearScan::any_reg;
  LinearScan::Name max_partial_reg = LinearScan::any_reg;
...
if(min_full_reg==LinearScan::any_reg||i==hint_reg||(_use_pos[i]<_use_pos[min_full_reg]&&min_full_reg!=hint_reg)){
min_full_reg=(LinearScan::Name)i;
      }
...
//  free_collect_unhandled(fixedKind, cur);
  ...
  LinearScan::Name hint_reg, hint_regHi;
...
assert(hint_reg!=LinearScan::any_reg&&hint_regHi==LinearScan::any_reg,"must be for fixed intervals");
hint_regHi=(LinearScan::Name)(hint_reg+1);//connect e.g. eax-edx
    }
    TRACE_LINEAR_SCAN(4, tty->print("      hint registers %d, %d from interval ", hint_reg, hint_regHi); register_hint->print());

  } else {
hint_reg=LinearScan::any_reg;
hint_regHi=LinearScan::any_reg;
  }
assert(hint_reg==LinearScan::any_reg||hint_reg!=hint_regHi,"hint reg and regHi equal");
assert(cur->assigned_reg()==LinearScan::any_reg&&cur->assigned_regHi()==LinearScan::any_reg,"register already assigned to interval");
  ...
  int split_pos = -1;
  LinearScan::Name reg = LinearScan::any_reg;
  LinearScan::Name regHi = LinearScan::any_reg;

  if (_adjacent_regs) {
    reg = find_free_double_reg(reg_needed_until, interval_to, hint_reg, &need_split);
    regHi = (LinearScan::Name)(reg + 1);
    if (reg == LinearScan::any_reg) {
      return false;
    }
    split_pos = MIN2(_use_pos[reg], _use_pos[regHi]);

  } else {
reg=find_free_reg(cur->type(),reg_needed_until,interval_to,hint_reg,LinearScan::any_reg,cur->is_temp(),&need_split);
if(reg==LinearScan::any_reg){
      return false;
    }
    ...
    if (_num_phys_regs == 2) {
regHi=find_free_reg(cur->type(),reg_needed_until,interval_to,hint_regHi,reg,cur->is_temp(),&need_split);

if(_use_pos[reg]<interval_to&&regHi==LinearScan::any_reg){
...
return _num_phys_regs==1||regHi!=LinearScan::any_reg;
}


LinearScan::Name LinearScanWalker::find_locked_reg(BasicType type, int reg_needed_until, int interval_to, LinearScan::Name hint_reg, LinearScan::Name ignore_reg, bool* need_split) {
  LinearScan::Name max_reg = LinearScan::any_reg;

  for (int i = _first_reg; i <= _last_reg; i++) {
    if (i == ignore_reg) {
      // this register must be ignored

    } else if (!LinearScan::is_processed_reg_num(i) || ((type==T_OBJECT || type==T_ARRAY) && !LinearScan::is_caller_save(i)) ) {
      // These are not allowed
    } else if (_use_pos[i] > reg_needed_until) {
if(max_reg==LinearScan::any_reg||i==hint_reg||(_use_pos[i]>_use_pos[max_reg]&&max_reg!=hint_reg)){
max_reg=(LinearScan::Name)i;
      }
    }
  }

  if (max_reg != LinearScan::any_reg && _block_pos[max_reg] <= interval_to) {
    *need_split = true;
  }
...
LinearScan::Name LinearScanWalker::find_locked_double_reg(int reg_needed_until, int interval_to, LinearScan::Name hint_reg, bool* need_split) {
  assert((_last_reg - _first_reg + 1) % 2 == 0, "adjust algorithm");
...
if(max_reg==LinearScan::any_reg||_use_pos[i]>_use_pos[max_reg]){
max_reg=(LinearScan::Name)i;
      }
    }
  }
...
void LinearScanWalker::split_and_spill_intersecting_intervals(LinearScan::Name reg, LinearScan::Name regHi) {
  assert(reg != LinearScan::any_reg, "no register assigned");
...
//  spill_block_unhandled_fixed(cur);
  ...
  int use_pos = 0;
  bool need_split = false;
  LinearScan::Name reg, regHi;

  if (_adjacent_regs) {
reg=find_locked_double_reg(reg_needed_until,interval_to,LinearScan::any_reg,&need_split);
regHi=(LinearScan::Name)(reg+1);
...
reg=find_locked_reg(cur->type(),reg_needed_until,interval_to,LinearScan::any_reg,cur->assigned_reg(),&need_split);
regHi=LinearScan::any_reg;
...
if(reg!=LinearScan::any_reg){
      ...
      if (_num_phys_regs == 2) {
if(cur->assigned_reg()!=LinearScan::any_reg){
          regHi = reg;
          reg = cur->assigned_reg();
        } else {
regHi=find_locked_reg(cur->type(),reg_needed_until,interval_to,LinearScan::any_reg,reg,&need_split);
if(regHi!=LinearScan::any_reg){
        ...
        if (regHi != LinearScan::any_reg && reg > regHi) {
          // sort register numbers to prevent e.g. a move from eax,ebx to ebx,eax
          LinearScan::Name temp = reg;
          reg = regHi;
          regHi = temp;
  ...
  if (reg == LinearScan::any_reg || (_num_phys_regs == 2 && regHi == LinearScan::any_reg) || use_pos <= cur->first_usage(mustHaveRegister)) {
...
assert(reg!=LinearScan::any_reg&&(_num_phys_regs==1||regHi!=LinearScan::any_reg),"no register found");
    assert(split_pos > 0, "invalid split_pos");
...
bool LinearScanWalker::no_allocation_possible(Interval* cur) {
  ...
  // AZUL: we have callee-save registers even on x86 so do nothing here
  return false;
}
...
void LinearScanWalker::combine_spilled_intervals(Interval* cur) {
  ...
  assert(begin_hint->assigned_reg() != LinearScan::any_reg, "must have register assigned");
  assert(end_hint->assigned_reg() == LinearScan::any_reg, "must not have register assigned");
...
assert(cur->assigned_reg()==LinearScan::any_reg&&cur->assigned_regHi()==LinearScan::any_reg,"register already assigned");

    allocator()->assign_spill_slot(cur);
    split_stack_interval(cur);
    result = false;

  } else if (cur->assigned_reg() == LinearScan::any_reg) {
...
void EdgeMoveOptimizer::optimize_moves_at_block_end(BlockBegin* block) {
  ...
  while (true) {
    ...
    TRACE_LINEAR_SCAN(4, tty->print("found instruction that is equal in all %d predecessors: ", num_preds); op->print(tty));
...
void EdgeMoveOptimizer::optimize_moves_at_block_begin(BlockBegin* block) {
  ...
  while (true) {
    ...
    TRACE_LINEAR_SCAN(4, tty->print("----- found instruction that is equal in all %d successors: ", num_sux); op->print(tty));
...
if(counter_idx==counter_loop_method||counter_idx==counter_exception_method){
