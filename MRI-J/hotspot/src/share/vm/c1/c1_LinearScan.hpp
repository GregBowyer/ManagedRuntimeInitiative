...
#ifndef C1_LINEARSCAN_HPP
#define C1_LINEARSCAN_HPP
...
class LinearScan : public CompilationResourceObj {
 ...
 public:
  // Representation of locations known to the register allocator. From 0 to
  // the number of registers are the physical (fixed) registers. After these
  // are argcount method arguments on the stack. Finally come spill slots.
  enum Name {
    any_reg = -1,
    nof_cpu_regs = pd_nof_cpu_regs_linearscan,
    nof_xmm_regs = pd_nof_xmm_regs_linearscan,
nof_regs=nof_cpu_regs+nof_xmm_regs+pd_nof_extra_regs_linearscan
  };
...
int _max_spills;//number of stack slots used for intervals allocated to memory (0 ...)
int _unused_spill_slot;//unused spill slot for a single-word value because of alignment of a double-word value (-1 or a value between 0 and _max_spills)
  ...
  bool has_call_in_range(int start_op, int end_op)  { return _has_call.get_next_one_offset(start_op>>1) < (BitMap::idx_t)(end_op>>1); }
...
static bool is_valid_reg_num(Name reg_num){return reg_num>any_reg;}
static Name reg_num(LIR_Opr opr);
static Name reg_numHi(LIR_Opr opr);
...
Name allocate_spill_slot(bool double_word);
  void        assign_spill_slot(Interval* it);
  void        propagate_spill_slots();

  Interval*   create_interval(Name reg_num);
...
private:
  ...
  void add_def (Name reg_num, int def_pos,      IntervalUseKind use_kind, BasicType type);
  void add_use (Name reg_num, int from, int to, IntervalUseKind use_kind, BasicType type);
  void add_temp(Name reg_num, int temp_pos,     IntervalUseKind use_kind, BasicType type);
...
Interval*interval_at_op_id(Name reg_num,int op_id);
...
void resolve_exception_edge(XHandler*handler,int throwing_op_id,Name reg_num,Phi*phi,MoveResolver&move_resolver);
...
VOopReg::VR vm_reg_for_interval(Interval*interval);
VOopReg::VR vm_reg_for_operand(LIR_Opr opr);
...
OopMap2*compute_oop_map(IntervalWalker*iw,LIR_Op*op,CodeEmitInfo*info,bool is_call_site);
  ...
  VReg::VR location_for_monitor_index(int monitor_index);
  //LocationValue* location_for_name(int name, Location::Type loc_type);

  int append_scope_value_for_constant(LIR_Opr opr,  DebugScopeBuilder *dbg, DebugInfoBuilder::JVM_Part part, int idx );
  int append_scope_value_for_operand(LIR_Opr opr, DebugScopeBuilder *dbg, DebugInfoBuilder::JVM_Part part, int idx );
  int append_scope_value( int op_id, Value value, DebugScopeBuilder *dbg, DebugInfoBuilder::JVM_Part part, int idx );

DebugScopeBuilder*compute_debug_info_for_scope(int op_id,IRScope*cur_scope,ValueStack*cur_state,ValueStack*innermost_state,int cur_bci,int stack_end,int locks_end);
...
class Interval : public CompilationResourceObj {
 private:
  ...
  bool             _has_non_temp_def; // does this interval just hold temp definitions?

  LinearScan::Name _assigned_reg;
  LinearScan::Name _assigned_regHi;

  int              _cached_to;    // cached value: to of last range (-1: not cached)
  LIR_Opr          _cached_opr;
  VOopReg::VR   _cached_vm_reg;
  ...
  LinearScan::Name _canonical_spill_slot;   // the stack slot where all split parts of this interval are spilled to (always stored in split parents)
 ...
 public:
  Interval(LinearScan::Name reg_num);

  static void      initialize();
  static Interval* end()                         { return ThreadLocals->_interval_end; }

  // accessors
LinearScan::Name reg_num()const{return _reg_num;}
void set_reg_num(LinearScan::Name r){assert(_reg_num==LinearScan::any_reg,"cannot change reg_num");_reg_num=r;}
BasicType type()const{assert(_reg_num==LinearScan::any_reg||_reg_num>=(int)LIR_OprDesc::vreg_base,"cannot access type for fixed interval");return _type;}
void set_type(BasicType type){assert(_reg_num<(int)LIR_OprDesc::vreg_base||_type==T_ILLEGAL||_type==type,"overwriting existing type");_type=type;}

  void             set_has_non_temp_def()        { _has_non_temp_def = true;  }
  bool             is_temp()                     { return !_has_non_temp_def; }
  ...
  LinearScan::Name assigned_reg() const          { return _assigned_reg; }
  LinearScan::Name assigned_regHi() const        { return _assigned_regHi; }
  void             assign_reg(LinearScan::Name reg)                       { assert0( _type!=T_DOUBLE || reg >=16 ); _assigned_reg = reg; _assigned_regHi = LinearScan::any_reg; }
  void             assign_reg(LinearScan::Name reg, LinearScan::Name rHi) { assert0( _type!=T_DOUBLE || reg >=16 ); _assigned_reg = reg; _assigned_regHi = rHi; }
...
LinearScan::Name canonical_spill_slot()const{return split_parent()->_canonical_spill_slot;}
void set_canonical_spill_slot(LinearScan::Name slot){assert(split_parent()->_canonical_spill_slot==LinearScan::any_reg,"overwriting existing value");split_parent()->_canonical_spill_slot=slot;}
...
VOopReg::VR cached_vm_reg()const{return _cached_vm_reg;}
  void             set_cached_opr(LIR_Opr opr)   { _cached_opr = opr; }
void set_cached_vm_reg(VOopReg::VR reg){_cached_vm_reg=reg;}
...
void next_range(){assert(this!=end(),"not allowed on sentinel");_current=_current->next();}
...
class LinearScanWalker : public IntervalWalker {
 private:
  ...
  LinearScan::Name find_free_reg(BasicType type, int reg_needed_until, int interval_to, int hint_reg, int ignore_reg, bool is_temp, bool* need_split);
  LinearScan::Name find_free_double_reg(int reg_needed_until, int interval_to, int hint_reg, bool* need_split);
  bool alloc_free_reg(Interval* cur);

  LinearScan::Name  find_locked_reg(BasicType type, int reg_needed_until, int interval_to, LinearScan::Name hint_reg, LinearScan::Name ignore_reg, bool* need_split);
  LinearScan::Name  find_locked_double_reg(int reg_needed_until, int interval_to, LinearScan::Name hint_reg, bool* need_split);
  void split_and_spill_intersecting_intervals(LinearScan::Name reg, LinearScan::Name regHi);
...
#include "c1_LinearScan_pd.hpp"

#endif // C1_LINEARSCAN_HPP
