...
public:
  ...
  void do_IncrementCount (IncrementCount*  x);
};
...
void NullCheckVisitor::do_IncrementCount(IncrementCount*x){}


void NullCheckEliminator::do_value(Value* p) {
  assert(*p != NULL, "should not find NULL instructions");
if(ThreadLocals->_static_nce->visitable(*p)){
ThreadLocals->_static_nce->mark_visited(*p);
(*p)->visit(&ThreadLocals->_static_nce->_visitor);
  }
}
...
assert(local0->type()==ThreadLocals->_objectType,"invalid type of receiver");
