...
methodData()->set_eflag(CodeProfile::estimated);
    return;
  }
  ...
  //
if(!has_dependencies()&&!methodData()->total_count()){
    ...
    if (_return_local) {
methodData()->set_eflag(CodeProfile::return_local);
    }
methodData()->set_eflag(CodeProfile::estimated);
  }
}
...
_return_local=methodData()->eflag_set(CodeProfile::return_local);
...
BCEscapeAnalyzer::BCEscapeAnalyzer(ciMethod* method, BCEscapeAnalyzer* parent)
    ...
    , _method(method)
,_methodData(method?method->codeprofile(false/*no clone*/):NULL)
