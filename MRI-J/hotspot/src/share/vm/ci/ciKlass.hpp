...
protected:
  ...
  klassRef get_klassRef() const { 
    klassRef k = klassRef(get_ref()._objref);
assert(k.not_null(),"illegal use of unloaded klass");
    return k;
  }
...
virtual oop loader()const{return NULL;}
  virtual jobject loader_handle()  { return NULL; }

virtual oop protection_domain()const{return NULL;}
  virtual jobject protection_domain_handle()  { return NULL; }

  const char* type_string() const { return "ciKlass"; }
...
jint layout_helper()const{return _layout_helper;}
  ...
  unsigned klassId();
  unsigned element_klassId();
  static ciKlass *make_from_klassId(int);
