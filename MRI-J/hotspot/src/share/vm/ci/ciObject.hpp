...
private:
  ...
  jobject  _handle;
  ciKlass* _klass;
  // _is_loaded must be assigned lazily since it depends
  // on the basic_type of a fully constructed ciType.
  bool     _is_loaded;
  bool     _is_loaded_initialized;
  uint     _ident;
...
protected:
  ...
  ciObject(FAMPtr old_cio);

  virtual void fixupFAMPointers();

  jobject      handle()  const { return _handle; }
  static bool is_fam_active();
  // Get the VM oop that this object holds.
  oop get_oop() const {
    assert( !UseFreezeAndMelt || !is_fam_active(), "FAM access not allowed");
...
objectRef get_ref()const;
  ...
  bool is_perm() const { return (_ident & PERM_FLAG) != 0; }
  void set_perm() {
    _ident |=  PERM_FLAG;
  }

  // Virtual behavior of the print() method.
virtual void print_impl(outputStream*out)const{}

virtual const char*type_string()const{return"ciObject";}
...
void print_oop(outputStream*st=tty)const;
};
