...
ciKlass* ciBytecodeStream::get_declared_method_holder() {
  bool ignore;
ciKlass*klass=CURRENT_ENV->get_klass_by_index(_holder,get_method_holder_index(),ignore);
if(klass->is_array_klass()){
    // methods on arrays are equivalent to methods on java.lang.Object
    klass = CURRENT_ENV->Object_klass();
  }
  return klass;
}
...
int ciBytecodeStream::get_method_holder_index() {
  if (FAM) {
    return _holder->get_klassref_by_cpool_index(get_method_index());
  }
  VM_ENTRY_MARK;
  constantPoolOop cpool = _holder->get_instanceKlass()->constants();
int klassref=cpool->klass_ref_index_at(get_method_index());
  _holder->add_klassref_by_cpool_index(get_method_index(), klassref);
  return klassref;
}
