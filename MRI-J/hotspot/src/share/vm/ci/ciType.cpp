...
ciType::ciType(FAMPtr old_type):ciObject(old_type){
  FAM->mapNewToOldPtr(this, old_type);

  _basic_type = (BasicType)FAM->getInt("((struct ciType*)%p)->_basic_type", old_type);
}

void ciType::fixupFAMPointers() {
...
void ciType::print_impl(outputStream*out)const{
out->print(" type=");
print_name(out);
}
...
void ciType::print_name(outputStream*out)const{
out->print(type2name(basic_type()));
}
...
//
// Implementation of the print method.
void ciReturnAddress::print_impl(outputStream*out)const{
out->print(" bci=%d",_bci);
}
