...
constantPoolHandle ClassFileParser::parse_constant_pool(TRAPS) {
  ...
  for (index = 1; index < length; index++) {          // Index 0 is unused
    if (!cp->is_within_bounds(index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
    switch (cp->tag_at(index).value()) {
      ...
      case JVM_CONSTANT_Methodref :
        // fall through
      case JVM_CONSTANT_InterfaceMethodref : {
        if (!cp->is_within_bounds(cp->klass_ref_index_at(index))) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
      ...
      case JVM_CONSTANT_Double :
        ...
        break;
      case JVM_CONSTANT_NameAndType : {
        if (!cp->is_within_bounds(cp->name_ref_index_at(index))) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
        if (!_need_verify) break;
      ...
      case JVM_CONSTANT_ClassIndex :
        {
          ...
          if (!cp->is_within_bounds(class_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
          check_property(
      ...
      case JVM_CONSTANT_StringIndex :
        {
          ...
          if (!cp->is_within_bounds(string_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
          check_property(
        ...
        break;
      case JVM_CONSTANT_Dependencyref:
        {
          ShouldNotReachHere();
        }
        break;
...
void ClassFileParser::parse_field_attributes(constantPoolHandle cp,
  ...
  while (attributes_count--) {
    ...
    if (!cp->is_within_bounds(attribute_name_index)) { THROW_MSG(vmSymbols::java_lang_ClassFormatError(), "malformed class file"); }
...
typeArrayHandle ClassFileParser::parse_fields(constantPoolHandle cp, bool is_interface, 
  ...
  for (int n = 0; n < length; n++) {
    ...
    if (!cp->is_within_bounds(name_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
    check_property(
    ...
    u2 signature_index = cfs->get_u2_fast();
    if (!cp->is_within_bounds(signature_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
    check_property(
...
u2*localvariable_table_length=0;
u2**localvariable_table_start=0;
u2*localvariable_type_table_length=0;
u2**localvariable_type_table_start=0;
  ...
  while (method_attributes_count--) {   
    ...
    if (!cp->is_within_bounds(method_attribute_name_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
    check_property(
    ...
    if (method_attribute_name == vmSymbols::tag_code()) {
      ...
      while (code_attributes_count--) {
        ...
        if (!cp->is_within_bounds(code_attribute_name_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
...
u2 ClassFileParser::parse_classfile_inner_classes_attribute(constantPoolHandle cp, instanceKlassHandle k, TRAPS) {  
  ...
  for (int n = 0; n < length; n++) {
    ...
    if (!cp->is_within_bounds(inner_class_info_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", 0); }
    check_property(
    ...
    if (!cp->is_within_bounds(outer_class_info_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", 0); }
    check_property(
    ...
    if (!cp->is_within_bounds(inner_name_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", 0); }
    check_property(
...
instanceKlassHandle ClassFileParser::parseClassFile(symbolHandle name, 
  ...
  if (!cp->is_within_bounds(this_class_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
  check_property(
  ...
  { HandleMark hm(THREAD);
    ...
    if (super_class_index == 0) {
      ...
      if (!cp->is_within_bounds(super_class_index)) { THROW_MSG_(vmSymbols::java_lang_ClassFormatError(), "malformed class file", nullHandle); }
...
vtable_size+itable_size)*wordSize;
...
int real_offset=0;
    ...
    // Size of instances
int instance_size=next_nonstatic_type_offset/wordSize;
...
tty->print("[Loaded %s",klass_name);
      if (cfs->source()) tty->print(" from %s", cfs->source());
      else if (class_loader.not_null()) tty->print(" from %s", instanceKlass::cast(class_loader->klass())->external_name());
      else if( THREAD->is_Java_thread() )
tty->print(" by instance of %s",
                   instanceKlass::cast(((JavaThread*)THREAD)->security_get_caller_class(1))->external_name());
      tty->print_cr("]");
    }
