...
Handle h=asd_klass->allocate_instance_handle(true/*SBA*/,CHECK_NULL);

  int len;
  typeArrayOop t;
  len = OptionList::count(_packages);
objArrayOop pn=oopFactory::new_objArray(SystemDictionary::string_klass(),len,true/*SBA*/,CHECK_NULL);
  objArrayHandle pkgNames (THREAD, pn);
t=oopFactory::new_typeArray(T_BOOLEAN,len,true/*SBA*/,CHECK_NULL);
...
objArrayOop cn=oopFactory::new_objArray(SystemDictionary::string_klass(),len,true/*SBA*/,CHECK_NULL);
...
Handle s=java_lang_String::create_from_str(p->name(),true/*SBA*/,CHECK);
