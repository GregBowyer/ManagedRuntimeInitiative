...
#ifndef JAVACLASSES_HPP
#define JAVACLASSES_HPP
...
hc_hash_offset=1
  ...
  static Handle basic_create(int length, bool tenured, intptr_t sba_hint, TRAPS);
  static Handle basic_create_from_unicode(jchar* unicode, int length, bool tenured, intptr_t sba_hint, TRAPS);

 public:
  // Instance creation
static Handle create_from_unicode(jchar*unicode,int len,intptr_t sba_hint,TRAPS);
  static Handle create_tenured_from_unicode(jchar* unicode, int len, TRAPS);
static oop create_oop_from_unicode(jchar*unicode,int len,intptr_t sba_hint,TRAPS);
static Handle create_from_str(const char*utf8_str,intptr_t sba_hint,TRAPS);
static oop create_oop_from_str(const char*utf8_str,intptr_t sba_hint,TRAPS);
static Handle create_from_symbol(symbolHandle symbol,intptr_t sba_hint,TRAPS);
  ...
  static objectRef* value_addr(oop string) { return (objectRef*)(((char*)string)+value_offset_in_bytes()); }
  ...
  static int length(oop java_string) {
    typeArrayOop v  = value(java_string);
if(v==NULL)return 0;
return v->length();
  }
...
static bool GC_weak_equals(oop java_string,jchar*chars,int len);
...
class java_lang_Class : AllStatic {
 ...
 public:
  ...
  static klassRef as_klassRef(objectRef java_class);
  // Testing
...
class java_lang_Thread : AllStatic {
 ...
 public:
  ...
  // Priority
static JavaThreadPriority priority(oop java_thread);
static void set_priority(oop java_thread,JavaThreadPriority priority);
  // Thread group
  static oop  threadGroup(oop java_thread);
  static void set_threadGroup(oop java_thread, oop thread_group);
  // Stillborn
...
static JavaThreadPriority maxPriority(oop java_thread_group);
  // Destroyed
...
class java_lang_Throwable: AllStatic {
 ...
 private:
  ...
  static void clear_stacktrace(objectRef throwable);
  // No stack trace available
  static const char* no_stack_trace_message();

 public:
  // Backtrace
static objArrayRef backtrace(objectRef throwable);
  static void set_backtrace(objectRef throwable, objectRef value);
  ...
  // Message
  static objectRef message(objectRef throwable);
  static objectRef message(Handle throwable);
  static void set_message(objectRef throwable, objectRef value);
  ...
  static void print_stack_trace(objectRef throwable, objectRef print_stream);
  ...
  static objectRef get_stack_trace_element(objectRef throwable, int index, TRAPS);
  static int  get_stack_trace_depth(objectRef throwable, TRAPS);
  // Printing
  static void print(objectRef throwable, outputStream* st);
  static void print(Handle throwable, outputStream* st);
  static void print_stack_trace(objectRef throwable, outputStream* st);
  // Debugging
  friend class JavaClasses;
};
...
class java_lang_reflect_Method : public java_lang_reflect_AccessibleObject {
 ...
 public:
  // Allocation
static Handle create(intptr_t sba_hint,TRAPS);
...
hc_queue_offset=2,
hc_next_offset=3,
hc_discovered_offset=4//Is not last, see SoftRefs.
  };
  enum {
   hc_static_lock_offset         = 0,
   hc_static_pending_list_offset = 1
  };
  ...
  static int static_pending_list_offset;
  static int number_of_fake_oop_fields;
 
  // Accessors
static oop referent(oop ref){return lvb_ref(referent_addr(ref)).as_oop();}
  static void set_referent(oop ref, oop value);
static heapRef*referent_addr(oop ref);//No stack refs

static oop pending(oop ref){return lvb_ref(pending_addr(ref)).as_oop();}
static void set_pending(oop ref,oop value);
static objectRef*pending_addr(oop ref);

  static oop next(oop ref)            { return lvb_ref(next_addr(ref)).as_oop(); }
  static void set_next(oop ref, oop value);
  static heapRef* next_addr(oop ref);

  static oop discovered(oop ref)      { return lvb_ref(discovered_addr(ref)).as_oop(); }
  static void set_discovered(oop ref, oop value);
  static heapRef* discovered_addr(oop ref);

  // Accessors for statics
static oop pending_list_lock(){return lvb_ref(pending_list_lock_addr()).as_oop();}
static oop pending_list(){return lvb_ref(pending_list_addr()).as_oop();}

  static heapRef* pending_list_lock_addr();
  static heapRef* pending_list_addr();
};
...
class java_lang_StackTraceElement: AllStatic {
 ...
 public:
  ...
  static objectRef create(methodHandle m, int bci, TRAPS);
...
class JavaClasses : AllStatic {
 ...
 public:
  static bool check_offset(instanceKlassHandle ikh, const char *klass_name, int offset, const char *field_name, const char* field_sig);
  static void compute_hard_coded_offsets();
  static void compute_offsets();
  static void check_offsets() PRODUCT_RETURN;
};

#endif // JAVACLASSES_HPP
