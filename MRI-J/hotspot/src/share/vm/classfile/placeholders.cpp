...
PlaceholderEntry* PlaceholderTable::find_and_add(int index, unsigned int hash, symbolHandle name, Handle loader, classloadAction action, symbolHandle supername, Thread* thread) {
  ...
  if (probe == NULL) {
    ...
    if (action == LOAD_SUPER) {
      ...
      probe->set_supername(POISON_SYMBOLREF(symbolRef(supername())));
    }
  }
...
void PlaceholderEntry::oops_do(OopClosure* blk) {
  ...
  blk->do_oop((objectRef*)klass_addr());
if(_loader.not_null()){
    blk->do_oop(loader_addr());
  }
if(_supername.not_null()){
    blk->do_oop((objectRef*)supername_addr());
  }
if(_instanceKlass.not_null()){
    blk->do_oop((objectRef*)instanceKlass_addr());
  }
}
...
void PlaceholderEntry::verify() const {
  oop placeholder_loader = loader();
guarantee(placeholder_loader==NULL||placeholder_loader->is_instance(),
