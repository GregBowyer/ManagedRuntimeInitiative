...
class PlaceholderEntry : public HashtableEntry {
 ...
 private:
  objectRef         _loader;        // initiating loader
  bool              _havesupername; // distinguish between null supername, and unknown
  symbolRef         _supername;
  Thread*           _definer;       // owner of define token
  klassRef          _instanceKlass; // instanceKlass from successful define
...
symbolOop klass()const{return(symbolOop)literal().as_oop();}
  symbolOop*         klass_addr()          { return (symbolOop*)literal_addr(); }

  oop                loader()              const { return lvb_ref(&_loader).as_oop(); }
  void               set_loader(objectRef loader) { _loader = loader; }
  ...
  symbolOop          supername()           const { return lvb_symbolRef(&_supername).as_symbolOop(); }
  void               set_supername(symbolRef supername) { _supername = supername; }
  symbolRef*         supername_addr()      { return &_supername; }

  Thread*            definer()             const {return _definer; }
  void               set_definer(Thread* definer) { _definer = definer; }

  klassOop           instanceKlass()     const {return lvb_klassRef(&_instanceKlass).as_klassOop(); }
  void               set_instanceKlass(klassRef instanceKlass) { _instanceKlass = instanceKlass; }
  klassRef*          instanceKlass_addr()   { return &_instanceKlass; }
