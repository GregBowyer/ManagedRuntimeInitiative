...
#ifndef VERIFICATIONTYPE_HPP
#define VERIFICATIONTYPE_HPP
...
class VerificationType VALUE_OBJ_CLASS_SPEC {
  private:
    ...
    enum { 
      ...
      BciForThis         = ((u2)InvocationEntryBci),   // A bci of InvocationEntryBci is an Unintialized-This
 ...
 public:
  ...
  symbolOop name() const { 
    assert(is_reference() && !is_null(), "Must be a non-null reference");
return lvb_symbolRef(_u._handle).as_symbolOop();
  }
