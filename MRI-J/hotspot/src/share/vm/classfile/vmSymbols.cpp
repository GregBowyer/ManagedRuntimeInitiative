...
symbolRef vmSymbols::_symbols[vmSymbols::SID_LIMIT];

symbolRef vmSymbols::_type_signatures[T_VOID+1] = { symbolRef((uint64_t)NULL) /*, NULL...*/ };

inline int compare_symbol(symbolRef a, symbolRef b) {
  symbolOop aoop = lvb_symbolRef(&a).as_symbolOop();
  symbolOop boop = lvb_symbolRef(&b).as_symbolOop();
  if (aoop == boop)  return 0;
  return (intptr_t)aoop > (intptr_t)boop ? +1 : -1;
}
...
extern "C" {
  static int compare_vmsymbol_sid(const void* void_a, const void* void_b) {
    symbolRef a = vmSymbols::symbol_at(*((vmSymbols::SID*) void_a));
    symbolRef b = vmSymbols::symbol_at(*((vmSymbols::SID*) void_b));
...
void vmSymbols::initialize(TRAPS) {
  ...
  for (int index = (int)FIRST_SID; index < (int)SID_LIMIT; index++) {
    ...
    _symbols[index] = POISON_SYMBOLREF(symbolRef(sym));
    string += strlen(string); // skip string body
    string += 1;              // skip trailing null
  }

  for (int i = 0; i < T_VOID+1; i++) {
    _type_signatures[i] = nullRef;
  }
  _type_signatures[T_BYTE]    = POISON_SYMBOLREF(symbolRef(byte_signature()));
  _type_signatures[T_CHAR]    = POISON_SYMBOLREF(symbolRef(char_signature()));
  _type_signatures[T_DOUBLE]  = POISON_SYMBOLREF(symbolRef(double_signature()));
  _type_signatures[T_FLOAT]   = POISON_SYMBOLREF(symbolRef(float_signature()));
  _type_signatures[T_INT]     = POISON_SYMBOLREF(symbolRef(int_signature()));
  _type_signatures[T_LONG]    = POISON_SYMBOLREF(symbolRef(long_signature()));
  _type_signatures[T_SHORT]   = POISON_SYMBOLREF(symbolRef(short_signature()));
  _type_signatures[T_BOOLEAN] = POISON_SYMBOLREF(symbolRef(bool_signature()));
  _type_signatures[T_VOID]    = POISON_SYMBOLREF(symbolRef(void_signature()));
  // no single signatures for T_OBJECT or T_ARRAY

#ifdef ASSERT
  // Check for duplicates:
  for (int i1 = (int)FIRST_SID; i1 < (int)SID_LIMIT; i1++) {
symbolOop sym=symbol_at((SID)i1).as_symbolOop();
...
assert(_symbols[NO_SID].is_null(),"must be");
    ...
    for (int index = (int)FIRST_SID; index < (int)SID_LIMIT; index++) {
sym=lvb_symbolRef(&symbol_at((SID)index)).as_symbolOop();
      sid = find_sid(sym);
...
void vmSymbols::oops_do(OopClosure* f, bool do_all) {
  for (int index = (int)FIRST_SID; index < (int)SID_LIMIT; index++) {
    f->do_oop((objectRef*) &_symbols[index]);
  }
  for (int i = 0; i < T_VOID+1; i++) {
if(_type_signatures[i].not_null()){
      assert(i >= T_BOOLEAN, "checking");
      f->do_oop((objectRef*)&_type_signatures[i]);
    } else if (do_all) {
      f->do_oop((objectRef*)&_type_signatures[i]);
    }
  }
...
BasicType vmSymbols::signature_type(symbolOop s) {
  ...
  symbolRef sr = symbolRef(s);
  for (int i = T_BOOLEAN; i < T_VOID+1; i++) {
if(sr==lvb_ref(&_type_signatures[i])){
      return (BasicType)i;
    }
  }
  return T_OBJECT;
...
vmSymbols::SID vmSymbols::find_sid(symbolOop symbol) {
  ...
  if ((uint)++find_sid_check_count > (uint)100) {
    ...
    SID sid2 = NO_SID;
    for (int index = (int)FIRST_SID; index < (int)SID_LIMIT; index++) {
symbolOop sym2=lvb_symbolRef(&symbol_at((SID)index)).as_symbolOop();
      if (sym2 == symbol) {
        sid2 = (SID)index;
        break;
      }
...
void vmIntrinsics::verify_method(ID actual_id, methodOop m) {
  ...
  ttyLocker ttyl;
if(tty!=NULL){
tty->print("intrinsic_misdeclared actual='%s' declared='%s'",
