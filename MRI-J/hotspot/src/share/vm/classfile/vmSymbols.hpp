...
#ifndef VMSYMBOLS_HPP
#define VMSYMBOLS_HPP
...
#define VM_SYMBOLS_DO(template, do_alias)                                                         \
  ...
  template(java_io_ObjectInputStream,                 "java/io/ObjectInputStream")                \
  ...
  template(slot_name,                                 "slot")                                     \
                                                                                                  \
  template(com_azulsystems_util_Prefetch,             "com/azulsystems/util/Prefetch")            \
  template(com_azulsystems_misc_BlockingHint,         "com/azulsystems/misc/BlockingHint")        \
  template(com_azulsystems_misc_BlockingHint_message_name,      "_msg")                                                  \
  template(com_azulsystems_misc_BlockingHint_lock_name,         "_lock")                                                 \
  template(com_azulsystems_misc_BlockingHint_sync_name,         "_sync")                                                 \
  template(com_azulsystems_misc_BlockingHint_nanos_name,        "_nanos")                                                \
  template(java_lang_Thread_blockingHint_name,             "blockingHint")                                          \
                                                                                                                         \
...
#define VM_INTRINSICS_DO(do_intrinsic, do_class, do_name, do_signature, do_alias)                                       \
  ...
  do_intrinsic(_clone,                    java_lang_Object,       clone_name, void_object_signature,             F_R)   \
   do_name(     clone_name,                                      "clone")                                               \
                                                                                                                        \
  do_intrinsic(_concurrentGetReferent,    java_lang_ref_Reference,concurrentGetReferent_name, void_object_signature, F_RN) \
do_name(concurrentGetReferent_name,"concurrentGetReferent")\
                                                                                                                        \
...
do_name(tan_name,"tan")do_name(atan2_name,"atan2")do_name(sqrt_name,"sqrt")do_name(sqrtf_name,"sqrtf")\
...
do_intrinsic(_fsqrt,java_lang_Math,sqrtf_name,float_float_signature,F_S)\
...
do_intrinsic(_stringEquals,java_lang_String,stringEquals_name,object_boolean_signature,F_R)\
do_name(stringEquals_name,"equals")\
  ...
  do_intrinsic(_attemptUpdate,            sun_misc_AtomicLongCSImpl, attemptUpdate_name, attemptUpdate_signature, F_R)  \
   ...
   do_signature(attemptUpdate_signature,                            "(JJ)Z")                                            \
                                                                                                                        \
  do_intrinsic(_shared,                   com_azulsystems_util_Prefetch, shared_name,    shared_signature,        F_S)  \
do_name(shared_name,"shared")\
   do_alias   (shared_signature,                                    prefetch_signature)                                 \
                                                                                                                        \
  do_intrinsic(_exclusive,                com_azulsystems_util_Prefetch, exclusive_name, exclusive_signature,     F_S)  \
do_name(exclusive_name,"exclusive")\
   do_alias   (exclusive_signature,                                 prefetch_signature)                                 \
                                                                                                                        \
  do_intrinsic(_internal_set,             com_azulsystems_misc_BlockingHint, internal_set_name, internal_set_signature,     F_S)  \
do_name(internal_set_name,"internal_set")\
do_signature(internal_set_signature,"(Ljava/lang/String;Ljava/lang/Object;Ljava/lang/Object;)V")\
                                                                                                                        \
...
class vmSymbols: AllStatic {
 ...
 private:
  // The symbol array
  static symbolRef _symbols[];

  // Field signatures indexed by BasicType.
  static symbolRef _type_signatures[T_VOID+1];
...
static symbolOop name(){return(symbolOop)lvb_ref(&_symbols[VM_SYMBOL_ENUM_NAME(name)]).as_oop();}
...
assert(_type_signatures[t].not_null(),"domain check");
return(symbolOop)lvb_ref(&_type_signatures[t]).as_oop();
  }
  // inverse of type_signature; returns T_OBJECT if s is not recognized
  static BasicType signature_type(symbolOop s);

  static symbolRef symbol_at(SID id) {
    assert(id >= FIRST_SID && id < SID_LIMIT, "oob");
assert(!_symbols[id].is_null(),"init");
return UNPOISON_SYMBOLREF(_symbols[id]);
  }
...
class vmSymbolHandles: AllStatic {
 ...
 public:
  ...
  static int symbol_handle_count() { return (int)vmSymbols::SID_LIMIT; }

  static symbolHandle symbol_handle_at(vmSymbols::SID id) {
assert(id>=vmSymbols::FIRST_SID&&id<vmSymbols::SID_LIMIT,"oob");
