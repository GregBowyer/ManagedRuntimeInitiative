...
#ifndef EXCEPTIONHANDLERTABLE_HPP
#define EXCEPTIONHANDLERTABLE_HPP
...
// relative to the CodeBlob code start for the compiled exception
...
// the CatchNode in the corresponding CodeBlob. Empty subtables are dis-
// carded.
//
...
// CodeBlob (for lookup purposes) in which case the table cannot be
// modified.
...
class ExceptionHandlerTable VALUE_OBJ_CLASS_SPEC {
 ...
 public:
  ...
  // (run-time) construction from CodeBlob
  ExceptionHandlerTable(const CodeBlob* cb);
  ...
  void copy_to(CodeBlob* cb);

  // lookup
  HandlerTableEntry* entry_for(int catch_pco, int handler_bci, int scope_depth) const;

  // debugging
void print_subtable(HandlerTableEntry*t,outputStream*out)const;
  void print(outputStream* out) const;
  void print_subtable_for(int catch_pco, outputStream* out) const;
};
...
// CodeBlob a zero length table takes no space.  This is detected by
...
public:
  ...
  // (run-time) construction from CodeBlob
  ImplicitExceptionTable( const CodeBlob *cb );
  ...
  void copy_to(CodeBlob* cb);
  void print(address base, outputStream* out) const;
  void verify(CodeBlob *cb) const;
};

class ImplicitRangeCheckTable VALUE_OBJ_CLASS_SPEC{
  uint _size;
  uint _len;
  implicit_null_entry *_data;
  implicit_null_entry *adr( uint idx ) const { return &_data[2*idx]; }
  ReallocMark          _nesting;  // assertion check for reallocations
public:
ImplicitRangeCheckTable():_data(0),_size(0),_len(0){}
  // (run-time) construction from CodeBlob
  ImplicitRangeCheckTable( const CodeBlob *cb );
...
#endif // EXCEPTIONHANDLERTABLE_HPP
