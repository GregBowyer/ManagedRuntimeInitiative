...
CheckForUnmarkedOops(PSYoungGen*young_gen,CardTableExtension*card_table,oop container):
_young_gen(young_gen),_card_table(card_table),_container(container),_unmarked_addr(NULL){}

  virtual void do_oop(objectRef* p) {
    oop oop_to_check = ALWAYS_UNPOISON_OBJECTREF(*p).as_oop();
    if (_young_gen->is_in_reserved(oop_to_check) && 
...
CheckForUnmarkedOops object_check(_young_gen,_card_table,obj);
    obj->oop_iterate(&object_check);
    if (object_check.has_unmarked_oop()) {
if(_card_table->addr_is_marked_imprecise(obj)){
tty->print_cr("Warning, found imprecise card mark at addr %p unmarked card %p",
                      object_check._unmarked_addr, object_check._unmarked_card);
      } else {
        tty->print_cr("Unmarked addr %p unmarked card %p", object_check._unmarked_addr, object_check._unmarked_card);
tty->print_cr("\nObject pointed at\n");
        oop pointee = ALWAYS_UNPOISON_OBJECTREF(*(objectRef*)(object_check._unmarked_addr)).as_oop();
pointee->print(tty);
tty->print_cr("\nObject containing unmarked pointer\n");
obj->print(tty);
assert(false,"Found unmarked young_gen object");
      }
    }
  }
};
...
void CardTableExtension::scavenge_contents_parallel(ObjectStartArray* start_array,
  ...
  for (jbyte* slice = start_card; slice < end_card; slice += ssize*ParallelGCThreads) {
    ...
    if (!start_array->object_starts_in_range(slice_start, slice_end < (HeapWord*)sp_top ? slice_end : (HeapWord*)sp_top-1)) {
      continue;
    }
...
void CardTableExtension::resize_commit_uncommit(int changed_region,
  ...
  // AZUL - cannot have overlapping regions.
  // So ensure new_committed_start_aligned starts atmost from the previous
  // lower committed region's end.
  if (UseAdaptiveGCBoundary) {
    if (changed_region > 0) {
MemRegion prev_committed=_committed[changed_region-1];
      if (new_start_aligned < prev_committed.end()) {
        new_start_aligned = prev_committed.end();
      }
    } 
  }
  ...
  if (new_start_aligned < cur_committed.start()) {
    ...
    if(!new_committed.is_empty()) {
      if (!os::commit_memory((char*)new_committed.start(),
                             new_committed.byte_size(), Modules::ParGC_CardTableExtension)) {
...
os::uncommit_memory((char*)uncommit_region.start(),uncommit_region.byte_size(),Modules::ParGC_CardTableExtension);
    }
  }
