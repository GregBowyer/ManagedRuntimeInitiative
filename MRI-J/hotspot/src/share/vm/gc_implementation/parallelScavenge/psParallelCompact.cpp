...
#include "ostream.hpp"
#include "pcTasks.hpp"
#include "psCompactionManager.hpp"
...
verify_mark_bitmap(ParMarkBitMap& _mark_bitmap)
{
  ...
  do {
    ...
    if (_mark_bitmap.is_marked(beg_bit)) {
      oop obj = (oop)addr;
assert(obj->mark()->is_marked(),"obj header is not marked");
...
assert(!obj->mark()->is_marked(),"obj marked in header, not in bitmap");
	tmp_bit = beg_bit + 1;
...
const size_t page_sz=os::vm_page_size();
  ...
  ReservedSpace rs(bytes, (char*) __PARALLEL_COMPACT_START_ADDR__, 0);
...
void PSParallelCompact::KeepAliveClosure::do_oop(objectRef* p) {
mark_and_push(_compaction_manager,(heapRef*)p);
}
...
"dp="PTR_FORMAT" ""cdw="SIZE_FORMAT_W(08),
					...
					bool maximum_compaction)
{
  ...
  if (TraceParallelOldGCDensePrefix) {
    tty->print_cr("space_live=" SIZE_FORMAT " " "space_used=" SIZE_FORMAT " "
		  ...
		  space_live, space_used,
		  space_capacity);
tty->print_cr("dead_wood_limiter(%6.4f, %ld)=%6.4f "
				      ...
				      bool maximum_compaction)
{
  ...
  //LoggerMark m(NOTAG, Log::M_GC, "2 summarize");
  BufferedLoggerMark m(NOTAG, Log::M_GC, PrintGCDetails && PrintParallelOldGCPhaseTimes, true, gclog_or_tty);
  TraceTime tm("summary phase", m.enabled(), true, m.stream());
m.enter();
  // trace(" 2");
...
#ifndef PRODUCT
    if (TraceParallelOldGCMarkingPhase) {
gclog_or_tty->print_cr("marking_phase: cas_tries %zd  cas_retries %zd "
"cas_by_another %zd",
...
assert(DerivedPointerTable::is_active(),"Sanity");
    ...
    // klassID in the markOop to get their size. Also, we can't do this after we
    // prepare perm gen for compaction since this clears the GC mark in the
    // markOop of all Klasses which aren't moving so all these Klasses would
    // appear dead.
KlassTable::unlink(is_alive_closure());
...
gclog_or_tty->print_cr("VM-Thread %lld %lld %lld",
...
ChunkTaskQueueSet*qset=ParCompactionManager::chunk_array();
  ...
  BufferedLoggerMark m2(NOTAG, Log::M_GC, PrintGCDetails && PrintParallelOldGCPhaseTimes, true, gclog_or_tty);
  m2.enter();
  {
...
q->enqueue(new MarkFromRootsTask(MarkFromRootsTask::arta_objects));
...
void PSParallelCompact::adjust_roots() {
  ...
  //LoggerMark m(NOTAG, Log::M_GC, "3 adjust roots");
  BufferedLoggerMark m(NOTAG, Log::M_GC, PrintGCDetails && PrintParallelOldGCPhaseTimes, true, gclog_or_tty);
  TraceTime tm("adjust roots", m.enabled(), true, m.stream());
m.enter();
  ...
  // NOTE! ArtaObjects are not normal roots. During scavenges, they are considered
  // strong roots. During a mark sweep they are weak roots.
ArtaObjects::oops_do(adjust_root_pointer_closure());
...
CodeCache::oops_do(adjust_root_pointer_closure());
CodeCacheOopTable::oops_do(adjust_root_pointer_closure());
KlassTable::oops_do(adjust_root_pointer_closure());
}

void PSParallelCompact::compact_perm(ParCompactionManager* cm) {
  //LoggerMark m(NOTAG, Log::M_GC, "4 compact perm");
  BufferedLoggerMark m(NOTAG, Log::M_GC, PrintGCDetails && PrintParallelOldGCPhaseTimes, true, gclog_or_tty);
  TraceTime tm("compact perm gen", m.enabled(), true, m.stream());
m.enter();
  // trace("4");
...
gclog_or_tty->print_cr("%zu initially fillable chunks",fillable_chunks);
  }
}
...
void PSParallelCompact::compact() {
  //LoggerMark m(NOTAG, Log::M_GC, "5 compact");
  BufferedLoggerMark m(NOTAG, Log::M_GC, PrintGCDetails && PrintParallelOldGCPhaseTimes, true, gclog_or_tty);
...
void PSParallelCompact::compact_serial(ParCompactionManager* cm) {
  //LoggerMark m(NOTAG, Log::M_GC, "5 compact serial");
  BufferedLoggerMark m(NOTAG, Log::M_GC, PrintGCDetails && PrintParallelOldGCPhaseTimes, true, gclog_or_tty);
  TraceTime tm("compact serial", m.enabled(), true, m.stream());
m.enter();
...
void PSParallelCompact::follow_root(ParCompactionManager* cm, heapRef* p) {
  assert(!Universe::heap()->is_in_reserved(p),
...
void PSParallelCompact::adjust_pointer(heapRef* p, bool isroot) {
if(p->not_null()){
    heapRef r = UNPOISON_OBJECTREF(*p, p);
oop obj=r.as_oop();
...
PSParallelCompact::update_and_deadwood_in_dense_prefix(ParCompactionManager* cm,
  ...
  if (beg_addr < end_addr) {
    ...
    //FIXME:  The following line generates a -Warray-bounds warning when 
    // compilere -DASSERT -O3.  I haven't been able to figure out why.
    // I think it is complaining that space_id is out of bounds when used
    // in PSParallelCompact::dense_prefix in the expressions _space_info[id].
    //
    // Note:  I think this is probably a real error because this warning is
    // generatd only when the compiler can prove the array index is out
    // of bounds.
...
gclog_or_tty->print_cr("first_block 0x%zx"
" _offset 0x%x"
      " _first_is_start_bit %d",
      first_block, 
...
void PSParallelCompact::check_kid(oop obj, unsigned int kid) {
