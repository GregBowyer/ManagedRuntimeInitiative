...
#ifndef PSPARALLELCOMPACT_HPP
#define PSPARALLELCOMPACT_HPP


#include "objectStartArray.hpp"
#include "parMarkBitMap.hpp"
...
public:
 ...
 // Mask for the bits in a pointer to get the address of the start of a chunk.
  ...
  public:
    ...
    // destination_count to the claimed value.  After a chunk has
...
assert(destination_count()>0,"count must not go negative");
  Atomic::add_ptr((intptr_t)dc_mask, (volatile intptr_t*)&_dc_and_los);
}
...
Atomic::add_ptr((intptr_t)words,(volatile intptr_t*)&_dc_and_los);
}
...
void do_oop(objectRef*p){follow_root(_compaction_manager,(heapRef*)p);}
...
void do_oop(objectRef*p){adjust_pointer((heapRef*)p,_is_root);}
  };
  ...
  static void mark_and_push_internal(ParCompactionManager* cm, heapRef* p);
 ...
 private:
  ...
  static void adjust_pointer(heapRef* p, bool is_root);
  static void adjust_root_pointer(heapRef* p) { adjust_pointer(p, true); }

  static void check_kid(oop, unsigned int);

  static void follow_root(ParCompactionManager* cm, heapRef* p);
...
void do_oop(objectRef*p){mark_and_push(_compaction_manager,(heapRef*)p);}
    virtual const bool do_nmethods() const { return true; }
  };
  ...
  // Marking support
  static inline bool mark_obj(heapRef obj);
  static bool mark_obj(heapRef* p)  { 
    if (UNPOISON_OBJECTREF(*p, p).not_null()) {
      return mark_obj(*p); 
    } else {
      return false;
    }
  }
  static void mark_and_push(ParCompactionManager* cm, heapRef* p) {     
  ...
  static bool is_unmarked_and_discover_reference(ParCompactionManager* cm, objectRef referent, oop obj, ReferenceType type) {
    return ( PSParallelCompact::mark_bitmap()->is_unmarked(referent.as_oop()) &&
             PSParallelCompact::ref_processor()->discover_reference(obj, type) );
  }

  // Compaction support.
  // Return true if p is in the range [beg_addr, end_addr).
  static inline bool is_in(HeapWord* p, HeapWord* beg_addr, HeapWord* end_addr);
  static inline bool is_in(heapRef* p, HeapWord* beg_addr, HeapWord* end_addr);
  ...
  static void mark_and_follow(ParCompactionManager* cm, heapRef* p);    

  static ParMarkBitMap* mark_bitmap() { return &_mark_bitmap; }
  static ParallelCompactData& summary_data() { return _summary_data; }

  static inline void adjust_pointer(heapRef* p) { adjust_pointer(p, false); }
  static inline void adjust_pointer(heapRef* p,
				    HeapWord* beg_addr,
				    HeapWord* end_addr);
...
bool PSParallelCompact::mark_obj(heapRef p){
oop obj=p.as_oop();
...
inline bool
PSParallelCompact::is_in(heapRef* p, HeapWord* beg_addr, HeapWord* end_addr) {
...
inline void PSParallelCompact::adjust_pointer(heapRef* p,
					      HeapWord* beg_addr,
					      HeapWord* end_addr) {
