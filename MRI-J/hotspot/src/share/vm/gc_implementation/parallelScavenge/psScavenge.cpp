...
elapsedTimer PSScavenge::_accumulated_gc_time;
elapsedTimer PSScavenge::_accumulated_undo_time;
GrowableArray<markWord*>*  PSScavenge::_preserved_mark_stack = NULL;
GrowableArray<oop>*        PSScavenge::_preserved_oop_stack = NULL;
CollectorCounters*         PSScavenge::_counters = NULL;
unsigned int PSScavenge::_total_invocations=0;
unsigned int               PSScavenge::_total_promotion_failures = 0;
...
public:
  ...
  void do_oop(objectRef* p) {
    assert0(objectRef::is_null_or_heap(p));
    do_ref((heapRef*)p);
  }

void do_ref(heapRef*p){
    assert (p->not_null(), "expected non-null ref");
    assert (ALWAYS_UNPOISON_OBJECTREF(*p).as_oop()->is_oop(), "expected an oop while scanning weak refs");
  
    // Weak refs may be visited more than once.
if(PSScavenge::should_scavenge(ALWAYS_UNPOISON_OBJECTREF(*p),_to_space)){
...
private:
virtual const char*name(){return(char*)"Process referents by policy in parallel";}
...
public:
  ...
  virtual const char* name() { return (char *)"Enqueue reference objects in parallel"; }
  virtual void do_it(GCTaskManager* manager, uint which)
  {
...
// fails. Some markWords will need preserving, some will not. Note
...
void PSScavenge::oop_promotion_failed(oop obj,markWord*obj_mark){
...
_tenuring_threshold=markWord::max_age+1;
  } else {
