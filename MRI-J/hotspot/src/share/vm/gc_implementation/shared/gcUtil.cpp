...
double AdaptiveWeightedAverage::compute_adaptive_average(double new_sample, 
                                                         double average) {
  ...
  double new_avg = exp_avg(average, new_sample, adaptive_weight);

  return new_avg;
}

void AdaptiveWeightedAverage::sample(double new_sample) {
  increment_count();
  assert(count() != 0,
         "Wraparound -- history would be incorrectly discarded");

  // Compute the new weighted average
double new_avg=compute_adaptive_average(new_sample,average());
  set_average(new_avg);
...
void AdaptivePaddedNoZeroDevAverage::sample(double new_sample) {
  // Compute our parent classes sample information
  AdaptiveWeightedAverage::sample(new_sample);

  double new_avg = average();
  if (new_sample != 0) {
