...
#ifndef COLLECTEDHEAP_HPP
#define COLLECTEDHEAP_HPP
...
class CollectedHeap : public CHeapObj {
  ...
  friend class StackBasedAllocation; // access to allocate_new_tlab
#ifdef ASSERT
 ...
 protected:
  ...
  void ensure_prezero(); // reverse procedure by zero'ing over filler objects
  ...
  // Used to give control over tlab sizing to the collector.
  // Return tlab size or remaining space in eden such that the
  // space is large enough to hold obj_size and alignment_reserve.
  // Otherwise return 0. This method is a bit of a hack.
  virtual size_t compute_tlab_size(Thread *thread, size_t requested_free_size, size_t obj_size, size_t alignment_reserve);

  debug_only(static void check_for_valid_allocation_state();)

 public:
  // Values for the zero_mem parameter of various CollectedHeap methods:
  enum {
    ZeroMemory     = true,
    DontZeroMemory = false
  };
  ...
  // If size is large or OOM-after-GC, return NULL.  If zero_mem==true, the block returned
  // by a successful allocation is initialized to zeros.
  inline static HeapWord* allocate_chunk_from_tlab(Thread* thread, size_t size, bool zero_mem);
  // Allocate a single object of size < Universe::heap()->max_tlab_size().
  // If size is large or OOM-after-GC, return NULL.  If zero_mem==true, the block returned
  // by a successful allocation is initialized to zeros.
  inline static HeapWord* allocate_from_tlab(Thread* thread, size_t size, bool zero_mem);
  // Broken-out slow path.  If zero_mem==true, the block returned by a successful allocation
  // is initialized to zeros.
  static HeapWord* allocate_from_tlab_slow(Thread* thread, size_t size, bool zero_mem);

 protected:
  // Allocate a block of the given size, or returns NULL if this is impossible.
  // If zero_mem==true, the block returned by a successful allocation, aside from the oop header,
  // is initialized to zeros.
  inline static HeapWord* common_mem_allocate(size_t size, bool is_noref, bool zero_mem, TRAPS);
...
inline static void post_allocation_setup_common(KlassHandle klass,HeapWord*obj,size_t size);
...
inline static HeapWord*init_obj(HeapWord*obj,size_t size);
...
size_t reserved_obj_bytes()const{
...
virtual bool is_maximal_no_gc(){return false;};
  ...
  // Management data collection would like to know memory usage values from
  // the most recent GC cycle.
  virtual size_t last_gc_live_bytes() const = 0;
  ...
  // Clears an allocated object with periodic safepoints for large objects.
  virtual HeapWord* incremental_init_obj(HeapWord* obj, size_t size);
...
inline static oop obj_allocate_noinit(KlassHandle klass,int size,TRAPS);
inline static oop array_allocate_noinit(KlassHandle klass,int size,int length,TRAPS);
inline static oop large_typearray_allocate_noinit(KlassHandle klass,int size,int length,TRAPS);
  ...
  // Post TLAB-allocation oop handling.  Oops are not yet initialized when this is called.
  virtual void tlab_allocation_mark_new_oop(HeapWord* obj) {}
  ...
  // Some heaps may support "pinning" objects in place, so that the jni_Get*Critical functions
  // don't need to acquire the GC_locker jni_critical lock.
  virtual bool jni_critical_pin_object  (oop obj) { return false; }
  virtual bool jni_critical_unpin_object(oop obj) { return false; }
  ...
  // Returns the maximum size of a tlab.  Used by SBA to allocate large
  // chunks of memory for copying objects into.  Allocations of this length
  // or below should behave properly given an allocation sequence which bypasses
  // the standard allocators and uses a bump pointer.
  virtual size_t max_tlab_size() = 0;
  ...
  // In a generational collector, for example, this is probably the largest
...
virtual void oop_iterate(OopClosure*cl){return;}

  // Iterate over all objects, calling "cl.do_object" on each.
  // This includes objects in permanent memory.
virtual void object_iterate(ObjectClosure*cl){return;}
...
virtual HeapWord*block_start(const void*addr){return NULL;};
  virtual HeapWord* block_start_debug(const void* addr) { return NULL; } ; // A more robust version for debug printouts
...
virtual bool block_is_obj(const HeapWord*addr){return false;};
  ...
  virtual void post_verify() = 0;

  virtual void print() const = 0;
  virtual void print_on(outputStream* st) const = 0;
  virtual void print_xml_on(xmlBuffer *xb, bool ref = true) const = 0;
...
virtual void print_tracing_info(){return;};
  ...
  ///////////////////////////////////////////////
  //
  // New Heap methods to support Azul's features.
  //

  // Return the correct space ID for the oop.
  virtual uint64_t space_id_for_address(const void* addr) const = 0;

  //
  // During safepoints, the vm thread may ask the heap to protect itself
  // from writes. This is used to help catch threads that are writing into
  // the heap when they should not be.
  //
  virtual void mprotect() = 0;
  virtual void munprotect() = 0;
};
