 ...
 public:
  ...
  Bytecodes::Code next() {
    ...
    if (is_last_bytecode()) {
      ...
      // If the byte code is a _breakpoint, we need the moop to find out the
      // original byte code. However, PGC doesn't support the needed 
      // block_start() functionality which is needed to map the bcp back to
      // moop. Since we have method stored in the BytecodeStream, we just need
      // to pass it down explicitly here.
      code        = Bytecodes::java_code_at(bcp, _method());
      // set next bytecode position
      //
