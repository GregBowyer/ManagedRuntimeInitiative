...
if(THREAD->is_Compiler_thread())return;//don't force compilation, resolve was on behalf of compiler
  if( UseC1 && CompilationPolicy::mustBeCompiled(selected_method,1) )
    CompileBroker::_c1.producer_add_task(selected_method, selected_method, InvocationEntryBci);
  if( UseC2 && CompilationPolicy::mustBeCompiled(selected_method,2) )
    CompileBroker::_c2.producer_add_task(selected_method, selected_method, InvocationEntryBci);
}
...
assert(recv_klass->is_subtype_of(resolved_klass()),"Lookup mismatch");
    
  // Virtual methods cannot be resolved before their klasses have been linked,
  // because otherwise the methodOops have not been rewritten and the vtable
  // initialized.  Make sure to do this after the nullcheck, since a missing
  // receiver might result in a bogus lookup.
