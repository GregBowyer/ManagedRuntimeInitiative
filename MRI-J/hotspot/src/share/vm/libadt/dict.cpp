...
bool Dict::contains(const void *key) const {
  uint i = _hash( key ) & (_size-1);	// Get hash key, corrected for size
  bucket *b = &_bin[i];		// Handy shortcut
  for( uint j=0; j<b->_cnt; j++ )
    if( !_cmp(key,b->_keyvals[j+j]) )
...
void Dict::print() {
  ...
  tty->print("Dict@" PTR_FORMAT "[%d] = {", this, _cnt);
  for( ; i.test(); ++i ) {
    tty->print("(" PTR_FORMAT "," PTR_FORMAT "),", i._key, i._value);
  }
  tty->print_cr("}");
}
...
int hashtriple(const void*key){
  Triple* t = (Triple*)key;
  return (uintptr_t)t->_a ^ (uintptr_t)t->_b ^ (uintptr_t)t->_c;
}
...
int32 cmptriple(const void*key1,const void*key2){
  Triple* t1 = (Triple*)key1;
  Triple* t2 = (Triple*)key2;
  return (int32)((t1->_a != t2->_a) || (t1->_b != t2->_b) || (t1->_c != t2->_c));
}
...
// Find the next key-value pair in the dictionary, or return a NULL key and
// value.
void DictFAMI::operator ++(void) {
  if( _j-- ) {			// Still working in current bin?
    _key   = _fam->getOldPtr("((struct Dict*)%p)->_bin[%d]._keyvals[%d]", _old_dict, _i, _j+_j);
    _value = _fam->getOldPtr("((struct Dict*)%p)->_bin[%d]._keyvals[%d]", _old_dict, _i, _j+_j+1);
    return;
  }

  while( ++_i < (uint)_fam->getInt("((struct Dict*)%p)->_size", _old_dict) ) {	// Else scan for non-zero bucket
    _j = _fam->getInt("((struct Dict*)%p)->_bin[%d]._cnt", _old_dict, _i);
    if( !_j ) continue;
    _j--;
    _key   = _fam->getOldPtr("((struct Dict*)%p)->_bin[%d]._keyvals[%d]", _old_dict, _i, _j+_j);
    _value = _fam->getOldPtr("((struct Dict*)%p)->_bin[%d]._keyvals[%d]", _old_dict, _i, _j+_j+1);
...
void DictFAMI::reset( FAMPtr old_dict ) {
  _old_dict = old_dict;		// The dictionary
  _i = (uint)-1;		// Before the first bin
  _j = 0;			// Nothing left in the current bin
  ++(*this);			// Step to first real value
}

int DictFAMI::test(void) {
  return _i< (uint)_fam->getInt("((struct Dict*)%p)->_size", _old_dict);
}
