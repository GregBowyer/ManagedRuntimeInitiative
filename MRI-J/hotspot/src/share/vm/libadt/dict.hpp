...
class Triple:public ResourceObj{
 public:
  Triple() {}
  Triple(void *a, void *b, void *c) {
    _a = a; _b = b; _c = c;
  }
  void *_a, *_b, *_c;
};
...
class DictFAMI {
 private:
  FAMPtr _old_dict;             // Dictionary being iterated over
  uint _i;                      // Counter over the bins
  uint _j;                      // Counter inside each bin
  FreezeAndMelt *_fam;
 public:
  FAMPtr _key, _value;          // Easy access to the key-value pair
  DictFAMI( FreezeAndMelt* fam, FAMPtr old_dict ) : _fam(fam) {reset(old_dict);} // Create a new iterator
  void reset( FAMPtr old_dict );     // Reset existing iterator
  void operator ++(void);             // Increment iterator
  int test();
};
