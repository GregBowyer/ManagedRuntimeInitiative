...
tty->print_cr("Heap malloc "PTR_FORMAT" %7zd %s",p,size,name==NULL?"":name);
}


void trace_heap_free(void* p) {
  // A lock is not needed here - tty uses a lock internally
tty->print_cr("Heap free   "PTR_FORMAT,p);
}
...
class ChunkPool { 
 ...
 public:
  ...
  void* allocate(size_t bytes) {
    ...
    void* p = NULL;    
{MutexLockerNested ml(ThreadCritical_lock);
      _num_used++;
      p = get_first();
...
void AllocatedObj::print(outputStream* st) const { print_on(st); }
void AllocatedObj::print_value() const { print_value_on(tty); }

void AllocatedObj::print_on(outputStream* st) const {
st->print_cr("AllocatedObj("PTR_FORMAT")",this);
}

void AllocatedObj::print_value_on(outputStream* st) const {
st->print("AllocatedObj("PTR_FORMAT")",this);
}
