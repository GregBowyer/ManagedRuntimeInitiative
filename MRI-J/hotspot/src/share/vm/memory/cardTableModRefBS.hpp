...
#ifndef CARDTABLEMODREFBS_HPP
#define CARDTABLEMODREFBS_HPP
...
class CardTableModRefBS: public ModRefBarrierSet {
 ...
 public:
  // Made public for verification.  Note that the math encapsulated here
  // is already well public for the compilers.
 ...
 protected:
  ...
  HeapWord* addr_for(const jbyte* p) const { 
    ...
    // Some users of this method ask for the card *past* the last valid address,
    // to serve as a limit on scanning. Because that card will be == the last address
    // in an exlusive MemRegion, we also need to test (result-1) in this assert
    assert(_whole_heap.contains(result) || _whole_heap.contains(result-1),
...
public:
  ...
  inline bool write_ref_needs_barrier(objectRef* field, oop new_val) {
...
protected:
  ...
  void write_ref_field_work(objectRef* field, objectRef newVal);
public:
...
public:
  ...
  inline void inline_write_ref_field(objectRef* field, objectRef newVal) {
if(newVal.is_null())return;
    if( newVal.is_old() ) return;
#ifdef ASSERT
    if ( !newVal.is_new() ) fatal1("new expected - " INTPTR_FORMAT, newVal.raw_value())
#endif
  ...
  PrecisionStyle precision() {
    return Precise; // Azul uses a write barrier that always does precise cardmarks
  }
