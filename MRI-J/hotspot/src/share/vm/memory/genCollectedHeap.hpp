...
#ifndef GENCOLLECTEDHEAP_HPP
#define GENCOLLECTEDHEAP_HPP
...
public:
  ...
  // override
bool is_in_closed_subset(const void*p)const{return is_in(p);}

  // Returns "TRUE" iff "p" points into the youngest generation.
bool is_in_youngest(const void*p)const;
  ...
  virtual HeapWord* block_start_debug(const void* addr) const { return block_start(addr); }
  ...
  void print_heap_change(size_t prev_used, outputStream* st = gclog_or_tty);
  void print_perm_heap_change(size_t perm_prev_used, outputStream* st = gclog_or_tty) const;
...
public:
  ...
  // Return the correct space ID for the address.
  virtual uint64_t space_id_for_address(const void* addr) const;

  virtual void mprotect();
  virtual void munprotect();
};

#endif // GENCOLLECTEDHEAP_HPP
