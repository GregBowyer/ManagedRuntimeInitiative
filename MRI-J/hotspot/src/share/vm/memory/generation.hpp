...
class Generation: public CHeapObj {
 ...
 public:
  ...
  // verification only.
bool is_in(const void*p)const{Unimplemented();return false;}
  ...
  virtual oop promote(oop obj, size_t obj_size, objectRef* ref);

  // promote or return NULL if there isn't enough room
virtual oop attempt_promotion(oop obj,size_t obj_size,objectRef*ref);
...
warning("time_of_last_gc: time warp: %lld to %lld",_time_of_last_gc,now);
      }
    )
...
#undef Generation_SINCE_SAVE_MARKS_DECL
  ...
  void print_heap_change(size_t prev_used, outputStream* st = gclog_or_tty) const;

  // PrintHeapAtGC support
  virtual void print() const;
  virtual void print_on(outputStream* st) const;
  virtual void print_xml_on(xmlBuffer *xb, bool ref = true) const;
...
StatRecord():invocations(0),accumulated_time(){}
  };
private:
...
class OneContigSpaceCardGeneration: public CardGeneration {
 ...
 public:
  ...
  inline bool is_in(const void* p) const { Unimplemented(); return false; }
