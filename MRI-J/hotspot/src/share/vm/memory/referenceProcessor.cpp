...
public:
DiscoveredList():_head(nullRef),_len(0){}
  objectRef    head() const     { return lvb_ref(&_head).as_oop(); }
  objectRef*   head_ptr()       { return &_head; }
  void   set_head(objectRef o)  { _head = o; }
...
objectRef  ReferenceProcessor::_sentinelRef;
...
NEEDS_CLEANUP;//The initial value is set before arguments are parsed, thus it needs to be this literal.
assert(_sentinelRef.is_null(),"should be initialized precisely once");
  EXCEPTION_MARK;
_sentinelRef=objectRef(instanceKlass::cast(
                   SystemDictionary::object_klass())->
allocate_permanent_instance(THREAD));
  _sentinelRef = ALWAYS_POISON_OBJECTREF(_sentinelRef);
...
assert(_sentinelRef.not_null()&&ALWAYS_UNPOISON_OBJECTREF(_sentinelRef).as_oop()->is_oop(),
         "Just constructed it!");
...
ReferenceProcessor::ReferenceProcessor(MemRegion span,
  ...
  _next_id(0)
{
assert(SubclassesOfJavaLangRefReference==4,"Number of java.lang.ref.Reference subclasses has changed");

  _span = span;
  _discovery_is_atomic = atomic_discovery;
  _discovery_is_mt     = mt_discovery;
  _num_q               = mt_degree;
_discoveredSoftRefs=NEW_C_HEAP_ARRAY(DiscoveredList,_num_q*SubclassesOfJavaLangRefReference);
...
assert(_sentinelRef.not_null(),"_sentinelRef is NULL");
  // Initialized all entries to _sentinelRef
for(int i=0;i<_num_q*SubclassesOfJavaLangRefReference;i++){
...
bool ReferenceProcessor::enqueue_discovered_references(AbstractRefProcTaskExecutor* task_executor) {
  ...
  objectRef old_pending_list_value = *pending_list_addr;
  ...
  ref_store(objectRef(SystemDictionary::reference_klass()), pending_list_addr, ALWAYS_UNPOISON_OBJECTREF(*(pending_list_addr)));
...
void ReferenceProcessor::enqueue_discovered_reflist(DiscoveredList& refs_list,
  ...
  if (TraceReferenceGC && PrintGCDetails) {
    gclog_or_tty->print_cr("ReferenceProcessor::enqueue_discovered_reflist list "
                           PTR_FORMAT, (address)refs_list.head().raw_value());
  }
  objectRef obj = refs_list.head();
  ...
  while (obj != ALWAYS_UNPOISON_OBJECTREF(_sentinelRef)) {
    oop obj_oop = obj.as_oop();
assert(obj_oop->is_instanceRef(),"should be reference object");
    objectRef next = lvb_ref(java_lang_ref_Reference::discovered_addr(obj_oop));

    if (TraceReferenceGC && PrintGCDetails) {
      gclog_or_tty->print_cr("	obj " INTPTR_FORMAT "/next " INTPTR_FORMAT,
                             obj.raw_value(), next.raw_value());
    }
assert(java_lang_ref_Reference::next(obj_oop)==NULL,
      "The reference should not be enqueued");

    if (next == ALWAYS_UNPOISON_OBJECTREF(_sentinelRef)) {  // obj is last
      // Swap refs_list into pending_list_addr and
      // set obj's next to what we read from pending_list_addr.
objectRef old=(objectRef)Atomic::xchg_ptr((intptr_t)ALWAYS_POISON_OBJECTREF(refs_list.head()).raw_value(),(intptr_t*)pending_list_addr);
...
java_lang_ref_Reference::set_next(obj_oop,obj_oop);
      } else {
java_lang_ref_Reference::set_next(obj_oop,old.as_oop());
      }
    } else {
java_lang_ref_Reference::set_next(obj_oop,next.as_oop());
    }
java_lang_ref_Reference::set_discovered(obj_oop,(oop)NULL);
    obj = next;
  }
}
...
public:
  ...
  virtual void work(unsigned int work_id)
  {
    ...
    int index = work_id;
for(int j=0;j<SubclassesOfJavaLangRefReference;j++,index+=_n_queues){
...
void ReferenceProcessor::enqueue_discovered_reflists(objectRef* pending_list_addr, 
  AbstractRefProcTaskExecutor* task_executor) {
...
inline oop obj()const{return ALWAYS_UNPOISON_OBJECTREF(_ref).as_oop();}

  // Get oop to the referent object.
inline oop referent()const{return ALWAYS_UNPOISON_OBJECTREF(_referent).as_oop();}
...
inline void make_active(){java_lang_ref_Reference::set_next(ALWAYS_UNPOISON_OBJECTREF(_ref).as_oop(),NULL);}
...
inline void clear_referent(){*_referent_addr=nullRef;}
...
private:
  ...
  objectRef*         _prev_next;
  objectRef          _ref;
  objectRef*         _discovered_addr;
  objectRef          _next;
  objectRef*         _referent_addr;
  objectRef          _referent;
  OopClosure*        _keep_alive;
  BoolObjectClosure* _is_alive;
  DEBUG_ONLY(
  objectRef          _first_seen; // cyclic linked list check
  )
  NOT_PRODUCT(
  size_t             _processed;
  size_t             _removed;
...
_ref(ALWAYS_POISON_OBJECTREF(refs_list.head())),
#ifdef ASSERT
_first_seen(ALWAYS_POISON_OBJECTREF(refs_list.head())),
#endif
#ifndef PRODUCT
    _processed(0),
    _removed(0),
#endif
_next(ALWAYS_POISON_OBJECTREF(refs_list.head())),
...
{
return _is_alive->do_object_b(ALWAYS_UNPOISON_OBJECTREF(_referent).as_oop());
}

inline void DiscoveredListIterator::load_ptrs(DEBUG_ONLY(bool allow_null_referent))
{
_discovered_addr=java_lang_ref_Reference::discovered_addr(ALWAYS_UNPOISON_OBJECTREF(_ref).as_oop());
assert(_discovered_addr&&(lvb_ref(_discovered_addr).as_oop())->is_oop_or_null(),
         "discovered field is bad");
_next=ALWAYS_POISON_OBJECTREF(lvb_ref(_discovered_addr));
_referent_addr=java_lang_ref_Reference::referent_addr(ALWAYS_UNPOISON_OBJECTREF(_ref).as_oop());
_referent=ALWAYS_POISON_OBJECTREF(lvb_ref(_referent_addr));
assert(Universe::heap()->is_in_reserved_or_null(ALWAYS_UNPOISON_OBJECTREF(_referent).as_oop()),
         "Wrong oop found in java.lang.Reference object");
  assert(allow_null_referent ?
ALWAYS_UNPOISON_OBJECTREF(_referent).as_oop()->is_oop_or_null()
:ALWAYS_UNPOISON_OBJECTREF(_referent).as_oop()->is_oop(),
         "bad referent");
}
...
{
assert(ALWAYS_UNPOISON_OBJECTREF(_ref).as_oop()->is_oop(),"Dropping a bad reference");
  // Clear the discovered_addr field so that the object does
  // not look like it has been discovered.
*_discovered_addr=nullRef;
  // Remove Reference object from list.
POISON_AND_STORE_REF(_prev_next,ALWAYS_UNPOISON_OBJECTREF(_next));
...
{
_ref=_next;//Under RefPoisoning, should move a poisoned field to a poisoned field
...
ReferenceProcessor::process_phase1(DiscoveredList&    refs_list_addr,
  ...
  while (iter.has_next()) {
    ...
    if (referent_is_dead && !policy->should_clear_reference(iter.obj())) {
      if (TraceReferenceGC) {
gclog_or_tty->print_cr("Dropping reference ("PTR_FORMAT": %s"") by policy",
...
gclog_or_tty->print(" Dropped %zd dead Refs out of %zd "
                             ...
                             OopClosure*        keep_alive)
{
  ...
  while (iter.has_next()) {
    ...
    DEBUG_ONLY(objectRef* next_addr = java_lang_ref_Reference::next_addr(iter.obj());)
assert(ALWAYS_UNPOISON_OBJECTREF(*next_addr).is_null(),"Should not discover inactive Reference");
    if (iter.is_referent_alive()) {
      if (TraceReferenceGC) {
gclog_or_tty->print_cr("Dropping strongly reachable reference ("PTR_FORMAT": %s)",
                               (address)iter.obj(), iter.obj()->blueprint()->internal_name()); 
      }
  ...
  NOT_PRODUCT(
    if (PrintGCDetails && TraceReferenceGC) {
gclog_or_tty->print(" Dropped %zd active Refs out of %zd "
  ...
  VoidClosure*       complete_gc)
{
  ...
  while (iter.has_next()) {
    ...
    objectRef* next_addr = java_lang_ref_Reference::next_addr(iter.obj());
    oop next_oop = ALWAYS_UNPOISON_OBJECTREF(*next_addr).as_oop();    
    if ((iter.referent() == NULL || iter.is_referent_alive() ||
next_oop!=NULL)){
assert(next_oop->is_oop_or_null(),"bad next field");
      // Remove Reference object from list
      iter.remove();
      // Trace the cohorts
      iter.make_referent_alive();
...
ReferenceProcessor::process_phase3(DiscoveredList&    refs_list_addr,
  ...
  while (iter.has_next()) {
    ...
    if (TraceReferenceGC) {
gclog_or_tty->print_cr("Adding %sreference ("PTR_FORMAT": %s) as pending",
                             clear_referent ? "cleared " : "",  
                             (address)iter.obj(), iter.obj()->blueprint()->internal_name()); 
    }
assert(iter.obj()->is_oop(),"Adding a bad reference");
...
||(!discovery_is_atomic()&&iter.referent()->is_oop_or_null()),
...
ReferenceProcessor::abandon_partial_discovered_list(DiscoveredList& ref_list) {
  objectRef obj = ref_list.head();
while(obj!=ALWAYS_UNPOISON_OBJECTREF(_sentinelRef)){
objectRef*discovered_addr=java_lang_ref_Reference::discovered_addr(obj.as_oop());
obj=lvb_ref(discovered_addr);
*discovered_addr=nullRef;
  }
  ...
  for (int from_idx = 0; from_idx < _num_q; from_idx++) {
    while (ref_lists[from_idx].length() > avg_refs) {
      ...
      if (ref_lists[to_idx].length() < avg_refs) {
        ...
        objectRef move_head = ref_lists[from_idx].head();
        objectRef move_tail = move_head;
        objectRef new_head  = move_head;
        // find an element to split the list on
        for (size_t j = 0; j < refs_to_move; ++j) {
          move_tail = new_head;
new_head=lvb_ref(java_lang_ref_Reference::discovered_addr(new_head.as_oop()));
        }
java_lang_ref_Reference::set_discovered(move_tail.as_oop(),ref_lists[to_idx].head().as_oop());
...
gclog_or_tty->print(", %zu refs",total);
  }
...
objectRef obj_ref,objectRef*discovered_addr){
  assert(_discovery_is_mt, "!_discovery_is_mt should have been handled by caller");
  // First we must make sure this object is only enqueued once. CAS in a non null
  // discovered_addr.
intptr_t retest=Atomic::cmpxchg_ptr((intptr_t)ALWAYS_POISON_OBJECTREF(list.head()).raw_value(),(intptr_t*)discovered_addr,NULL);
if(retest==0){
    // This thread just won the right to enqueue the object.
    // We have separate lists for enqueueing so no synchronization
    // is necessary.
list.set_head(obj_ref);
    list.set_length(list.length() + 1);
  } else {
    // If retest was non NULL, another thread beat us to it:
    // The reference has already been discovered...
    if (TraceReferenceGC) {
      oop obj_oop = obj_ref.as_oop();
      gclog_or_tty->print_cr("Already enqueued reference (" INTPTR_FORMAT ": %s)",  
                             obj_ref.raw_value(), obj_oop->blueprint()->internal_name()); 
    }
  }
}
...
assert(discovered_addr!=NULL&&lvb_ref(discovered_addr).as_oop()->is_oop_or_null(),
         "bad discovered field");
if(discovered_addr->not_null()){
    // The reference has already been discovered...
    if (TraceReferenceGC) {
gclog_or_tty->print_cr("Already enqueued reference ("PTR_FORMAT": %s)",
    ...
    if (RefDiscoveryPolicy == ReferentBasedDiscovery) {
      ...
      assert(!UseSerialGC && !UseParallelGC && !UseParallelOldGC, 
  ...
  objectRef obj_ref = ALWAYS_POISON_OBJECTREF(objectRef(obj));
  if (_discovery_is_mt) {
add_to_discovered_list_mt(*list,obj_ref,discovered_addr);
  } else {
POISON_AND_STORE_REF(discovered_addr,list->head());
list->set_head(obj_ref);
    list->set_length(list->length() + 1);
  }
  ...
  if (TraceReferenceGC) {
    oop referent = java_lang_ref_Reference::referent(obj);
    if (PrintGCDetails) {
gclog_or_tty->print_cr("Enqueued reference ("PTR_FORMAT": %s)",
                             (oopDesc*) obj, obj->blueprint()->internal_name()); 
    }
...
void ReferenceProcessor::preclean_discovered_reflist(
  ...
  while (iter.has_next()) {
    ...
    if (iter.referent() == NULL || iter.is_referent_alive() || 
      ...
      if (TraceReferenceGC) {
gclog_or_tty->print_cr("Precleaning Reference ("PTR_FORMAT": %s)",
  ...
  NOT_PRODUCT(
    if (PrintGCDetails && PrintReferenceGC) {
gclog_or_tty->print(" Dropped %zd Refs out of %zd "
        "Refs in discovered list ", iter.removed(), iter.processed());
    }
  )
}
...
assert(i>=0&&i<=_num_q*SubclassesOfJavaLangRefReference,"Out of bounds index");
   int j = i / _num_q;
   switch (j) {
...
guarantee(_sentinelRef.not_null()&&ALWAYS_UNPOISON_OBJECTREF(_sentinelRef).as_oop()->is_oop(),"Lost _sentinelRef");
}
...
void ReferenceProcessor::clear_discovered_references() {
  guarantee(!_discovering_refs, "Discovering refs?");
for(int i=0;i<_num_q*SubclassesOfJavaLangRefReference;i++){
    objectRef obj = ALWAYS_UNPOISON_OBJECTREF(_discoveredSoftRefs[i].head());
    while (obj != ALWAYS_UNPOISON_OBJECTREF(_sentinelRef)) {
      oop obj_oop = obj.as_oop();
      objectRef next = lvb_ref(java_lang_ref_Reference::discovered_addr(obj_oop));
java_lang_ref_Reference::set_discovered(obj_oop,(oop)NULL);
