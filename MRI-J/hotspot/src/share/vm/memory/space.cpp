...
void FilteringClosure::do_oop(objectRef* p) {
  do_oop_nv(p);
}
...
q->forward_to_pointer(compact_top);
    assert(q->is_gc_marked(), "encoding the pointer should preserve the mark");
  } else {
    // if the object isn't moving we can just set the mark to the default
    // mark and handle it specially later on.  
q->clear_mark();
assert(q->forwarded_pointer()==NULL,"should be forwarded to NULL");
  }
...
MemRegion mr(q,deadlength);
  mr.fill();
oop(q)->clear_and_mark();
  assert0((int) deadlength == oop(q)->size());
...
void ContiguousSpace::allocate_temporary_filler(int factor) {
  ...
  MemRegion mr(allocate(size),size);
  mr.fill();
}
...
_par_alloc_lock(Patching_lock._rank,"OffsetTableContigSpace par alloc lock",false)
{
