...
#ifndef CONSTMETHODOOP_HPP
#define CONSTMETHODOOP_HPP
...
public:
  heapRef* oop_block_beg() const { return adr_method(); }
  heapRef* oop_block_end() const { return adr_exception_table() + 1; }
...
void set_method(methodOop m){ref_store_without_check(&_method,(methodRef)m);}


  // stackmap table data
typeArrayOop stackmap_data()const{return lvb_typeArrayRef(&_stackmap_data).as_typeArrayOop();}
void set_stackmap_data(typeArrayOop sd){ref_store_without_check(&_stackmap_data,(typeArrayRef)sd);}
bool has_stackmap_table()const{return stackmap_data()!=NULL;}

  // exception handler table
typeArrayOop exception_table()const{return lvb_typeArrayRef(&_exception_table).as_typeArrayOop();}
typeArrayRef exception_tableRef()const{return lvb_typeArrayRef(&_exception_table);}
  void set_exception_table(typeArrayOop e)       { ref_store_without_check(&_exception_table, (typeArrayRef) e); }
  ...
  heapRef*  adr_stackmap_data() const      { return (heapRef*)&_stackmap_data;   }
  heapRef*  adr_exception_table() const    { return (heapRef*)&_exception_table; }
