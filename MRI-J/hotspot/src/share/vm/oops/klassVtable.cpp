...
for(int i=0;i<_length;i++){
    start[i].set_ref( table()[i].method_ref() );
  }
}

void klassVtable::adjust_method_entries(methodRef* old_methods, methodRef* new_methods,
...
void klassVtable::oop_adjust_pointers() {
  int len = length();
  for (int i = 0; i < len; i++) {
MarkSweep::adjust_pointer((heapRef*)adr_method_at(i));
  }
}
...
void klassVtable::oop_update_pointers(ParCompactionManager* cm) {
  const int n = length();
  for (int i = 0; i < n; i++) {
PSParallelCompact::adjust_pointer((heapRef*)adr_method_at(i));
  }
}
...
void klassVtable::oop_update_pointers(ParCompactionManager* cm,
  ...
  heapRef* const beg_oop = (heapRef*)adr_method_at(beg_idx);
  heapRef* const end_oop = MIN2((heapRef*)end_addr, (heapRef*)adr_method_at(n));
  for (heapRef* cur_oop = beg_oop; cur_oop < end_oop; cur_oop += entry_size) {
...
void itableMethodEntry::initialize(methodOop m) {
  ...
  POISON_AND_STORE_REF(&_method, methodRef(m));
}
...
void klassItable::oop_adjust_pointers() {
  ...
  for(int i = 0; i < _size_offset_table; i++) {
    MarkSweep::adjust_pointer((heapRef*)&ioe->_interface);
    ioe++;
  }
  ...
  for(int j = 0; j < _size_method_table; j++) {
    MarkSweep::adjust_pointer((heapRef*)&ime->_method);
    ime++;
  }
}
...
void klassItable::oop_update_pointers(ParCompactionManager* cm) {
  ...
  itableOffsetEntry* ioe = offset_entry(0);
  for(int i = 0; i < _size_offset_table; i++) {
    PSParallelCompact::adjust_pointer((heapRef*)&ioe->_interface);
    ioe++;
  }
  ...
  itableMethodEntry* ime = method_entry(0);
  for(int j = 0; j < _size_method_table; j++) {
    PSParallelCompact::adjust_pointer((heapRef*)&ime->_method);
    ime++;
  }
}
...
void klassItable::oop_update_pointers(ParCompactionManager* cm,
  ...
  itableOffsetEntry* ioe = offset_entry(0);
  for(int i = 0; i < _size_offset_table; i++) {
    heapRef* p = (heapRef*)&ioe->_interface;
...
void klassItable::oop_oop_iterate(OopClosure* blk) {
  ...
  itableOffsetEntry* ioe = offset_entry(0);
  for(int i = 0; i < _size_offset_table; i++) {
    blk->do_oop((objectRef*)&ioe->_interface);
    ioe++;
  }
  ...
  itableMethodEntry* ime = method_entry(0);
  for(int j = 0; j < _size_method_table; j++) {
    blk->do_oop((objectRef*)&ime->_method);
    ime++;
  }
}
...
void klassItable::oop_oop_iterate_m(OopClosure* blk, MemRegion mr) {
  ...
  itableOffsetEntry* ioe = offset_entry(0);
  for(int i = 0; i < _size_offset_table; i++) {
    objectRef* adr = (objectRef*)&ioe->_interface;
  ...
  itableMethodEntry* ime = method_entry(0);
  for(int j = 0; j < _size_method_table; j++) {
    objectRef* adr = (objectRef*)&ime->_method;
    if (mr.contains(adr)) blk->do_oop(adr);
    ime++;
  }
}
...
void klassItable::adjust_method_entries(methodRef* old_methods, methodRef* new_methods,
                                        int methods_length, bool * trace_name_printed) {
...
void klassItable::setup_itable_offset_table(instanceKlassHandle klass) {  
  ...
  intptr_t* end               = klass->end_of_itable();
  assert((objectRef*)(ime + nof_methods) <= klass->start_of_static_fields(), "wrong offset calculation (1)");
  assert((objectRef*)(end) == (objectRef*)(ime + nof_methods),               "wrong offset calculation (2)");
