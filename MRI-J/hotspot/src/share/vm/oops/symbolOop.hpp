...
#ifndef SYMBOLOOP_HPP
#define SYMBOLOOP_HPP
...
return size;
  ...
  const char* as_klass_pretty_name() const;
...
//
// For UsePauselessGC, object order in perm does change, so we do an alphabetical
// compare instead.
int symbolOopDesc::fast_compare(symbolOop other) const {
  if ( UseGenPauselessGC ) {
    // since the base() is 2 byte aligned, we could probably do a fast
    // compare using shorts instead of chars, but the tail handling
    // would be ugly.
    int my_len  = this->utf8_length();
    int his_len = other->utf8_length();
    int min_len = MIN2(my_len, his_len);

for(int i=0;i<min_len;i++){
      if ( this->byte_at(i) != other->byte_at(i) )
        return this->byte_at(i) - other->byte_at(i);
    }
