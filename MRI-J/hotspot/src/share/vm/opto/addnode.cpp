...
if(t12->singleton()&&t12!=Type::TOP&&
        add1 != a11 && this != a11 && (a11->req()<=1 || this != a11->in(1)) ) {
      add2 = add1->clone();
      add2->set_req(2, in(2));
...
Node *AddINode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  // Look for repeated sums of the same thing.  Happens after loop unrolling.
  // Convert (x + (x+y)) into ((x<<1)+y)
  if( op2 == Op_AddI && in(2)->in(1) == in(1) ) {
Node*s=phase->transform(new(phase->C,3)LShiftINode(in(1),phase->intcon(1)));
return new(phase->C,3)AddINode(s,in(2)->in(2));
  }
...
return(phase->type(in(Offset))->higher_equal(TypeLong::ZERO))?in(Address):this;
}

//------------------------------Idealize---------------------------------------
Node *AddPNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  // Bail out if dead inputs
const Type*tadr=phase->type(in(Address));
if(tadr==Type::TOP)return NULL;
...
const TypeLong*t2=temp_t2->is_long();
      if( t2->is_con() ) {
        // The Add of the flattened expression
        set_req(Address, addp->in(Address));
set_req(Offset,phase->longcon(t2->get_con()+t12->get_con()));
...
if(add->Opcode()==Op_AddL&&add->in(1)!=add){
  ...
  // If this is an add of a constant and the base & address are from a
  // CheckCastPP and the offset exists in the pre-CheckCastPP type,
  // then move the AddPNode around it.  
  intptr_t off = 0;
  if( in(Base) == in(Address) &&
in(Base)->Opcode()==Op_CheckCastPP&&
      in(Offset)->get_intptr_t(&off) &&
      !tadr->isa_aryptr() ) { // Array refs get strange offsets all the time
Node*precast=in(Base)->in(1);
const TypeOopPtr*tp=phase->type(precast)->isa_oopptr();
    if( tp ) {                  // It is an OOP type - so it has a size
ciKlass*klass=tp->klass();
if(klass->is_instance_klass()&&
          ((ciInstanceKlass*)klass)->contains_field_offset(off) ) {
        set_req(Base   ,precast);
        set_req(Address,precast);
      }
    }
  }

  // Handle AddP of convert - bring it into the shape used by the array
  // addressing math.  This is not a semantically equivalent transform, except
  // that pointer-math does not specify how many bits are used.  In
  // particular, sub-address math will not properly wrap here.
  if( in(Offset)->Opcode() == Op_ConvI2L ) {
Node*off=in(Offset)->in(1);
    if( off->Opcode() == Op_AddI && off->in(1) != off && phase->type(off->in(2))->singleton() ) {
      Node *c1 = phase->transform( new (phase->C, 2) ConvI2LNode( off->in(1) ) );
      Node *c2 = phase->transform( new (phase->C, 2) ConvI2LNode( off->in(2) ) );
      Node *ad1= phase->transform( new (phase->C, 4)    AddPNode( in(Base), in(Address), c1 ) );
      set_req( Address, ad1 );
      set_req( Offset, c2 );
      return this;
    }
if(off->Opcode()==Op_LShiftI){
      Node *c1 = phase->transform( new (phase->C, 2) ConvI2LNode( off->in(1) ) );
Node*ad=phase->transform(new(phase->C,3)LShiftLNode(c1,off->in(2)));
      set_req( Offset, ad );
      return this;
    }
  }
...
const Type *AddPNode::Value( PhaseTransform *phase ) const {
  ...
  assert0( !p1->isa_klassptr() || !p1->is_klassptr()->_is_kid );// a ptr and not a KID
  // Right input is an int
const TypeLong*p2=t2->is_long();
  // Add 'em
...
offset=phase->find_long_con(offs,Type::OffsetBot);
...
const Type *AddPNode::mach_bottom_type( const MachNode* n) {
  ...
  for ( uint i = 1; i < numopnds; i++ ) {
    ...
    for ( uint j = opnd->num_edges(); j > 0; j-- ) {
      ...
      if ( !et->singleton() )  goto bottom_out;
      if( et->isa_int() ) offset += ((TypeInt*)et)->_lo;
      else if( et->isa_long() ) offset += ((TypeLong*)et)->_lo;
    }
  }
