...
int Block::is_Empty() const {
  ...
  // emit() functions.
  while ((end_idx > 0) && !_nodes[end_idx]->is_Mach() && _nodes[end_idx]->Opcode()!=Op_ArrayCopy) { 
    end_idx--;
  }
...
if(_pre_order)C2OUT->print("B%d",_pre_order);
else C2OUT->print("N%d",head()->_idx);
}
...
if(bra->is_RangeCheck())return true;
