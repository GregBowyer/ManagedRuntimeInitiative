...
#ifndef CALLGENERATOR_HPP
#define CALLGENERATOR_HPP
...
class CallGenerator : public ResourceObj {
 ...
 public:
  ...
  virtual JVMState* generate(JVMState* jvms, CPData_Invoke *cpdi, bool cloned_in_citypeflow) = 0;

  // How to generate a call site that is inlined:
static CallGenerator*for_inline(ciMethod*m,CodeProfile*c1_cp,int c1_cp_inloff,float expected_uses=-1);
  // How to generate code for an on-stack replacement handler.
  static CallGenerator* for_osr(ciMethod* m, CodeProfile *c1_cp, int c1_cp_inloff, int osr_bci);
  ...
  static CallGenerator* for_predicted_call(ciKlass* predicted_receiver,
                                           ...
                                           float hit_prob);
static CallGenerator*for_bimorphic_predicted_call(ciKlass*predicted_receiver,
...
static CallGenerator*for_uncommon_trap(ciMethod*m,Deoptimization::DeoptReason trap_index,const char*comment,bool must_throw);
...
class WarmCallInfo : public ResourceObj {
 private:
  ...
  CPData_Invoke _cpdi;   // Profiling info about the original call site
 ...
 public:
  ...
  WarmCallInfo( WarmCallInfo *wc, CPData_Invoke *cpdi, bool cloned_in_citypeflow ) {
    *this = *wc;
    _cpdi = *cpdi;
...
void init(JVMState*call_site,ciMethod*call_method,int call_count,float prof_factor,CPData_Invoke*cpdi);
