...
static Node *merge_region(RegionNode *region, PhaseGVN *phase) {
  ...
  if( rreq == 3 &&              // Do not do this on 2-input merges untils loops are discovered
      !phase->C->loops_found() ) 
    return NULL;                

  // Count Phi-uses from 'region'
  int nphis = 0;
for(DUIterator_Fast jmax,j=region->fast_outs(jmax);j<jmax;j++)
    if( region->fast_out(j)->is_Phi() ) nphis++;
...
r!=region){//Avoid stupid situations
      // Only only single-use Phi's, used by the main merging Region Phi
      bool fail = false;
      int nphis2 = 0;           // Number of Phis uses of 'r'
      for (DUIterator_Fast jmax, j = r->fast_outs(jmax); j < jmax; j++) {
Node*phi=r->fast_out(j);
        if( phi == r || phi == region ) continue; // Ignore self use
        // Valid uses are exactly and only Phis hanging off of 'r' that have
        // exactly 1 use by a Phi off of 'region'.
        if( !phi->is_Phi() ||  
            phi->outcnt() != 1 ||
            !phi->unique_out()->is_Phi() ||
            phi->unique_out()->in(0) != region )
          fail = true;
        if( fail ) break;
        nphis2++;
      }
      // Also, if 'region' has a Phi which isn't merging data from a Phi along
      // 'r' then we'd have to replicate a series of edges into that Phi,
      // which isn't profitable.
      if( nphis2 < nphis ) fail = true;
      if( fail ) continue;

      // Move everything from 'r' to 'region'
for(DUIterator_Fast jmax,j=region->fast_outs(jmax);j<jmax;j++){
Node*phi=region->fast_out(j);
        if( !phi->is_Phi() && phi != region )
          continue;             // Ignore things not part of the merge
        if( !progress ) igvn->hash_delete(phi); // 1st time remove from hash table
        
        // Move all the things coming in from 'r' onto the end of the 'region'
        // merge point.  If the old value is controlled by 'r' then it is
        // either 'r' itself or a Phi of 'r' (gated by the test above).  If it
        // is NOT controlled by 'r' then it is invariant into the merge and
        // gets replicated.
Node*old=phi->in(i);//Old value on path i
for(uint k=1;k<r->req();k++)
          phi->add_req(old->in(0) == r ? old->in(k) : old);
        phi->set_req(i,phase->C->top()); // Kill path i
      }
  ...
  for( uint i=1; i<req(); ++i ){// For all paths in
    ...
    if( n != NULL ) {
      ...
      if( n->is_Proj() ) {      // Remove useless rethrows
        Node *call = n->in(0);
if(call->is_Call()&&call->as_Call()->entry_point()==StubRoutines::forward_exception_entry2()){
          set_req(i, call->in(0));
          i--;
          continue;
        }
    ...
    if( init != NULL && limit != NULL && l->stride_is_con() ) {
      ...
      if( lo && hi ) {            // Dying loops might have TOP here
        ...
        if( stride < 0 ) {          // Down-counter loop
          lo_lo = hi->_lo-stride; // Endpoint is excluded
          hi_hi = lo->_hi;
          const TypeInt *tmp = lo; lo = hi; hi = tmp; // swap for reversed-endpoint test
        } else {
          lo_lo = lo->_lo;
          hi_hi = hi->_hi-stride; // Endpoint is excluded
        }
        if( lo->_hi < hi->_lo )     // Reversed endpoints are well defined :-(
          return TypeInt::make(lo_lo,hi_hi,TypeInt::WidenMax);
      }
    }
  }
...
t=t->meet(phase->type(in(i)));

t=phase->saturate(t,phase->type_or_null(this),_type);
...
int PhiNode::is_diamond_phi() const {
  ...
  if(         req() != 3 ) return 0;
  if( in(1) == this || in(2) == this ) return 0; // Ignore cycles
  ...
  if( iff->in(0)->is_top() ) return 0; // Dead & dying
...
if(progress||!can_reshape||type()!=Type::MEMORY)return progress;

  const TypePtr* at = adr_type();
if(at!=TypePtr::BOTTOM){//Skinny-Phi
  ...
  // This next process tends to make lots of PhiNodes, so do not do it unless
  // we have lots of headroom.
  if( phase->C->unique() > 32000 ) return NULL;

  // see if this phi should be sliced
  uint merge_width = 0;
  MergeMemNode *mm_uniq = 0;            // -1 for NOT unique, otherwise uniqe MM node 
  for( uint i=1; i<req(); ++i ) {// For all paths in
Node*n=in(i);
if(n->is_MergeMem()){
      merge_width = MAX2(merge_width, n->req());
      //if( n->base_memory() == this ) return NULL;
      if( n->as_MergeMem()->base_memory() == n    ) return NULL;
      mm_uniq = mm_uniq ? (MergeMemNode*)-1 : n->as_MergeMem(); // Determine if more than 1 MM node merging
    }
  }

  // No MergeMem to split through at all
if(mm_uniq==0)return NULL;
  // Find at least one non-empty MergeMem
  if (merge_width <= Compile::AliasIdxRaw) return NULL;

  // We might spin this optimization around a loop forever, and some loops may
  // not be labeled as such.  Thus only do this if we can guarantee some kind
  // of forward progress.  Do not drop a single MM into a loop, lest we spin.
  // (Dropping an MM in along both paths is OK, because when we're done there
  // is one less MM in the world).
  if( req() > LoopNode::LoopBackControl && 
      in(LoopNode::LoopBackControl) == mm_uniq && 
      r->is_Loop() ) {
    // Loop-invariant base memory is OK to expand across a backedge
    // because the base-memory Phi will go away.
    if( mm_uniq->base_memory() != in(LoopNode::EntryControl) )
      return NULL;
  }

  // Dropping a single MM through a non-loop requires a stricter progress test.
  if( mm_uniq != (MergeMemNode*)-1 && !r->is_Loop() ) {
    // See that no Phi input has a higher index than ourself.  This is to
    // prevent pushing a single MM around an unlabeled loop of Phis forever.
for(uint i=1;i<mm_uniq->req();i++)
      if( mm_uniq->in(i)->is_Phi() &&
          mm_uniq->in(i)->_idx > _idx )
        return NULL;
  }
  ...
  PhaseIterGVN *igvn = phase->is_IterGVN();
Node*hook=new(phase->C,1)Node((Node*)NULL);
...
Node*n=in(i);
if(n->is_MergeMem()){
      for (MergeMemStream mms(result, n->as_MergeMem()); mms.next_non_empty2(); ) {
...
call->entry_point()==StubRoutines::forward_exception_entry2()))
    return this;
