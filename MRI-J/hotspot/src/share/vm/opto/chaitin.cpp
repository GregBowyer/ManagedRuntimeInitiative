...
#ifndef PRODUCT
void LRG::dump( ) const {
  C2OUT->print("%d ",_fat_proj ? 1 : _fat_proj_kill_regs);
  _mask.dump();
  if( _msize_valid ) {
if(mask_size()==compute_mask_size())C2OUT->print(", #%d ",_mask_size);
else C2OUT->print(", #!!!_%d_vs_%d ",_mask_size,_mask.Size());
  } else {
C2OUT->print(", #?(%d) ",_mask.Size());
  }

C2OUT->print("EffDeg: ");
if(_degree_valid)C2OUT->print("%d ",_eff_degree);
else C2OUT->print("? ");

  if( is_multidef() ) {
C2OUT->print("MultiDef ");
    if (_defs != NULL) {
C2OUT->print("(");
      for (int i = 0; i < _defs->length(); i++) {
C2OUT->print("N%d ",_defs->at(i)->_idx);
      }
C2OUT->print(") ");
    }
  }
else if(_def==0)C2OUT->print("Dead ");
else C2OUT->print("Def: N%d ",_def->_idx);

C2OUT->print("Cost:%4.2g Area:%4.2g Score:%4.2g ",_cost,_area,score());
  // Flags
if(_is_oop)C2OUT->print("Oop ");
if(_is_float)C2OUT->print("Float ");
if(_was_spilled1)C2OUT->print("Spilled ");
if(_was_spilled2)C2OUT->print("Spilled2 ");
if(_direct_conflict)C2OUT->print("Direct_conflict ");
if(_fat_proj)C2OUT->print("Fat ");
if(_was_lo)C2OUT->print("Lo ");
if(_has_copy)C2OUT->print("Copy ");
if(_at_risk)C2OUT->print("Risk ");

if(_must_spill)C2OUT->print("Must_spill ");
if(_is_bound)C2OUT->print("Bound ");
  if( _msize_valid ) {
if(_degree_valid&&lo_degree())C2OUT->print("Trivial ");
  }
...
//
//// --- sortem ----------------------------------------------------------------
//static uint sortem( Block** blks, uint num_blocks, Block_List &bs ) {
//  // Experiment with sorting strategies to speed compilation
//  double  cutoff = BLOCK_FREQUENCY(100000.0); // Cutoff for high frequency bucket
...
//    buckets[i] = NEW_RESOURCE_ARRAY( Block *, num_blocks );
//    buckcnt[i] = 0;
//    // Bump by two orders of magnitude each time
//    cutoff *= 0.01;
//    buckval[i] = cutoff;
...
//      if( (j == NUMBUCKS-1) || (bs[i]->_freq > buckval[j]) ) {
//        // Assign block to end of list for appropriate bucket
//        buckets[j][buckcnt[j]++] = bs[i];
...
//  tty->print("SORTEM: ");
//  for( uint i = 0; i < NUMBUCKS; i++ ) 
//    tty->print("%6g:%d(%%%2.2d) ",buckval[i],buckcnt[i],buckcnt[i]*100/num_blocks);
//  tty->cr();
//  
//  // Dump buckets into final block array
//  uint blkcnt = 0;
//  for( uint i = 0; i < NUMBUCKS; i++ ) {
//    // Empty blocks first, as these might end up with copies that, if
//    // coalesced, would leave the block empty and avoid a branch.
//    for( uint j = 0; j < buckcnt[i]; j++ ) {
//      Block *b = buckets[i][j];
//      if(  b->is_Empty() ) blks[blkcnt++] = b;
//    }
//    for( uint j = 0; j < buckcnt[i]; j++ ) {
//      Block *b = buckets[i][j];
//      if( !b->is_Empty() ) blks[blkcnt++] = b;
...
// --- sortem ----------------------------------------------------------------
static int cmpblockp(const void *p1, const void *p2) {
  // The actual arguments to this function are "pointers to pointers to
  // blocks"
  Block *const* b1 = (Block *const*)p1;
  Block *const* b2 = (Block *const*)p2;
  float f1 = (*b1)->_freq;
  float f2 = (*b2)->_freq;
  if( (*b1)->is_Empty() ) f1 *= 1.2;
  if( (*b2)->is_Empty() ) f2 *= 1.2;
  return (int)(f2 - f1);
}
...
static uint sortem( Block** blks, uint num_blocks, Block_List &bs ) {
  ...
  for( uint i=0; i<num_blocks; i++ )
    blks[i] = bs._blocks[i];
  qsort(blks,num_blocks, sizeof(Block*), cmpblockp);
  return num_blocks;
}
...
#endif
{
  ...
  // Experiment with sorting strategies to speed compilation
  uint blkcnt = sortem( _blks, _cfg._num_blocks, _cfg._blocks );
...
void PhaseChaitin::Register_Allocate() {
  ...
  while( spills ) {
    if( _trip_cnt++ > 24 ) {
      ...
      if( _trip_cnt > 27 ) {
C->record_failure("failed spill-split-recycle sanity check",false);
        return;
      }
    }
...
if(_max_reg<=_matcher._new_SP){
    // Incase we never spilled, _max_reg will be < max physical registers.
    // in that case _framesize is really whatever outgoing preserved
    // stack slots are.
  ...
  _framesize += C->in_preserve_stack_slots(); // Required incoming space
  if (Matcher::stack_alignment_in_bytes() > 0)
    _framesize = round_to(_framesize, Matcher::stack_alignment_in_bytes()>>3);
...
_node_regs[i]=lrg.reg();
      if( lrg._is_oop ) _node_oops.set(i);
    } else {
_node_regs[i]=OptoReg::Bad;
    }
  }
...
lrg._fat_proj_kill_regs=rm.Size();
lrg.set_reg_pressure(lrg._fat_proj_kill_regs);
          lrg._fat_proj = 1;
          lrg._is_bound = 1;
          break;
        case Op_RegP:
...
if(n_type->isa_oop_ptr()&&
              !(n_type->isa_klassptr() && n_type->is_klassptr()->_is_kid))
            lrg._is_oop = 1;
          break;
...
void PhaseChaitin::set_was_low() {
  ...
  for( uint i = 1; i < _maxlrg; i++ ) {
    int size = lrgs(i)._fat_proj ? lrgs(i)._fat_proj_kill_regs : 1;
    ...
    if( lrgs(i).lo_degree() ) {
      ...
      while((lidx = elements.next()) != 0) {
        if( !lrgs(lidx).lo_degree() ) 
          briggs_degree += MAX2(size,lrgs(lidx)._fat_proj ? lrgs(lidx)._fat_proj_kill_regs : 1);
      }
...
void PhaseChaitin::Pre_Simplify( ) {
  ...
  while( lo_no_copy ) {
    ...
    int size = lrgs(lo)._fat_proj ? lrgs(lo)._fat_proj_kill_regs : 1;
...
OptoReg::Name reg=tempmask.find_first_elem();
...
OptoReg::Name PhaseChaitin::choose_color( LRG &lrg, int chunk ) {  
  ...
  if( !lrg._fat_proj )   // Aligned+adjacent pairs ok
    // Use a heuristic to "bias" the color choice
    return bias_color(lrg, chunk);

  // Fat-proj case or misaligned double argument. 
assert(lrg.compute_mask_size()==lrg._fat_proj_kill_regs,"fat projs exactly color");
...
uint spill_reg=OptoReg::SPILL_REG;
...
if(!lrg->_fat_proj){//Size 1 live range
...
return spill_reg-OptoReg::SPILL_REG;//Return number of spills
}
...
Node *PhaseChaitin::find_base_for_derived( Node **derived_base_map, Node *derived, uint &maxlrg ) {
  ...
  if( derived->is_Con() ) {
    Node *base = ConPNode::make( C, (unsigned char*)NULL );
    new_lrg(base,maxlrg++);
    b->_nodes.insert(b->find_node(derived),base ); // Must insert created NULL here as a base
...
C2OUT->print("[");
OptoReg::Name first=get_reg(n);
      if( OptoReg::is_reg(first) )
C2OUT->print("%s]",Matcher::regName[first]);
      else
C2OUT->print("%s+%d]",Matcher::regName[OptoReg::c_frame_pointer],reg2offset_unchecked(first));
    } else
    n->out_RegMask().dump();
  }
C2OUT->print("/N%d\t",n->_idx);
...
C2OUT->print("L%d",r);
      ...
      int op = n->Opcode();
if(r&&op!=Op_Phi&&op!=Op_Proj&&op!=Op_SCMemProj&&op!=Op_CProj){
        if( _node_regs ) {
C2OUT->print("[");
OptoReg::Name first=get_reg(n->in(k));
          if( OptoReg::is_reg(first) )
C2OUT->print("%s]",Matcher::regName[first]);
          else
            C2OUT->print("%s+%d]",Matcher::regName[OptoReg::c_frame_pointer],
                       reg2offset_unchecked(first));
        } else
          n->in_RegMask(k).dump();
      }
C2OUT->print("/N%d ",m->_idx);
    }
  }
if(k<n->len()&&n->in(k))C2OUT->print("| ");
  for( ; k < n->len(); k++ ) {
    Node *m = n->in(k);
    if( !m ) break;
    uint r = (m->_idx < _names.Size() ) ? Find_const(m) : 0;
...
if(n->is_Mach())n->as_Mach()->dump_spec(C2OUT);
else n->dump_spec(C2OUT);
  if( _spilled_once.test(n->_idx ) ) {
C2OUT->print(" Spill_1");
    if( _spilled_twice.test(n->_idx ) ) 
C2OUT->print(" Spill_2");
...
C2OUT->print("--- Chaitin -- argsize: %d  framesize: %d ---\n",
...
print_reg(get_reg(n),buf);
    ...
    if( !_ifg ) {
      ...
      strcpy(buf,"Special");
}else if(!lrgs(lidx).mask().is_bound1()){
      sprintf(buf,"L%d",lidx); // No register binding yet
    } else {                    // Hah!  We have a bound machine register
print_reg(lrgs(lidx).reg(),buf);
    }
  }
...
C2OUT->print_cr("Graph-Coloring Iteration %d will split the following live ranges",_trip_cnt);
    for( uint bidx = 1; bidx < _maxlrg; bidx++ ) {
if(lrgs(bidx).alive()&&lrgs(bidx).reg()>=OptoReg::SPILL_REG){
C2OUT->print("L%d: ",bidx);
        lrgs(bidx).dump();
      }
    }
C2OUT->cr();
...
void PhaseChaitin::dump_frame() const {
  const char *fp = Matcher::regName[OptoReg::c_frame_pointer];
  const TypeTuple *domain = C->tf()->domain();
  const int        argcnt = domain->cnt() - TypeFunc::Parms;

  // Incoming arguments in registers dump
  for( int k = 0; k < argcnt; k++ ) {
VReg::VR parmreg=_matcher._parm_regs[k];
if(VReg::is_reg(parmreg)){
const char*reg_name=Matcher::regName[parmreg];
C2OUT->print("#r%3.3d %s",parmreg,reg_name);
C2OUT->print("   : parm %d: ",k);
...
C2OUT->print_cr("#r%3.3d %s+%2d: pad0, owned by CALLER",reg,fp,reg2offset_unchecked(reg));
  }
  ...
  while( reg > begin_in_arg ) {
    reg = OptoReg::add(reg, -1);
C2OUT->print("#r%3.3d %s+%2d: ",reg,fp,reg2offset_unchecked(reg));
    int j;
    for( j = 0; j < argcnt; j++) {
if(_matcher._parm_regs[j]==reg){
C2OUT->print("parm %d: ",j);
...
C2OUT->cr();
    ...
    if( j >= argcnt ) 
      C2OUT->print_cr(C->out_preserve_stack_slots() == 0 ? "HOLE, owned by CALLER" : "HOLE, owned by SELF");
  }

  // Old outgoing preserve area
  while( reg > _matcher._old_SP ) {
    reg = OptoReg::add(reg, -1);
C2OUT->print_cr("#r%3.3d %s+%2d: old out preserve",reg,fp,reg2offset_unchecked(reg));
  }

  // Old SP
C2OUT->print_cr("# -- Old %s -- Framesize: %d --",fp,
    reg2offset_unchecked(OptoReg::add(_matcher._old_SP,-1)) - reg2offset_unchecked(_matcher._new_SP)+8);
...
C2OUT->print_cr("return address");
    else
C2OUT->print_cr("pad2, in_preserve");
    reg = OptoReg::add(reg, -1);
  }

  // Spill area dump
reg=OptoReg::add(_matcher._new_SP,_framesize-C->in_preserve_stack_slots());
  while( reg > _matcher._out_arg_limit ) {
    reg = OptoReg::add(reg, -1);
C2OUT->print_cr("#r%3.3d %s+%2d: spill",reg,fp,reg2offset_unchecked(reg));
  }

  // Outgoing argument area dump
  while( reg > OptoReg::add(_matcher._new_SP, C->out_preserve_stack_slots()) ) {
    reg = OptoReg::add(reg, -1);
C2OUT->print_cr("#r%3.3d %s+%2d: outgoing argument",reg,fp,reg2offset_unchecked(reg));
  }

  // Outgoing new preserve area
  while( reg > _matcher._new_SP ) {
    reg = OptoReg::add(reg, -1);
C2OUT->print_cr("#r%3.3d %s+%2d: new out preserve",reg,fp,reg2offset_unchecked(reg));
  }
C2OUT->print_cr("#");
}
...
C2OUT->print_cr("---dump of B%d---",pre_order);
  for( uint i = 0; i < _cfg._num_blocks; i++ ) {
    Block *b = _cfg._blocks[i];
...
C2OUT->print_cr("---dump of L%d---",lidx);

  if( _ifg ) {
    if( lidx >= _maxlrg ) {
C2OUT->print("Attempt to print live range index beyond max live range.\n");
      return;
    }
C2OUT->print("L%d: ",lidx);
    lrgs(lidx).dump( );
  }
if(_ifg){C2OUT->print("Neighbors: %d - ",_ifg->neighbor_cnt(lidx));
...
C2OUT->print_cr("Inserted %d spill loads, %d spill stores, %d mem-mem moves and %d copies.",_final_loads,_final_stores,_final_memoves,_final_copies);
C2OUT->print_cr("Total load cost= %6.0f, store cost = %6.0f, mem-mem cost = %5.2f, copy cost = %5.0f.",_final_load_cost,_final_store_cost,_final_memove_cost,_final_copy_cost);
C2OUT->print_cr("Adjusted spill cost = %7.0f.",
                _final_load_cost*4.0 + _final_store_cost  * 2.0 + 
                _final_copy_cost*1.0 + _final_memove_cost*12.0);
C2OUT->print("Conservatively coalesced %d copies, %d pairs",
                _conserv_coalesce, _conserv_coalesce_pair);
  if( _conserv_coalesce_trie || _conserv_coalesce_quad )
C2OUT->print(", %d tries, %d quads",_conserv_coalesce_trie,_conserv_coalesce_quad);
C2OUT->print_cr(", %d post alloc.",_post_alloc);
  if( _lost_opp_pp_coalesce || _lost_opp_cflow_coalesce )
C2OUT->print_cr("Lost coalesce opportunity, %d private-private, and %d cflow interfered.",
                  _lost_opp_pp_coalesce, _lost_opp_cflow_coalesce );
  if( _used_cisc_instructions || _unused_cisc_instructions )
C2OUT->print_cr("Used cisc instruction  %d,  remained in register %d",
                   _used_cisc_instructions, _unused_cisc_instructions);
  if( _allocator_successes != 0 )
C2OUT->print_cr("Average allocation trips %f",(float)_allocator_attempts/(float)_allocator_successes);
C2OUT->print_cr("High Pressure Blocks = %d, Low Pressure Blocks = %d",_high_pressure,_low_pressure);
}
