...
if(PrintOpto)C2OUT->print_cr("CMOV to I2B");
...
Node *CheckCastPPNode::Identity( PhaseTransform *phase ) {
  ...
  // Azul note: not needed after the interface re-writing
...
const Type *CheckCastPPNode::Value( PhaseTransform *phase ) const {
  ...
  // Azul Note: interface types are here!!!  Back to the simple join call.
  return in_type->join(my_type);
}
...
if(in(1)->Opcode()==Op_AndI){
    jint mask;
    if( in(1)->in(2)->is_Con() &&
        (mask = in(1)->in(2)->get_int()) &&
        is_power_of_2(mask) ) { // Masking to a single bit
      return new (phase->C, 3) URShiftINode(in(1),phase->intcon(log2_intptr(mask)));
    }
  }
  return NULL;
}
...
//------------------------------Ideal------------------------------------------
// Use the faster float-sqrt instead of double-sqrt if possible.
// Is this a double-con with an exact float-con representation?
static bool is_double_float_con( const Type *t ) {
  if( t == Type::TOP ) return false;
if(t==Type::DOUBLE)return false;
  const TypeD *td = t->is_double_constant();
  double d = td->getd();
  float f = (float)d;
  return (double)f == d;
...
Node*ConvD2FNode::Ideal(PhaseGVN*phase,bool can_reshape){
  if( !can_reshape ) return NULL; // Need accurate DU info
  // Look for ConvF2D, SqrtD, ConvD2F and use only SqrtF.
Node*dop=in(1);//Some Double opcode
int opc=dop->Opcode();
  if( dop->outcnt() == 1 &&     // I am the single user
      opc == Op_SqrtD &&        // Single-input operand
      dop->in(1)->Opcode() == Op_ConvF2D )
    return new (phase->C, 2) SqrtFNode(dop->in(1)->in(1));

  // Same hack for MulD/DivD/SubD/AddD: if followed by D2F and preceeded by a
  // F2D then do the whole thing in single precision.
  if( dop->outcnt() == 1 &&   // I am the single user
      (opc == Op_MulD || opc == Op_DivD || opc == Op_AddD || opc == Op_SubD) &&
      (dop->in(1)->Opcode() == Op_ConvF2D || is_double_float_con(phase->type(dop->in(1)))) &&
      (dop->in(2)->Opcode() == Op_ConvF2D || is_double_float_con(phase->type(dop->in(2)))) ) {
Node*fop=NULL;
    switch( opc ) {
    case Op_MulD: fop = new (phase->C, 3) MulFNode(           0,0); break;
    case Op_DivD: fop = new (phase->C, 3) DivFNode(dop->in(0),0,0); break;
    case Op_AddD: fop = new (phase->C, 3) AddFNode(           0,0); break;
    case Op_SubD: fop = new (phase->C, 3) SubFNode(           0,0); break;
    }
Node*d1=dop->in(1);
Node*d2=dop->in(2);
    fop->set_req(1, d1->Opcode() == Op_ConvF2D ? d1->in(1) : phase->transform(new (phase->C, 2) ConvD2FNode(d1)));
    fop->set_req(2, d2->Opcode() == Op_ConvF2D ? d2->in(1) : phase->transform(new (phase->C, 2) ConvD2FNode(d2)));
    return fop;
  }
...
const Type *ConvI2LNode::Value( PhaseTransform *phase ) const {
  ...
  tl = tl->filter(_type);
  return tl;
  //// AzulBug 4181: if (tl->empty())  return TypeLong::make(type()->is_long()->_lo);
  //// Are types lifting or lowering?
  //if( phase->is_CCP() ) {       // Types are monotonically lowering
  //  // Ensure monotonicity
  //  const Type *told = phase->type(this);
  //  const Type *tx = told->meet(tj); // Forces monotonicity lowering
  //  // JOIN is empty but input is not (already checked for TOP) - which can
  //  // lead to an instance of Azulbug 6391 and friends.  Instead return the
  //  // input type, which is pessimestic but correct.
  //  return tx->empty() ? tl : tx;
  //} else if( phase->is_IterGVN() ) { // Else types are lifting
  //  // Ensure monotonicity, but also do not lift types to TOP (correct but
  //  // slightly pessimal).
  //  const Type *told = phase->type(this);
  //  return (tj->empty() && !told->empty()) ? told : tj;
  //} else {                      // Else initial parse and no prior types
  //  if (tj->empty())  return Type::TOP;
  //  return tj;
  //}
}
...
Node *ConvI2LNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  if (op == Op_AddI || op == Op_SubI) {
    ...
    // See if we produced a ConvI2L which is identical to another expect for
    // minor Type differences - which prevents useful folding of address
    // expressions.
    if( cx->Opcode() == Op_ConvI2L && cx->in(1)->outcnt() > 1 && can_reshape ) {
Node*x=cx->in(1);
      const Type *t = ((ConvI2LNode*)cx)->type();
for(DUIterator_Fast imax,i=x->fast_outs(imax);i<imax;i++){
Node*n=x->fast_out(i);
        if( n != cx && n->Opcode() == Op_ConvI2L )
          t = t->meet(((ConvI2LNode*)n)->type());
      }
...
Node*n=x->fast_out(i);
        if( n->Opcode() == Op_ConvI2L &&
            ((ConvI2LNode*)n)->type() != t ) {
          igvn->hash_delete(n);
          ((ConvI2LNode*)n)->set_type(t);
...
Node *ConvL2INode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  if( andl_op == Op_AndL ) {
    // Blow off prior masking to int
const Type*t_mask=phase->type(andl->in(2));
if(t_mask==TypeLong::make(0xFFFFFFFF)){
      set_req(1,andl->in(1));
      return this;
    }
const TypeLong*t_mask_l=t_mask->isa_long();
    if( t_mask_l && t_mask_l->is_con() && // If bit 31 will be clear then the 
        !is_set_nth_bit(t_mask_l->get_con(),31) ) { // result will be positive
      // If the AndL mask clears off bit 31, then the result will be a
      // positive integer and we can swap the ConvL2I and AndL, which in turn
      // will let me use an AndI mask.  The pattern ConvL2I/AndI is usually a
      // single instruction (no sign extension needed).  Happens for fast
      // Hashcode extraction.
      Node *cl2i = phase->transform(new (phase->C, 2) ConvL2INode(andl->in(1)));
      return new (phase->C, 3) AndINode( cl2i, phase->intcon(t_mask_l->get_con()&0x7fffffff) );
    }
  }
  ...
  if( andl_op == Op_LoadL ) { 
    ...
    // adr = phase->transform( new (phase->C, 4) AddPNode(base,adr,phase->longcon(sizeof(jint))));
    return NULL;
#else
...
static inline bool fits_in_int(const Type* t, bool but_not_min_int = false) {
  if (t == Type::TOP)  return false;
const TypeLong*tl=t->is_intptr_t();
  jint lo = min_jint;
  jint hi = max_jint;
...
static inline Node* addP_of_X2P(PhaseGVN *phase,
                                ...
                                bool negate = false) {
  if (negate) {
dispX=new(phase->C,3)SubLNode(phase->longcon(0),phase->transform(dispX));
  }
...
const TypeOopPtr*toop=tp->isa_oopptr();
  if( !toop && tp->singleton() ) { // RAW & NULL ptrs
    jlong bits = (jlong) tp->get_con();
return TypeLong::make(bits);
  }
  // An Azul special: if we are casting a known-typed object, then the
  // resulting Long has known constant klassID bits.
  if( toop && toop->klass_is_exact() )
    return TypeLong::make((jlong)(toop->klass()->klassId()));
...
//=============================================================================
//------------------------------Ideal------------------------------------------
Node*PartialSubtypeCheckNode::Ideal(PhaseGVN*phase,bool can_reshape){
...
//=============================================================================
// --- Ideal_DU_postCCP ------------------------------------------------------
Node*GetKIDNode::Ideal_DU_postCCP(PhaseCCP*ccp){
  // Need a null check?
  if( in(MemNode::Control) ) return NULL;

  return MemNode::Ideal_DU_postCCP_shared(ccp,this,in(1));
}

//------------------------------Value------------------------------------------
const Type*GetKIDNode::Value(PhaseTransform*phase)const{
  if( in(0) && phase->type(in(0)) == Type::TOP ) return Type::TOP;
  if( phase->type(in(1)) == TypePtr::NULL_PTR &&
      _type->isa_ptr() && _type->is_ptr()->_ptr == TypePtr::NotNull )
    return Type::TOP;
...
const TypeOopPtr*tp=t->is_oopptr();
  if( tp->above_centerline() ) return Type::TOP;
  if( tp->isa_klassptr() ) return Type::TOP;
  return TypeKlassPtr::make_kid(tp->klass(),tp->klass_is_exact());
}

//=============================================================================
//------------------------------Value------------------------------------------
const Type*KID2KlassNode::Value(PhaseTransform*phase)const{
  const Type *t = phase->type( in(1) );
  if( t == Type::TOP ) return Type::TOP;
if(t==TypePtr::NULL_PTR)return TypePtr::NULL_PTR;
const TypeOopPtr*tp=t->is_oopptr();
  if( tp->above_centerline() ) return Type::TOP;
  if( !tp->isa_klassptr() ) return Type::TOP;
  if( !tp->is_klassptr()->_is_kid ) return Type::TOP;
  return tp->is_klassptr()->cast_to_kid(false);
}

//------------------------------Identity---------------------------------------
// Remove back-to-back kid-to-klass-to-kid conversions
Node*KID2KlassNode::Identity(PhaseTransform*phase){
Node*lk=in(1);
  if( lk->Opcode() != Op_LoadKlass ) return this;
Node*adr=lk->in(LoadKlassNode::Address);
  if( !adr->is_AddP() ) return this;
Node*off=adr->in(AddPNode::Offset);
  if( off->find_long_con(0) != Klass::klassId_offset_in_bytes() + (int)sizeof(oopDesc) )
    return this;
return adr->in(AddPNode::Base);
}
...
jint i=ti->get_con();
    ...
    if (i == 0)
      return TypeInt::make(BitsPerInt);
    int m = 31;
    y = i << 16; if (y != 0) { m = m - 16; i = y; }
    y = i <<  8; if (y != 0) { m = m -  8; i = y; }
    y = i <<  4; if (y != 0) { m = m -  4; i = y; }
    y = i <<  2; if (y != 0) { m = m -  2; i = y; }
    y = i <<  1; if (y != 0) { m = m -  1; }
    return TypeInt::make(m);
  }
return TypeInt::INT;
...
const TypeLong*lt=t->isa_long();
  if (lt && lt->is_con()) {
    ...
    if (l == 0)
      return TypeInt::make(BitsPerLong);
    int m = 63;
    y = (int) l; if (y != 0) { m = m - 32; x = y; } else x = (((julong) l) >> 32);
    y = x << 16; if (y != 0) { m = m - 16; x = y; }
    y = x <<  8; if (y != 0) { m = m -  8; x = y; }
    y = x <<  4; if (y != 0) { m = m -  4; x = y; }
    y = x <<  2; if (y != 0) { m = m -  2; x = y; }
    y = x <<  1; if (y != 0) { m = m -  1; }
    return TypeInt::make(m);
  }
return TypeInt::INT;
