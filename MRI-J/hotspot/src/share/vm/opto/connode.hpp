...
public:
  ...
  virtual int Opcode() const;
  virtual int get_intptr_t( intptr_t *con ) const { *con = _type->is_int()->get_con(); return true; }
...
return new(C,1)ConPNode(con?TypeRawPtr::make(con):TypePtr::NULL_PTR);
  }
...
public:
  ...
  virtual int Opcode() const;
  virtual int get_intptr_t( intptr_t *con ) const { *con = _type->is_long()->get_con(); return true; }
...
//------------------------------CastP2XNode-------------------------------------
// Azul: All pointers are 64 bits
// Used for unsafe pointer math, but not card marks.
...
//------------------------------TicksNode-------------------------------------
// Ideal Node which returns the current hardware tick register
class TicksNode:public Node{
public:
  // Pin it to the current basic block, so reading from the hardware register
  // does not move to far from the intended point.  Without this control edge
  // the Ticks read might happen at method start, while the intended use is
  // e.g. after some long loop.  Still allows hoisting & folding within the
  // current basic block.
  TicksNode(Node *ctrl ) : Node(ctrl) {}
  virtual int Opcode() const;
virtual const Type*bottom_type()const{return TypeLong::BOTTOM;}
virtual uint ideal_reg()const{return Op_RegL;}
};
...
//------------------------------GetKIDNode------------------------------------
// Object-to-Klass-ID conversion.  Extracts the KID for a given Object.
class GetKIDNode:public TypeNode{
public:
  GetKIDNode( Node *ctrl, Node *ref, const TypeKlassPtr *tk ) : TypeNode(tk,2) { init_req(0,ctrl); init_req(1,ref); }
  virtual int Opcode() const;
  virtual const Type* Value(PhaseTransform* phase) const;
  virtual uint ideal_reg() const { return Op_RegP; }
...
//------------------------------KID2KlassNode---------------------------------
// KID-to-Klass conversion.  Loads the Klass for a given KID.  On Azul's
// implementation uses the KID bits to avoid a memory load.  Notice there is
// no control edge; getting the Klass of NULL returns a NULL, not a crash.
class KID2KlassNode:public TypeNode{
public:
  KID2KlassNode( Node *kid ) : TypeNode(TypeKlassPtr::OBJECT,2) { 
    assert0( kid->bottom_type()->is_klassptr()->_is_kid );
set_req(1,kid);
  }
  virtual int Opcode() const;
  virtual const Type* Value(PhaseTransform* phase) const;
