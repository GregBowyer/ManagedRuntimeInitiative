...
Node *DivLNode::Ideal( PhaseGVN *phase, bool can_reshape) {
  ...
  // Check for some powers-of-10 used in long<->ascii conversions
if(i==10){
    Node *tmp1 = phase->longcon(CONST64(0x6666666666666667));
    Node *tmp2 = phase->transform( new (phase->C, 3) MulHiLNode(tmp1,in(1)));
Node*tmp3=phase->transform(new(phase->C,3)RShiftLNode(tmp2,phase->intcon(2)));
Node*sign=phase->transform(new(phase->C,3)RShiftLNode(in(1),phase->intcon(63)));
    return new (phase->C, 3) SubLNode( tmp3, sign );
  }

if(i==100){
    Node *tmp1 = phase->longcon(CONST64(0xa3d70a3d70a3d70b));
    Node *tmp2 = phase->transform( new (phase->C, 3) MulHiLNode(tmp1,in(1)));
    Node *tmp3 = phase->transform( new (phase->C, 3) AddLNode(tmp2,in(1)));
Node*tmp4=phase->transform(new(phase->C,3)RShiftLNode(tmp3,phase->intcon(6)));
Node*sign=phase->transform(new(phase->C,3)RShiftLNode(in(1),phase->intcon(63)));
    return new (phase->C, 3) SubLNode( tmp4, sign );
  }

if(i==1000){
    Node *tmp1 = phase->longcon(CONST64(0x20c49ba5e353f7cf));
    Node *tmp2 = phase->transform( new (phase->C, 3) MulHiLNode(tmp1,in(1)));
Node*tmp3=phase->transform(new(phase->C,3)RShiftLNode(tmp2,phase->intcon(7)));
Node*sign=phase->transform(new(phase->C,3)RShiftLNode(in(1),phase->intcon(63)));
    return new (phase->C, 3) SubLNode( tmp3, sign );
  }

  // Check for the EXTREMELY popular div by millis-to-days conversion
if(i==86400000){
    Node *tmp1 = phase->longcon(CONST64(0x636ba875fd33dc87));
    Node *tmp2 = phase->transform( new (phase->C, 3) MulHiLNode(tmp1,in(1)));
Node*tmp3=phase->transform(new(phase->C,3)RShiftLNode(tmp2,phase->intcon(25)));
Node*sign=phase->transform(new(phase->C,3)RShiftLNode(in(1),phase->intcon(63)));
    return new (phase->C, 3) SubLNode( tmp3, sign );
  }
