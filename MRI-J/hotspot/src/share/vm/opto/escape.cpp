...
C2OUT->print("%s  %s  [[",node_type_names[(int)nt],esc_names[(int)es]);
  for (uint i = 0; i < edge_count(); i++) {
C2OUT->print(" %d%s",edge_target(i),edge_type_suffix[(int)edge_type(i)]);
  }
C2OUT->print("]]  ");
  if (_node == NULL)
C2OUT->print_cr("<null>");
...
Unimplemented();//Couldnt get the following to compile yet
#if 0
...
void ConnectionGraph::process_call_result(ProjNode *resproj, PhaseTransform *phase) {
  ...
  switch (call->Opcode()) {
    ...
    {
      Untested("");
      ciKlass* cik = call->tf()->domain()->field_at(TypeFunc::Parms+0)->is_instptr()->klass();
...
void ConnectionGraph::record_escape_work(Node *n, PhaseTransform *phase) {
  ...
  switch (opc) {
    ...
    case Op_StoreP:
#if 0 // unsupported Op_StorePConditional
    ...
    default:
      Unimplemented();
      // new in JAVA6 - would prefer to handle all cases explicitly
      ;
      // nothing to do
  }
}
...
if(es==PointsToNode::NoEscape){
      ...
      if (esp->_node->Opcode() != Op_ConP || igvn->type(esp->_node) != TypePtr::NULL_PTR) {
        if (first) {
C2OUT->print("======== Connection graph for ");
