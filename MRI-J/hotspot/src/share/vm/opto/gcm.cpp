...
void PhaseCFG::schedule_node_into_block( Node *n, Block *b ) {
  ...
  // If we decided to eat some stall time and inserted a long-latency op
  // into a short block (usually because hoisting enters a loop nest), then
  // we need to update the start-time for this block.
Node*least_head=b->_nodes[0];
  if( _node_latency.at_grow(n->_idx) > _node_latency.at_grow(least_head->_idx) ) {
    _node_latency.at_put_grow(least_head->_idx,_node_latency.at_grow(n->_idx));
partial_latency_of_defs(least_head);
  }
...
void PhaseCFG::schedule_pinned_nodes( VectorSet &visited ) {
  ...
  while ( spstack.is_nonempty() ) {
    Node *n = spstack.pop();
    assert0( C->node_arena()->contains(n) );
...
C2OUT->print_cr("!!! Unschedulable graph !!!");
...
C2OUT->print("B%d idom=B%d depth=%2d ",inb->_pre_order,
                   inb->_idom ? inb->_idom->_pre_order : 0, inb->_dom_depth);
      inn->dump();
    }
C2OUT->print("Failing node: ");
    n->dump();
...
//while_nstack_nonempty:
    while (true) {
      ...
      while (i < n->len()) {         // For all inputs
        ...
        assert0( C->node_arena()->contains(in) );
...
static Block* memory_early_block(Node* load, Block* early, Block_Array &bbs) {
  if( load->Opcode() == Op_ArrayCopySrc ) return early; // No change for array-copy-source
  if( load->Opcode() == Op_EscapeMemory ) return early; // No change for escape-memory
  Node* base;
  Node* index;
...
C2OUT->print_cr("*** Possible Anti-Dependence Bug:  Load consumes all of memory.");
    load->dump(2);
  ...
  if( load_alias_idx == Compile::AliasIdxBot && load->req() == 2 ) {
    assert0( load->is_Mach() );
    assert0( ((MachNode*)load)->num_opnds() == 2 );
    // If this is a load from Raw memory with no possibility of variant
    // address edges, then we assume it is coming from the OopTable and
    // represents a constant load - invariant memory so no aliasing.
    // Loads from TLS - pending exception - DO alias with null stores
    // into pending exception.
    if( ((MachNode*)load)->_opnds[1]->constant_disp() != in_bytes(JavaThread::pending_exception_offset()) )
      return LCA;
  }
  ...
  while (worklist_store.size() > 0) {
    ...
    if (store->is_Mach()) {
      ...
      int miop = mstore->ideal_Opcode();
      // On Azul, Safepoints do not write 'raw' memory - allocation is done
      // inside millicode.  This means Safepoints really only ever read memory.
      if ( miop == Op_SafePoint )
        continue;
...
if(miop==Op_CallStaticJava){
        ...
        // These runtime calls do not write to Java visible memory
        // (other than Raw) and so do not require anti-dependence edges.
        if( mstore->is_MachAllocate() )
          continue;
        // Also ignore multianewarray calls.
        if( mstore->is_MachCallVM() && !strncmp(mstore->as_MachCallVM()->_name,"multianewarray",10) )
          continue;
...
if(miop==Op_SafePoint)
        ...
        if (mstore->ideal_Opcode() == Op_SafePoint && load->in(0) == mstore)
          continue;
#endif
        if( miop == Op_MemBarRelease ||
            miop == Op_MemBarAcquire ||
            miop == Op_MemBarVolatile ||
            miop == Op_MemBarCPUOrder )
          continue;
      }
    }
...
#ifndef PRODUCT
  if (trace_opto_pipelining()) {
C2OUT->print("# latency_to_inputs: node_latency[%d] = %d for node",
...
#ifndef PRODUCT
    if (trace_opto_pipelining()) {
C2OUT->print_cr("#      %d + edge_latency(%d) == %d -> %d, node_latency[%d] = %d",
...
#ifndef PRODUCT
        if (trace_opto_pipelining()) {
C2OUT->print_cr("#      %d + edge_latency(%d) == %d -> %d, latency = %d",
                          nl, j, ul, l, latency);
        }
#endif
      }
...
#ifndef PRODUCT
  if (trace_opto_pipelining()) {
C2OUT->print("# latency_from_outputs: node_latency[%d] = %d for node",
                 n->_idx, _node_latency.at_grow(n->_idx));
    dump();
  }
#endif
...
static double latency_cost( double use_freq, double def_freq, int target_latency, int def_start_latency ) {
  // Latency cost is: the cost to uselessly execute an instruction prematurely
  // in the def-block vs failing to cover the long latency if we make it down
  // to the use-block.  

  // Issue cost to schedule the instruction in the def-block
  double cost = def_freq;

  // Latency not covered; assume the op is scheduled at the def-block's start
  int latency_not_covered = target_latency - def_start_latency;
  if( latency_not_covered > 0 ) 
    // Any un-covered latency causes a stall in the use-block
    cost += (double)latency_not_covered*MIN2(def_freq,use_freq);
  return cost;
}
...
double use_freq=least->_freq;
  ...
  double least_cost  = latency_cost(use_freq,least_freq,target,start_latency);

  // Turn off latency scheduling if scheduling is just plain off
  if ( !OptoSchedulingPre )
    in_latency = true;
...
#ifndef PRODUCT
  if (trace_opto_pipelining()) {
C2OUT->print("# Find cheaper block for latency %d: ",
      _node_latency.at_grow(self->_idx));
    self->dump();
C2OUT->print_cr("#   B%d: start latency for [%4d]=%d, end latency for [%4d]=%d, freq=%g",
      LCA->_pre_order,
      LCA->_nodes[0]->_idx,
      start_latency,
...
C2OUT->print_cr("#   B%d: start latency for [%4d]=%d, end latency for [%4d]=%d, freq=%g",
    ...
    if (LCA_freq < least_freq              || // Better Frequency
        ( !in_latency                   &&    // No block containing latency
          // Covered latency cost is worth premature execution?
          (LCA_cost=latency_cost(use_freq,LCA_freq,target,start_lat)) < least_cost &&
...
#ifndef PRODUCT
  if (trace_opto_pipelining()) {
C2OUT->print_cr("#  Choose block B%d with start latency=%d and freq=%g",
...
#ifndef PRODUCT
    if (trace_opto_pipelining()) {
C2OUT->print_cr("#  Change latency for [%4d] from %d to %d",self->_idx,target,end_latency);
    }
#endif
...
while((self=iter.next())!=NULL){
    ...
    MachNode* mach = self->is_Mach() ? self->as_Mach() : NULL;
    // I do not understand how a CheckCastPP can ever see a RawPtr input,
    // except perhaps as part of an inlined new allocation.  Since Azul does
    // not inline allocations, I am merely going to assert here.
    assert0( !mach || mach->ideal_Opcode() != Op_CheckCastPP ||
             !mach->in(1) || mach->in(1)->bottom_type()->base() != Type::RawPtr );
...
C->record_failure(C2Compiler::retry_no_subsuming_loads(),false);
      } else {
...
ResourceArea*a=Thread::current()->resource_area();
  VectorSet visited(a);
  GrowableArray<uint> node_latency(C->unique()+8); // hoist the init
  ...
  if (!schedule_early(visited, stack)) {
    // Bailout without retry
C->record_failure("early schedule failed",false);
    return;
  }
...
C2OUT->print("\n---- Detect implicit null checks ----\n");
  ...
  {
    // SUN added a cutout here which Azul does not need nor want.
...
_bbs[proj->_idx]->implicit_null_check(this,proj,val);
    }
  }

  // Insert LVBs after pointer loads from the heap and before the next safepoint.
  if( UseLVBs || RefPoisoning ) {
    for( uint i = 0; i < _num_blocks; i++) {
      Block *b = _blocks[i];
      for( uint j = 0; j < b->_nodes.size(); j++ )
        MachLVBNode::insert(b->_nodes[j], node_latency);
    }
  }
...
C->record_failure("local schedule failed",false);
      }
      return;
    }
  }
...
void PhaseCFG::Estimate_Block_Frequency() {
  int cnts = C->c2_cp()->invoke_count(); // Note: can be zero for OSR compiles
...
C2OUT->print_cr("CFG Block Frequencies");
    _root_loop->dump_tree();
    if (Verbose) {
C2OUT->print_cr("PhaseCFG dump");
      dump();
C2OUT->print_cr("Node dump");
      _root->dump(99999);
    }
  }
#endif
...
for(int i=0;i<_depth;i++)C2OUT->print("   ");
C2OUT->print("%s: %d  trip_count: %6.0f freq: %6.0f\n",
               _depth == 0 ? "Method" : "Loop", _id, trip_count(), _freq);
for(int i=0;i<_depth;i++)C2OUT->print("   ");
C2OUT->print("         members: %d",_id);
  int k = 0;
  for (int i = 0; i < _members.length(); i++) {
    if (k++ >= 6) {
C2OUT->print("\n              ");
for(int j=0;j<_depth+1;j++)C2OUT->print("   ");
      k = 0;
    }
    CFGElement *s = _members.at(i);
    if (s->is_block()) {
      Block *b = s->as_Block();
C2OUT->print(" B%d(%6.3f)",b->_pre_order,b->_freq);
    } else {
      CFGLoop* lp = s->as_CFGLoop();
C2OUT->print(" L%d(%6.3f)",lp->_id,lp->_freq);
    }
  }
C2OUT->print("\n");
for(int i=0;i<_depth;i++)C2OUT->print("   ");
...
C2OUT->print(" ->%d@%d%%",blk->_pre_order,(int)(prob*100));
  }
C2OUT->print("\n");
}
#endif
