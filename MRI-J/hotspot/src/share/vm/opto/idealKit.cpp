...
Node*ld=LoadNode::make(C,ctl,mem,adr,adr_type,t,bt);
  return transform(ld);
}
...
Node* IdealKit::store(Node* ctl, Node* adr, Node *val, BasicType bt,
  ...
  debug_only(adr_type = C->get_adr_type(adr_idx));
  Node *mem = memory(adr_idx);
Node*st=StoreNode::make(C,ctl,mem,adr,adr_type,val,bt);
  st = transform(st);
...
CallNode*call=(CallNode*)new(C,size)CallLeafNode(slow_call_type,slow_call,leaf_name,NULL,false,adr_type);
