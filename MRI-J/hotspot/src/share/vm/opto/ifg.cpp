...
//------------------------------compute_degree---------------------------------
// Compute the degree between 2 live ranges.  If both live ranges are normal
// then we use size 1.  If either is Fat-Projections (not-adjacent) then we
// have to MULTIPLY the sizes.  Inspect Brigg's thesis on register pairs to
// see why this is so.
int LRG::compute_degree( LRG &l ) const {
  assert0( !(_fat_proj && l._fat_proj) ); // cannot have 2 fatproj live ranges alive at once
  if(   _fat_proj ) return   _fat_proj_kill_regs;
  if( l._fat_proj ) return l._fat_proj_kill_regs;
  return 1;
}
...
int PhaseIFG::effective_degree( uint lidx ) const {
  ...
  uint nidx;
if(lrgs(lidx)._fat_proj){
int num_regs=lrgs(lidx)._fat_proj_kill_regs;
    while((nidx = elements.next()) != 0) {
      assert0( !lrgs(nidx)._fat_proj ); // cannot have 2 fatproj live ranges alive at once
      eff += num_regs;
    }
    ...
    while((nidx = elements.next()) != 0) {
      LRG &lrgn = lrgs(nidx);
      eff += (lrgn._fat_proj ? lrgn._fat_proj_kill_regs : 1);
    }
  }
  return eff;
}
...
C2OUT->print_cr("-- Interference Graph --%s--",
                _is_square ? "square" : "triangular" );
  if( _is_square ) {
    for( uint i = 0; i < _maxlrg; i++ ) {
C2OUT->print((*_yanked)[i]?"XX ":"  ");
C2OUT->print("L%d: { ",i);
      IndexSetIterator elements(&_adjs[i]);
      uint datum;
      while ((datum = elements.next()) != 0) {
C2OUT->print("L%d ",datum);
      }
C2OUT->print_cr("}");
...
C2OUT->print_cr("--Histogram of counts--");
  for( i = 0; i < _maxlrg*2; i++ ) 
    if( h_cnt[i] )
C2OUT->print("%d/%d ",i,h_cnt[i]);
C2OUT->cr();
}
...
if(!liveout.member(r)&&!n->is_RangeCheck()){
          Node *def = n->in(0);
          if( !n->is_Proj() ||
          ...
          lrgs(r)._area -= cost;
	  if( n->is_RangeCheck() && lrgs(r)._area < 0 )
	    lrgs(r)._area += cost; // Unused range-check value gets screwy area
...
OptoReg::Name r_reg=(!lrgs(r)._fat_proj)?rmask.find_first_elem():OptoReg::Physical;
...
lrg.set_reg(OptoReg::SPILL_REG);
      ...
      } else if( (n->flags() & Node::Flag_is_safepoint_node) != 0 &&
                 n->is_MachCall() ) {  // exclude MachSafepoints that are not MachCalls ("real" safepoints)
        address entry = n->as_MachCall()->_entry_point;
        if( entry == StubRoutines::resolve_and_patch_call_entry() ||
            entry == StubRoutines::new_sba() ||
            entry == StubRoutines::new_fast() ||
            entry == StubRoutines::new_sba_array() ||
            entry == StubRoutines::new_fast_array() ||
            entry == (address)SharedRuntime::monitorenter ||
            entry == (address)SharedRuntime::_new ||
            0 ) {
          // Kill all oops in registers across safepoint calls, except
          // Official Safepoints which support any & all oops-in-regs.
          IndexSetIterator elements(&liveout);
          uint l;
          while ((l = elements.next()) != 0) {
            LRG &lrg = lrgs(l);
if(lrg._is_oop){
              lower_pressure( &lrg, j-1, b, &pressure[0], &hrp_index[0] );
              lrg.SUBTRACT(*_matcher.idealreg2regmask[Op_RegP]);
              lrg.compute_set_mask_size();
            }  
          }
        }
