...
static Node* split_if(IfNode *iff, PhaseIterGVN *igvn) {
  ...
  if (b->outcnt() != 1 || cmp->outcnt() != 1) {
    //C2OUT->print_cr("many users of cmp/bool");
    return NULL;
  }
  ...
  for (DUIterator_Fast jmax, j = r->fast_outs(jmax); j < jmax; j++) {
    ...
    if( !u->is_Phi() ) {
      /*
      if( u->is_Start() ) {
        C2OUT->print_cr("Region has inlined start use");
      } else {
        C2OUT->print_cr("Region has odd use");
        u->dump(2);
      }*/
      return NULL;
    }
    if( u != phi ) {
      ...
      //C2OUT->print_cr("Merging another value");
      //u->dump(2);
      return NULL;
    }
    ...
    for (DUIterator_Fast kmax, k = u->fast_outs(kmax); k < kmax; k++) {
      ...
      if( !v->is_Call() ) {
        /*
        if( v->Opcode() == Op_AddP ) {
          C2OUT->print_cr("Phi has AddP use");
        } else if( v->Opcode() == Op_CastPP ) {
          C2OUT->print_cr("Phi has CastPP use");
        } else if( v->Opcode() == Op_CastII ) {
          C2OUT->print_cr("Phi has CastII use");
        } else {
          C2OUT->print_cr("Phi has use I cant be bothered with");
        }
        */
      }
      return NULL;
      ...
      if( !v->in(0) && v != cmp ) {
        C2OUT->print_cr("Phi has free-floating use");
        v->dump(2);
        return NULL;
      }
...
int IfNode::is_range_check(Node*&range,Node*&index,jint&offset,PhaseGVN*phase){
  Node* b = in(1);
  if (b == NULL || !b->is_Bool())  return 0;
  BoolNode* bn = b->as_Bool();
  Node* cmp = bn->in(1);
  if (cmp == NULL)  return 0;
int cmpop=cmp->Opcode();
  if( cmpop != Op_CmpU &&
      cmpop != Op_CmpI ) return 0; // Not a compare

  Node* l = cmp->in(1);		// Preload compare values
  Node* r = cmp->in(2);
  if( cmpop == Op_CmpI && 	// Allow signed compare against positives
      !(phase->type(l)->higher_equal(TypeInt::POS) &&
	phase->type(r)->higher_equal(TypeInt::POS) ) )
    return 0;			// Signed compare; but one was not positive
  ...
  if (bn->_test._test == BoolTest::le) {
    ...
    r = cmp->in(1);
    flip_test = 2;
  } else if (bn->_test._test == BoolTest::ne &&
             phase->type(r)->higher_equal(TypeInt::ZERO) ) {
    l = cmp->in(2);
    r = cmp->in(1);
...
if(req==Deoptimization::Reason_range_check){
          found_trap = true;
        }
      }
    }
...
//------------------------------find_unk---------------------------------------
// Require the widened check to bail out to an uncommon-trap, which will
// (slowly) execute the correct semantics starting at the bytecode of the
// widened check.  No good to fatten the check, only to have it lead off to
// arbitrary user code.
static Node*find_unk(Node*prev_chk){
assert(prev_chk->in(0)->is_If(),"");
assert(prev_chk->is_Proj(),"");
  IfNode *iff = (IfNode*)(prev_chk->in(0));
  // Find the Projection for the fail path.
  Node *fail = iff->proj_out( 1-((ProjNode*)prev_chk)->_con );
  int cnt = 0;
  while( 1 ) {
    // See if we can find 1 control user.
Node*x=NULL;
for(DUIterator_Fast jmax,j=fail->fast_outs(jmax);j<jmax;j++){
      x = fail->fast_out(j);
      if( x != fail && x->is_CFG() )  break;
    }
    if( !x ) return false;      // No control users?  Partially dead code?
fail=x;
    if( fail->is_Call() &&
        ((CallNode*)fail)->entry_point() == CAST_FROM_FN_PTR(address, StubRoutines::uncommon_trap_entry()) ) {
      CallNode *cfail = (CallNode*)fail;
      if( cfail->_cpd) {
        if (cfail->_cpd->is_Branch(cfail->bc())) {
          CPData_Branch* cpdb = (CPData_Branch*)cfail->_cpd;
          debug_only( assert0(cpdb->is_Branch()) );
          if (cpdb->_taken != 0 && cpdb->_nottaken != 0) break;
        } else {
          CPData_Null* cpdn = (CPData_Null*)cfail->_cpd;
          debug_only( assert0(cpdn->is_Null()) );
          if (cpdn->did_rchk_wide()) break;
        }
      }
      int didx = fail->in(TypeFunc::Parms)->bottom_type()->is_int()->_lo;
      if( didx == Deoptimization::Reason_range_check ||
          didx == Deoptimization::Reason_range_check_widened )
        return fail;
    }
    if( !fail->is_Region() )  break;
    if( cnt++ > 50 ) break;     // Infinite loop?
  }
  return NULL;
}
...
int flip,int off_lo,PhaseIterGVN*igvn){
  PhaseGVN *gvn = igvn;
  // Break apart the old check
  Node *iff = proj->in(0);
  Node *bol = iff->in(1);
if(!bol->is_Bool())return;//In case a partially dead range check appears
  Node *cmp = bol->in(1);
  // Compute a new check
  ...
  // Any loads from this adjusted check can no longer freely hoist.
  // Check both projections underneath the IfNode.
for(DUIterator_Fast jmax,j=proj->fast_outs(jmax);j<jmax;j++){
Node*ld=proj->fast_out(j);
    if( ld->is_Load() ) ld->as_Load()->guarding_test_no_longer_precise();
  }
  assert0( iff->is_If() );
  proj = ((IfNode*)iff)->proj_out(1-((ProjNode*)proj)->_con);
  assert0( proj->is_Proj() );
for(DUIterator_Fast jmax,j=proj->fast_outs(jmax);j<jmax;j++){
Node*ld=proj->fast_out(j);
    if( ld->is_Load() ) ld->as_Load()->guarding_test_no_longer_precise();
  }
...
// Return an equivalent dominating test, if any.
static Node *dominating_test( IfNode* iff, int is_range_check ) {
  int dist = 0;               // Cutoff limit for search
  Node *cmp;
int op=iff->Opcode();
  if( op == Op_If && 
      (cmp=iff->in(1)->in(1))->Opcode() == Op_CmpP ) {
...
dist=6;//Do not bother for random pointer tests
    }
  } else {
dist=6;//Limit for random junky scans
  }
  if( is_range_check ) 
dist=999;//Cutoff for equivalent range-check scans
...
Node*dom=iff->in(0);
  ...
  while( dom->Opcode() != op    ||  // Not same opcode?
         dom->in(1)    != iff->in(1) ||  // Not same input 1?
         (iff->req() == 3 && dom->in(2) != iff->in(2)) || // Not same input 2?
...
Node *IfNode::Ideal(PhaseGVN *phase, bool can_reshape) {
  ...
  Node *index1, *range1;
  jint offset1;
int flip1=is_range_check(range1,index1,offset1,phase);

//Check for a prior equivalent dominating test
  Node *prev_dom = dominating_test(this,flip1);

  // If that fails, check for a pair of covering prior range checks
  if( !prev_dom && !flip1 ) return NULL;
  if( !prev_dom && flip1 ) {
    // Try to remove extra range checks.  All 'up_one_dom' gives up at merges
    // so all checks we inspect post-dominate the top-most check we find.
    // If we are going to fail the current check and we reach the top check
    // then we are guarenteed to fail, so just start interpreting there.
    // We 'expand' the top 2 range checks to include all post-dominating checks.
    Node *dom = in(0);
    prev_dom = this;
    ...
    for( int dist = 0; dist < 999; dist++ ) { // Range-Check scan limit
      if( dom->Opcode() == Op_If &&  // Not same opcode?
        ...
        Node *index2, *range2;
        jint offset2;
int flip2=dom->as_If()->is_range_check(range2,index2,offset2,phase);
...
if(dom->in(1)==in(1)){
            up_one_dom(dom);    // defeat compiler bug in Forte 7 C 5.4, 2002/03/09
            prev_chk2 = prev_chk1;
          }
        }
      }
    ...
    // Also require the prev check to bail out to an uncommon-trap, which will
    // (slowly) execute the correct semantics starting at the bytecode of the
    // prev check.  No good to fatten the check, only to have it lead off to
    // arbitrary user code.
    Node *unk1 = find_unk(prev_chk1);
if(!unk1)return NULL;
    ...
    if( index1 ) {
      ...
      // prev check.  No good to fatten the check, only to have it lead off to
      // arbitrary user code.
      Node *unk2 = find_unk(prev_chk2);
if(!unk2)return NULL;
      // Change the uncommon-trap function to recognize a failed widen attempt
      unk2->set_req(TypeFunc::Parms, igvn->intcon(Deoptimization::Reason_range_check_widened));
    ...
    // Change the uncommon-trap function to recognize a failed widen attempt
    unk1->set_req(TypeFunc::Parms, igvn->intcon(Deoptimization::Reason_range_check_widened));
  } // End of scan for covering range checks

  // Hit!  Remove this IF
#ifndef PRODUCT
  if( TraceIterativeGVN ) {
C2OUT->print("   Removing IfNode: ");this->dump();
  }
