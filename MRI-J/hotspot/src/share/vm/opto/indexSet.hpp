...
static ResourceArea*arena(){return Compile::current()->indexSet_arena();}
...
static void reset_memory(Compile*compile,ResourceArea*arena){
...
C2OUT->print_cr("IndexSet %d : %s ( %d )",_serial_number,operation,operand);
      }
    }
  }
  void check_watch(const char *operation) const {
    if (IndexSetWatch != 0) {
      if (IndexSetWatch == -1 || _serial_number == IndexSetWatch) {
C2OUT->print_cr("IndexSet %d : %s",_serial_number,operation);
      }
    }
  }
