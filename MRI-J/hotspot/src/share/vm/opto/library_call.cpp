...
class LibraryCallKit : public GraphKit {
 private:
  ...
  CPData_Invoke *_cpdi;
  bool  _cloned_in_citypeflow;

 public:
  LibraryCallKit(JVMState* caller, LibraryIntrinsic* intrinsic, CPData_Invoke *cpdi, bool cloned_in_citypeflow)
    : GraphKit(caller),
...
Node*generate_virtual_guard(Node*obj_klass,RegionNode*slow_region,int*vtable_entry);
...
bool inline_prefetch(vmIntrinsics::ID id);
...
Node*dest_elem_kid,
  ...
  bool inline_blockinghint();
};


//---------------------------make_vm_intrinsic----------------------------
CallGenerator* Compile::make_vm_intrinsic(ciMethod* m, bool is_virtual) {
if(!UseIntrinsics)return NULL;
...
case vmIntrinsics::_stringEquals:
...
JVMState* LibraryIntrinsic::generate(JVMState* jvms, CPData_Invoke *cpdi, bool cloned_in_citypeflow) {
  LibraryCallKit kit(jvms, this, cpdi, cloned_in_citypeflow);
  Compile* C = kit.C;
  int nodes = C->unique();
#ifndef PRODUCT
...
C2OUT->print("Inlining intrinsic %s%s at bci:%d in",
                 vmIntrinsics::name_at(intrinsic_id()),
                 (is_virtual() ? " (virtual)" : ""), kit.bci());
kit.caller()->print_short_name(C2OUT);
C2OUT->print_cr(" (%d bytes)",kit.caller()->code_size());
    }
...
bool LibraryCallKit::try_to_inline() {
  // Failed at this BCI already?  Do not try again!
  if( _cpdi->did_fail() ) { Untested(); return false; }
...
case vmIntrinsics::_shared:
case vmIntrinsics::_exclusive:
return inline_prefetch(intrinsic_id());
...
case vmIntrinsics::_internal_set:
    return inline_blockinghint();
  case vmIntrinsics::_get_AtomicLong:
...
const Type*thread_type=TypeOopPtr::make_from_klass_unique(thread_klass)->cast_to_ptr_type(TypePtr::NotNull);
...
bool LibraryCallKit::inline_string_compareTo() {
  ...
  _sp += 2;
  receiver = do_null_check(receiver, T_OBJECT, Deoptimization::Reason_null_check, "", _cpdi);
  argument = do_null_check(argument, T_OBJECT, Deoptimization::Reason_null_check, "", _cpdi);
  _sp -= 2;
  if (stopped()) {
    return true;
  }
  ...
  Node* compare =
_gvn.transform(new(C,5)StrCompNode(
                        control(), 
...
bool LibraryCallKit::inline_string_equals(){
  ...
  // which handles NullPointerExceptions.
  _sp += 2;
  receiver = do_null_check(receiver, T_OBJECT, Deoptimization::Reason_null_check, "intrinsic null chk", _cpdi);
  if (stopped()) {
    _sp -= 2;
    return true;
  }
  ...
  // Check if String
  ciInstanceKlass* klass = env()->String_klass();
  Node *phi = gen_instanceof(argument, makecon(TypeKlassPtr::make(klass)), "intrinsic null chk 2nd arg", _cpdi);
_sp-=2;//Now pop stack
  // Could be a NULL passed to String.equals
  if( phi->is_Phi() ) {

    // Path #1 into the instanceof is the "yes its a String" path.
    const TypeInstPtr* string_type = TypeInstPtr::make(TypePtr::BotPTR, klass, false, NULL, 0);
    Node* compare = 
      _gvn.transform(new (C, 5) StrEqualsNode(control()->in(1), 
                                              memory(TypeAryPtr::CHARS),
                                              memory(string_type->add_offset(value_offset)),
                                              receiver, 
                                              argument));
phi->set_req(1,compare);
  }
...
Node*sourceCount=load_array_length(source);
...
Node*targ=load_array_element(__ ctrl(),target,tpj,target_type);
              Node* ipj  = __ AddI(__ value(i), __ value(j));
Node*src2=load_array_element(__ ctrl(),source,ipj,TypeAryPtr::CHARS);
...
int o=0;
  ...
  // which handles NullPointerExceptions.
  _sp += 2;
  receiver = do_null_check(receiver, T_OBJECT, Deoptimization::Reason_null_check, "unsafe null chk", _cpdi);
...
Node * LibraryCallKit::pop_math_arg() {
  Node *arg = pop_pair();
if(Matcher::strict_fp_requires_explicit_rounding&&/*UseSSE*/2<=1)
...
bool LibraryCallKit::inline_trig(vmIntrinsics::ID id) {
  ...
  if( Matcher::strict_fp_requires_explicit_rounding ) {
    ...
    switch (id) {
    case vmIntrinsics::_dsin:
call=make_runtime_call(RC_LEAF,false/*!must_callruntimenode*/,
...
assert(id==vmIntrinsics::_dsqrt||id==vmIntrinsics::_fsqrt,"Not square root");
  _sp += arg_size();        // restore stack pointer
  Node *arg = pop_math_arg();
  Node *sqrt = id == vmIntrinsics::_dsqrt 
    ? (Node*)new (C, 2) SqrtDNode(arg) 
    : (Node*)new (C, 2) SqrtFNode(arg);
push_pair(_gvn.transform(sqrt));
  return true;
}
...
bool LibraryCallKit::inline_exp(vmIntrinsics::ID id) {
  ...
  if( _cpdi->did_fail() ) return false;
Untested("JAVA6");
  ...
  { BuildCutout unless(this, bolisnum, PROB_STATIC_FREQUENT);
    ...
    uncommon_trap(Deoptimization::Reason_intrinsic_check, NULL, "intrinsic Math.exp failed", false );
  }
...
Untested("JAVA6");
  ...
  { BuildCutout unless(this, bolisnum, PROB_STATIC_FREQUENT);
    ...
    uncommon_trap(Deoptimization::Reason_intrinsic_check, NULL, "intrinsic Math.pow failed", false );
  }
...
Node*trig=make_runtime_call(RC_LEAF,false/*!must_callruntimenode*/,
call_type,funcAddr,funcName,
                                 no_memory_effects,
                                 a, top(), b, b ? top() : NULL);
...
//------------------------------inline_prefetch--------------------------------
// Inline Java specificed (hand generated) prefetch ops
bool LibraryCallKit::inline_prefetch(vmIntrinsics::ID id){
_sp+=arg_size();//restore the arguments so I can pop with names
  Node *off = pop_pair();       // Long offset
Node*obj=pop();//Object base
Node*adr=_gvn.transform(new(C,4)AddPNode(obj,obj,off));
Node*pre=NULL;
  if (intrinsic_id() == vmIntrinsics::_shared) {
    pre = _gvn.transform( new (C, 3) PrefetchReadNode( memory(Compile::AliasIdxRaw), adr ) );
  } else if (intrinsic_id() == vmIntrinsics::_exclusive) {
    pre = _gvn.transform( new (C, 3) PrefetchWriteNode( memory(Compile::AliasIdxRaw), adr ) );
  } else {
    ShouldNotReachHere();
  }
  set_memory(pre,Compile::AliasIdxRaw);
  return true;
}
...
const TypeLong*offset_type=_gvn.type(offset)->isa_intptr_t();
    if (offset_type != NULL &&
...
const TypePtr*tjp=TypeOopPtr::make_from_klass_unique(sharpened_klass);
  ...
  // which handles NullPointerExceptions.
  _sp += nargs;
  do_null_check(receiver, T_OBJECT, Deoptimization::Reason_null_check, "unsafe null chk", _cpdi);
  _sp -= nargs;
  if (stopped()) {
    return true;
  }
  ...
  if (!is_store) {
    ...
    if (type != T_OBJECT ) {
(void)store_to_memory(control(),adr,val,type,adr_type);
    } else {
...
bool LibraryCallKit::inline_unsafe_prefetch(bool is_native_ptr, bool is_store, bool is_static) {
  ...
  if (is_static) {
    ...
    // which handles NullPointerExceptions.
    _sp += nargs;
    do_null_check(receiver, T_OBJECT, Deoptimization::Reason_null_check, "intrinsic Unsafe.prefetch null chk", _cpdi);
    _sp -= nargs;
    if (stopped()) {
      return true;
    }
...
bool LibraryCallKit::inline_unsafe_CAS(BasicType type) {
  ...
  //  Null check receiver.
  _sp += nargs;
  do_null_check(receiver, T_OBJECT, Deoptimization::Reason_null_check, "Unsafe.CAS null chk", _cpdi);
  _sp -= nargs;
  if (stopped()) {
    return true;
  }
...
Node*cas=NULL;
  ...
  // The ideal CAS produce flags, but the Unsafe code wants a boolean
Node*bol=_gvn.transform(new(C,2)BoolNode(cas,BoolTest::eq));
  IfNode *iff = create_and_map_if( control(), bol, PROB_MAX/*CAS rarely fails*/, -1.0F );

RegionNode*region=new(C,3)RegionNode(3);
Node*phi=new(C,3)PhiNode(region,TypeInt::BOOL);

region->init_req(1,_gvn.transform(new(C,1)IfFalseNode(iff)));
  phi->init_req(1,_gvn.intcon(0));

region->init_req(2,_gvn.transform(new(C,1)IfTrueNode(iff)));
  phi->init_req(2,_gvn.intcon(1));
...
bool LibraryCallKit::inline_unsafe_ordered_store(BasicType type) {
  ...
  //  Null check receiver.
  _sp += nargs;
  do_null_check(receiver, T_OBJECT, Deoptimization::Reason_null_check, "intrinsic Unsafe.ordered null chk", _cpdi);
  _sp -= nargs;
  if (stopped()) {
    return true;
  }
...
store=store_to_memory(control(),adr,val,type,adr_type);
  else {
store=store_to_memory(control(),adr,val,type,adr_type);
  }
  insert_mem_bar(Op_MemBarCPUOrder);
...
null_check_receiver(callee(),_cpdi);//check then ignore argument(0)
  _sp += nargs;  // set original stack for use by uncommon_trap
  Node* cls = do_null_check(argument(1), T_OBJECT, Deoptimization::Reason_null_check, "unsafe null chk allocate", _cpdi);
  _sp -= nargs;
  if (stopped())  return true;

  Node* kls = load_klass_from_mirror(cls, false, nargs, NULL, 0);
  _sp += nargs;  // set original stack for use by uncommon_trap
  kls = do_null_check(kls, T_OBJECT, Deoptimization::Reason_null_check, "unsafe null chk allocate", _cpdi);
  _sp -= nargs;
  if (stopped())  return true;  // argument was like int.class

const TypeKlassPtr*tkls=_gvn.type(kls)->is_klassptr();
  Node *kid_adr = basic_plus_adr(kls, Klass::klassId_offset_in_bytes() + (int)sizeof(oopDesc));
  Node *kid_node = _gvn.transform(new(C,3) LoadKlassNode(NULL,immutable_memory(),kid_adr,_gvn.type(kid_adr)->is_ptr(),tkls->cast_to_kid(true)));
  ...
  Node* obj = new_instance(kid_node,test,_cpdi);
  push(obj);
...
bool LibraryCallKit::inline_native_time_funcs(bool isNano) {
  if( !isNano && UseAznixSystemCTM ) {
    // System.currentTimeMillis() has this cool implementation on Azul.
    // We read a global memory word, and that's the answer.
    const TypeRawPtr *t_adr = TypeRawPtr::make(os::System_CTM_addr());
Node*adr=makecon(t_adr);
    Node *tm = _gvn.transform(make_load(NULL,adr,TypeLong::LONG,T_LONG));
push_pair(tm);
    return true;
  }
  ...
  const TypePtr* no_memory_effects = NULL;
Node*time=make_runtime_call(RC_LEAF,false/*!must_callruntimenode*/,tf,funcAddr,funcName,no_memory_effects);
...
Node*kls=_gvn.transform(new(C,3)LoadKlassNode(0,immutable_memory(),p,_gvn.type(p)->is_ptr(),kls_type));
  _sp += nargs; // any deopt will start just before call to enclosing method
  Node* null_ctl = top();
kls=null_check_oop(kls,&null_ctl,never_see_null,_cpdi);
  if (region != NULL) {
...
bool LibraryCallKit::inline_native_Class_query(vmIntrinsics::ID id) {
  ...
  bool never_see_null = !_cpdi->saw_null();
  ...
  if (PrintIntrinsics || PrintInlining || PrintOptoInlining) {
    ...
    if (k) {
C2OUT->print("Inlining %s on constant Class ",vmIntrinsics::name_at(intrinsic_id()));
      k->print_name(C2OUT);
C2OUT->cr();
    }
  }
  ...
  // situation.
  _sp += nargs;  // set original stack for use by uncommon_trap
  mirror = do_null_check(mirror, T_OBJECT, Deoptimization::Reason_null_check, "null chk class mirror", _cpdi);
  _sp -= nargs;
  // If mirror or obj is dead, only null-path is taken.
  if (stopped())  return true;

  // Azul has stronger profiling of failing conditions; do not go conservative here
  // on inlining primitives.
  ...
  case vmIntrinsics::_isInstance:
    {
      ...
      _sp += nargs;  // set original stack for use by uncommon_trap
const TypeKlassPtr*tkls=_gvn.type(kls)->is_klassptr();
      Node* adr   = basic_plus_adr(kls, Klass::klassId_offset_in_bytes() + (int)sizeof(oopDesc));
      kls         = _gvn.transform(new (C, 3) LoadKlassNode(NULL,immutable_memory(),adr,_gvn.type(adr)->is_ptr(),tkls->cast_to_kid(true)));
      query_value = _gvn.transform(gen_instanceof( obj, kls, "null chk, isInstance", _cpdi ));
      _sp -= nargs;
      break;
    }
  ...
  case vmIntrinsics::_getSuperclass:
    ...
    null_ctl = top();
    _sp += nargs;  // set original stack for use by uncommon_trap
    kls = null_check_oop(kls, &null_ctl, !_cpdi->saw_null(), _cpdi);
    _sp -= nargs;  // set original stack for use by uncommon_trap
    if (null_ctl != top()) {
...
bool LibraryCallKit::inline_native_subtype_check() {
  ...
  for (which_arg = 0; which_arg <= 1; which_arg++) {
    ...
    _sp += nargs;  // set original stack for use by uncommon_trap
    arg = do_null_check(arg, T_OBJECT, Deoptimization::Reason_null_check, "null chk subtype_check", _cpdi);
    _sp -= nargs;
    if (stopped())  break;
    args[which_arg] = _gvn.transform(arg);

    Node* p = basic_plus_adr(arg, class_klass_offset);
Node*kls=new(C,3)LoadKlassNode(0,immutable_memory(),p,_gvn.type(p)->is_ptr(),kls_type);
  ...
  for (which_arg = 0; which_arg <= 1; which_arg++) {
    ...
    Node* null_ctl = top();
    _sp += nargs;  // set original stack for use by uncommon_trap
    kls = null_check_oop(kls, &null_ctl, !_cpdi->saw_null(), _cpdi); // does region->init_req(3) for null
    _sp -= nargs;
  ...
  if (!stopped()) {
    ...
    const TypeKlassPtr *tkls_sub = _gvn.type(klasses[1])->is_klassptr();
    Node* subadr   = basic_plus_adr(klasses[1], Klass::klassId_offset_in_bytes() + (int)sizeof(oopDesc));
    Node* subk     = _gvn.transform(new (C, 3) LoadKlassNode(NULL,immutable_memory(),subadr,_gvn.type(subadr)->is_ptr(),tkls_sub->cast_to_kid(true)));
    const TypeKlassPtr *tkls_super = _gvn.type(klasses[0])->is_klassptr();
    // A subtle bail-out to handle a weird corner case.  If there are no
    // implemetations of the superklass then the ease answer is to report "no"
    // to any isAssignableFrom test.  But we might have an assignable - but
    // abstract class with no implementations - which does not count as
    // implementing the superklass.  In which case, the subtype test reports
    // back as "since no impls, nothing is assignable-from", but really should
    // any instances of the abstract subklass appear - they will be
    // assignable.  So the *Classes* are assignable, even if there are no
    // instances to be assigned.  Just bail out for now.
    const TypePtr *no_impls = TypeOopPtr::make_from_klass_unique(tkls_super->klass());
if(no_impls==TypePtr::NULL_PTR)
      return false;
    Node* superadr = basic_plus_adr(klasses[0], Klass::klassId_offset_in_bytes() + (int)sizeof(oopDesc));
    Node* superk   = _gvn.transform(new (C, 3) LoadKlassNode(NULL,immutable_memory(),superadr,_gvn.type(superadr)->is_ptr(),tkls_super->cast_to_kid(true)));
    region->set_req(_both_ref_path, gen_subtype_check(subk, superk, NULL));
...
Node* LibraryCallKit::generate_array_guard_common(Node* kls, RegionNode* region,
  ...
  jint  layout_con = 0;
jint dummy=0;
Node*layout_val=get_layout_helper(kls,layout_con,dummy);
  if (layout_val == NULL) {
...
bool LibraryCallKit::inline_native_newArray() {
  ...
  _sp += nargs;  // set original stack for use by uncommon_trap
  mirror = do_null_check(mirror, T_OBJECT, Deoptimization::Reason_null_check, "null chk native newArray", _cpdi);
  _sp -= nargs;
  ...
  if (!stopped()) {
const TypeKlassPtr*tkls=_gvn.type(klass_node)->is_klassptr();
    Node *kid_adr = basic_plus_adr(klass_node, Klass::klassId_offset_in_bytes() + (int)sizeof(oopDesc));
    Node *kid_node = _gvn.transform(new(C,3) LoadKlassNode(NULL,immutable_memory(),kid_adr,_gvn.type(kid_adr)->is_ptr(),tkls->cast_to_kid(true)));
    // Normal case:  The array type has been cached in the java.lang.Class.
    // The following call works fine even if the array type is polymorphic.
    // It could be a dynamic mix of int[], boolean[], Object[], etc.
    _sp += nargs;  // set original stack for use by uncommon_trap
Node*obj=new_array(kid_node,count_val,_cpdi);
    _sp -= nargs;
...
bool LibraryCallKit::inline_native_getLength() {
  ...
  _sp += nargs;  // set original stack for use by uncommon_trap
  array = do_null_check(array, T_OBJECT, Deoptimization::Reason_null_check, "null chk native getLength", _cpdi);
  _sp -= nargs;
  ...
  if (non_array != NULL) {
    ...
    uncommon_trap(Deoptimization::Reason_intrinsic_check, NULL, "", false);
  }
...
bool LibraryCallKit::inline_array_copyOf(bool is_copyOfRange) {
  ...
  _sp += nargs;  // set original stack for use by uncommon_trap
  array_type_mirror = do_null_check(array_type_mirror, T_OBJECT, Deoptimization::Reason_null_check, "null chk array copyOf 1", _cpdi);
  original          = do_null_check(original, T_OBJECT, Deoptimization::Reason_null_check, "null chk array copyOf 2", _cpdi);
  _sp -= nargs;
  ...
  Node* klass_node = load_klass_from_mirror(array_type_mirror, false, nargs,
                                            NULL, 0);
  _sp += nargs;  // set original stack for use by uncommon_trap
  klass_node = do_null_check(klass_node, T_OBJECT, Deoptimization::Reason_null_check, "null chk array copyOf 3", _cpdi);
  _sp -= nargs;
  ...
  if (bailout->req() > 1) {
    ...
    _sp += nargs;  // push the arguments back on the stack
    uncommon_trap(Deoptimization::Reason_intrinsic_check, NULL, "array copyof failed", false);
  }
  ...
  if (!stopped()) {
    ...
    const TypePtr *adr_type = _gvn.type(original)->is_ptr()->add_offset(Type::OffsetBot);
const TypeKlassPtr*tkls=_gvn.type(klass_node)->is_klassptr();
    Node *kid_adr = basic_plus_adr(klass_node, Klass::klassId_offset_in_bytes() + (int)sizeof(oopDesc));
    Node *kid_node = _gvn.transform(new(C,3) LoadKlassNode(NULL,immutable_memory(),kid_adr,_gvn.type(kid_adr)->is_ptr(),tkls->cast_to_kid(true)));

    _sp += nargs;  // set original stack for use by uncommon_trap
Node*newcopy=new_array(kid_node,length,_cpdi);
    _sp -= nargs;
...
generate_arraycopy(adr_type,T_OBJECT,
...
int hashCode_vtable_entry_offset = 0;  // Uninitialized value
int clone_vtable_entry_offset = 0;     // Uninitialized value
...
RegionNode*slow_region,
  ...
  assert0( *vtable_entry == 0 || *vtable_entry == entry_offset ); // set only once
  *vtable_entry = entry_offset;
...
int vidx=methodOopDesc::invalid_vtable_index;
address adr=StubRoutines::resolve_and_patch_call_entry();
  if( !is_static )    null_check_receiver(method, _cpdi);
  CallJavaNode *slow_call = is_virtual
    ? (CallJavaNode*)new (C, tfdc) CallDynamicJavaNode(tf, adr, method, vidx, bci(), _cpdi, _cloned_in_citypeflow)
    : (CallJavaNode*)new (C, tfdc) CallStaticJavaNode (tf, adr, method,       bci(), _cpdi, _cloned_in_citypeflow);
  set_arguments_for_java_call(slow_call);
set_edges_for_java_call(slow_call,_cpdi,false);
  return slow_call;
}
...
obj=null_check_receiver(callee(),_cpdi);
    ...
    _sp += nargs;
    Node* null_ctl = top();
    obj = null_check_oop(obj, &null_ctl, !_cpdi->saw_null(), _cpdi);
    _sp -= nargs;
  ...
  // Azul: Convert KID to a Klass
obj_klass=_gvn.transform(new(C,2)KID2KlassNode(obj_klass));
  ...
  // Object hashCode().
  if (is_virtual) {
generate_virtual_guard(obj_klass,slow_region,&hashCode_vtable_entry_offset);
  }

  // Get the header out of the object, use LoadMarkNode when available
  Node* header_addr = basic_plus_adr(obj, oopDesc::mark_offset_in_bytes());
Node*header=make_load(NULL,header_addr,TypeInt::INT,T_INT);

  // Get the hash value and check to see that it has been properly assigned.
Node*hash_val=_gvn.transform(new(C,3)RShiftINode(header,_gvn.intcon(1)));

Node*lobit=_gvn.transform(new(C,3)AndINode(header,_gvn.intcon(1)));
Node*no_hash_val=_gvn.intcon(0);
Node*chk_assigned=_gvn.transform(new(C,3)CmpINode(lobit,no_hash_val));
...
Node*obj=null_check_receiver(callee(),_cpdi);
  if (stopped())  return true;
Node*kid=load_object_klass(obj);
Node*klass=_gvn.transform(new(C,2)KID2KlassNode(kid));
  push( load_mirror_from_klass(klass) );
  return true;
}
...
C2OUT->print_cr("Attempting to inline sun.reflect.Reflection.getCallerClass");
  }
#endif
...
bool LibraryCallKit::inline_native_AtomicLong_get() {
  ...
  _sp++;
  obj = do_null_check(obj, T_OBJECT, Deoptimization::Reason_null_check, "null chk AtomicLong_get", _cpdi);
  _sp--;
  ...
  Node *result = _gvn.transform(new (C, 3) LoadLNode(control(), memory(alias_idx), adr, adr_type, TypeLong::LONG, true));
...
bool LibraryCallKit::inline_native_AtomicLong_attemptUpdate() {
  ...
  // Null check without removing any arguments.
  _sp+=5;
  obj = do_null_check(obj, T_OBJECT, Deoptimization::Reason_null_check, "null chk AtomicLong_update", _cpdi);
  _sp-=5;
  // Check for locking null object
  if (stopped()) return true;
...
Node*result=_gvn.transform(new(C,5)CompareAndSwapLNode(control(),memory(alias_idx),adr,newVal,oldVal));
  Node *store_proj = _gvn.transform( new (C, 1) SCMemProjNode(result));
  set_memory(store_proj, alias_idx);

  // The result is typed as flags, but Java wants a boolean.  Make a boolean
  // and rely on the optimizer to straighten it all out.
Node*bol=_gvn.transform(new(C,2)BoolNode(result,BoolTest::eq));
  IfNode *iff = create_and_map_if( control(), bol, PROB_MAX/*CAS rarely fails*/, -1.0F );
...
//------------------------------inline_blockinghint----------------------------
// Inline blocking hints when JDK 6 j.u.c.locks lock.
bool LibraryCallKit::inline_blockinghint(){
_sp+=arg_size();//restore the arguments so I can pop with names
Node*sync=pop();//The JDK 6 j.u.c.locks.Sync
Node*lock=pop();//The JDK 6 j.u.c.locks.Lock
Node*msg=pop();//A Java String
Node*tiks=_gvn.transform(new(C,1)TicksNode(control()));

  Node *thread = _gvn.transform( new (C, 1) ThreadLocalNode() );

Node*msg__adr=basic_plus_adr(top(),thread,in_bytes(JavaThread::hint_block_msg_offset()));
Node*lock_adr=basic_plus_adr(top(),thread,in_bytes(JavaThread::hint_block_lock_offset()));
Node*sync_adr=basic_plus_adr(top(),thread,in_bytes(JavaThread::hint_block_sync_offset()));
Node*tiks_adr=basic_plus_adr(top(),thread,in_bytes(JavaThread::time_block_offset()));

  Node *st_msg = store_to_memory(control(),msg__adr,msg ,T_OBJECT,msg__adr->bottom_type()->is_ptr());
  Node *st_lock= store_to_memory(control(),lock_adr,lock,T_OBJECT,lock_adr->bottom_type()->is_ptr());
  Node *st_sync= store_to_memory(control(),sync_adr,sync,T_OBJECT,sync_adr->bottom_type()->is_ptr());
  Node *st_tiks= store_to_memory(control(),tiks_adr,tiks,T_LONG  ,tiks_adr->bottom_type()->is_ptr());
...
Node*obj=null_check_receiver(callee(),_cpdi);
  if (stopped())  return true;
Node*obj_klass=load_object_klass(obj);//actually loads a KID, but called obj_klass to match Sun
  Node *obj_klass2 = _gvn.transform( new (C, 2) KID2KlassNode(obj_klass) ); // really the Klass
  ...
  // CNC: Huh?  all java-visible stores should be ordered before the allocation
  // (which is a full-blown deopt-able safepoint) and arraycopy below.
  //insert_mem_bar(Op_MemBarCPUOrder);
  ...
  RegionNode* result_reg = new(C, PATH_LIMIT) RegionNode(PATH_LIMIT);
PhiNode*result_val=new(C,PATH_LIMIT)PhiNode(result_reg,toop);
  PhiNode*    result_mem = new(C, PATH_LIMIT) PhiNode(result_reg, Type::MEMORY, TypePtr::BOTTOM);
...
PhiNode*alloc_val=new(C,ALLOC_LIMIT)PhiNode(alloc_reg,toop);
PhiNode*alloc_siz=new(C,ALLOC_LIMIT)PhiNode(alloc_reg,TypeLong::LONG);
  PhiNode*    alloc_mem = new(C, ALLOC_LIMIT) PhiNode(alloc_reg, Type::MEMORY,
                                                      raw_adr_type);
  record_for_igvn(alloc_reg);

Node*array_ctl=generate_array_guard(obj_klass2,(RegionNode*)NULL);
  if (array_ctl != NULL) {
    // It's an array.
    PreserveJVMState pjvms(this);
    ...
    _sp += nargs;  // set original stack for use by uncommon_trap
Node*alloc_obj=new_array(obj_klass,obj_length,_cpdi);
    _sp -= nargs;
assert(alloc_obj->is_Proj()&&alloc_obj->in(0)->is_Allocate(),"");
    Node *obj_size = alloc_obj->in(0)->in(AllocateNode::AllocSize);
    assert(obj_size != NULL, "");

    // Azul: no skipping the oop-array-copy.  We need read-barriers and SBA
    // write-barriers.

      // If it is an oop array, it requires very special treatment,
      // because card marking is required on each card of the array.
Node*is_obja=generate_objArray_guard(obj_klass2,(RegionNode*)NULL);
      if (is_obja != NULL) {
...
alloc_val->init_req(_typeArray_alloc,alloc_obj);
    alloc_siz->init_req(_typeArray_alloc, obj_size);
    alloc_reg->init_req(_typeArray_alloc, control());
alloc_mem->init_req(_typeArray_alloc,reset_memory());
  }
  ...
  if (!stopped()) {
    ...
    // Object clone().
    if (is_virtual) {
      generate_virtual_guard(obj_klass2, slow_region, &clone_vtable_entry_offset);
    }
    ...
    // Azul: Convert KID to a Klass
generate_access_flags_guard(obj_klass2,
  ...
  if (!stopped()) {
    ...
    jint  layout_con = 0;
    jint  dummy = 0;
    Node* layout_val = get_layout_helper(obj_klass2, layout_con, dummy);
    assert0( dummy == 0 );
    Node* obj_size = layout_val ? ConvI2L(layout_val) : _gvn.longcon(layout_con);
    Node* alloc_obj = new_instance(obj_klass,_gvn.intcon(0)/*never force slowpath*/,_cpdi);
    assert(obj_size != NULL, "");
assert(alloc_obj->is_Proj()&&alloc_obj->in(0)->is_Allocate(),"");

alloc_val->init_req(_instance_alloc,alloc_obj);
    alloc_siz->init_req(_instance_alloc, obj_size);
    alloc_reg->init_req(_instance_alloc, control());
alloc_mem->init_req(_instance_alloc,reset_memory());
  }
...
set_all_memory(_gvn.transform(alloc_mem));
  ...
  if (!stopped()) {
    ...
    Node* countx = size;
countx=_gvn.transform(new(C,3)SubLNode(countx,longcon(base_off)));
countx=_gvn.transform(new(C,3)URShiftLNode(countx,intcon(LogBytesPerLong)));
    // Azul: ArrayCopy auto-skips the presumed array header
    base_off -= arrayOopDesc::base_offset_in_bytes(T_LONG);
    ...
    generate_unchecked_arraycopy(raw_adr_type, T_LONG, disjoint_bases,
                                 src, _gvn.intcon(base_off>>3), dest, _gvn.intcon(base_off>>3), countx);

    // Azul: No need to write-barrier the arraycopy as the clone does not have
    // any 'final field' semantics.  

    // Azul: Our allocation idiom always returns a strongly typed result.  No
    // need to cast the result.

Node*new_obj=raw_obj;
...
#define RETURN_STUB(xxx_arraycopy) { \
  ...
  case T_BYTE:
  case T_BOOLEAN:
    name = "prim_arraycopy1"; return StubRoutines::prim_arraycopy(1,(COPYFUNC_CONJOINT&selector)!=0);
    //switch (selector) {
  ...
  case T_CHAR:
  case T_SHORT:
    name = "prim_arraycopy2"; return StubRoutines::prim_arraycopy(2,(COPYFUNC_CONJOINT&selector)!=0);
    //switch (selector) {
  ...
  case T_INT:
  case T_FLOAT:
    name = "prim_arraycopy4"; return StubRoutines::prim_arraycopy(4,(COPYFUNC_CONJOINT&selector)!=0);
    //switch (selector) {
  ...
  case T_DOUBLE:
  case T_LONG:
    name = "prim_arraycopy8"; return StubRoutines::prim_arraycopy(8,(COPYFUNC_CONJOINT&selector)!=0);
    //switch (selector) {
  ...
  case T_OBJECT:
    switch (selector) {
case COPYFUNC_CONJOINT|COPYFUNC_UNALIGNED:RETURN_STUB(objectRef_arraycopy);
case COPYFUNC_CONJOINT|COPYFUNC_ALIGNED:RETURN_STUB(arrayof_objectRef_arraycopy);
case COPYFUNC_DISJOINT|COPYFUNC_UNALIGNED:RETURN_STUB(objectRef_disjoint_arraycopy);
case COPYFUNC_DISJOINT|COPYFUNC_ALIGNED:RETURN_STUB(arrayof_objectRef_disjoint_arraycopy);
    }
  default:
    ShouldNotReachHere();
    return NULL;
...
bool LibraryCallKit::inline_arraycopy() {
  ...
  _sp += nargs;
  src  = do_null_check(src,  T_ARRAY, Deoptimization::Reason_null_check, "", _cpdi);
  dest = do_null_check(dest, T_ARRAY, Deoptimization::Reason_null_check, "", _cpdi);
  _sp -= nargs;
...
LibraryCallKit::generate_arraycopy(const TypePtr* adr_type,
  ...
  // Azul: we optimize bulk zero's much later by looking for AllocateNode
  // followed by an ArrayCopy closer to Matching and not during parsing.
  ...
  if (!stopped() && copy_type == T_OBJECT) {
    ...
    // Azul optimization: use E-KIDs
    int element_kid_offset = objArrayOopDesc::ekid_offset_in_bytes();
    Node *sek_ptr = basic_plus_adr( src,  src, element_kid_offset);
    Node *dek_ptr = basic_plus_adr(dest, dest, element_kid_offset);
const TypeAryPtr*sak=_gvn.type(sek_ptr)->is_aryptr();
const TypeAryPtr*dak=_gvn.type(dek_ptr)->is_aryptr();
    // Azul optimization:  Check for copying from an array of unloaded - i.e., all NULLs.
    // We do not need the subtype check.
    if( sak->elem() != TypePtr::NULL_PTR ) {
    // Azul: Funny indent to match Sun's code whitespace-for-whitespace
    //const TypeKlassPtr *sekid = TypeKlassPtr::make_kid(sak->elem()->is_oopptr()->klass(),sak->klass_is_exact());      
Node*d_e_kid;
    if (dak->elem()->isa_oopptr()) {
      const TypeKlassPtr *dekid = TypeKlassPtr::make_kid(dak->elem()->is_oopptr()->klass(),dak->klass_is_exact());
      d_e_kid = _gvn.transform(new (C, 3) LoadKlassNode(0, memory(dak), dek_ptr, dak, dekid));
    } 
    else {
      assert(dak->klass()->is_obj_array_klass(), "Handle non obj_array_klass in generate_arraycopy.");
      // Load element klass
      const TypeKlassPtr *taryk = TypeKlassPtr::make(dak->klass())->cast_to_exactness(dak->klass_is_exact())->is_klassptr();
      Node* array_kid   = _gvn.transform(new (C, 2) GetKIDNode(0/*array is known not-null here*/,dek_ptr,taryk->cast_to_kid(true)));
...
const TypePtr*tek=_gvn.type(lekp)->is_ptr();
      ...
      d_e_kid = _gvn.transform(new(C,3) LoadKlassNode(NULL,immutable_memory(),kid_adr,_gvn.type(kid_adr)->is_ptr(),TypeKlassPtr::KID));
    }
...
//Node* not_subtype_ctrl = gen_subtype_check(s_e_kid  , d_e_kid   , NULL);
Node*not_subtype_ctrl=gen_subtype_check(src_klass,dest_klass,NULL);
    ...
    } // End Azul test for copying from unloaded array
...
LibraryCallKit::generate_slow_arraycopy(const TypePtr* adr_type,
  ...
  Node* call = make_runtime_call(RC_NO_LEAF | RC_UNCOMMON,
                                 true /* must_callruntimenode */,
                                 OptoRuntime::slow_arraycopy_Type(),
                                 (address)SharedRuntime::slow_arraycopy_C,
                                 "slow_arraycopy", adr_type,
thread,src,src_offset,dest,dest_offset,
                                 copy_length);
  _sp -= nargs;

  // in the case of an exception slow_arraycopy_C will deopt the caller
}
...
Node*dest_elem_klass=_gvn.transform(new(C,2)KID2KlassNode(dest_elem_kid));
  Node* p3 = basic_plus_adr(dest_elem_klass, sco_offset);
  Node* n3 = new(C, 3) LoadINode(NULL, immutable_memory(), p3, TypeRawPtr::BOTTOM);
  Node* check_offset = _gvn.transform(n3);
Node*check_value=dest_elem_kid;

  Node* src_start  = array_element_address(src,  src_offset,  T_OBJECT, NULL);

uint header=arrayOopDesc::base_offset_in_bytes(T_OBJECT);
  Node *dest_index = _gvn.transform(new(C,3)AddINode(dest_offset,_gvn.intcon(header>>3)));
  ...
  Node* call = make_runtime_call(RC_LEAF|RC_NO_FP,
                                 ...
                                 // Azul arg layout:
                                 dest_index, // index to 1st dest element, including all offsets
                                 src_start,  // derived ptr to start
                                 copy_length,  // elements to copy
                                 check_offset, // supercheck offset
                                 check_value,  // element kid
                                 dest);        // dest array base
...
LibraryCallKit::generate_unchecked_arraycopy(const TypePtr* adr_type,
                                             BasicType basic_elem_type,
                                             bool disjoint_bases,
Node*src,Node*src_off,
Node*dst,Node*dst_off,
Node*length){
  if (stopped())  return;               // nothing to do

  // Azul: Use ArrayCopyNodes instead of OJDK runtime calls.
  // Alias types and memory edges
  MergeMemNode* old_mem = merged_memory(); // Get old memory state
  const TypePtr *srctype = _gvn.type(src)->is_ptr()->add_offset(Type::OffsetBot);
  const TypePtr *dsttype = _gvn.type(dst)->is_ptr()->add_offset(Type::OffsetBot);
int srcidx=C->get_alias_index(srctype);
int dstidx=C->get_alias_index(dsttype);
Node*srcmem=memory(srcidx);
Node*dstmem=memory(dstidx);
  // Also do the array-lengths, so that array-length stores (happens just after
  // a CallNew and are exposed to the optimizer) do not bypass the ArrayCopy.
  const TypePtr *srclentype = _gvn.type(src)->is_ptr()->add_offset(arrayOopDesc::length_offset_in_bytes());
  const TypePtr *dstlentype = _gvn.type(dst)->is_ptr()->add_offset(arrayOopDesc::length_offset_in_bytes());
int srclenidx=C->get_alias_index(srclentype);
int dstlenidx=C->get_alias_index(dstlentype);
Node*srclenmem=memory(srclenidx);
Node*dstlenmem=memory(dstlenidx);

  const size_t elem_size = type2aelembytes[basic_elem_type];
  // Is the first level component type a primitive type?  
  const bool is_primitive = basic_elem_type != T_OBJECT && basic_elem_type != T_ARRAY;

  // Make the ArrayCopyNode
  Node *acsn = _gvn.transform(new (C, 5) ArrayCopySrcNode( srcmem, srclenmem, src, src_off, is_primitive ? elem_size : 0, disjoint_bases));
  Node *acn  = _gvn.transform(new (C, 7) ArrayCopyNode( control(), acsn, dstmem, dstlenmem, dst, dst_off, length) );

  // Only change the dst memory.  This is a leaf call, no GC, so no
  // need to set RawPtr memory state.  All the other memory states
  // just flow around the call.
  set_memory(acn, dstidx);
}
