...
Node*PhaseIdealLoop::get_early_ctrl(Node*n,bool assert_placement){
...
Node*x=n->in(i);
    if( assert_placement ) {
      assert0( !x->in(0) || !x->in(0)->is_CFG() || dom_lca(x->in(0),get_ctrl_no_update(x)) == x->in(0) );
    }
    Node *cin = get_ctrl_no_assert(x);
    assert( cin, "" );
...
if(back_control->Opcode()==Op_SafePoint&&!KeepSafepointsInCountedLoops)
...
(trunc1!=NULL&&phi->in(LoopNode::LoopBackControl)!=trunc1)){
  ...
  if (!KeepSafepointsInCountedLoops || loop->_has_call) {
  ...
  if( iftrue_op == Op_IfFalse ) {
    ...
    if_f   = iff2;
    iftrue = ift2;
    le->_prob = 1.0 - le->_prob; // Flip branch probabilities
  } else {
...
if(is_main_loop()){
st->print("main");
st->print(" unrolled %dx",_unroll_ratio);
  }
...
landing_pad=igvn.register_new_node_with_optimizer(landing_pad,_head);
...
static float estimate_path_freq( Node *n ) {
  ...
  for( int i = 0; i < 50; i++ ) { // Skip through a bunch of uncommon tests
    ...
    if( nop == Op_CatchProj ) {   // Get count from a prior call
      ...
      CPData_Invoke *cpdi = call->as_Call()->get_cpdi();
      if( !cpdi ) {
        // no call profile available, try call's control input
        n = n->in(0);
        continue;
      }
      return cpdi->site_count();
    }
...
void IdealLoopTree::counted_loop( PhaseIdealLoop *phase ) {
  ...
  if( _head->is_CountedLoop() ||
    ...
    if (!KeepSafepointsInCountedLoops) {
...
C2OUT->print("  ");
C2OUT->print("Loop: N%d/N%d ",_head->_idx,_tail->_idx);
if(_irreducible)C2OUT->print(" IRREDUCIBLE");
  if( _head->is_CountedLoop() ) {
    CountedLoopNode *cl = _head->as_CountedLoop();
C2OUT->print(" counted");
if(cl->is_pre_loop())C2OUT->print(" pre");
if(cl->is_main_loop())C2OUT->print(" main");
if(cl->is_post_loop())C2OUT->print(" post");
  }
C2OUT->cr();
}
...
PhaseIdealLoop::PhaseIdealLoop(PhaseIterGVN&igvn)
  : PhaseTransform(Ideal_Loop), 
    _igvn(igvn), 
    _dom_lca_tags(C->comp_arena()) {
}

void PhaseIdealLoop::do_loop_opts(const PhaseIdealLoop *verify_me, bool do_split_ifs, bool after_ccp) {
...
#ifndef PRODUCT
  if (C->method()->should_print_ir()) {
    ttyLocker ttyl;
C2OUT->print_cr("========= Before Lock Coarsen =============");
    C->method()->print_short_name(C2OUT); C2OUT->cr();
    dump();
  }
#endif // !PRODUCT
  ...
  if (C->method()->should_print_ir()) {
    ttyLocker ttyl;
C2OUT->print_cr("========= After Lock Coarsen ===========");
    C->method()->print_short_name(C2OUT); C2OUT->cr();
    dump();
  }
#endif // !PRODUCT
  ...
  if (C->method()->should_print_ir()) {
    ttyLocker ttyl;
C2OUT->print_cr("========= Just after split_if_with_blocks =============");
    C->method()->print_short_name(C2OUT); C2OUT->cr();
    dump();
  }
#endif // !PRODUCT
...
_ltree_root->_child->iteration_split(this,worklist,after_ccp);
  ...
  if (C->method()->should_print_ir()) {
    ttyLocker ttyl;
C2OUT->print_cr("========= Before Optimize =============");
    C->method()->print_short_name(C2OUT); C2OUT->cr();
    dump();
  }
#endif // !PRODUCT
...
C2OUT->print_cr("PhaseIdealLoop=%d, sum _unique=%d",_loop_invokes,_loop_work);
}
...
void PhaseIdealLoop::verify() const {
  ...
  ResourceMark rm;
PhaseIdealLoop loop_verify(_igvn);
  loop_verify.do_loop_opts( this, false, false );
...
C2OUT->print("Mismatched control setting for: ");
      n->dump();
      if( fail++ > 10 ) return;
      Node *c = get_ctrl_no_update(n);
C2OUT->print("We have it as: ");
      if( c->in(0) ) c->dump();
else C2OUT->print_cr("N%d",c->_idx);
C2OUT->print("Verify thinks: ");
...
C2OUT->cr();
    ...
    if( loop_verify->has_ctrl(n) ) {
C2OUT->print("Mismatched loop setting for: ");
      n->dump();
      if( fail++ > 10 ) return;
C2OUT->print("We have it as: ");
      us->dump();
...
C2OUT->print("Unequals loops for: ");
        n->dump();
        if( fail++ > 10 ) return;
C2OUT->print("We have it as: ");
        us->dump();
...
C2OUT->cr();
  ...
  if( i >= _idom_size ) {
    if( !n->is_CFG() ) return;
C2OUT->print("CFG Node with no idom: ");
    n->dump();
    return;
  }
  if( !n->is_CFG() ) return;
...
C2OUT->print("Unequals idoms for: ");
    n->dump();
    if( fail++ > 10 ) return;
C2OUT->print("We have it as: ");
    id->dump();
...
void IdealLoopTree::verify_tree(IdealLoopTree *loop, const IdealLoopTree *parent) const {
  ...
  int fail = 0;
if(!Compile::current()->major_progress()&&_child==NULL&&_parent){
    for( uint i = 0; i < _body.size(); i++ ) {
      ...
      if( j == loop->_body.size() ) { // Not found in loop body
        ...
        if (Compile::current()->root()->find(n->_idx)) {
          fail++;
C2OUT->print("We have that verify does not: ");
          n->dump();
        }
      }
    }
    for( uint i2 = 0; i2 < loop->_body.size(); i2++ ) {
      ...
      if( j == _body.size() ) { // Not found in loop body
        ...
        if (Compile::current()->root()->find(n->_idx)) {
          fail++;
C2OUT->print("Verify has that we do not: ");
          n->dump();
        }
      }
    }
...
int PhaseIdealLoop::build_loop_tree_impl( Node *n, int pre_order ) {
  ...
  // Gather locks for later lock-coarsening optimization
if(n->is_Lock()||n->is_Unlock())
_locks.push(n);
  ...
  for (DUIterator_Fast imax, i = n->fast_outs(imax); i < imax; i++) {
    ...
    if( !is_postvisited(m) ) {  // Child visited but not post-visited?
      ...
      if( !l ) {
        ...
        // Halt & Catch Fire
Node*halt=new(C,TypeFunc::Parms)HaltNode(if_f,frame,"infinite loop");
...
if(!verify_me&&n->Opcode()==Op_SafePoint&&
...
Node *PhaseIdealLoop::get_late_ctrl( Node *n, Node *early ) {
  ...
  if (n->is_Load() && LCA != early) {
    ...
    while(worklist.size() != 0 && LCA != early) {
      ...
      // Skip the occasional dead node
if(_nodes[s->_idx]==NULL)
        continue;

      if (s->is_Load() || s->Opcode() == Op_EscapeMemory) {
        continue;
...
case Op_StrEquals://Does a bunch of load-like effects
      pinned = false;
    }
    if( pinned ) {
...
C2OUT->print("  ");
C2OUT->print_cr("---- Loop N%d-N%d ----",loop->_head->_idx,loop->_tail->_idx);
...
C2OUT->print(" ID:%d",computed_idom->_idx);
      }
      n->dump();
      if( cached_idom != computed_idom ) {
C2OUT->print_cr("*** BROKEN IDOM!  Computed as: %d, cached as: %d",
...
C2OUT->print_cr("*** BROKEN CTRL ACCESSOR!  _nodes[k] is %p, ctrl is %p",
