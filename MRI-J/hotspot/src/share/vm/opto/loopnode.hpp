...
class CountedLoopNode : public LoopNode {
  ...
  // Record the number of original iterations each current loop body represents.
  // 1 means not unrolled yet.
  int _unroll_ratio;
...
public:
  ...
  void set_unroll_ratio(int ur) { _unroll_ratio = ur; }
  int unroll_ratio()            { return _unroll_ratio; }
...
void iteration_split(PhaseIdealLoop*phase,Node_List&old_new,bool after_cpp);

  // Driver for various flavors of iteration splitting
void iteration_split_impl(PhaseIdealLoop*phase,Node_List&old_new,bool after_ccp);
  ...
  // Return TRUE or FALSE if the loop should be optimized.  This performs
  // some basic checks to see if we should do any loop optimizations at all.
bool policy_do_aggressive_loopopts(PhaseIdealLoop*phase)const;

  // Return 0 if no prefetch insertion and no unrolling is needed
  // Return 1 if prefetch is needed and no unrolling is needed
  // Return 2 if unrolling is needed for future prefetch insertion
  int policy_unroll_and_prefetch( PhaseIdealLoop *phase ) const;
  ...
  bool policy_unroll_by_N( PhaseIdealLoop *phase, int N ) const;

  // Return TRUE or FALSE if the loop should be unrolled or not.  Unroll if 
  // the loop is a CountedLoop and the body is small enough.
bool policy_unroll_small_loops(PhaseIdealLoop*phase)const;
  ...
  // Return TRUE or FALSE if the loop contains array references
  // that should be prefetched.
bool do_prefetch(PhaseIdealLoop*phase);
...
Node*get_early_ctrl(Node*n,bool assert_placement);
  void set_early_ctrl( Node *n );
  void set_subtree_ctrl( Node *root );
  void set_ctrl( Node *n, Node *ctrl ) {
    assert0( !n->in(0) || !n->in(0)->is_CFG() || dom_lca(n->in(0),ctrl)==n->in(0) );
    assert( ctrl == find_non_split_ctrl(ctrl), "must set legal crtl" );
set_ctrl_no_assert(n,ctrl);
  }
  void set_ctrl_no_assert( Node *n, Node *ctrl ) { // Only valid to call during clone_loop
    assert0( !has_node(n) || has_ctrl(n) );
  ...
  Node *get_ctrl( Node *i ) {
    assert0( !i->in(0) || !i->in(0)->is_CFG() || 
             dom_lca(i->in(0),get_ctrl_no_update(i)) == i->in(0) );
return get_ctrl_no_assert(i);
  }
Node*get_ctrl_no_assert(Node*i){
    assert(has_node(i), "");
    Node *n = get_ctrl_no_update(i);
    if( n != (Node*)(((intptr_t)_nodes[i->_idx]) & ~1) )
...
_igvn.add_users_to_worklist(old_node);
...
private:
  ...
  // Local copy
  Node **copy_idom(ResourceArea *area) const;
...
void do_loop_opts(const PhaseIdealLoop*verify_me,bool do_split_ifs,bool after_ccp);
...
Node*side_by_side_idom,bool clone_after);
  void handle_invariant_uses( Node *old, IdealLoopTree *loop, Node_List &old_new, Node_List *worklist, Node **old_idoms, int old_idom_size, uint new_counter, uint loopbodysize, Node_List **split_if_set, Node_List **split_bool_set);
...
Node*split_thru_region(Node*n,Node*region,bool is_loop);
  // Split Node 'n' through merge point if there is enough win.
Node*split_thru_phi(Node*n,Node*region,int policy,bool is_split_if);
...
public:
  ...
  // issues.  No coarsening such that a new path has to acquire a lock - as
  // this could lead to deadlock.  I do allow inserting an Unlock on a new
  // path as this much flexibility appears to allow a lot more unlock/lock
  // sequences to be removed.
Node_List _locks;
  void lock_coarsen( );
  void try_to_coarsen( Node *obj );
  bool scan_mem_unlock( Node *def, VectorSet &visited, VectorSet &ulregion, Node_Array &base_mem_defs );
  Node *find_base_mem_def(Node *ctrl, VectorSet &ulregion, Node_Array &base_mem_defs );
  void set_narrow_slice(Node *ctrl, Node *def, int aix, Node_Array &base_mem_defs, VectorSet& visited);
  void remove_unlock( Node *n, VectorSet &visited, VectorSet &ulregion, Node *obj, Node_Array &base_mem_defs);
