...
public:
  ...
  static const Register _regEncode[];
  // Machine Node names
  ...
  void  match( );
  // Helper for match
OptoReg::Name warp_incoming_stk_arg(VReg::VR reg);
...
OptoReg::Name warp_outgoing_stk_arg(VReg::VR reg,OptoReg::Name begin_out_arg_area,OptoReg::Name&out_arg_limit_per_call);
  ...
  VReg::VR *_parm_regs;        // Array of machine registers per argument
...
static void calling_convention(BasicType*,VReg::VR*,uint len,bool is_outgoing);
...
static VReg::VR return_value(int ideal_reg,bool is_outgoing);
