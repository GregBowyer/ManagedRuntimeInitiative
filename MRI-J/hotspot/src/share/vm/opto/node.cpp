...
C2OUT->print_cr("BreakAtNode: _idx=%d _debug_idx=%d",_idx,_debug_idx);
    BREAKPOINT;
  }
...
void Node::emit(PhaseRegAlloc*ra_)const{}
...
int Node::get_intptr_t( intptr_t * ) const { return false; }
...
static void find_recur( Node* &result, Node *n, int idx, bool only_ctrl, 
  ...
  if( (int)n->_idx == node_idx
    ...
    if (result != NULL)
C2OUT->print("find: "INTPTR_FORMAT" and "INTPTR_FORMAT" both have idx==%d\n",
...
dump_spec(C2OUT);
  ...
  // Check for "nuked" node
  uint i;
  for( i=0; i<req(); i++ )
    if( in(i) )
      break;
  // Get bottom type, assuming node is "not nuked"
  const Type *t = (i == req()) ? Type::TOP : bottom_type();

  if (!is_Type() && // TypeNodes already print their type, do not double-print
      !is_Load() && // LoadNodes already print their type, do not double-print
      !Opcode() == Op_Parm && // ParmNodes already print their type
      !(((Node*)this)->is_Mach() && ((MachNode*)this)->is_MachType()) &&
      t != NULL &&              // Find a pointer-type result?
      (t->isa_instptr() || t->isa_klassptr())) {
    const TypeInstPtr  *toop = t->isa_instptr();
    const TypeKlassPtr *tkls = t->isa_klassptr();
    ciKlass*           klass = toop ? toop->klass() : (tkls ? tkls->klass() : NULL );
    if( klass && klass->is_loaded() && klass->is_interface() ) {
C2OUT->print("  Interface:");
    } else if( toop ) {
C2OUT->print("  Oop:");
    } else if( tkls ) {
C2OUT->print("  Klass:");
    }
t->dump();//Print pointer result types
  } else if( t == Type::MEMORY ) {
C2OUT->print("  Memory:");
MemNode::dump_adr_type(this,adr_type(),C2OUT);
  }
  if (is_new) {
...
C2OUT->print("NotANode ");//uninitialized, sentinel, garbage, etc.
    } else {
C2OUT->print("%c%d ",Compile::current()->node_arena()->contains(d)?' ':'o',d->_idx);
    }
  }
}
...
if(NotANode(p)){C2OUT->print("NotANode ");continue;}
C2OUT->print("%c%d ",Compile::current()->node_arena()->contains(in(i))?' ':'o',in(i)->_idx);
    }
  }
}
...
C2OUT->print("NotANode ");
    } else {
C2OUT->print("%c%d ",Compile::current()->node_arena()->contains(u)?' ':'o',u->_idx);
    }
  }
C2OUT->print("]] ");
}
...
//=============================================================================
//-----------------------------------------------------------------------------
void Node_Array::reset(ResourceArea*new_arena){
