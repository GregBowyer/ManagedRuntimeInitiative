...
DEFINE_CLASS_ID(Lock,CallStaticJava,0)
            DEFINE_CLASS_ID(CallDynamicJava,  CallJava, 1)
          DEFINE_CLASS_ID(CallRuntime,      Call, 1)
            DEFINE_CLASS_ID(CallLeaf,         CallRuntime, 0)
DEFINE_CLASS_ID(Unlock,CallLeaf,0)
DEFINE_CLASS_ID(Allocate,CallRuntime,1)
...
DEFINE_CLASS_ID(FastLock,If,1)
DEFINE_CLASS_ID(FastUnlock,If,2)
...
DEFINE_CLASS_ID(MachCallVM,MachCall,1)
DEFINE_CLASS_ID(MachCallRuntime,MachCallVM,0)
DEFINE_CLASS_ID(MachAllocate,MachCallRuntime,0)
DEFINE_CLASS_ID(MachCallLeaf,MachCallVM,1)
...
DEFINE_CLASS_ID(RangeCheck,Mach,5)
...
DEFINE_CLASS_ID(Add,Node,10)
DEFINE_CLASS_ID(Mul,Node,11)
DEFINE_CLASS_ID(Halt,Node,12)
...
Flag_is_Con=Flag_needs_anti_dependence_check<<1,
...
Flag_is_safepoint_node=Flag_is_dead_loop_safe<<1,
Flag_is_Vector=Flag_is_safepoint_node<<1,
...
DEFINE_CLASS_QUERY(MachAllocate)
...
//----------------- Code Generation
  ...
  // If this is a StoreP, CompareAndSwapP or ArrayCopy of ptrs, AND UseSBA is
  // on, then we need to check for heap escapes.
  virtual int base_derived_idx() const { return 0; }
  virtual void set_oopmap( OopMap2 *om ) { ShouldNotReachHere(); }
  virtual OopMap2 * get_oopmap( ) const { ShouldNotReachHere(); return 0; }
...
virtual void emit(PhaseRegAlloc*ra_)const;
  ...
  virtual int get_intptr_t( intptr_t *con ) const;
...
C2OUT->print("%4d: %-17s",_idx,Name());
    for (uint i = 0; i < len(); i++)
      if (in(i))
C2OUT->print(" %4d",in(i)->_idx);
      else
C2OUT->print(" NULL");
C2OUT->print("\n");
  }
...
protected:
ResourceArea*_a;//Arena to allocate in
  uint   _max;
  Node **_nodes;
int _nesting;//Used to assert on resource marks
  void   grow( uint i );        // Grow array node to fit
public:
  Node_Array(ResourceArea *a) : _a(a), _max(OptoNodeListSize), _nesting(DEBUG_ONLY(_a->nesting()) NOT_DEBUG(0)) {
  ...
  Node_Array(Node_Array *na) : _a(na->_a), _max(na->_max), _nodes(na->_nodes), _nesting(na->_nesting) {}
...
void map(uint i,Node*n){assert0(_nesting==_a->nesting());if(i>=_max)grow(i);_nodes[i]=n;}
  void insert( uint i, Node *n );
  void remove( uint i );        // Remove, preserving order
  void sort( C_sort_func_t func);
void reset(ResourceArea*new_a);//Zap mapping to empty; reclaim storage
...
Node_List(ResourceArea*a):Node_Array(a),_cnt(0){}
...
Unique_Node_List(ResourceArea*a):Node_List(a),_in_worklist(a),_clock_index(0){}
...
u2 indx;//Index of next node's child
    u2    nreq; // Node's old node->req()
  };
...
public:
  ...
  uint nreq () const { return _inode_top->nreq; }
  void set_node(Node *n) {
    _inode_top->node = n;
    _inode_top->nreq = n->req();
  }
