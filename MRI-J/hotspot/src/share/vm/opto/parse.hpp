...
InlineTree*build_inline_tree_for_callee(CPData_Invoke*caller_cpd,
                                           ciMethod* callee_method,
                                           JVMState* caller_jvms,
                                           int caller_bci);
InliningFailureID try_to_inline(ciMethod*callee_method,int caller_bci,int call_site_count,WarmCallInfo*wci_result);
InliningFailureID shouldInline(ciMethod*callee_method,int caller_bci,int call_site_count,WarmCallInfo*wci_result)const;
InliningFailureID shouldNotInline(ciMethod*callee_method,int call_site_count,WarmCallInfo*wci_result)const;
void print_inlining(ciMethod*callee_method,int caller_bci,InliningFailureID failure_msg)const PRODUCT_RETURN;
...
public:
  ...
  WarmCallInfo* ok_to_inline(CPData_Invoke *c2_caller_cpd, CPData_Invoke *caller_cpd, ciMethod *call_method, JVMState* caller_jvms, int call_site_count, WarmCallInfo* wci);
...
class Parse : public GraphKit {
 ...
 private:
  ...
  CodeProfile*  _c1_cp;         // CodeProfile from C1
  int           _c1_cp_inloff;  // Offset in the C1 CodeProfile to this inlined method's profile data
  bool          _matches_c1_inlining;
  ...
  Node* _synch_obj;             // object for synchronized method

 public:
  // Constructor
  Parse(JVMState* caller, ciMethod* parse_method, CodeProfile *c1_cp, int c1_cp_inloff, bool matches_c1_inlining, float expected_uses, CPData_Invoke *cpdi);
 ...
 public:
  ...
  int           c1_cp_inloff()  const { return _c1_cp_inloff; }
  CPData*       cpdata()        const;
  CPData*       c2cpdata()      const;
  CPData_Null * cpdata_null()   const { return cpdata()->as_Null(java_bc()); }
  Node* do_null_check(Node* value, BasicType type, const char *msg) { return GraphKit::do_null_check(value,type, Deoptimization::Reason_null_check, msg, cpdata_null()); }
  Node* do_div0_check(Node* value, BasicType type, const char *msg) { return GraphKit::do_null_check(value,type, Deoptimization::Reason_div0_check, msg, cpdata_null()); }
  ...
  bool is_private_copy() const { return _block->flow()->is_private_copy(); }
 ...
 private:
  ...
  SafePointNode* create_entry_map(CPData_Invoke *cpdi);
  ...
  void do_method_entry(CPData_Invoke *caller_cpdi);
...
Node*array_addressing(BasicType type,int vals);
...
if(target_bci<=bci()){
  ...
  void    repush_if_args();
void adjust_map_after_if(BoolTest::mask btest,Node*c,float prob,int cnt,
  ...
  void profile_taken_branch(int target_bci, bool force_update = false) { /*not used on Azul*/ }
  void profile_not_taken_branch(bool force_update = false) { /*not used on Azul*/}
  void profile_null_checkcast() { /*not used on Azul*/ }
