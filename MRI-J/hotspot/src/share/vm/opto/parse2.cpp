...
Node*adr=array_addressing(elem_type,0);
  if (stopped())  return;     // guarenteed null or range check
  _sp -= 2;                   // Pop array and index
const TypeAryPtr*tary=_gvn.type(adr)->is_aryptr();
const Type*elem=tary->elem();
  // Sharpen base elements loaded from oop arrays
ciKlass*aklass=tary->klass();
  if( aklass->is_obj_array_klass() )
    elem = TypeKlassPtr::make_from_klass_unique(((ciObjArrayKlass*)aklass)->element_klass());
...
Node*Parse::array_addressing(BasicType type,int vals){
  Node *idx   = peek(0+vals);   // Get from stack without popping
  Node *ary   = peek(1+vals);   // in case of exception

  // Null check the array base, with correct stack contents
ary=do_null_check(ary,T_ARRAY,"null chk array");
...
uncommon_trap(Deoptimization::Reason_unloaded,arytype->klass(),"!loaded array",false);
    return top();
  }
...
if(need_range_check){
    ...
    { BuildCutout unless(this, tst, PROB_MAX);
      // If we have not failed here before then uncommon-trap.  This exit
      // point needs full debug info, because we can speculatively widen the
      // check in the optimizer - and if we hit this uncommon trap we are not
      // guarenteed to be throwing an exception.

      // If we are throwing real range check exceptions here, start catching them.
      CPData_Null* cpdn = cpdata_null();
      builtin_throw(Deoptimization::Reason_range_check, "range check is throwing", cpdn, cpdn->did_rchk(), /*must_throw=*/cpdn->did_rchk_wide());
    }
  }
...
public:
  ...
  void print(ciEnv* env) {
    if (is_singleton())
C2OUT->print(" {%d}=>%d",lo(),dest());
    else if (lo() == min_jint)
C2OUT->print(" {..%d}=>%d",hi(),dest());
    else if (hi() == max_jint)
C2OUT->print(" {%d..}=>%d",lo(),dest());
    else
C2OUT->print(" {%d..%d}=>%d",lo(),hi(),dest());
  }
};
...
int table_index=NullTableIndex;
...
bool Parse::create_jump_tables(Node* key_val, SwitchRange* lo, SwitchRange* hi) {
  ...
  // Azul PCs are limited to 32-bits.  So the scale-and-add better fit in 32 bits.
...
void Parse::modf() {
  Node *f2 = pop();
  Node *f1 = pop();
Node*c=make_runtime_call(RC_LEAF,false/*!must_callruntimenode*/,
...
float Parse::dynamic_branch_prediction(float &cnt) {
  ...
  CPData_Jump   *cpd_jmp = cpd->isa_Jump  (java_bc());
  CPData_Branch *cpd_bra = cpd->isa_Branch(java_bc());
  if( cpd_jmp ) {
    taken = sum = cpd_jmp->reached();
    if( sum > 0 ) Untested("");
  } else if( cpd_bra ) {
    sum   = cpd_bra->reached();
    taken = cpd_bra->taken()  ;
...
float Parse::branch_prediction(float& cnt,
  ...
  if (target_bci < bci()) {
    if (is_osr_parse()) {    // Could be a hot OSR'd loop; force deopt
      ...
      CPData_Branch *cpdb = (CPData_Branch*)cpdata();
      {
...
static inline bool seems_never_taken(float prob,int count){
  return prob < PROB_MIN && count > PROB_COUNT_THRESHOLD;
}
static inline bool seems_always_taken(float prob, int count) {
  return prob > PROB_MAX && count > PROB_COUNT_THRESHOLD;
}
...
adjust_map_after_if(btest,c,prob,cnt,branch_block,next_block);
      if (!stopped())
        merge(target_bci);
    }
  }
...
adjust_map_after_if(BoolTest(btest).negate(),c,1.0-prob,cnt,
...
false);
  ...
  { PreserveJVMState pjvms(this);
    ...
    if (stopped()) {
      ...
      // Update method data
      profile_taken_branch(target_bci);
adjust_map_after_if(taken_btest,c,prob,cnt,branch_block,next_block);
      if (!stopped())
        merge(target_bci);
    }
  }
  ...
  if (stopped()) {
    ...
    // Update method data
    profile_not_taken_branch();
adjust_map_after_if(untaken_btest,c,untaken_prob,cnt,
                        next_block, branch_block);
  }
}
...
void Parse::adjust_map_after_if(BoolTest::mask btest,Node*c,float prob,int cnt,
  ...
  int cop = c->Opcode();
if(seems_never_taken(prob,cnt)&&cop==Op_CmpP&&btest==BoolTest::eq){
...
Node*ary=do_null_check(peek(),T_ARRAY,"null chk array length");
    // Compile-time detect of null-exception?
    if (stopped())  return;
    a = pop();
...
store_to_memory(control(),d,c,T_OBJECT,adr_type);
    break;
  }
...
do_null_check(peek(1),T_LONG,"lrem");
    // Compile-time detect of null-exception?
    if (stopped())  return;
    b = pop_pair();
    a = pop_pair();
  ...
  case Bytecodes::_ldiv: 
    // Must keep both values on the expression-stack during null-check
    assert(peek(0) == top(), "long word order");
do_null_check(peek(1),T_LONG,"ldiv");
    // Compile-time detect of null-exception?
    if (stopped())  return;
    b = pop_pair();
    a = pop_pair();
...
do_null_check(peek(),T_OBJECT,"null chk athrow");
    if (stopped())  return;
    if (JvmtiExport::can_post_exceptions()) {
      // "Full-speed throwing" is not necessary here,
      // since we're notifying the VM on every throw.
      uncommon_trap(Deoptimization::Reason_jvmti, NULL, "jvmti cutout on athrow", false);
      return;
    }
  ...
  case Bytecodes::_goto_w: {
    ...
    CPData_Jump *cpdj = (CPData_Jump*)cpdata();
    int taken = cpdj->reached();
    if( taken == 0 && !UseInterpreter )  break; // -Xcomp mode rarely has useful counts
    taken *= prof_factor();
    if( taken != 0 ) Untested("");
...
C->record_failure("breakpoint in method",true);
    return;
