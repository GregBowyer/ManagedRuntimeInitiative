...
void Parse::do_field_access(bool is_get, bool is_field) {
  ...
  if (is_field == field->is_static()) {
    ...
    uncommon_trap(Deoptimization::Reason_unhandled, field_holder, "incompatible class change", false);
    return;
  }

  if (!is_field && !field_holder->is_initialized()) {
    if (!static_field_ok_in_clinit(field, method())) {
uncommon_trap(Deoptimization::Reason_static_in_clinit,NULL,"!static_field_ok_in_clinit",false);
      return;
    }
  }
...
obj=do_null_check(peek(obj_depth),T_OBJECT,"null chk field");
...
type=TypeOopPtr::make_from_klass_unique(field_klass->as_klass());
    }
  } else {
...
void Parse::do_put_xxx(const TypePtr* obj_type, Node* obj, ciField* field, bool is_field) {
  ...
  // Store the value.  
  Node* store;
store=store_to_memory(control(),adr,val,bt,adr_type);
...
uncommon_trap(Deoptimization::Reason_unloaded,array_klass,"array class unloaded",false);
    return;
  }

  kill_dead_locals();

  do_array(array_klass,T_OBJECT, NULL);
}
...
void Parse::do_newarray(BasicType elem_type) {
  ...
  ciTypeArrayKlass *array_klass = ciTypeArrayKlass::make(elem_type);
  do_array(array_klass,elem_type, NULL);
}
...
Node* Parse::expand_multianewarray(ciArrayKlass* array_klass, Node* *lengths, int ndimensions) {
  ...
  assert(length != NULL, "");
  ciType *etype = array_klass->element_type();
ciArrayKlass*earray_klass;
  if (etype->is_primitive_type()) earray_klass = ciTypeArrayKlass::make(array_klass->element_type()->basic_type());
  else earray_klass = ciObjArrayKlass::make(array_klass->element_type()->as_klass());
push(length);
  do_array(earray_klass,T_OBJECT, cpdata());
Node*array=pop();
  if (ndimensions > 1) {
...
store_to_memory(control(),eaddr,elem,T_OBJECT,adr_type);
    }
  }
  return array;
}
...
//Handle the JCK test where the outer dimension is known to be negative.
      peek(ndimensions-1)->bottom_type()->join(TypeInt::POS)->empty() ) {
    uncommon_trap(Deoptimization::Reason_unhandled, array_klass, "multi array class unloaded", false );
    return;
  }
  ...
  case 2: fun = (address)OptoRuntime::multianewarray2_Java; name = "multianewarray2"; break;
  case 3: fun = (address)OptoRuntime::multianewarray3_Java; name = "multianewarray3"; break;
  case 4: fun = (address)OptoRuntime::multianewarray4_Java; name = "multianewarray4"; break;
  case 5: fun = (address)OptoRuntime::multianewarray5_Java; name = "multianewarray5"; break;
  default: name = ""; ShouldNotReachHere();
  };

const Type*aelem=Type::get_const_type(array_klass->element_type());
  const TypeAryPtr *arr = TypeAryPtr::make(TypePtr::NotNull,TypeAry::make(aelem,TypeInt::POS),array_klass,0);
  Node* thread = _gvn.transform(new (C, 1) ThreadLocalNode());
  Node* c = make_runtime_call(RC_NO_LEAF | RC_NO_IO,
true/*must_callruntimenode*/,
OptoRuntime::multianewarray_Type(ndimensions,arr),
fun,name,TypeRawPtr::BOTTOM,
thread,makecon(TypeKlassPtr::make(array_klass)),
