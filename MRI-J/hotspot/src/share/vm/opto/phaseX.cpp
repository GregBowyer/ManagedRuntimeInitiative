...
//------------------------------PhaseTransform---------------------------------
PhaseTransform::PhaseTransform(ResourceArea*arena,PhaseNumber pnum):Phase(pnum),
  _arena(arena),
  _nodes(arena),
  _types(arena)
{
...
void PhaseTransform::dump_new( uint nidx ) const {
  for( uint i=0; i<_nodes.Size(); i++ )
    if( _nodes[i] && _nodes[i]->_idx == nidx ) {
      _nodes[i]->dump();
C2OUT->cr();
C2OUT->print_cr("Old index= %d",i);
      return;
    }
C2OUT->print_cr("Node %d not found in the new indices",nidx);
}
...
C2OUT->print("      ");type(n)->dump();C2OUT->cr();
  }
}
...
PhaseValues::PhaseValues(ResourceArea*arena,uint est_max_size):PhaseTransform(arena,GVN),_table(arena,est_max_size){
...
ConNode* PhaseTransform::makecon(const Type *t) {
  ...
  // Azul: Actually we see 1/2 dead CheckCastPP's and the like all the time.
  // These things constant-fold into various vacuous constants before being
  // discovered as dead.
...
C2OUT->print_cr("VerifyIterativeGVN: %ld transforms and verify passes",
                    _verify_full_passes);
    else
C2OUT->print_cr("VerifyIterativeGVN: %ld transforms, %ld full verify passes",
  ...
  if (!dead->is_Con()) { // Don't kill cons but uses
    ...
    for( uint i = 0; i < dead->req(); i++ ) {
      ...
      if( in ) {                 // Points to something?
        ...
        } else if( in->is_Allocate() ) { // Allocate nodes going dead are special
          _worklist.push(in);
        }
      }
    }
...
//------------------------------subsume_node_keep_old--------------------------
// Remove users from node 'old' and add them to node 'nn'.
void PhaseIterGVN::subsume_node_keep_old(Node*old,Node*nn){
...
void PhaseIterGVN::add_users_to_worklist( Node *n ) {
  ...
  for (DUIterator_Fast imax, i = n->fast_outs(imax); i < imax; i++) {
    ...
    if( type_or_null(use) == Type::TOP ) continue; // Do not en-worklist dead stuff
...
void PhaseCCP::analyze() {
  ...
  while( worklist.size() ) {
    ...
    if (t != type(n)) {
      ...
      if( TracePhaseCCP ) {
        t->dump();
do{C2OUT->print("\t");}while(C2OUT->position()<16);
        n->dump();
      }
#endif
      set_type(n, t);
...
void PhaseCCP::do_transform() {
  ...
  // "add_users_to_worklist" will put lots of extra stuff on the worklist,
  // including dead unreachable stuff.  Pull the unreachable stuff from the
  // worklist so we don't have to try and optimize dead stuff.
for(uint i=0;i<_worklist.size();i++){
    if( !_nodes[_worklist.at(i)->_idx] ) {
      // Found a Node on worklist that was not reached by CCP walk of live
_worklist.remove(i);//Yank from worklist
i--;//Retry at same worklist index
    }
  }
}
...
C2OUT->print_cr("CCP: %d  constants found: %d",_total_invokes,_total_constants);
}
#endif
...
block->_nodes.at(instruction_index-i)->as_Mach()->dump(1);C2OUT->cr();
            }
C2OUT->print_cr("replaced with");
...
C2OUT->print_cr("Peephole: peephole rules applied: %d",_total_peepholes);
}
#endif
...
void Type_Array::dump() const {
  ...
  for( uint i = 0; i < max; i++ ) {
    if( _types[i] != NULL ) {
C2OUT->print("  %d\t== ",i);_types[i]->dump();C2OUT->cr();
    }
  }
}
#endif
