...
PhaseTransform(ResourceArea*arena,PhaseNumber pnum);
  PhaseTransform( PhaseTransform *phase, PhaseNumber pnum );

ResourceArea*arena(){return _arena;}
...
public:
PhaseValues(ResourceArea*arena,uint est_max_size);
...
public:
PhaseGVN(ResourceArea*arena,uint est_max_size):PhaseValues(arena,est_max_size){}
...
void subsume_node_keep_old(Node*old,Node*nn);
