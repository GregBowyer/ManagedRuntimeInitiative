...
uint oldidx=def->_idx;
    ...
    if( def->is_Copy() )        // Copies carry value through
      ...
      return false;             // Unrelated computation
    // Only count for increasing index - to prevent cycles
    if( def->_idx >= oldidx &&  ++cnt >= limit )
...
#if 0
  // Added another condition before its considered OK to use a different register
  // on a chain of copies.
  // This condition will ensure, incase of debug Info inputs nodes, we 
  // must not replace existing register/stack-slot if its supposed to be
  // killed after the call. 
  MachCallNode *mcall = n->is_MachCall() ? n->as_MachCall() : NULL;
  if( can_use && mcall && idx >= mcall->tf()->domain()->cnt() ) {
uint cnt=mcall->tf()->range()->cnt();

    RegMask out_mask = RegMask::Empty;
for(DUIterator_Fast imax,i=mcall->fast_outs(imax);i<imax;i++){
Node*p=mcall->fast_out(i);
      if( !p->is_Proj() ) {
        continue;
      }
ProjNode*fatProj=p->as_Proj();
      if( fatProj->_con == cnt+10000 ) {
        out_mask = fatProj->out_RegMask();
        break;
      }
    }

    can_use = !(out_mask.Member(def_reg));
  }
#endif
...
int win=use_prior_register(n,k,regnd[val_reg],current_block,value,regnd);
blk_adjust+=win;
if(win>0&&n->in(k)==regnd[val_reg])//Success!  Quit trying
      return blk_adjust;
  }
...
OptoReg::is_reg((OptoReg::Name)reg)||//turning into a register use OR
          regnd[reg]->outcnt()==1 ) { // last use of a spill-load turns into a CISC use
int win=use_prior_register(n,k,regnd[reg],current_block,value,regnd);
blk_adjust+=win;
if(win>0&&n->in(k)==regnd[reg])//Success!  Quit trying
          return blk_adjust;
...
yank_if_dead(x,b,&value,&regnd);
    ...
    for( j = phi_dex; j < b->_nodes.size(); j++ ) {
      ...
      CodeBlob *cb;
      if( (n->flags() & Node::Flag_is_safepoint_node) != 0 && 
        ...
        if( entry == StubRoutines::resolve_and_patch_call_entry() ||
            ...
            0 ) {
          for( uint reg = 0; reg < (uint)_max_reg; reg++ ) {
            if( value[reg] && value[reg]->bottom_type()->isa_oopptr() ) {
              value.map(reg,0);
              regnd.map(reg,0);
            }
          }
