...
C->record_failure("attempted to spill a non-spillable item",false);
    return NULL;
  }
  ...
  if( w_mask->overlap( *o_mask ) ) { // overlapped
    w_o_mask = w_mask;
...
uint PhaseChaitin::split_DEF( Node *def, Block *b, int loc, uint maxlrg, Node **Reachblock, Node **debug_defs, GrowableArray<uint> splits, int slidx ) {
  ...
  // Same sad story for spilling the results of an ladd instruction:
  // the spill happens on the non-faulting path.
  if( be==def && be->is_RangeCheck() ) {
    // Spill goes in the fall-through block
    int eidx = b->end_idx();
    uint j = 0;
    while( b->_nodes[eidx+1+j]->as_Proj()->_con != 0 )
      j++;
b=b->_succs[j];//The fall-through block
    loc = 0;                    // Just past the Region
  }
...
if(lidx<_maxlrg&&lrgs(lidx).reg()>=OptoReg::SPILL_REG){
...
int lrg_pres=(lrg->get_invalid_mask_size()>1)?lrg->get_invalid_mask_size():bound_pres;
...
if(lrgs(bidx).alive()&&lrgs(bidx).reg()>=OptoReg::SPILL_REG){
      ...
      spill_cnt++;
      lidxs.append(bidx);
      // One-time compute and cache the "rematerialize" flag for single-def
      // live ranges, in case the flag changes as the number of USEs change.
      if( lrgs(bidx)._def != (Node*)-1 )
        lrgs(bidx)._rematerialize = lrgs(bidx)._def->rematerialize();
#ifdef ASSERT
...
C2OUT->print("Reaching Definitions after Phi handling\n");
      for( uint x = 0; x < spill_cnt; x++ ) {
C2OUT->print("Spill Idx %d: UP %d: Node\n",x,UPblock[x]);
        if( Reachblock[x] )
          Reachblock[x]->dump();
        else
C2OUT->print("Undefined\n");
      }
    }
#endif
...
if(lrgs(defidx).reg()<OptoReg::SPILL_REG){
            uint i;
            Node *u = NULL;
...
if(useidx<_maxlrg&&lrgs(useidx).reg()>=OptoReg::SPILL_REG){
...
!def->rematerialize()){
      ...
      // UP info.
if(deflrg.reg()>=OptoReg::SPILL_REG){//Spilled?
...
(defup&&is_high_pressure(b,&deflrg,insidx))||
             // Check for LRG being single-def single-use.
             (defup && lrgs(defidx)._def == n && n->outcnt() == 1)
             ) ) {
...
deflrg.reg()<OptoReg::SPILL_REG){//And DEF is from stack
          LRG &uselrg = lrgs(useidx);
          if( OptoReg::is_stack(uselrg.reg()) &&
uselrg.reg()<OptoReg::SPILL_REG&&//USE is from stack
    ...
    for( slidx = 0; slidx < spill_cnt; slidx++ ) {
      ...
      if( !liveout->member(defidx) ) {
        ...
        // Assert turned off because it puts the overall algorithm in O(n^2)
  ...
  if(!noprogress) {
C2OUT->print_cr("Failed to make progress in Split");
    //BREAKPOINT;
  }
#endif
  // Return updated count of live ranges
