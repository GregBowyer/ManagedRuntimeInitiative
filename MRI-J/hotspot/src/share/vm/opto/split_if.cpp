...
Node*PhaseIdealLoop::split_thru_region(Node*n,Node*region,bool is_loop){
  uint wins = 0;
  assert( n->is_CFG(), "" );
  assert( region->is_Region(), "" );
  RegionNode *rl = is_loop 
    ? (RegionNode*)new (C, region->req()) LoopNode  ( region->in(1), region->in(2) ) 
    :              new (C, region->req()) RegionNode( region->req() );
  ...
  for( uint i = 1; i < region->req(); i++ ) {
    ...
    if( n->is_If() ) loop = get_loop(x->in(0));
    set_loop(x, loop);
    if( !loop->_child )
loop->_body.push(x);
...
rl->set_req(i,x);
  ...
  // Record region
rl->set_req(0,region);//Not a TRUE RegionNode
_igvn.register_new_node_with_optimizer(rl);
  return rl;
}
...
set_ctrl(bol,use_c);
          if( !get_loop(use_c)->_child )
get_loop(use_c)->_body.push(bol);
          continue;
        }
...
C2OUT->print("Cloning up: ");
  ...
  _igvn.hash_delete(n);
_igvn.subsume_node_keep_old(n,phi);
_igvn.hash_find_insert(n);
  for( uint j = 1; j < blk1->req(); j++ ) {
    ...
    Node *y = _igvn.hash_find(x);
    if( y ) {                   // Found a prior node?
x->destruct();//Use prior node
      x = y;
      if( n == x ) {            // No block-local inputs any more?
assert(j==1,"");
        set_ctrl(n,find_non_split_ctrl(idom(blk1))); // Must be able to hoist n alot
        _igvn.subsume_node(phi,n); // No need for phi
phi->destruct();
        return false;
      }
      Node *l = dom_lca(get_ctrl(x),blk1->in(j));
      if( get_ctrl(x) != l && dom_depth(get_ctrl(x)) > dom_depth(l) ) {
        set_ctrl(x,l);          // Lift x because of the new phi use
      }
    } else {
...
Node *PhaseIdealLoop::find_use_block( Node *use, Node *def, Node *old_false, Node *new_false, Node *old_true, Node *new_true ) {
  ...
  // Normal (non-phi) use
Node*use_blk=get_ctrl_no_assert(use);
...
if(i==0){//Record earliest legal locationa
    Node *early = get_early_ctrl(use, false);
set_ctrl(use,early);
  }
...
void PhaseIdealLoop::do_split_if( Node *iff ) {
  ...
  bool was_loop = region->is_Loop();
  if( PrintOpto && VerifyLoopOptimizations ) {
    C2OUT->print(was_loop ? "Split-if at loop head  %d" : "Split-if %d",C->_split_ctr);
iff->dump();
  }
  ...
  while (progress) {
    ...
    for (i = region->outs(); region->has_out(i); i++) {
      ...
      if( was_loop && n->in(LoopNode::LoopBackControl) == n ) {
        _igvn.subsume_node(n,n->in(LoopNode::EntryControl));
        progress = true;
	continue;
      }
  ...
  // Split the IF.
Node*new_iff=split_thru_region(iff,region,false);
...
Node*new_false=NULL,*new_true=NULL;
  for (DUIterator_Last j2min, j2 = iff->last_outs(j2min); j2 >= j2min; --j2) {
    ...
    Node *ifpx = split_thru_region( ifp, region, (was_loop && get_loop(iff)==get_loop(ifp)) );

    // Replace 'If' projection of a Region with a Region of
    // 'If' projections.
    ifpx->set_req(0, ifpx);       // A TRUE RegionNode
    // Loop is same as original projection
IdealLoopTree*loop=get_loop(ifp);
set_loop(ifpx,loop);
    if( !loop->_child )
loop->_body.push(ifpx);
  ...
  if( was_loop ) {              // Make the loop node an official loop head
IdealLoopTree*loop=get_loop(region);
    Node *head = get_loop(new_true)==loop ? new_true : new_false;
    Node *post = get_loop(new_true)==loop ? new_false : new_true;
loop->_head=head;
    loop->_tail = head->in(LoopNode::LoopBackControl);
set_loop(head,loop);
    Node *entry = head->in(LoopNode::EntryControl);
    set_idom(head, entry, dom_depth(entry)+1);
    set_idom(post, entry->in(0), dom_depth(entry));
    // New loop-entry path got placed inside the loop;
    // move it to outside the loop.
    set_loop(entry,get_loop(entry->in(0)));
    if( !loop->_child )
loop->_body.yank(entry);

    region_dom = entry->in(0);
  }
