...
C2OUT->print("%3d ",m);_mem_slice_head.at(m)->dump();
C2OUT->print("    ");_mem_slice_tail.at(m)->dump();
    }
  }
#endif
...
#ifndef PRODUCT
  if (TraceSuperWord && Verbose)
C2OUT->print_cr("compute_max_depth iterated: %d times",ct);
#endif
}
...
#ifndef PRODUCT
C2OUT->print_cr("packset");
  for (int i = 0; i < _packset.length(); i++) {
C2OUT->print_cr("Pack: %d",i);
    Node_List* p = _packset.at(i);
    print_pack(p);
  }
#endif
...
#ifndef PRODUCT
C2OUT->print(" align: %d \t",alignment(s));
  s->dump();
#endif
}
...
#ifndef PRODUCT
C2OUT->print("base: %d  adr: %d  scale: %d  offset: %d  invar: %c%d\n",
...
void DepEdge::print() {
#ifndef PRODUCT
C2OUT->print_cr("DepEdge: %d [ %d ]",_pred->node()->_idx,_succ->node()->_idx);
#endif
}
