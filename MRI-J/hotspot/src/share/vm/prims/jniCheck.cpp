...
"jniCheck examining oops in without jvm lock.")
...
__ENTRY(result_type,"checked JNI",header,thr)
...
methodOop jniCheck::validate_jmethod_id(JavaThread* thr, jmethodID method_id) {
  ASSERT_OOPS_ALLOWED;
methodOop moop=JNIHandles::checked_resolve_jmethod_id(method_id).as_methodOop();
  if (moop == NULL) {
...
JNI_ENTRY_CHECKED(jstring,checked_jni_NewStringUTF(JNIEnv*env,const char*utf))
    functionEnter(thr);
