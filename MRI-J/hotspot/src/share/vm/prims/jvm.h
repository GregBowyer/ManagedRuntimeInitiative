...
JNIEXPORT jobject JNICALL
JVM_ShallowCopy(JNIEnv*env,jobject obj);

JNIEXPORT jboolean JNICALL
JVM_ReferenceEquality(JNIEnv*env,jobject ref,jobject obj);
...
JNIEXPORT void JNICALL
JVM_SystemResourceLimit_GC(void);
...
JNIEXPORT void JNICALL
JVM_MetaTick(jint meta, jlong info, jlong tag);

/* 
 *  AVM JNIEnv -> Proxy JNIEnv conversion 
 */
JNIEXPORT uint64_t JNICALL
JVM_Avm2ProxyJNIEnv(JNIEnv*env);

JNIEXPORT char* JNICALL
JVM_GetMethodSignature(jmethodID methodID);

JNIEXPORT jchar JNICALL
JVM_GetMethodReturnType(jmethodID methodID);

JNIEXPORT jboolean JNICALL
JVM_GetUseDebugLibrarySuffix(JNIEnv*env);

/* For JVMTI Heap Iteration */

// this needs to be in sync with jvm.h under j2se5 and avmProxyJVMTI.hpp
typedef enum {
  JVMTI_ITERATE_OVER_HEAP = 0,
  JVMTI_ITERATE_OVER_REACHABLE_OBJECT
} jvmti_heap_iteration_type;

JNIEXPORT char* JNICALL JVM_GetMethodSignatureFromUnknownThread(jmethodID methodID);
JNIEXPORT jint JNICALL JVM_GetThreadId();
JNIEXPORT void JNICALL JVM_GetTagAndThreadId(jlong* callbackwrapper_ptr, jlong* tag_addr, jint* thread_id_addr);
JNIEXPORT void  JNICALL JVM_SetCallbackWrapperTag(jboolean update_tag, jlong callback_wrapper_ptr, jlong tag);
JNIEXPORT void  JNICALL JVM_SetHeapIterationControl(jvmtiIterationControl value, jvmti_heap_iteration_type type);
JNIEXPORT jboolean  JNICALL JVM_ShouldUseSynchronousHeapCallbacks();

/* ThreadCounters */
JNIEXPORT jlong JNICALL JVM_TicksToNanos(JNIEnv* env, jlong ticks);
JNIEXPORT jlong JNICALL JVM_CollectWallTicks(JNIEnv* env);
JNIEXPORT jlong JNICALL JVM_CollectCpuTicks(JNIEnv* env);
JNIEXPORT jlong JNICALL JVM_CollectBlockedTicks(JNIEnv* env);
JNIEXPORT jlong JNICALL JVM_CollectObjWaitTicks(JNIEnv* env);
JNIEXPORT jlong JNICALL JVM_CollectNetWaitTicks(JNIEnv* env);
JNIEXPORT jlong JNICALL JVM_CollectFileWaitTicks(JNIEnv* env);
JNIEXPORT jlong JNICALL JVM_CollectGCWaitTicks(JNIEnv* env);
JNIEXPORT jlong JNICALL JVM_CollectSafepointWaitTicks(JNIEnv* env);
JNIEXPORT jlong JNICALL JVM_CollectCpuWaitTicks(JNIEnv* env);

/* To request TLAB parking from j2se or proxy */
JNIEXPORT void JNICALL JVM_ParkTLAB();

JNIEXPORT void JNICALL JVM_UnparkTLAB();

JNIEXPORT jboolean JNICALL JVM_GetUseLockedCollections();
JNIEXPORT jboolean JNICALL JVM_GetUseCheckedCollections();

JNIEXPORT jint JNICALL
JVM_ReadByteArrayRegion(JNIEnv *env, jbyteArray array, jsize start,
                        jsize len, jint fd);

JNIEXPORT jint JNICALL
JVM_WriteByteArrayRegion(JNIEnv *env, jbyteArray array, jsize start,
                         jsize len, jint fd);

JNIEXPORT void JNICALL
JVM_InitFileDescriptorField(JNIEnv *env, jclass fdClass, jfieldID fdid);

JNIEXPORT jboolean JNICALL
JVM_IsFileDescriptorField(JNIEnv *env, jobject obj, jfieldID id);



/* Get/set an int field for each element of a region of an object array into a buffer. These
 * calls are used by the Azul implementation of the Weblogic socket muxer to reduce the number
 * of JNI calls performed.
 *
 * Throws:
 *   NullPointerException - if 'array' or an element of 'array' are null
 *   ArrayIndexOutOfBoundsException - if 'start' and 'len' don't specify a valid range of 'array' */
JNIEXPORT void JNICALL
JVM_GetObjectArrayRegionIntField(JNIEnv *env, jobjectArray array, jsize start,
                                 jsize len, jint *buf, jfieldID fieldID);
JNIEXPORT void JNICALL
JVM_SetObjectArrayRegionIntField(JNIEnv *env, jobjectArray array, jsize start,
                                 jsize len, const jint *buf, jfieldID fieldID);

/*
 * Get information about the object and the values of all it's fields
 */
JNIEXPORT int JNICALL
JVM_GetObjectFields(JNIEnv *env, jobject obj, void *objserialzer);

JNIEXPORT void JNICALL
JVM_CachejfieldID(JNIEnv *env, jclass clazz, jfieldID id, jfieldID mapid);

/*
 * java.lang.ref.Finalizer
 */
JNIEXPORT void JNICALL
JVM_SetFinalizerThread(JNIEnv*env,jclass ignored,jboolean flag);
 ...
 */
JNIEXPORT jobject JNICALL
JVM_ReferenceGetReferent(JNIEnv*env,jobject reference);

JNIEXPORT jboolean JNICALL
JVM_ReferencePendingInNext(JNIEnv*env);

/*
 * Extract fields of a datagram socket object 
 */
enum {
    DATAGRAM_IO_FD_FD_FIELDID = 0,
    DATAGRAM_PDSI_FD_FIELDID,
    DATAGRAM_PDSI_FD1_FIELDID,
    DATAGRAM_PDSI_TIMEOUT_FIELDID,
    DATAGRAM_PDSI_TRAFFICCLASS_FIELDID,
    DATAGRAM_PDSI_LOCALPORT_FIELDID,
    DATAGRAM_PDSI_CONNECTED_FIELDID,
    DATAGRAM_PDSI_CONNECTEDADDRESS_FIELDID,
    DATAGRAM_PDSI_CONNECTEDPORT_FIELDID,
    DATAGRAM_PDSI_MULTICASTINTERFACE_FIELDID,
    DATAGRAM_PDSI_LOOPBACK_FIELDID,
    DATAGRAM_PDSI_TTL_FIELDID,

    DATAGRAM_DPBUFFER_FIELDID,
    DATAGRAM_DPADDRESS_FIELDID,
    DATAGRAM_DPOFFSET_FIELDID,
    DATAGRAM_DPBUFLENGTH_FIELDID,
    DATAGRAM_DPLENGTH_FIELDID,
    DATAGRAM_DPPORT_FIELDID,

    DATAGRAM_IAADDRESS_FIELDID,
    DATAGRAM_IAFAMILY_FIELDID,

    /* Add new fields before this */
    DATAGRAM_NUMBER_OF_FIELDIDS
};

JNIEXPORT int JNICALL
JVM_DatagramSendTo(JNIEnv *env, jobject socket, jobject packet, jfieldID *idarray,
                   jint *fd, jboolean *connected, jint *buflen, char **buffer,
                   jint *family, jint *rport, uint32_t *raddr);

JNIEXPORT int JNICALL
JVM_DatagramRecvFrom(JNIEnv *env, jobject socket, jobject packet, jfieldID *idarray,
                     jint *fd, jint *bufoffset, size_t *buflen, unsigned int *timeout,
                     jint *family, uint32_t *caddr, uint32_t *raddr);

JNIEXPORT void JNICALL
JVM_DatagramRecvFromSet(JNIEnv *env, jobject packet, jfieldID *idarray,
