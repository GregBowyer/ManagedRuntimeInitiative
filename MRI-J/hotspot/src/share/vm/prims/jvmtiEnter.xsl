...
static jvmtiError JNICALL
  ...
  <xsl:choose>
    ...
    <xsl:text>    return JVMTI_ERROR_WRONG_PHASE;
      ...
      <xsl:text>  
  Thread* this_thread = (Thread*)Thread::current(); </xsl:text>
  ...
  if (!jvmti_env->is_valid()) {
    ...
    <xsl:if test="$trace='Trace'">
      <xsl:text>    if (trace_flags) {
          tty->print_cr("JVMTI [%s] %s %s  env=%p",  curr_thread_name, func_name, 
  ...
  if (Threads::number_of_threads() == 0) {
    ...
    this_thread = (Thread*)Thread::current();
    transition = ((this_thread != NULL) &amp;&amp; !this_thread->is_VM_thread() &amp;&amp; !this_thread->is_GenPauselessGC_thread());
  }
...
<xsl:template match="jframeID" mode="dochecks">
  ...
  if (depth &lt; 0) {
    ...
    <xsl:apply-templates select=".." mode="traceError">     
      ...
      <xsl:with-param name="comment"> - negative depth - depth = %d</xsl:with-param>
      <xsl:with-param name="extraValue">, <xsl:value-of select="$name"/></xsl:with-param>
    </xsl:apply-templates>
    <xsl:text>
  }
...
<xsl:template match="jclass" mode="dochecks">
 ...
 <xsl:if test="count(@method)=0">
  ...
  if (k_oop == NULL) {
    ...
    <xsl:apply-templates select=".." mode="traceError">     
      <xsl:with-param name="err">JVMTI_ERROR_INVALID_CLASS</xsl:with-param>
      <xsl:with-param name="comment"> - no klassOop - jclass = " PTR_FORMAT "</xsl:with-param>
      <xsl:with-param name="extraValue">, <xsl:value-of select="$name"/></xsl:with-param>
    </xsl:apply-templates>
    <xsl:text>
  }
...
<xsl:template match="jlong|jlocation" mode="traceInFormat">
  <xsl:param name="name"/>
  <xsl:text> </xsl:text>
  <xsl:value-of select="$name"/>
  <xsl:text>=%lld</xsl:text>
</xsl:template>
