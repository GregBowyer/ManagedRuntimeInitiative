...
// FIXME - AZUL - Fix code under AZUL_PORT_16
...
#ifdef AZUL_PORT_16 // how do we test for native if not at a safepoint here?
    // CNC - You Don't because running inside the VM-but-jvmlock-unlocked is
    // functionally identical to running inside native code, e.g. the compiler
    // threads are in this mode most all the time.  But you CAN ask
    // "thr->jvm_lock_is_free" which is true inside of natives as well as just
    // before blocking.
    if (thr != Thread::current() && !thr->is_hint_blocked()) {
      // not blocked but at safepoint?  must be in native
...
if(java_thread->is_being_ext_suspended()){
  ...
  jvmtiError suspendErr = JvmtiSuspendControl::suspend(java_thread);
  if (suspendErr == JVMTI_ERROR_INVALID_THREAD) {
...
JvmtiEnv::ResumeThread(JavaThread* java_thread) {
  ...
  if (!(java_thread->please_self_suspend() & JavaThread::jvmti_suspend)) {
...
JvmtiEnv::StopThread(JavaThread* java_thread, jobject exception) {
  JavaThread* thread = JavaThread::current();   // replaces (JavaThread*) ThreadLocalStorage::thread();

  StackBasedAllocation::ensure_in_heap(thread, exception,"jvmti StopThread");
objectRef e=JNIHandles::resolve_as_ref_external_guard(exception);
  NULL_CHECK(e.as_oop(), JVMTI_ERROR_NULL_POINTER);
...
MutexLockerAllowGC ml(Threads_lock,thread);
  java_thread->install_async_exception(e, true /*overwrite*/, true /*remote thread*/);
...
MutexLockerAllowGC ml(current_thread->threadObj()==thread_oop?NULL:&Threads_lock,1);
...
JavaThreadPriority priority;
  ...
  { MutexLockerAllowGC mu(Threads_lock,current_thread);
...
JvmtiEnv::GetOwnedMonitorInfo(JavaThread* java_thread, jint* owned_monitor_count_ptr, jobject** owned_monitors_ptr) {
  ...
  if (is_thread_fully_suspended(java_thread, true)) {
...
JvmtiEnv::RunAgentThread(jthread thread, jvmtiStartFunction proc, const void* arg, jint priority) {
  ...
  {
MutexLockerAllowGC mu(Threads_lock,current_thread);//grab Threads_lock

JvmtiAgentThread*new_thread=new(ttype::java_thread)JvmtiAgentThread(this,proc,arg);
...
Threads::add(new_thread,false);

    new_thread->set_priority((JavaThreadPriority)priority);
...
JvmtiEnv::GetThreadGroupInfo(jthreadGroup group, jvmtiThreadGroupInfo* info_ptr) {
  ...
  Handle parent_group;
  bool is_daemon;
JavaThreadPriority max_priority;
...
if(!is_thread_fully_suspended(java_thread,true/*wait for suspend completion*/)){
    return JVMTI_ERROR_THREAD_NOT_SUSPENDED;
  }
...

  {
    ...
    if (java_thread->is_hint_waiting()) { // MONITOR_WAIT
...

  {
    ...
    for (vframe vf = java_thread->last_java_vframe(/* true */); !vf.done(); vf.next()) {
methodHandle mh(current_thread,vf.method());
...
if(vframeFor(java_thread,1).done()){
    ...
    {
      MutexLockerAllowGC mu1(Compile_lock, current_thread);
      ...
      //   nmethod* nm = NULL;
      //   {
      //     vframe vf = vframeFor(java_thread, i);
      //     fr = vf.get_frame();
      //     nm = vf.nm();
      //     if (vf.done()) {
      //       break;
      ...
      //     MutexLocker mu2(NMethodBucket_lock);
      //     num_marked++;
      //     nm->deoptimize();
      //   }
      //   i++;
...
JvmtiEnv::NotifyFramePop(JavaThread* java_thread, jint depth) {
  ...
  if (!JvmtiEnv::is_thread_fully_suspended(java_thread, true)) {
      return JVMTI_ERROR_THREAD_NOT_SUSPENDED;
  }
...

  {
    ...
    if (!vf.get_frame().is_java_frame() || vf.method()->is_native()) {
      return JVMTI_ERROR_OPAQUE_FRAME;
    }
...
JvmtiEnv::ForceEarlyReturnObject(JavaThread* java_thread, jobject value) {
  jvalue val;
  val.l = value;
return force_early_return(java_thread,val,T_OBJECT);
...
JvmtiEnv::ForceEarlyReturnInt(JavaThread* java_thread, jint value) {
  jvalue val;
  val.i = value;
return force_early_return(java_thread,val,T_INT);
...
JvmtiEnv::ForceEarlyReturnLong(JavaThread* java_thread, jlong value) {
  jvalue val;
  val.j = value;
return force_early_return(java_thread,val,T_LONG);
...
JvmtiEnv::ForceEarlyReturnFloat(JavaThread* java_thread, jfloat value) {
  jvalue val;
  val.f = value;
return force_early_return(java_thread,val,T_FLOAT);
...
JvmtiEnv::ForceEarlyReturnDouble(JavaThread* java_thread, jdouble value) {
  jvalue val;
  val.d = value;
return force_early_return(java_thread,val,T_DOUBLE);
...
JvmtiEnv::ForceEarlyReturnVoid(JavaThread* java_thread) {
  jvalue val;
  val.j = 0L;
return force_early_return(java_thread,val,T_VOID);
...
JvmtiEnv::GetLocalObject(JavaThread* java_thread, jint depth, jint slot, jobject* value_ptr) {
  ...
  MutexLockerAllowGC ml1(NewGC_ConcurrentRefProcessing_lock, current_thread);
  MutexLockerAllowGC ml2(OldGC_ConcurrentRefProcessing_lock, current_thread);
  
  VM_GetOrSetLocal op(java_thread, current_thread, depth, slot);
  {
    // FIXME
    Unimplemented();
    // vframeAcrossSafepoint vas(op.jvf_addr());
    VMThread::execute(&op);
  }
...
JvmtiEnv::GetConstantPool(oop k_mirror, jint* constant_pool_count_ptr, jint* constant_pool_byte_count_ptr, unsigned char** constant_pool_bytes_ptr) {
  ...
  //FIXME  -- ObjectLocker semantics...
Unimplemented();//ObjectLocker ol(constants, thread);    // lock constant pool while we query it
...
int generic_signature_index=0;
generic_signature_index=(int)table[i].signature_cp_index;
...
JvmtiEnv::DestroyRawMonitor(JvmtiRawMonitor * rmonitor) {
  if (Threads::number_of_threads() == 0) {
    ...
    Thread* thread  = Thread::current();  
    // FIXME  -- need ObjectMonitor constant equivalents and RawMonitor method
    Unimplemented();
  }
...
JvmtiEnv::RawMonitorEnter(JvmtiRawMonitor * rmonitor) {
  if (Threads::number_of_threads() == 0) {
    ...
    if (thread->is_Java_thread()) {
      ...
      // TODO - A bunch of jdb tests hang if we don't do the
      // proper transitions.  Need to fix those before enabling the ifdef below
      // If we don't do the transition here, raw_enter, raw_wait and
      // wait_for_monitor need to be modified to account for not holding
      // the jvm_lock
//#ifdef PROPER_TRANSITIONS
      // Can't do proper transitions because VM thread posting events can deadlock. 
      // Enable this code vmthread posting is fixed.
      // Not really unknown but ThreadInVMfromNative does more than we want
      ThreadInVMfromUnknown __tiv;
//#endif // PROPER_TRANSITIONS
      // FIXME  -- RawMonitor::raw_enter  and  ObjectMonitor::constants
      Unimplemented();
      // r = rmonitor->raw_enter(current_thread, true, false);
      // assert(r == ObjectMonitor::OM_OK, "raw_enter should have worked");
    } else {
if(thread->is_VM_thread()||thread->is_GenPauselessGC_thread()){
      // FIXME  -- RawMonitor::raw_enter  and  ObjectMonitor::constants
      Unimplemented();
	    //  r = rmonitor->raw_enter(thread, true, false);
      } else {
	ShouldNotReachHere();
      }
    }

    // FIXME  -- need new ObjectMonitor constant equivalents...
    Unimplemented();
...
JvmtiEnv::RawMonitorExit(JvmtiRawMonitor * rmonitor) {
  ...
  if (Threads::number_of_threads() == 0) {
    ...
    if (thread->is_Java_thread()) {
      ...
      // FIXME  -- RawMonitor::raw_enter  and  ObjectMonitor::constants
      Unimplemented();
      // r = rmonitor->raw_exit(current_thread, true, false);
    } else {
if(thread->is_VM_thread()||thread->is_GenPauselessGC_thread()){
      // FIXME  -- RawMonitor::raw_enter  and  ObjectMonitor::constants
      Unimplemented();
	    //   r = rmonitor->raw_exit(thread, true, false);
      } else {
	ShouldNotReachHere();
      }
    }
...
//#ifdef PROPER_TRANSITIONS
    ...
    ThreadInVMfromUnknown __tiv;
//#endif /* PROPER_TRANSITIONS */
    // FIXME  -- RawMonitor implementation
    Unimplemented();
...
JvmtiEnv::GetAvailableProcessors(jint* processor_count_ptr) {
  if (AvailableProcessors > 0) {
      *processor_count_ptr = AvailableProcessors;
  }

  if (AllowDynamicCPUCount) {
      *processor_count_ptr = os::active_processor_count();
  } else {
*processor_count_ptr=os::processor_count();
  }
  return JVMTI_ERROR_NONE;
...
JvmtiEnv::AddToBootstrapClassLoaderSearch(const char* segment) {
  ...
  if (phase == JVMTI_PHASE_ONLOAD) {
    ...
    //FIXME  -- ObjectLocker semantics...
Unimplemented();//ObjectLocker ol(loader_lock, thread);
...
JvmtiEnv::AddToSystemClassLoaderSearch(const char* segment) {
  ...
  if (phase == JVMTI_PHASE_ONLOAD) {
    ...
    //FIXME  -- ObjectLocker semantics...
Unimplemented();//ObjectLocker ol(loader, THREAD);

    // need the path as java.lang.String
    Handle path = java_lang_String::create_from_str(segment, true/*SBA*/, THREAD);
    if (HAS_PENDING_EXCEPTION) {
      CLEAR_PENDING_EXCEPTION;
      return JVMTI_ERROR_INTERNAL;
    }
