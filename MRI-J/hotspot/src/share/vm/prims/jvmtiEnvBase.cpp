...
// Automatically generated file; must come last
...
assert(SafepointSynchronize::is_at_safepoint()||(UseGenPauselessGC),"sanity check");
  ...
  if (_needs_clean_up) {
    ...
    WaitLock* threadsLock = NULL;
    Thread* current_thread = Thread::current();
    if (UseGenPauselessGC) {
      assert(current_thread->is_GenPauselessGC_thread(), "Must be GPGC thread");
      ((GPGC_Thread*)current_thread)->set_blocked(true);
      threadsLock = &Threads_lock;
    }

    {
      MutexLockerAllowGC ml(threadsLock,1);
    ...
    if (UseGenPauselessGC) {
      ((GPGC_Thread*)current_thread)->set_blocked(false);
    }
  }
}
...
// Called from JVMTI entry points which perform stack walking.
//
bool 
JvmtiEnvBase::is_thread_fully_suspended(JavaThread*thr,bool wait_for_suspend){
  // "other" threads require special handling
if(thr==JavaThread::current())return true;

//See if other thread is ready to be stack-walked.
  return thr->wait_for_suspend_completion(wait_for_suspend);
}
...
Unimplemented();//FIXME last_java_vframe need a FALSE operand?
vframe vf=java_thread->last_java_vframe();
  int d = 0;
  while ((!vf.done()) && (d < depth)) {
vf.next();
    d++;
  }
  return vf;
...
if(java_thread->root_Java_frame()){
  ...
  // CNC - note that due to BiasedLocking we have a fast custom routine for this already....
  for(vframe jvf=java_thread->last_java_vframe(); !jvf.done(); jvf.next()) {
    Unimplemented();
    //GrowableArray<objectRef>* mons = jvf.monitors(java_thread);
    //if (!mons->is_empty()) {
    //  for (int i = 0; i < mons->length(); i++) {
    //    Unimplemented();
    //    objectRef mi = mons->at(i);
    //
...
is_thread_fully_suspended(java_thread,false)),
...
if(!java_thread->root_Java_frame()){
    ...
    int depth = 0;
for(vframe jvf=java_thread->last_java_vframe();!jvf.done();
jvf.next()){
...
// Save JNI handles (local for JVMTI) for any objects that this frame owns.
jvmtiError
JvmtiEnvBase::get_locked_objects_in_frame(JavaThread* calling_thread, JavaThread* java_thread, 
const vframe&jvf,GrowableArray<jvmtiMonitorStackDepthInfo*>*owned_monitors_list,int stack_depth){
  jvmtiError err = JVMTI_ERROR_NONE;
  ResourceMark rm;

  GrowableArray<objectRef>* mons = jvf.monitors(java_thread);
  if (mons->is_empty()) {
return err;//this vframe holds no monitors
  }
...
vframe jvf=java_thread->last_java_vframe();
    HandleMark hm(current_thread);
    if (start_depth != 0) {
      if (start_depth > 0) {
for(int j=0;j<start_depth&&!jvf.done();j++){
jvf.next();
	}
	if (jvf.done()) {
...
vframe jvf_cursor=jvf;
vframe jvf_prev=vframe(frame(0,0));
	vframe jvf_prev_prev = vframe(frame(0,0));
	int j = 0;
	while (!jvf_cursor.done()) {
	  jvf_prev_prev = jvf_prev;
	  jvf_prev = jvf_cursor;
for(j=0;j>start_depth&&!jvf_cursor.done();j--){
jvf_cursor.next();
	  }
	}
	if (j == start_depth) {
...
jvf.next();
    ...
    for (; count < max_count && !jvf.done(); count++) {
      {
methodOop m=jvf.method();
        frame_buffer[count].location = (m->is_native() ? InvocationEntryBci : jvf.bci());
        frame_buffer[count].method = m->jmethod_id(); // might GC and blow 'm'
      }
...
is_thread_fully_suspended(java_thread,false)),
  ...
  methodOop method = NULL;
  {
    ...
    // Azul - check for a native method as well.
#ifdef PRODUCT
...
// Checks that the frame method signature matches the return type (type).
...
jvalue value,BasicType type,Handle*ret_ob_h){
  ResourceMark rm(current_thread);
...
//  }
  ...
  BasicType return_type = vf.method()->result_type();

  if (return_type != type) {
    if (type != T_INT || (return_type != T_BYTE && return_type != T_BOOLEAN && return_type != T_CHAR && return_type != T_SHORT)) {
...
if(type==T_OBJECT&&jobj!=NULL){//NULL reference is allowed
...
char*ty_sign=1+strchr(vf.method()->signature()->as_C_string(),')');
...
jvmtiError
JvmtiEnvBase::force_early_return(JavaThread*java_thread,jvalue value,BasicType type){
...
jvmtiError err=check_top_frame(current_thread,java_thread,value,type,&ret_ob_h);
  if (err != JVMTI_ERROR_NONE) {
    return err;
  }
assert(type!=T_OBJECT||value.l==NULL||ret_ob_h()!=NULL,
...
state->set_earlyret_value(value,type);
