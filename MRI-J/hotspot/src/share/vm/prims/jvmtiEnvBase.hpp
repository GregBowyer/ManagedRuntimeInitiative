...
#ifndef JVMTIENVBASE_HPP
#define JVMTIENVBASE_HPP
...
bool is_valid_jvmti(){return _magic==JVMTI_MAGIC;}
  bool is_valid()                                  { return is_valid_jvmti(); }
  ...
  // method to create global handle for local handle for jvmti.    
  jobject jni_reference(Handle hndl) {
    return JNIHandles::make_local(hndl());  
  }

  // method to create a local handle for jvmti. 
  ...
  // method to destroy global handle for local handle for jvmti.    
  void destroy_jni_reference(jobject jobj) {
    JNIHandles::destroy_local(jobj);  
  }
    
  // method to destroy local handle.
  // This function allows caller to specify which
...
jvmtiError force_early_return(JavaThread*java_thread,jvalue value,BasicType type);
};
