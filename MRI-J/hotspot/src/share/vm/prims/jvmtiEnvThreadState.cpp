...
assert(get_thread()==Thread::current()||JvmtiEnv::is_thread_fully_suspended(get_thread(),false),"frame pop data only accessible from same thread or while suspended");
...
vframe vf=_thread->last_java_vframe(/*false*/);
...
if(event_type==JVMTI_EVENT_SINGLE_STEP&&!_thread->root_Java_frame()){
      jmethodID method_id;
      int bci;
