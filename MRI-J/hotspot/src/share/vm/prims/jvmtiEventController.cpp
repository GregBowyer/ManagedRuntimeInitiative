...
void VM_EnterInterpOnlyMode::doit() {
  ...
  // Compile_lock first.
MutexLocker mcl(Compile_lock,Thread::current());
  EnforceSafepoint es;
...
//    vframe vf = thread->last_java_vframe(false);
//
//    while (!vf.done()) {
//      if (vf.get_frame().is_compiled_frame()) {
//        vframeAcrossSafepoint vas(&vf);
//
...
JvmtiEventControllerPrivate::recompute_enabled() {
  ...
  if (    (any_env_thread_enabled & THREAD_FILTERED_EVENT_BITS) != 0 &&
    ...
    { 
MutexLockerAllowGC mu(Threads_lock,JavaThread::current());//hold the Threads_lock for the iteration
...
MutexLockerAllowGC mu(*JvmtiThreadState_lock,thread);
