...
#ifndef JVMTIEXPORT_HPP
#define JVMTIEXPORT_HPP
...
static void post_method_exit(JavaThread*thread,methodOop method,frame current_frame,uint64_t return_value)KERNEL_RETURN;
...
static void post_compiled_method_load(methodCodeOop nm)KERNEL_RETURN;
  ...
  // call after GPGC has completed referencing processing
...
class JvmtiVMObjectAllocEventCollector : public JvmtiEventCollector {
 private:
GrowableArray<uint64_t>*_allocated;//field to record vm internally allocated object oop.
 ...
 public:
  ...
  oop oop_at(int i) { return lvb_ref((objectRef*) (_allocated->adr_at(i))).as_oop(); }
...
class JvmtiGCMarker : public StackObj {
 private:
  ...
  bool _pre_iterate_collection; 		// Used by [G]PGC

 protected:
JvmtiGCMarker(bool full,bool pre_iterate_collection=false)KERNEL_RETURN;//protected
...
class JvmtiGCForAllocationMarker : public JvmtiGCMarker {
 public:
  JvmtiGCForAllocationMarker(bool pre_iterate_collection = false) : JvmtiGCMarker(false, pre_iterate_collection) { 
  }
};
...
class JvmtiGCFullMarker : public JvmtiGCMarker {
 public:
  JvmtiGCFullMarker(bool pre_iterate_collection = false) : JvmtiGCMarker(true, pre_iterate_collection) { 
  }
};
