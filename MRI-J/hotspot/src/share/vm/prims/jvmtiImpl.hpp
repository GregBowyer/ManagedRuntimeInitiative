...
#ifndef JVMTIIMPLE_HPP
#define JVMTIIMPLE_HPP
...
public: 
  ...
  methodOop method() { return lvb_methodRef(&_method).as_methodOop(); }
...
void oops_do(OopClosure*f){f->do_oop(&_method);}
...
public:
  JvmtiRawMonitor(objectRef ref, const char *name);
  ~JvmtiRawMonitor();
...
vframe _jvf;
  ...
  vframe get_vframe();
  bool check_slot_type(const vframe& vf);
...
static jvmtiError suspend(JavaThread*java_thread);
  // resume the thread
