...
objArrayRef VM_RedefineClasses::_old_methods;
objArrayRef VM_RedefineClasses::_new_methods;
methodRef*  VM_RedefineClasses::_matching_old_methods    = NULL;
methodRef*  VM_RedefineClasses::_matching_new_methods    = NULL;  
methodRef*  VM_RedefineClasses::_deleted_methods         = NULL;
methodRef*  VM_RedefineClasses::_added_methods           = NULL;  
int         VM_RedefineClasses::_matching_methods_length = 0;  
int         VM_RedefineClasses::_deleted_methods_length  = 0;
int         VM_RedefineClasses::_added_methods_length    = 0;  
klassRef VM_RedefineClasses::_the_class_oop;

void jvmtiRedefineClasses_static_init(){
  VM_RedefineClasses::_old_methods = nullRef;
  VM_RedefineClasses::_new_methods = nullRef;
  VM_RedefineClasses::_the_class_oop = nullRef;
}
...
case JVM_CONSTANT_Dependencyref://fall through
...
instanceKlass::cast(the_class_oop())->class_loader()!=NULL;
...
if(ik->vtable_length()>0&&ik->is_subtype_of(the_class_oop())){
...
if(ik->itable_length()>0&&(Klass::cast(the_class_oop())->is_interface()
||ik->is_subclass_of(the_class_oop()))){
...
methodOop old_method=lvb_methodRef(&_matching_old_methods[j]).as_methodOop();
    jmethodID jmid = old_method->find_jmethod_id_or_null();
    if (jmid != NULL) {
      // There is a jmethodID, change it to point to the new method
methodHandle new_method_h(lvb_methodRef(&_matching_new_methods[j]).as_methodOop());
      JNIHandles::change_method_associated_with_jmethod_id(jmid, new_method_h);
assert(JNIHandles::resolve_jmethod_id(jmid)==lvb_methodRef(&_matching_new_methods[j]).as_methodOop(),
             "should be replaced");
    }
  }
}
...
methodOop old_method=lvb_methodRef(&_matching_old_methods[j]).as_methodOop();
methodOop new_method=lvb_methodRef(&_matching_new_methods[j]).as_methodOop();
    methodOop old_array_method;

    // Maintain an old_index into the _old_methods array by skipping
    // deleted methods
while((old_array_method=(methodOop)old_methods()->obj_at(old_index))
                                                            != old_method) {
      ++old_index;
    }
...
u2 num=instanceKlass::cast(the_class_oop())->next_method_idnum();
      if (num != constMethodOopDesc::UNSET_IDNUM) {
...
methodOop old_method=lvb_methodRef(&_deleted_methods[i]).as_methodOop();
...
assert((*emcp_method_count_p+obsolete_count)==old_methods()->length(),
    "sanity check");
}
...
class TransferNativeFunctionRegistration {
 ...
 public:
  ...
  void transfer_registrations(methodRef* old_methods, int methods_length) {
...
void VM_RedefineClasses::flush_dependent_code(instanceKlassHandle k_h, TRAPS) {
  ...
  if (JvmtiExport::all_dependencies_are_recorded()) {
    No_Safepoint_Verifier nsv;
instanceKlass::cast(k_h())->deoptimize_evol_dependent_on();
  } else {
    Unimplemented();
//    // Deoptimize all activations depending on marked nmethods
//    if (nmethod::deopt_list_len > 0) {
//      VM_Deoptimize vm_op;
...
void VM_RedefineClasses::compute_added_deleted_matching_methods() {
  ...
  _matching_old_methods = NEW_RESOURCE_ARRAY(methodRef, old_methods()->length());
  _matching_new_methods = NEW_RESOURCE_ARRAY(methodRef, old_methods()->length());
  _added_methods        = NEW_RESOURCE_ARRAY(methodRef, new_methods()->length());
  _deleted_methods      = NEW_RESOURCE_ARRAY(methodRef, old_methods()->length());
  ...
  int oj = 0;
  while (true) {
if(oj>=old_methods()->length()){
if(nj>=new_methods()->length()){
        break; // we've looked at everything, done
      }
      // New method at the end
new_method=(methodOop)new_methods()->obj_at(nj);
POISON_AND_STORE_REF(&_added_methods[_added_methods_length++],methodRef(new_method));
      ++nj;
}else if(nj>=new_methods()->length()){
      // Old method, at the end, is deleted
old_method=(methodOop)old_methods()->obj_at(oj);
POISON_AND_STORE_REF(&_deleted_methods[_deleted_methods_length++],methodRef(old_method));
      ++oj;
    } else {
old_method=(methodOop)old_methods()->obj_at(oj);
new_method=(methodOop)new_methods()->obj_at(nj);
...
POISON_AND_STORE_REF(&_matching_old_methods[_matching_methods_length],methodRef(old_method));
POISON_AND_STORE_REF(&_matching_new_methods[_matching_methods_length++],methodRef(new_method));
          ++nj;
          ++oj;
        } else {
...
assert(_matching_methods_length+_deleted_methods_length==old_methods()->length(),"sanity");
assert(_matching_methods_length+_added_methods_length==new_methods()->length(),"sanity");
}
...
void VM_RedefineClasses::redefine_single_class(jclass the_jclass,
  ...
  POISON_AND_STORE_REF(&_old_methods, objArrayRef(the_class->methods()));
  POISON_AND_STORE_REF(&_new_methods, objArrayRef(scratch_class->methods()));
  POISON_AND_STORE_REF(&_the_class_oop, klassRef(the_class_oop));
...
for(int i=old_methods()->length()-1;i>=0;i--){
methodOop method=(methodOop)old_methods()->obj_at(i);
...
the_class->set_methods(new_methods());
scratch_class->set_methods(old_methods());//To prevent potential GCing of the old methods,
...
BitMap emcp_methods(old_methods()->length());
...
for(j=0;j<old_methods()->length();++j){
methodOop m=(methodOop)old_methods()->obj_at(j);
...
for(j=0;j<new_methods()->length();++j){
methodOop m=(methodOop)new_methods()->obj_at(j);
          tty->print("%4d  (%5d)  ", j, m->vtable_index());
          m->access_flags().print_on(tty);
          tty->print(" --  ");
          m->print_name(tty);
...
methodOop m=lvb_methodRef(&_matching_old_methods[j]).as_methodOop();
          tty->print("%4d  (%5d)  ", j, m->vtable_index());
          m->access_flags().print_on(tty);
          tty->print(" --  ");
          m->print_name(tty);
          tty->cr();
m=lvb_methodRef(&_matching_new_methods[j]).as_methodOop();
...
methodOop m=lvb_methodRef(&_deleted_methods[j]).as_methodOop();
          tty->print("%4d  (%5d)  ", j, m->vtable_index());
          m->access_flags().print_on(tty);
          tty->print(" --  ");
          m->print_name(tty);
...
methodOop m=lvb_methodRef(&_added_methods[j]).as_methodOop();
          tty->print("%4d  (%5d)  ", j, m->vtable_index());
          m->access_flags().print_on(tty);
          tty->print(" --  ");
          m->print_name(tty);
