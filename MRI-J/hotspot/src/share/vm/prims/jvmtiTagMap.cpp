...
// Azul porting note
//
// serialize_jvmti_heap_reference_info and deserialize_jvmti_heap_reference_info
// assume that JVMTI_HEAP_REFERENCE_JNI_LOCAL and JVMTI_HEAP_REFERENCE_STACK_LOCAL
// populate the jvmtiHeapReferenceInfoStackLocal structure and all other
// jvmtiHeapReferenceKinds populate only the jvmtiHeapReferenceInfoField structure.
//
// Also the 1.1 JVMTI spec stipulates that
// Heap callbacks are single threaded - no more than one callback will be invoked
// at a time.
// I've made the HeapIteratorTaskThreads default to one and made that a develop mode
// flag.  But most tools did not like multithreaded callbacks even with the older
// Heap 1.0 callbacks. So we should cleanup the code to make it simpler.
//

jlong VM_HeapIterateOperation::_number_of_callbacks     = 0;
jlong VM_HeapIterateOperation::_callbacks_start_time    = 0;
bool  VM_HeapIterateOperation::_continue_heap_iteration = true;
bool  VM_HeapIterateOperation::_safe_to_iterate         = true;
bool  VM_HeapIterateOperation::_timeout_reached_warning = false;

jlong VM_HeapWalkOperation::_number_of_callbacks     = 0;
jlong VM_HeapWalkOperation::_callbacks_start_time    = 0;
bool  VM_HeapWalkOperation::_continue_heap_iteration = true;
bool  VM_HeapWalkOperation::_safe_to_iterate         = true;
bool  VM_HeapWalkOperation::_timeout_reached_warning = false;
...
MutexLocker ml(JvmtiTagHashmap_lock,Thread::current());
...
inline JvmtiTagHashmapEntry*find_locked(oop key){
assert_lock_strong(JvmtiTagHashmap_lock);
...
assert(find_locked(key)==NULL,"duplicate detected");
    unsigned int h = hash(key);
...
if(Universe::heap()->kind()==CollectedHeap::GenPauselessHeap||
      Universe::heap()->kind() == CollectedHeap::PauselessHeap) {
    // is_in_young below will check for is_in_new
...
inline bool JvmtiTagMap::is_in_young(oop o) {
  // Azul - for [G]PGC
  // Since objects may be moved during the mark/remap phase of the next
  // cycle, hashmap_for cannot work reliably with [G]PGC.
  // Maintain all objects in just one hashmap by returning true always
if(Universe::heap()->kind()==CollectedHeap::GenPauselessHeap){
    //return ((GPGC_Heap*)Universe::heap())->is_in_new(o);
    return true;
  } else if (Universe::heap()->kind() == CollectedHeap::PauselessHeap) {
    //return ((PauselessHeap*)Universe::heap())->is_in_new(o);
    return true;
  } else if (Universe::heap()->kind() == CollectedHeap::GenCollectedHeap ||
             Universe::heap()->kind() == CollectedHeap::ParallelScavengeHeap) {
  ...
  ShouldNotReachHere();
  return false;   // not sure what else to return
}
...
JvmtiTagMap::JvmtiTagMap(JvmtiEnv* env) :
  _env(env),
  _lock(JvmtiTagHashmap_lock._rank+1, "JvmtiTagMap._lock", false),
  _free_entries(NULL),
...
JvmtiTagHashmapEntry* JvmtiTagMap::create_entry(jweak ref, jlong tag) {
  ...
  assert(current_thread->is_VM_thread() || current_thread->is_GC_thread() || current_thread->is_jvmti_heap_iterator_thread() || is_locked(), "checking");
  ...
  if (_free_entries == NULL) {
    ...
    Atomic::dec_ptr(&_free_entries_count);

    // TODO: check for ABA
    intptr_t old_head;
    intptr_t new_head;
    do {
      old_head = (intptr_t) _free_entries;
      new_head = (intptr_t) _free_entries->next();
    } while (old_head != Atomic::cmpxchg_ptr(new_head, (intptr_t*)&_free_entries, old_head));

    entry = (JvmtiTagHashmapEntry*) old_head;
...
void JvmtiTagMap::destroy_entry(JvmtiTagHashmapEntry* entry) {
  ...
  if (_free_entries_count >= max_free_entries) {
    ...
    do {
      old_head = (intptr_t) _free_entries;
      entry->set_next((JvmtiTagHashmapEntry*) old_head);
      new_head = (intptr_t) entry;
    } while (old_head != Atomic::cmpxchg_ptr(new_head, (intptr_t*)&_free_entries, old_head));

Atomic::inc_ptr(&_free_entries_count);
  }
}
...
CallbackWrapper::CallbackWrapper(JvmtiTagMap* tag_map, oop o) {
  Thread* current_thread = Thread::current();
  assert(current_thread->is_VM_thread() || current_thread->is_GC_task_thread() ||
         tag_map->is_locked(), "MT unsafe or must be VM thread or GC Task Thread");
...
void inline CallbackWrapper::post_callback_tag_update(oop o, 
  ...
  if (entry == NULL) { 
    if (obj_tag != 0) {	
      ...
      Thread* current_thread = Thread::current();
      assert(current_thread->is_VM_thread() || current_thread->is_GC_task_thread() ||
             current_thread->is_jvmti_heap_iterator_thread(), "must be VMThread or GC Task Thread");
...
static void heap_iteration_prolog() {
  Thread* current_thread = Thread::current();

  // reset to true before starting the iteration
  VM_HeapIterateOperation::set_safe_to_iterate(true);
  VM_HeapWalkOperation::set_safe_to_iterate(true);

  if (UseGenPauselessGC) {
    if (!GPGC_Heap::is_safe_to_iterate()) {
      // TODO - Attempt a complete cycle before issuing the warning
warning("VM_HeapIterateOperation: It is not safe to iterate through the heap.  Need a complete cycle before the heap can be iterated over.  Try again later.  If the problem persists, please increase PGCSidebandPagesPercent and start over.");
      VM_HeapIterateOperation::set_safe_to_iterate(false);
      VM_HeapWalkOperation::set_safe_to_iterate(false);
      return;
    }
  } else {
    // Nothing to do here
...
static void heap_iteration_epilog(jlong number_of_callbacks) {
  // If the heap_iteration_prolog determined that it was not safe to iterate
  // we should have just returned from the doit operation and not reached here.
  assert(VM_HeapIterateOperation::is_safe_to_iterate() == true, "epilog reached without actual iteration");
  assert(VM_HeapWalkOperation::is_safe_to_iterate() == true, "epilog reached without actual iteration");

  Thread* current_thread = Thread::current();

#ifdef AZ_PROXIED
  // We don't have to check for completion in the synchronous case
  if (JVM_ShouldUseSynchronousHeapCallbacks() != true) {
assert(UseGenPauselessGC,"asynchronous callbacks only with GPGC");
    proxy_invoke_remote_jvmtiHeapCallbacksChecker(number_of_callbacks);
  }
#endif // AZ_PROXIED
}

void VM_HeapIterateOperation::doit(){
  // allows class files maps to be cached during iteration    
  ClassFieldMapCacheMark cm;

  reset_number_of_callbacks();
  reset_continue_heap_iteration();
  set_callbacks_start_time();

  heap_iteration_prolog();

  if (is_safe_to_iterate() != true) {
    return;
  }
  ...
  // do the iteration
  if (UseGenPauselessGC) {
    HeapIterator::heap_iterate(_blk);
  } else {
    Universe::heap()->object_iterate(_blk);
  }

  heap_iteration_epilog(number_of_callbacks());
}

void call_HeapIterateOperation(void* user_data) {
assert(UseGenPauselessGC,"can be called only by GPGC");

Thread*thread=Thread::current();

  ((VM_HeapIterateOperation*)user_data)->doit();
}
...
void IterateOverHeapObjectClosure::do_object(oop o) {
  ...
  if (VM_HeapIterateOperation::should_discontinue_heap_iteration()) return;
...
CallbackWrapper*wrapper=new CallbackWrapper(tag_map(),o);
...
return;
  ...
  Atomic::inc_ptr((intptr_t*)VM_HeapIterateOperation::number_of_callbacks_addr());

  // invoke the agent's callback
jvmtiIterationControl control;
...
(jlong*)wrapper,
  ...
  if (control == JVMTI_ITERATION_ABORT) {
    VM_HeapIterateOperation::set_heap_iteration_control(control);
  }
}
...
CallbackWrapper*wrapper=new CallbackWrapper(tag_map(),obj);
...
if(is_filtered_by_heap_filter(wrapper->obj_tag(),wrapper->klass_tag(),heap_filter())){
  ...
  if (callbacks()->heap_iteration_callback != NULL) {
    ...
    if (check_flags_for_abort(res)) {
      // FIXME - Deleting the wrapper here will not work for asynchronous callbacks
      delete wrapper;
      return;
    }
  }
                                    ...
                                    const void* user_data) 
{
  ...
  if (UseGenPauselessGC) {
    ...
    // This will mark/remap the entire heap and not relocate.
    // This way we can iterate through the entire heap safely.
    ((GPGC_Heap*)Universe::heap())->collect_and_iterate(GCCause::_jvmti_iterate_over_heap, call_HeapIterateOperation, &op);
  } else {
...
void JvmtiTagMap::iterate_through_heap(jint heap_filter, 
				       KlassHandle klass, 
                                       ...
                                       const void* user_data) 
{   
  guarantee (JVM_ShouldUseSynchronousHeapCallbacks() == true,
"asynchronous callbacks have not been implemented for IterateThroughHeap");
...
assert(o!=NULL&&o!=JNIHandles::deleted_handle().as_oop(),"sanity check");
...
jvmtiError JvmtiTagMap::get_objects_with_tags(const jlong* tags, 
  ...
  {    
    ...
    MutexLocker ml(*lock(), JavaThread::current());
...
class RestoreMarksClosure : public ObjectClosure {
 public:  
  void do_object(oop o) {   
    // FIXME -- implementation based upon markWord needed for x86?
    Unimplemented();
    // if (o != NULL) {
    //   markOop mark = o->mark();
    //   if (mark->is_marked()) {
    //     // For AVM, mark bits are seperate from lock bits
    //     o->set_mark(mark->reset_marked());
    //   }
    // }    
  }
};
...
void ObjectMarker::init() {
  Thread* current_thread = Thread::current();
  assert(current_thread->is_VM_thread() || current_thread->is_GC_task_thread(), "must be VMThread or GC Task Thread");

  // prepare heap for iteration 
  Universe::heap()->ensure_parsability(false);  // no need to retire TLABs

  // create stacks for interesting headers
  _saved_mark_stack = new (ResourceObj::C_HEAP) GrowableArray<markWord>(4000, true);
...
inline void ObjectMarker::mark(oop o) {
  ...
  // FIXME -- now we need an x86 way to do this...
  Unimplemented();  
...
inline bool CallbackInvoker::invoke_basic_heap_root_callback(jvmtiHeapRootKind root_kind, oop obj) {
  ...
  CallbackWrapper* wrapper = new CallbackWrapper(tag_map(), obj);
  Atomic::inc_ptr((intptr_t*)VM_HeapWalkOperation::number_of_callbacks_addr());

  jvmtiIterationControl control;

  if (JVM_ShouldUseSynchronousHeapCallbacks() != true) {
    control = (*cb)(root_kind, 
                                        wrapper->klass_tag(), 
wrapper->obj_size(),
                                        (jlong*)wrapper,
                                        (void*)user_data());
...
inline bool CallbackInvoker::invoke_basic_object_reference_callback(jvmtiObjectReferenceKind ref_kind,
  ...
  // do the callback  
CallbackWrapper*wrapper=new CallbackWrapper(tag_map(),referree);
...
oop o=UNPOISON_OBJECTREF(*obj_p,obj_p).as_oop();
if(o==NULL||o==JNIHandles::deleted_handle().as_oop()){
      return;
    }     
...
static inline bool verify_static_oop(instanceKlass* ik, objectRef* obj_p) {
  objectRef* start = ik->start_of_static_fields();
  objectRef* end = start + ik->static_oop_field_size();
...
inline bool VM_HeapWalkOperation::iterate_over_class(klassOop k) {
  ...
  if (klass->oop_is_instance()) {         
	...
	objectRef* f = (objectRef*)addr;
	assert(verify_static_oop(ik, f), "sanity check");
oop fld_o=lvb_ref(f).as_oop();
	if (fld_o != NULL) {	 
...
#define REPORT_STACK_REF_ROOT(root)                                              \
if(root==NULL){\
    continue;                                                                    \
  }                                                                              \
                                                                                 \
...
if(!java_thread->root_Java_frame()){
    ...
    vframe vf = java_thread->last_java_vframe();

    bool is_top_frame = true; 
    int depth = 0;
frame last_entry_frame=frame(NULL,NULL);
...
jmethodID method=moop->jmethod_id();

        if (!(moop->is_native())) {
          jlocation bci = (jlocation)vf.bci();     	
int maxlocals=moop->max_locals();
oop o=NULL;

if(fr.is_compiled_frame()){
Unimplemented();//FIXME -- ScopeValue requires replacement implementation...
            // intptr_t buffer = 0;
            // ScopeValue* scope_val = NULL;
            // GrowableArray<ScopeValue*>* locals = vf.scope()->locals();
            //
            // for (int slot = 0; slot < maxlocals; slot++) {
            //   scope_val = locals->at(slot);
            //   if (scope_val->is_location() &&
            //       ((LocationValue*) scope_val)->location().type() == Location::oop) {
            //     vf.java_local(java_thread, slot, T_ILLEGAL, (intptr_t*) &buffer);
            //     o = objectRef((uint64_t) buffer).as_oop();
            //
            //     REPORT_STACK_REF_ROOT(o)
            //   }
            // }
          } else {
            intptr_t buffer = 0;
            intptr_t* start_address = NULL; // Locals start
            intptr_t* end_address = NULL; // Locals end
            const char* stack_type = "Locals (collect_stack_roots)";
            
            if (maxlocals > 0) {
              end_address = (intptr_t*)fr.interpreter_frame_local_at(maxlocals);
              start_address = (intptr_t*)fr.interpreter_frame_local_at(0);
              
              int slot = 0;
              while (start_address > end_address) {
                switch (fr.tag_at_address(start_address)) {
                case frame::single_slot_primitive_type:
                  start_address--;
                  break;
                  
                case frame::double_slot_primitive_type:
                  assert((start_address - 2) >= end_address, "Must have at least two stack slots");
                  start_address -= 2;
                  break;
                ...
                case frame::single_slot_ref_type:
                  ...
                  o = objectRef((uint64_t) buffer).as_oop();
assert(o==NULL||o->is_oop(),"ref type not oop?");
        ...
        // externalVFrame - or an entry frame then we report the JNI locals
        // as roots
        frame fr = vf.get_frame();
...
void VM_HeapWalkOperation::doit() {
...

  {
    ...
    if (initial_object().is_null()) {
      if (!collect_simple_roots() || should_discontinue_heap_iteration()) return;  
      if (!collect_stack_roots() || should_discontinue_heap_iteration()) return;
    } else {
...
if(!visit(o)||should_discontinue_heap_iteration()){
      break;
    }
        }
      }
  ...
  heap_iteration_epilog(_number_of_callbacks);
}

void call_IterateOverReachableObjectOperation(void* user_data) {
assert(UseGenPauselessGC,"can be called only by GPGC");

Thread*thread=Thread::current();

  ((VM_HeapWalkOperation*)user_data)->doit();
}
...
void JvmtiTagMap::iterate_over_reachable_objects(jvmtiHeapRootCallback heap_root_callback, 
  ...
  if (UseGenPauselessGC) {
    ...
    // This will mark/remap the entire heap and not relocate.
    // This way we can iterate through the entire heap safely.
    ((GPGC_Heap*)Universe::heap())->collect_and_iterate(GCCause::_jvmti_iterate_over_reachable_objects, call_IterateOverReachableObjectOperation, &op);
  } else {
MutexLockerAllowGC ml(Heap_lock,JavaThread::current());
...
{
  guarantee (JVM_ShouldUseSynchronousHeapCallbacks() == true,
"asynchronous callbacks have not been implemented for FollowReferences");
  oop obj = JNIHandles::resolve(object);
  Handle initial_object(Thread::current(), obj);
...
// GPGC has completed referencing processing so we may have JNI weak refs
// to objects that have been GC'ed. 
void JvmtiTagMap::gpgc_ref_processing_epilogue(){
  assert(SafepointSynchronize::is_at_safepoint(), "must be executed at a safepoint");
assert(UseGenPauselessGC,"should only be used with GPGC");
  ...
  if (JvmtiEnv::environments_might_exist()) {
    ...
    for (JvmtiEnvBase* env = it.first(); env != NULL; env = it.next(env)) {
      JvmtiTagMap* tag_map = ((JvmtiEnvBase *)env)->tag_map();
      if (tag_map != NULL && !tag_map->is_empty()) {
TraceTime t("JVMTI Rehash ([G]PGC) ",TraceJVMTIObjectTagging);
        // We have only one hashmap for [G]PGC.
        tag_map->rehash(0, 0);
      } 
    }
  }
...
void JvmtiTagMap::rehash(int start, int end) {
  ...
  // other hashmap.
  if (UseGenPauselessGC) {
assert(promoted_entries==NULL,"GPGC should have only the young_hashmap");
  }
