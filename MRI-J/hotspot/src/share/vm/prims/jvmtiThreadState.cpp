...
JvmtiEnv::is_thread_fully_suspended(get_thread(),false),
  ...
  if (get_thread()->root_Java_frame()) return 0;  // no Java frames

  ResourceMark rm;
vframe jvf=get_thread()->last_java_vframe();
  int n = 0;
...
void JvmtiThreadState::oops_do(OopClosure* f) {
  f->do_oop((objectRef*) &_earlyret_oop);
}
