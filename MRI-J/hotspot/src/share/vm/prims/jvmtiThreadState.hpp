...
#ifndef JVMTITHREADSTATE_HPP
#define JVMTITHREADSTATE_HPP
...
BasicType _earlyret_type;
  ...
  objectRef     _earlyret_oop;         // Used to return an oop result into Java code from
 ...
 public:
  ...
  BasicType earlyret_type()                          { return _earlyret_type; }
  oop  earlyret_oop() const                          { return lvb_ref(&_earlyret_oop).as_oop(); }
  void set_earlyret_oop (objectRef x)                { ref_store_without_check(&_earlyret_oop, x);    }
  jvalue earlyret_value()                            { return _earlyret_value; }
void set_earlyret_value(jvalue val,BasicType type){_earlyret_type=type;_earlyret_value=val;}
void clr_earlyret_value(){_earlyret_type=T_ILLEGAL;_earlyret_value.j=0L;}

  static ByteSize earlyret_state_offset() { return byte_offset_of(JvmtiThreadState, _earlyret_state); }
static ByteSize earlyret_type_offset(){return byte_offset_of(JvmtiThreadState,_earlyret_type);}
