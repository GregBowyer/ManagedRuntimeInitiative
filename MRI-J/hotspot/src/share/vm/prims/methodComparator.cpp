...
constantPoolRef MethodComparator::_old_cp;
constantPoolRef MethodComparator::_new_cp;
...
void methodComparator_static_init() {
  MethodComparator::_old_cp = nullRef;
  MethodComparator::_new_cp = nullRef;
}
...
bool MethodComparator::methods_EMCP(methodOop old_method, methodOop new_method) {
  ...
  POISON_AND_STORE_REF(&_old_cp, constantPoolRef(old_method->constants()));
  POISON_AND_STORE_REF(&_new_cp, constantPoolRef(new_method->constants()));
...
if(old_cp()->float_at(cpi_old)!=new_cp()->float_at(cpi_new))
          return false;
      }
...
if(old_cp()->double_at(cpi_old)!=new_cp()->double_at(cpi_new))
	return false;
    }
    break;
  }
