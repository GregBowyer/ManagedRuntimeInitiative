...
#ifdef AZ_PROXIED
static address lookup_remote_azproxiedvirtualmachine_functions(char* jni_name, TRAPS) {
  address entry = NULL;
  if (JDK_Version::is_gte_jdk16x_version()) {
    const char* azproxiedvm_string = "Java_sun_tools_attach_AzproxiedVirtualMachine";
    if (strncmp(jni_name, azproxiedvm_string, strlen(azproxiedvm_string)) == 0) {
      assert0(THREAD->is_Java_thread());
      {
        ThreadToNativeFromVM ttnv((JavaThread*)THREAD); // Lookup can be slow, so allow GC
...
address NativeLookup::lookup_style(methodHandle method, char* pure_name, const char* long_name, int args_size, bool os_style, bool& in_base_library, TRAPS) {
...
#ifdef AZ_PROXIED
  remoteJNI::init_native_lookup(jni_name, args_size);
#endif // AZ_PROXIED
  ...
  if (loader.is_null()) {
    ...
    if (entry == NULL) {
      ...
      if (entry != NULL) {
        entry = remoteJNI::set_native_entrypoint(method, entry);
      }
    } else {
      remoteJNI::init_native_lookup(NULL, 0);
    }
#endif // AZ_PROXIED
    if (entry != NULL) {
...
#ifdef AZ_PROXIED
  // Java_sun_tools_attach_AzproxiedVirtualMachine_* are implemented in the proxy.
  // Since libattach.so in the proxy is a dummy library, we need to use our
  // remote lookup mechanism to find the symbol in libjvm.so
  entry = lookup_remote_azproxiedvirtualmachine_functions(jni_name, THREAD);
  if (entry != NULL) {
    entry = remoteJNI::set_native_entrypoint(method, entry);
    return entry;
  }
...
Handle name_arg=java_lang_String::create_from_str(jni_name,true/*SBA*/,CHECK_NULL);
...
address adr=lookup_entry(wrapper_method,in_base_library,THREAD);
#ifdef AZ_PROXIED
        if (adr != NULL && wrapper_method->is_remote()) {
...
address NativeLookup::lookup(methodHandle method, bool& in_base_library, TRAPS) {
  if (!method->has_native_function()) {
    ...
    if (PrintJNIResolving) {
      ResourceMark rm(THREAD);
tty->print_cr("[Dynamic-linking %snative method %s.%s ... JNI]",
