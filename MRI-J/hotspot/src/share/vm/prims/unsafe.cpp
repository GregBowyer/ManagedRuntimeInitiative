...
#define UNSAFE_ENTRY(result_type, func, header) \
  JVM_ENTRY(result_type, func, header)
...
// #define UNSAFE_LEAF(result_type, header) JVM_LEAF(result_type, header)
...
UNSAFE_ENTRY(jobject,Unsafe_GetObject,(JNIEnv*env,jobject unsafe,jobject obj,jlong offset))
  ...
  oop p = JNIHandles::resolve(obj);
  objectRef v = lvb_ref((objectRef*)index_oop_from_field_offset_long(p, offset));
  return JNIHandles::make_local(env, v);
UNSAFE_END

UNSAFE_ENTRY(void, Unsafe_SetObject, (JNIEnv *env, jobject unsafe, jobject obj, jlong offset, jobject x_h))
  UnsafeWrapper("Unsafe_SetObject");
objectRef x=JNIHandles::resolve_as_ref(x_h);
objectRef p=JNIHandles::resolve_as_ref(obj);
  ref_store(p, (objectRef*) index_oop_from_field_offset_long(p.as_oop(), offset), x);
UNSAFE_END

UNSAFE_ENTRY(jobject, Unsafe_GetObjectVolatile, (JNIEnv *env, jobject unsafe, jobject obj, jlong offset))
  UnsafeWrapper("Unsafe_GetObjectVolatile");
  oop p = JNIHandles::resolve(obj);
  objectRef v = lvb_ref((volatile objectRef*)index_oop_from_field_offset_long(p, offset));
  return JNIHandles::make_local(env, v);
UNSAFE_END
...
assert0(VM_Version::supports_cx8());
...
UNSAFE_ENTRY(void, Unsafe_SetOrderedObject, (JNIEnv *env, jobject unsafe, jobject obj, jlong offset, jobject x_h))
  ...
  oop p = JNIHandles::resolve(obj);
ref_store(p,(objectRef*)index_oop_from_field_offset_long(p,offset),x);
  OrderAccess::fence();
UNSAFE_END
...
UNSAFE_ENTRY(void, Unsafe_SetMemory, (JNIEnv *env, jobject unsafe, jlong addr, jlong size, jbyte value))
  ...
  // Why is someone rewriting memset ? This way we get faster zeroing as well. aB.
#if 1
  memset(p, (int)value, sz);
#else
...
UNSAFE_ENTRY(jobject, Unsafe_StaticFieldBaseFromField, (JNIEnv *env, jobject unsafe, jobject field))
  ...
  return JNIHandles::make_local(env, java_lang_Class::as_klassRef(mirror));
UNSAFE_END
...
return JNIHandles::make_local(env,java_lang_Class::as_klassRef(JNIHandles::resolve_as_non_null_ref(clazz)));
UNSAFE_END
...
JNIHandles::resolve_non_null(jobj)->lock(INF_JNI_ENTER,CHECK);
  }
UNSAFE_END
...
UNSAFE_ENTRY(jboolean, Unsafe_TryMonitorEnter, (JNIEnv *env, jobject unsafe, jobject jobj))
  UnsafeWrapper("Unsafe_TryMonitorEnter");
  {
    ...
    //JNIHandles::resolve_non_null(jobj)->try_lock(INF_JNI_ENTER, CHECK_0);
...
oop o=JNIHandles::resolve_non_null(jobj);
if(!o->is_self_locked()){
...
objectRef e=JNIHandles::resolve_as_ref(e_h);
objectRef pref=JNIHandles::resolve_as_ref(obj);
  oop p = pref.as_oop();
  // Eagerly test - if we are likely to fail do so now instead of doing an SBA escape
  if( lvb_ref(p->ref_field_addr(offset)) != e ) return false;

objectRef x=JNIHandles::resolve_as_ref(x_h);
  if( pref.is_stack() ) {       // Stack objects are single-threaded so the CAS is not needed
    ref_store(p, p->ref_field_addr(offset), x);
    return true;                // Success
  }
  // CAS'ing into heap, so the exchange value better be in heap
  if( x.is_stack() ) {          // Exchange not in heap?
    StackBasedAllocation::ensure_in_heap((JavaThread*) THREAD, x_h, "unsafe CAS into heap");
    // Escape allows GC, reload
    e = JNIHandles::resolve_as_ref(e_h); 
    p = JNIHandles::resolve_as_ref(obj).as_oop();
    x = JNIHandles::resolve_as_ref(x_h); 
  }
  // Now CAS into heap
  intptr_t* addr = (intptr_t *) index_oop_from_field_offset_long(p, offset);
  intptr_t e_value = ALWAYS_POISON_OBJECTREF(e).raw_value();
  intptr_t res = Atomic::cmpxchg_ptr((intptr_t) ALWAYS_POISON_OBJECTREF(x).raw_value(), addr, e_value);
  
  objectRef resultRef((uint64_t) res);
  jboolean success  = (resultRef.raw_value() == e_value);
  if (success)
    update_barrier_set((objectRef*)addr, x);
  return success;
UNSAFE_END
...
Handle p(THREAD,JNIHandles::resolve_as_ref(obj));
