...
typedef enum unrecognized_flags_s {
    UNRECOGNIZED_FLAGS_IGNORE = 1,
    UNRECOGNIZED_FLAGS_WARN = 2,
    UNRECOGNIZED_FLAGS_ERROR = 4,
    UNSUPPORTED_NATIVE_VM_FLAGS_IGNORE = 8,
    UNSUPPORTED_NATIVE_VM_FLAGS_WARN = 16,
    UNSUPPORTED_NATIVE_VM_FLAGS_ERROR = 32,
} unrecognized_flags_t;
...
class Arguments : AllStatic {
 ...
 private:  
  ...
  static bool _C1BackgroundCompilation;
  static bool _C2BackgroundCompilation;
  ...
  // AZUL - Set gc flags consistently across all collectors.
  static void set_gc_flags();
  // GC ergonomics
...
static bool process_argument(const char*arg,int32_t ignore_unrecognized,FlagValueOrigin origin);
  static void process_java_launcher_argument(const char*, void*);
  static void process_java_compiler_argument(char* arg);
  static jint parse_options_environment_variable(const char* name, JavaVMInitArgs** vm_args,
                    char* buffer, size_t bufsize, const int N_MAX_OPTIONS);
  static jint parse_java_tool_options_environment_variable(JavaVMInitArgs** vm_args,
                    char* buffer, size_t bufsize, const int N_MAX_TOOL_OPTIONS);
  static jint parse_java_options_environment_variable(JavaVMInitArgs** vm_args,
                                                      char* buffer, size_t bufsize, const int N_MAX_OPTIONS);
...
static bool is_bad_option(const JavaVMOption*option,int32_t ignore_unrecognized,
                            const char* option_type);
static bool is_bad_option(const JavaVMOption*option,int32_t ignore_unrecognized){
return is_bad_option(option,ignore_unrecognized,NULL);
  }
...
#ifndef AZ_PROXIED
  static void add_to_valid_px_options(const JavaVMOption* option);
static bool is_valid_px_option(const JavaVMOption*option);
#endif // !AZ_PROXIED

  static int32_t _ignore_flags;
  static jint scan_for_Xflags_and_XXFlags(
    const JavaVMInitArgs* args, const char** tail_Flags,
  ...
  static void parseOnlyLine(const char* line, short* num, short* max, char*** strings);

  static short  ITRTraceOnlyThreadsNum;
  static short  ITRTraceOnlyThreadsMax;
  static char** ITRTraceOnlyThreads;
 ...
 public:
  ...
  // Used by os_solaris
static bool process_settings_file(const char*file_name,bool should_exist,int32_t ignore_unrecognized);
#ifndef AZ_PROXIED
  static void parse_px_options(const JavaVMInitArgs* args);
#endif // !AZ_PROXIED
  ...
  static bool TraceThread(char* name);
  static bool ITRTraceThreadsEnabled()      { return ITRTraceOnlyThreadsNum != 0; }
  ...
  static char *get_library_path() { return _java_library_path->value(); }
