...
void Flag::print_xml_on(xmlBuffer* xb) {
  xmlElement xf(xb, "vm_flag");
  { xmlElement xe(xb, "type");
    xb->print(type);
  }
  { xmlElement xe(xb, "name");
    xb->print(name);
  }
  { xmlElement xe(xb, "default");
    xb->print(origin == DEFAULT ? "true" : "false");
  }
  { xmlElement xe(xb, "kind");
    xb->print(kind);
  }
  { xmlElement xe(xb, "value");
    if (is_bool()){
      xb->print(get_bool() ? "true" : "false");
    } else if (is_intx()) {
      xb->print("%ld", get_intx());
    } else if (is_uintx()) {
      xb->print("%lu", get_uintx());
    } else if (is_ccstr()) {
      ccstr x = get_ccstr();
      if (!x) {
xb->print(" ");
...
xb->print("unrecognized type");
    }
  }
  { xmlElement xe(xb, "documentation");
    xb->print("%s", doc);
  }
}
...
#define RUNTIME_PRODUCT_FLAG_STRUCT(type, name, value, doc) { #type, XSTR(name), &name, "{product}", doc, DEFAULT },
#define RUNTIME_PD_PRODUCT_FLAG_STRUCT(type, name, doc)     { #type, XSTR(name), &name, "{pd product}", doc, DEFAULT },
#define RUNTIME_DIAGNOSTIC_FLAG_STRUCT(type, name, value, doc) { #type, XSTR(name), &name, "{diagnostic}", doc, DEFAULT },
#define RUNTIME_MANAGEABLE_FLAG_STRUCT(type, name, value, doc) { #type, XSTR(name), &name, "{manageable}", doc, DEFAULT },
#define RUNTIME_PRODUCT_RW_FLAG_STRUCT(type, name, value, doc) { #type, XSTR(name), &name, "{product rw}", doc, DEFAULT },
...
#define RUNTIME_PD_DEVELOP_FLAG_STRUCT(type,name,doc){#type,XSTR(name),&name,"{pd}",doc,DEFAULT},
#define RUNTIME_NOTPRODUCT_FLAG_STRUCT(type,name,value,doc){#type,XSTR(name),&name,"{notproduct}",doc,DEFAULT},
#endif

#define C1_PRODUCT_FLAG_STRUCT(type, name, value, doc) { #type, XSTR(name), &name, "{C1 product}", doc, DEFAULT },
#define C1_PD_PRODUCT_FLAG_STRUCT(type, name, doc)     { #type, XSTR(name), &name, "{C1 pd product}", doc, DEFAULT },
#ifdef PRODUCT
...
#else
#define C1_DEVELOP_FLAG_STRUCT(type,name,value,doc){#type,XSTR(name),&name,"{C1}",doc,DEFAULT},
#define C1_PD_DEVELOP_FLAG_STRUCT(type,name,doc){#type,XSTR(name),&name,"{C1 pd}",doc,DEFAULT},
#define C1_NOTPRODUCT_FLAG_STRUCT(type,name,value,doc){#type,XSTR(name),&name,"{C1 notproduct}",doc,DEFAULT},
#endif


#define C2_PRODUCT_FLAG_STRUCT(type, name, value, doc) { #type, XSTR(name), &name, "{C2 product}", doc, DEFAULT },
#define C2_PD_PRODUCT_FLAG_STRUCT(type, name, doc)     { #type, XSTR(name), &name, "{C2 pd product}", doc, DEFAULT },
#define C2_DIAGNOSTIC_FLAG_STRUCT(type, name, value, doc) { #type, XSTR(name), &name, "{C2 diagnostic}", doc, DEFAULT },
#ifdef PRODUCT
...
#else
#define C2_DEVELOP_FLAG_STRUCT(type,name,value,doc){#type,XSTR(name),&name,"{C2}",doc,DEFAULT},
#define C2_PD_DEVELOP_FLAG_STRUCT(type,name,doc){#type,XSTR(name),&name,"{C2 pd}",doc,DEFAULT},
#define C2_NOTPRODUCT_FLAG_STRUCT(type,name,value,doc){#type,XSTR(name),&name,"{C2 notproduct}",doc,DEFAULT},
#endif
...
static Flag** sort_flags(int* array_length) {
  // Compute size
  int length = 0;
...
void CommandLineFlags::print_on(outputStream*st){
  int length;
  Flag** array = sort_flags(&length);
  // Print
st->print_cr("[Global flags]");
  for (int i = 0; i < length; i++) {
...
void CommandLineFlags::print_xml_on(xmlBuffer* xb) {
  int length;
  Flag** array = sort_flags(&length);
  xmlElement xe(xb, "vm_flags");
  for (int i = 0; i < length; i++) {
    if (array[i]->is_unlocked()) {
array[i]->print_xml_on(xb);
    }
  }
  FREE_C_HEAP_ARRAY(Flag*, array);
}
