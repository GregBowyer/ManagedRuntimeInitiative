...
class Handle VALUE_OBJ_CLASS_SPEC {
 protected:
  objectRef* _handle;
  oop     obj         () const { return _handle == NULL ? (oop)NULL : lvb_ref(_handle).as_oop(); }
  oop     non_null_obj() const { assert(_handle != NULL, "resolving NULL handle"); return lvb_ref(_handle).as_oop(); }

 public:
  // Constructors
  Handle()                                       { _handle = NULL; }
inline Handle(oop obj);
inline Handle(objectRef r);
#ifndef ASSERT 
inline Handle(Thread*thread,oop obj,bool dummy=false);
  inline Handle(Thread* thread, objectRef r);
#else // ASSERT
  // Don't inline body with assert for current thread
Handle(Thread*thread,oop obj,bool dummy=false);
Handle(Thread*thread,objectRef r);
#endif // ASSERT 
  ...
  objectRef as_ref() const                       { return (_handle == NULL ? nullRef : lvb_ref(_handle)); }
  ...
  // Debugging
void print(){obj()->print(tty);}

  // Direct interface, use very sparingly.
  // Used by JavaCalls to quickly convert handles and to create handles static data structures.
  // Constructor takes a dummy argument to prevent unintentional type conversion in C++.
  Handle(objectRef *ref, bool dummy)             { if(ref) VERIFY_POISONING(*ref, ref); _handle = ref; }
  void set( objectRef ref ) { if( ref.is_null() ) _handle=NULL; else *_handle = ref; }

  // Raw handle access. Allows easy duplication of Handles. This can be very unsafe 
  // since duplicates is only valid as long as original handle is alive.
  objectRef* raw_value()                         { return _handle; }
static objectRef raw_resolve(objectRef*handle){return handle==NULL?nullRef:lvb_ref(handle);}
};
...
KlassHandle(objectRef ref):Handle(ref){
    assert0(is_null() || ref.as_oop()->is_klass());
  }
...
assert(is_null()||obj->is_klass(),"not a klassOop");
  ...
  klassRef    as_klassRef() const                { return klassRef((_handle == NULL ? nullRef : lvb_ref(_handle))._objref); }
};
...
assert(is_null()||((oop)obj)->is_a(),\
    ...
    type##Handle (type##Ref ref) : Handle(ref) { }                            \
    type##Handle (Thread* thread, type##Oop obj) : Handle(thread, (oop)obj) { \
assert(is_null()||((oop)obj)->is_a(),"illegal type");\
    }                                                                         \
    \
    /* Special constructor, use sparingly */ \
    type##Handle (type##Ref *handle, bool dummy) : Handle((objectRef*)handle, dummy) {} \
                                                 \
    /* Operators for ease of use */              \
    type##Oop    operator () () const            { return obj(); } \
    type##Oop    operator -> () const            { return non_null_obj(); } \
    type##Ref    as_ref() const                  { return (_handle == NULL ? type##Ref((uint64_t)0) : lvb_##type##Ref((const type##Ref*)_handle)); } \
  };


DEF_HANDLE(instance         , is_instance         )
DEF_HANDLE(method           , is_method           )
DEF_HANDLE(methodCode       , is_methodCode       )
...
assert(is_null()||obj->klass_part()->is_a(),\
             "illegal type");                                                 \
    }                                                                         \
type##Handle(klassRef ref):KlassHandle(ref){\
      assert(is_null() || ref.as_klassOop()->klass_part()->is_a(),            \
             "illegal type");                                                 \
    }                                                                         \
...
DEF_KLASS_HANDLE(methodCodeKlass       , oop_is_methodCode    )
...
class HandleArea: public Arena {
 ...
 public:
  ...
  // Handle allocation
 private:
  objectRef* real_allocate_handle(oop obj) {
#ifdef ASSERT
    objectRef* handle = (objectRef*) (UseMallocOnly ? internal_malloc_4(oopSize) : Amalloc_4(oopSize));
#else
    objectRef* handle = (objectRef*) Amalloc_4(oopSize);
#endif
*handle=ALWAYS_POISON_OBJECTREF(objectRef(obj));
    return handle;
  }

objectRef*real_allocate_handle(objectRef r){
#ifdef ASSERT
    objectRef* handle = (objectRef*) (UseMallocOnly ? internal_malloc_4(oopSize) : Amalloc_4(oopSize));
#else
    objectRef* handle = (objectRef*) Amalloc_4(oopSize);
#endif // ASSERT
    *handle = ALWAYS_POISON_OBJECTREF(r);
    return handle;
  }

 public:
#ifdef ASSERT
  objectRef* allocate_handle(oop obj);
#else
  objectRef* allocate_handle(oop obj) { return real_allocate_handle(obj); }
#endif

#ifdef ASSERT
objectRef*allocate_handle(objectRef r);
#else
objectRef*allocate_handle(objectRef r){return real_allocate_handle(r);}
#endif
