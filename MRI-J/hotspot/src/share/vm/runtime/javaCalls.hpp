...
#ifndef JAVACALLS_HPP
#define JAVACALLS_HPP

#include "allocation.hpp"
#include "javaFrameAnchor.hpp"
#include "jniTypes_pd.hpp"
...
class JavaCallWrapper: StackObj {
 private:
  ...
  methodRef        _callee_method;          // to be able to collect arguments if entry frame is top frame
  objectRef        _receiver;               // the receiver of the call (if a non-static call)

  JavaFrameAnchor  _anchor;                 // last thread anchor state that we must restore
  intptr_t *       _jexstk_top;             // Top of Java Execution Stack at time of call
 ...
 public:
  ...
  JavaFrameAnchor* anchor_known_walkable(void);
  
  JavaValue*       result() const           { return _result; }
  // GC support
methodOop callee_method(){return _callee_method.as_methodOop();}
  methodRef*       callee_method_addr()     { return &_callee_method; }
  oop              receiver()               { return _receiver.as_oop(); }
...
class JavaCallArguments : public StackObj {
 ...
 public:
  ...
  Handle receiver() {    
    ...
    return Handle((objectRef*)_value[0], false);
  }
