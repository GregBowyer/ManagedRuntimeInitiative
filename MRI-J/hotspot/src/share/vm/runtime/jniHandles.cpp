...
objectRef JNIHandles::_deleted_handle;


bool JNIHandles::thread_is_locked(){
...
JRT_LEAF(objectRef, JNIHandles, resolve_non_null_weak, (jobject handle))
assert(thread_is_locked(),"Can't work on object in heap without holding the JVM lock");
assert(handle!=NULL,"No NULL handles into JNIHandless::resolve_non_null_weak");
  assert(intptr_t(handle) & JNIWeakHandleTag, "No non-weak handles into JNIHandles::resolve_non_null_weak");
...
assert(!CheckJNICalls||is_weak_global_handle(handle),"JNI weak handle not in weak global handle block");
  ...
  objectRef result = GPGC_Collector::JNI_weak_reference_resolve((objectRef*)handle);

  return result;
JRT_END


jobject JNIHandles::make_local(objectRef ref){
if(ref.is_null()){
    return NULL;                // ignore null handles
  } else {
    Thread* thread = Thread::current();
    assert(Universe::heap()->is_in_reserved(ref.as_oop()) || JavaThread::sba_is_in_current((address)ref.as_oop()), "sanity check");
return thread->active_handles()->allocate_handle(ref);
  }
}
...
jobject JNIHandles::make_local(Thread*thread,objectRef ref){
if(ref.is_null()){
    return NULL;                // ignore null handles
  } else {
    assert(Universe::heap()->is_in_reserved(ref.as_oop()) || JavaThread::sba_is_in_current((address)ref.as_oop()), "sanity check");
...
jobject JNIHandles::make_local(JNIEnv*env,objectRef ref){
if(ref.is_null()){
    return NULL;                // ignore null handles
  } else {
...
jobject JNIHandles::make_global(Handle obj) {  
  jobject res = NULL;
if(!obj.is_null()){//ignore null handles
Thread*current_thread=Thread::current();
if(current_thread->is_Java_thread()){
      StackBasedAllocation::ensure_in_heap((JavaThread*) current_thread, obj, "global handle");
    }
...
Thread*current_thread=Thread::current();
if(current_thread->is_Java_thread()){
      StackBasedAllocation::ensure_in_heap((JavaThread*) current_thread, obj, "global weak handle");
    }
    MutexLocker ml(JNIGlobalHandle_lock);
    assert(Universe::heap()->is_in_reserved(obj()), "sanity check");
  ...
  if (UseGenPauselessGC && res!=NULL) {
    // JNI weak global handles are distinguished from other JNI handles by having a low bit set. 
    res = jobject(intptr_t(res) | JNIWeakHandleTag);
  }
  return res;
}
...
void JNIHandles::destroy_global(jobject handle) {
  if (handle != NULL) {
    ...
    *((objectRef*)handle) = deleted_handle(); // Mark the handle as deleted, allocate will reuse it
  }
}


void JNIHandles::destroy_weak_global(jobject handle) {
  if (UseGenPauselessGC && handle!=NULL) {
    // JNI weak global handles are distinguished from other JNI handles by having a low bit set.
    assert((intptr_t(handle)&JNIWeakHandleTag)==JNIWeakHandleTag, "Weak global handle doesn't have the tag bits set.");
...
void JNIHandles::weak_oops_do(BoolObjectRefClosure*is_alive,OopClosure*f){
  _weak_global_handles->weak_oops_do(is_alive, f);
}


void JNIHandles::GPGC_weak_oops_do(bool is_new_gc) {
  _weak_global_handles->GPGC_weak_oops_do(is_new_gc);
}
...
oop obj=instanceKlass::cast(k)->allocate_permanent_instance(CATCH);
  _deleted_handle = ALWAYS_POISON_OBJECTREF(objectRef(obj));
}
...
bool JNIHandles::is_frame_handle(JavaThread* thr, jobject obj) {
  ...
  // is not permitted.
  if( thr->root_Java_frame() ) return false;

  return thr->is_in_stack((intptr_t)obj);
}
...
public:
  void do_oop(objectRef* root) {
    UNPOISON_OBJECTREF(*root,root).as_oop()->verify();
  }
};
...
VirtualSpace    JNIHandleBlock::_vs;
char *          JNIHandleBlock::_curaddr              = NULL;
...
*(oop*)&_handles[index]=badJNIHandle;
  }
}
...
MutexLocker ml(JNIHandleBlockFreeList_lock);
    if (_block_free_list == NULL) {
      // Allocate new block
block=JNIHandleBlock::allocate_blockarea();
      _blocks_allocated++;
      if (TraceJNIHandleAllocation) {
tty->print_cr("JNIHandleBlock "PTR_FORMAT" allocated (%d total blocks)",
...
void JNIHandleBlock::oops_do(OopClosure* f) {
  ...
  bool is_local = (!JNIHandles::     is_global_handles(this) &&
                   !JNIHandles::is_weak_global_handles(this) );
  JavaThread *jtsba = is_local ? Thread::current()->sba_thread() : NULL;
  if( jtsba && !jtsba->sba_area() ) jtsba = NULL;
...
objectRef*root=(objectRef*)&(current->_handles)[index];
oop value=(oop)(UNPOISON_OBJECTREF_IF_NOT_PTR(*root).as_address());
//traverse heap pointers only, not deleted handles or free list pointers
if(value!=NULL&&
(Universe::heap()->is_in_reserved(value)||
             // Local JNIHandleBlocks allow stack objects
             (jtsba && jtsba->sba_is_in_or_oldgen((address)value) ) ) ) {
          f->do_oop(root);
        }
      }
...
oop value;
      if (UseGenPauselessGC) {
        root = (objectRef*)&(current->_handles)[index];
        value = (oop)(UNPOISON_OBJECTREF_IF_NOT_PTR(*root).as_address());

        // Need to remap heap pointers before doing is_alive check
        // remap heap pointers only, not deleted handles or free list pointers
        if (value != NULL && (Universe::heap()->is_in_reserved(value))) {
          objectRef obj = (objectRef) GPGC_Collector::remap_only((heapRef*)&(current->_handles)[index]);
          value         = (oop) obj.as_address();
        }
      } else {
        root  = (objectRef*)&(current->_handles)[index];
        value = (oop)(UNPOISON_OBJECTREF_IF_NOT_PTR(*root).as_address());
...
(Universe::heap()->is_in_reserved(value)||
//Local JNIHandleBlocks allow stack objects
(!JNIHandles::is_global_handles(this)&&
            !JNIHandles::is_weak_global_handles(this) &&
            JavaThread::sba_is_in_current((address)value) ) ) ) {
        if (is_alive->do_object_b(value)) {
          ...
          if (TraceReferenceGC) {
gclog_or_tty->print_cr("Clearing JNI weak reference ("PTR_FORMAT")",root);
          }
*root=nullRef;
        }
      }
...
void JNIHandleBlock::weak_oops_do(BoolObjectRefClosure*is_alive,
				  OopClosure* f) {
  bool is_local = (!JNIHandles::     is_global_handles(this) &&
                   !JNIHandles::is_weak_global_handles(this) );
  JavaThread *jtsba = is_local ? Thread::current()->sba_thread() : NULL;
...
objectRef*root;
objectRef value;
      // TODO: we need to clean up the code, the remapping should be done by the closure
      if (UseGenPauselessGC) {
        root = (objectRef*)&(current->_handles)[index];
        value = UNPOISON_OBJECTREF_IF_NOT_PTR(*root);

        // Need to remap heap pointers before doing is_alive check
        // remap heap pointers only, not deleted handles or free list pointers
        if (value.not_null() && (Universe::heap()->is_in_reserved(value.as_address()))) {
          value = (objectRef) GPGC_Collector::remap_only((heapRef*)&(current->_handles)[index]);
        }
      } else {
        root  = (objectRef*)&(current->_handles)[index];
        value = UNPOISON_OBJECTREF_IF_NOT_PTR(*root);
      ...
      // traverse heap pointers only, not deleted handles or free list pointers
      if (value.not_null() &&
          (Universe::heap()->is_in_reserved(value.as_address()) ||
           // Local JNIHandleBlocks allow stack objects
           (jtsba && jtsba->sba_is_in_or_oldgen(value.as_address()) ) ) ) {
        if (is_alive->do_object_b(value)) {
          // The weakly referenced object is alive, update pointer
          f->do_oop(root);
        } else {
...
void JNIHandleBlock::GPGC_weak_oops_do(bool is_new_gc) {
JavaThread*jtsba=NULL;
  for (JNIHandleBlock* current = this; current != NULL; current = current->_next) {
    assert(current->pop_frame_link() == NULL, 
      "blocks holding weak global JNI handles should not have pop frame link set");
    for (int index = 0; index < current->_top; index++) {
      objectRef* root  = (objectRef*)&(current->_handles)[index];
      objectRef  value = UNPOISON_OBJECTREF_IF_NOT_PTR(*root);

      if (value.not_null() && (Universe::heap()->is_in_reserved(value.as_address()))) {
        if ( is_new_gc ) {
          GPGC_NewCollector::unlink_weak_root(root, value);
        } else {
          GPGC_OldCollector::unlink_weak_root(root, value);
        }
      }
    }
    // the next handle block is valid only if current block is full
...
jobject JNIHandleBlock::allocate_handle(objectRef ref){
  assert(Universe::heap()->is_in_reserved(ref.as_oop()) || JavaThread::sba_is_in_current((address)ref.as_oop()), "sanity check");
  if (_top == 0) {
  ...
  if (_last->_top < block_size_in_oops) {
    objectRef* handle = &(_last->_handles)[_last->_top++];
    *handle = ALWAYS_POISON_OBJECTREF(ref);
    return (jobject) handle;    
  }

  // Try free list
  if (_free_list != NULL) {    
objectRef*handle=(objectRef*)_free_list;
    _free_list = (intptr_t*) *_free_list;
    *handle = ALWAYS_POISON_OBJECTREF(ref);
    return (jobject) handle;    
  }
...
return allocate_handle(ref);
  ...
  if (_allocate_before_rebuild == 0) {
    ...
    // Append new block
    Thread* thread = Thread::current();
Handle ref_handle(thread,ref);
    // This can block, so we need to preserve ref accross call.
    _last->_next = JNIHandleBlock::allocate_block(thread);
    _last = _last->_next;
    _allocate_before_rebuild--;    
    ref = ref_handle();
  }  
return allocate_handle(ref);//retry
}
...
void JNIHandleBlock::rebuild_free_list() {
  ...
  for (JNIHandleBlock* current = this; current != NULL; current = current->_next) {
    for (int index = 0; index < current->_top; index++) {
      objectRef* handle = &(current->_handles)[index];
      if (*handle ==  JNIHandles::deleted_handle()) {
        // this handle was cleared out by a delete call, reuse it
*(intptr_t*)handle=(intptr_t)_free_list;
_free_list=(intptr_t*)handle;
  ...
  if (TraceJNIHandleAllocation) {
tty->print_cr("Rebuild free list JNIHandleBlock "PTR_FORMAT" blocks=%d used=%d free=%d add=%d",
...
void JNIHandleBlock::initialize(){
  ReservedSpace jrs(JNIHandleBlock::JNIHANDLE_AREA_SIZE, (char*) __JNI_HANDLE_BLOCK_START_ADDR__, JNIHandleBlock::JNIHANDLE_AREA_ALIGNMENT);
if(!jrs.is_reserved()){
vm_exit_during_initialization("JNIHandleBlock allocation failure");
  }
  if (!_vs.initialize(jrs, os::vm_page_size())) {
vm_exit_during_initialization("JNIHandleBlock allocation failure");
  }
  _curaddr = _vs.low();
}

JNIHandleBlock *JNIHandleBlock::allocate_blockarea() {
if(_curaddr==NULL){
JNIHandleBlock::initialize();
  }
  size_t aligned_size = align_size_up(sizeof(class JNIHandleBlock), sizeof(intptr_t));
  char *retaddr = _curaddr;
  _curaddr += aligned_size;
  while (_curaddr > _vs.high()) {
    if (!_vs.expand_by(os::vm_page_size(), false)) {
        vm_exit_out_of_memory(os::vm_page_size(), "JNIHandleBlock");
    }
  }
  return (JNIHandleBlock *)retaddr;
}

extern "C" {
  char *jni_get_handlebase() {
    return JNIHandleBlock::jnihandle_base();
  }
  char *jni_get_handletop() {
    return JNIHandleBlock::jnihandle_top();
  }
}
