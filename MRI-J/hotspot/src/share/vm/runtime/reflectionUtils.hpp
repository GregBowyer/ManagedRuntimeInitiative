...
#ifndef REFLECTIONUTILS_HPP
#define REFLECTIONUTILS_HPP
...
class FilteredField {
 ...
 public:
  FilteredField(klassRef klass, int field_offset) {
POISON_AND_STORE_REF(&_klass,klass);
    _field_offset = field_offset;
  }
klassOop klass(){return lvb_klassRef(&_klass).as_klassOop();}
  objectRef* klass_addr() { return (objectRef*) &_klass; }
...
class FilteredFieldsMap : AllStatic {
 ...
 public:
  ...
  static bool is_filtered_field(klassRef klass, int field_offset) {
  ...
  static int  filtered_fields_count(klassRef klass, bool local_only) {
    int nflds = 0;
    for (int i=0; i < _filtered_fields->length(); i++) {
      if (local_only && klass == _filtered_fields->at(i)->klass()) {
        nflds++;
}else if(klass.as_klassOop()->klass_part()->is_subtype_of(_filtered_fields->at(i)->klass())){
        nflds++;
      }
    }
    return nflds;
  ...
  static void klasses_oops_do(OopClosure* f) {
    for (int i = 0; i < _filtered_fields->length(); i++) {
f->do_oop(_filtered_fields->at(i)->klass_addr());
    }
  }
};
...
while(_index>=0&&FilteredFieldsMap::is_filtered_field(_klass(),offset())){
        _index -= instanceKlass::next_offset;
      }
    }
  }
