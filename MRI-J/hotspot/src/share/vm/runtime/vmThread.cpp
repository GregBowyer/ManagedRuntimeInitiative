...
void VMThread::create() {
  ...
  _vm_thread = new (ttype::vm_thread) VMThread();
...
void VMThread::run() {
  ...
  {
MutexLocker ml(Notify_lock,this);
Notify_lock.notify();
  }
...
os::set_native_priority(this,VMThreadPriority);
  ...
  if (VerifyBeforeExit) {
    ...
    os::check_heap();
Universe::verify(true,true);//Silent verification to not pollute normal output
  }

  // signal other threads that VM process is gone
  {
MutexLocker ml(VMTerminator_lock,this);
    _terminated = true;
VMTerminator_lock.notify();
  }

  // Clean up any pending unmarked heapRefs for the pauseless collector.
  flush_unmarked_refs();
  ...
  // Azul Note: Need to remove VMThread here since otherwise we die trying to dereference the
  // (now deallocated) in-stack thread object in the calling thread.
  VMThread::destroy();
  // Do NOT make any references to the VMThread object itself (this) after this point.
}
...
jt->jvm_unlock_self();
  { MutexLocker ml(VMTerminator_lock,jt);
    while(!VMThread::is_terminated()) {
      VMTerminator_lock.wait();
    }
  }  
}
...
void VMThread::print_xml_on(xmlBuffer *xb, bool ref) {
  xmlElement te(xb, ref ? "thread_ref" : "thread");
  state_print_xml_on(xb, "VM Thread");
}
...
tty->print_cr("%s stall: %ld",_cur_vm_operation->name(),stall);
      ...
      while (!should_terminate() && _cur_vm_operation == NULL) {
        ...
        bool timedout = VMOperationQueue_lock.wait_micros(GuaranteedSafepointInterval*1000, false); // No safepoint check for VM thread
...
MutexUnlocker mul(VMOperationQueue_lock);
          TraceTime t("SafepointAlot pause", PrintPauses, true, gclog_or_tty);
    ...
    { HandleMark hm(VMThread::vm_thread());      
      ...
      LoggerMark lm(NOTAG, Log::M_VM, "Executing VM operation: %s", vm_operation()->name());
      ...
      if (_cur_vm_operation->evaluate_at_safepoint()) {
        ...
        { 
          TraceTime t("VM Operation pause", PrintPauses, true, gclog_or_tty);

          _vm_queue->set_drain_list(safepoint_ops); // ensure ops can be scanned

          SafepointSynchronize::begin();

          if (MProtectHeapAtSafepoint) {
Universe::heap()->mprotect();

if(Verbose)tty->print("mprotecting heap for %ldms ... ",MProtectHeapAtSafepointDelayMS);

os::sleep(Thread::current(),MProtectHeapAtSafepointDelayMS,false);
Universe::heap()->munprotect();

if(Verbose)tty->print_cr("done");
          }

          if (PrintPauses) {
gclog_or_tty->print(": %s",_cur_vm_operation->name());
          }
...
{MutexLocker mu(VMOperationRequest_lock,this);
...
void VMThread::async_execute(VM_Operation*op){
  // This is like VMThread::execute(), except that it doesn't wait for the
  // operation to be completed.
  Thread* t = Thread::current();
  // Mark operation as synchronous.
op->set_calling_thread(NULL);

  // Add VM operation to list of waiting threads.  We are guaranteed not to
  // block while holding the VMOperationQueue_lock, so we can block without
  // a safepoint check.  This allows vm operation requests to be queued up
  // during a safepoint synchronization.
...
op->set_calling_thread(t);
    ...
    // Get ticket number for VM operations
    long ticket = 0;
...
VMOperationQueue_lock.notify();
    ...
    {      
      MutexLockerAllowGC mu(&VMOperationRequest_lock,1/*locked by both JavaThreads and non-Javathreads*/);
...
op->set_calling_thread(prev_vm_operation->calling_thread());
    ...
    LoggerMark lm(NOTAG, Log::M_VM, "Executing %s VM operation: %s", prev_vm_operation ? "nested" : "", op->name());
