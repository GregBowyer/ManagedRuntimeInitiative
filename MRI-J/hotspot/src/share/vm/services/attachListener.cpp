...
java_lang_Throwable::print(PENDING_EXCEPTION_REF,out);
...
instanceHandle thread_oop=klass->allocate_instance_handle(false/*SBA*/,CHECK);

  const char thread_name[] = "Attach Listener";
Handle string=java_lang_String::create_from_str(thread_name,false/*SBA*/,CHECK);
  ...
  { MutexLockerAllowGC mu(Threads_lock, JavaThread::current());
    JavaThread* listener_thread = new (ttype::java_thread) JavaThread(&attach_listener_thread_entry);
...
Threads::add(listener_thread,false);

    listener_thread->set_priority(NearMaxPriority);
