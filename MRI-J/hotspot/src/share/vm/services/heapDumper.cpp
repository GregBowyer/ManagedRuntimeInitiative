...
n=::write(file_descriptor(),s,len);
      ...
      if (n != len) {
        if (n < 0) {
          ...
          assert(n < len, "wrote more than length?");
          s = ((char*)s) + n;
          len -= n;
        }
      }
...
objectRef r=PERMISSIVE_UNPOISON(*f,f);
      oop o = r.as_oop();
...
void SymbolTableDumper::do_oop(objectRef* obj_p) {
  ResourceMark rm;
  objectRef r = UNPOISON_OBJECTREF(*obj_p, obj_p);
  symbolOop sym = (symbolOop)r.as_oop();
...
void JNILocalsDumper::do_oop(objectRef* obj_p) {  
  // ignore null or deleted handles
objectRef r=UNPOISON_OBJECTREF(*obj_p,obj_p);
  oop o = r.as_oop();
  if (r.not_null() && r != JNIHandles::deleted_handle()) {
...
void JNIGlobalsDumper::do_oop(objectRef* obj_p) {
objectRef r=UNPOISON_OBJECTREF(*obj_p,obj_p);
  oop o = r.as_oop();

  // ignore these
if(r.is_null()||r==JNIHandles::deleted_handle())return;
...
class JavaFrameLocalsDumper:public OopClosure{
 private:
  DumpWriter* _writer;
  u4 _thread_serial_num;
  DumpWriter* writer() const		    { return _writer; }
 ...
 public:
JavaFrameLocalsDumper(DumpWriter*writer,u4 thread_serial_num){
    _writer = writer; 
    _thread_serial_num = thread_serial_num;
  }
  void do_oop(objectRef* obj_p);
...
void JavaFrameLocalsDumper::do_oop(objectRef*obj_p){
 objectRef r = PERMISSIVE_UNPOISON(*obj_p, obj_p);
 oop o = r.as_oop();
...
class JavaThreadLocalsDumper:public OopClosure{
 private:
  DumpWriter* _writer;
  u4 _thread_serial_num;
  DumpWriter* writer() const		    { return _writer; }
 ...
 public:
JavaThreadLocalsDumper(DumpWriter*writer,u4 thread_serial_num){
    _writer = writer; 
    _thread_serial_num = thread_serial_num;
  }
  void do_oop(objectRef* obj_p);
...
void JavaThreadLocalsDumper::do_oop(objectRef*obj_p){
 objectRef r = UNPOISON_OBJECTREF(*obj_p, obj_p);
 oop o = r.as_oop();

if(r.not_null()){
writer()->write_u1(HPROF_GC_ROOT_THREAD_BLOCK);
   writer()->write_objectID(o);
   writer()->write_u4(_thread_serial_num);
 }
}
...
void StickyClassDumper::do_oop(objectRef* obj_p) {  
objectRef r=UNPOISON_OBJECTREF(*obj_p,obj_p);
if(r.not_null()){
    objectRef r = UNPOISON_OBJECTREF(*obj_p, obj_p);
    oop o = r.as_oop();
...
if(o==ALWAYS_UNPOISON_OBJECTREF(JNIHandles::deleted_handle()).as_oop())
return;
...
void VM_HeapDumper::check_segment_length() {
  if (writer()->is_open()) {
    if (is_segmented_dump()) {   
      // don't use current_offset that would be too expensive on
      // a per record basis.
...
JavaFrameLocalsDumper fLocDmp(writer(),thread_serial_num);
JavaThreadLocalsDumper tLocDmp(writer(),thread_serial_num);
  ...
  // Handle Area
java_thread->handle_area()->oops_do(&tLocDmp);

  // TODO: I may be missing following:
  ...
  for( objectRef *optr = java_thread->_lckstk_base;
       optr < java_thread->_lckstk_top;
       optr++ )
  {
    fLocDmp.do_oop(optr);
  }
  ...
  for( intptr_t *optr = java_thread->jexstk_base_for_sp((intptr_t*)java_thread);
       optr < java_thread->_jexstk_top;
       optr++ )
  {
  ...
  if (java_thread->root_Java_frame()) return;

  // Traverse the privileged stack
  if( java_thread->privileged_stack_top() )
java_thread->privileged_stack_top()->oops_do(&fLocDmp);

  // Traverse the execution stack
  bool do_arg_oops = true;
  for( StackFrameStream fst(java_thread); !fst.is_done(); fst.next() ) {
    if( do_arg_oops )           // Handle youngest caller args?
      fst.current()->oops_arguments_do(&fLocDmp);

    fst.current()->oops_do(java_thread, &fLocDmp);
