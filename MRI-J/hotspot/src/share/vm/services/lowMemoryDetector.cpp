...
volatile intptr_t LowMemoryDetector::_disabled_count=0;
...
_detector_thread=new(ttype::java_thread)LowMemoryDetectorThread(&low_memory_detector_thread_entry);
    ...
    Threads::add(_detector_thread, false);

    _detector_thread->set_os_priority(LowMemoryDetectorPriority);
...
assert_lock_strong(LowMemory_lock);
...
void LowMemoryDetector::low_memory_detector_thread_entry(JavaThread* jt, TRAPS) {
  if( UseSBA && !jt->sba_area() ) // Allocate an initial SBA area
    jt->set_sba(new SBAArea(jt));
  while (true) {
    bool   sensors_changed = false;

    {
ThreadBlockInVM tbivm(jt,"low memory detection");

MutexLocker ml(LowMemory_lock,jt);
...
void LowMemoryDetector::detect_low_memory() {
  MutexLocker ml(LowMemory_lock, Thread::current()); // if called from JavaThreads this will block GC
...
void LowMemoryDetector::detect_low_memory(MemoryPool* pool) {
  ...
  {
MutexLocker ml(LowMemory_lock,Thread::current());
...
void SensorInfo::oops_do(OopClosure* f) {
  f->do_oop((objectRef*) &_sensor_obj);
}
...
MutexLocker ml(LowMemory_lock,THREAD);
    _sensor_on = true;
    _sensor_count += count;
...
if(sensorobj!=NULL){
    klassOop k = Management::sun_management_Sensor_klass(CHECK);
    instanceKlassHandle sensorKlass (THREAD, k);
Handle sensor(THREAD,sensorobj);
...
tty->print_cr("%s count = %ld pending_triggers = %d pending_clears = %d",
                (_sensor_on ? "on" : "off"),
