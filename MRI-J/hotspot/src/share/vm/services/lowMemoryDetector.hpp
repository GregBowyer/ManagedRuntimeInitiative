...
#ifndef LOWMEMORYDETECTOR_HPP
#define LOWMEMORYDETECTOR_HPP
...
public:
  ...
  instanceOop sensor_obj()         { return lvb_instanceRef(&_sensor_obj).as_instanceOop(); }

  void set_sensor(instanceOop sensor) {
assert(_sensor_obj.is_null(),"Should be set only once");
    ref_store_without_check(&_sensor_obj, instanceRef(sensor));
  }
...
static volatile intptr_t _disabled_count;
...
static void disable(){Atomic::inc_ptr(&_disabled_count);}
static void enable(){Atomic::dec_ptr(&_disabled_count);}
