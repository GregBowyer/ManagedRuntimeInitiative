...
klassRef Management::_sensor_klass = klassRef((const uint64_t) NULL);
klassRef Management::_threadInfo_klass = klassRef((const uint64_t) NULL);
klassRef Management::_memoryUsage_klass = klassRef((const uint64_t) NULL);
klassRef Management::_memoryPoolMXBean_klass = klassRef((const uint64_t) NULL);
klassRef Management::_memoryManagerMXBean_klass = klassRef((const uint64_t) NULL);
klassRef Management::_garbageCollectorMXBean_klass = klassRef((const uint64_t) NULL);
klassRef Management::_managementFactory_klass = klassRef((const uint64_t) NULL);
...
void Management::oops_do(OopClosure* f) {
  ...
  f->do_oop((objectRef*) &_sensor_klass);
  f->do_oop((objectRef*) &_threadInfo_klass);
  f->do_oop((objectRef*) &_memoryUsage_klass);
  f->do_oop((objectRef*) &_memoryPoolMXBean_klass);
  f->do_oop((objectRef*) &_memoryManagerMXBean_klass);
  f->do_oop((objectRef*) &_garbageCollectorMXBean_klass);
  f->do_oop((objectRef*) &_managementFactory_klass);
}

klassOop Management::java_lang_management_ThreadInfo_klass(TRAPS) {
if(_threadInfo_klass.is_null()){
klassRef tmp_ref=(klassRef)load_and_initialize_klass(vmSymbolHandles::java_lang_management_ThreadInfo(),CHECK_NULL);
    POISON_AND_STORE_REF(&_threadInfo_klass, tmp_ref);
  }
  return check_klass(lvb_klassRef(&_threadInfo_klass));
}

klassOop Management::java_lang_management_MemoryUsage_klass(TRAPS) {
if(_memoryUsage_klass.is_null()){
klassRef tmp_ref=(klassRef)load_and_initialize_klass(vmSymbolHandles::java_lang_management_MemoryUsage(),CHECK_NULL);
    POISON_AND_STORE_REF(&_memoryUsage_klass, tmp_ref); 
  }
  return check_klass(lvb_klassRef(&_memoryUsage_klass));
}

klassOop Management::java_lang_management_MemoryPoolMXBean_klass(TRAPS) {
if(_memoryPoolMXBean_klass.is_null()){
klassRef tmp_ref=(klassRef)load_and_initialize_klass(vmSymbolHandles::java_lang_management_MemoryPoolMXBean(),CHECK_0);
    POISON_AND_STORE_REF(&_memoryPoolMXBean_klass, tmp_ref); 
  }
return check_klass(lvb_klassRef(&_memoryPoolMXBean_klass));
}

klassOop Management::java_lang_management_MemoryManagerMXBean_klass(TRAPS) {
if(_memoryManagerMXBean_klass.is_null()){
klassRef tmp_ref=(klassRef)load_and_initialize_klass(vmSymbolHandles::java_lang_management_MemoryManagerMXBean(),CHECK_0);
    POISON_AND_STORE_REF(&_memoryManagerMXBean_klass, tmp_ref); 
  }
return check_klass(lvb_klassRef(&_memoryManagerMXBean_klass));
}

klassOop Management::java_lang_management_GarbageCollectorMXBean_klass(TRAPS) {
if(_garbageCollectorMXBean_klass.is_null()){
klassRef tmp_ref=(klassRef)load_and_initialize_klass(vmSymbolHandles::java_lang_management_GarbageCollectorMXBean(),CHECK_NULL);
      POISON_AND_STORE_REF(&_garbageCollectorMXBean_klass, tmp_ref); 
  }
return check_klass(lvb_klassRef(&_garbageCollectorMXBean_klass));
}

klassOop Management::sun_management_Sensor_klass(TRAPS) {
if(_sensor_klass.is_null()){
klassRef tmp_ref=(klassRef)load_and_initialize_klass(vmSymbolHandles::sun_management_Sensor(),CHECK_NULL);
    POISON_AND_STORE_REF(&_sensor_klass, tmp_ref); 
  }
  return check_klass(lvb_klassRef(&_sensor_klass));
}

klassOop Management::sun_management_ManagementFactory_klass(TRAPS) {
if(_managementFactory_klass.is_null()){
klassRef tmp_ref=(klassRef)load_and_initialize_klass(vmSymbolHandles::sun_management_ManagementFactory(),CHECK_NULL);
    POISON_AND_STORE_REF(&_managementFactory_klass, tmp_ref); 
  }
return check_klass(lvb_klassRef(&_managementFactory_klass));
}
...
Handle element=ik->allocate_instance_handle(true/*SBA*/,CHECK_NULL);
...
objArrayOop r=oopFactory::new_objArray(ik(),num_args+num_flags,true/*SBA*/,CHECK_NULL);
...
JVM_ENTRY(jobjectArray,jmm_GetMemoryPools,(JNIEnv*env,jobject obj))
  ...
  instanceKlassHandle ik (THREAD, k);
objArrayOop r=oopFactory::new_objArray(ik(),num_memory_pools,true/*SBA*/,CHECK_NULL);
...
JVM_ENTRY(jobjectArray,jmm_GetMemoryManagers,(JNIEnv*env,jobject obj))
  ...
  instanceKlassHandle ik (THREAD, k);
objArrayOop r=oopFactory::new_objArray(ik(),num_mgrs,true/*SBA*/,CHECK_NULL);
...
JVM_ENTRY(jobject,jmm_GetMemoryUsage,(JNIEnv*env,jboolean heap))
  ...
  if (heap) {
total_used=Universe::heap()->used();
total_committed=Universe::heap()->max_capacity();
	  total_max = MAX3(total_used, total_committed, Universe::heap()->capacity());
  }
...
if(ta!=NULL){
  ...
  klassOop k = Management::java_lang_management_ThreadInfo_klass(CHECK_NULL);
  instanceKlassHandle ik (THREAD, k);
objArrayOop r=oopFactory::new_objArray(ik(),num_snapshots,true/*SBA*/,CHECK_NULL);
  objArrayHandle result_h(THREAD, r);
...
if(ts->threadObj().is_null()){
    ...
    if (locked_monitors) {
      ...
      typeArrayOop tarray = oopFactory::new_typeArray(T_INT, num_locked_monitors, true/*SBA*/, CHECK_NULL);
      ...
      for (int depth = 0; depth < num_frames; depth++) {
        ...
        GrowableArray<objectRef>* locked_monitors = frame->locked_monitors();
        for (j = 0; j < len; j++) {
          objectRef monitor = locked_monitors->at(j);
assert(monitor.not_null()&&monitor.as_oop()->is_instance(),"must be a Java object");
monitors_array->ref_at_put(count,monitor);
          depths_array->int_at_put(count, depth);
          count++;
        }
      }

      GrowableArray<objectRef>* jni_locked_monitors = stacktrace->jni_locked_monitors();
      for (j = 0; j < jni_locked_monitors->length(); j++) {
        objectRef object = jni_locked_monitors->at(j);
assert(object.not_null()&&object.as_oop()->is_instance(),"must be a Java object");
monitors_array->ref_at_put(count,object);
...
assert(ts->threadObj().not_null(),"Must be a valid JavaThread");
...
objArrayOop r=oopFactory::new_objArray(SystemDictionary::class_klass(),num_classes,true/*SBA*/,CHECK_0);
...
nFlags,true/*SBA*/,CHECK_0);
  ...
  for (int i = 0; i < nFlags; i++) {
    ...
    // Exclude the diagnostic flags
    if (flag->is_unlocked() || flag->is_unlocker()) {
Handle s=java_lang_String::create_from_str(flag->name,true/*SBA*/,CHECK_0);
...
objArrayOop res=oopFactory::new_objArray(SystemDictionary::string_klass(),num_entries,true/*SBA*/,CHECK_0);
...
void add_global_entry(JNIEnv* env, Handle name, jmmVMGlobal *global, Flag *flag, TRAPS) {
  Handle flag_name;
  if (name() == NULL) {
flag_name=java_lang_String::create_from_str(flag->name,true/*SBA*/,CHECK);
  } else {
    flag_name = name;
  }
...
Handle str=java_lang_String::create_from_str(flag->get_ccstr(),true/*SBA*/,CHECK);
...
s=java_lang_String::create_from_str(thread->name(),true/*SBA*/,CHECK);
...
static Handle find_deadlocks(bool object_monitors_only, TRAPS) {
  ...
  objArrayOop r = oopFactory::new_objArray(SystemDictionary::thread_klass(), num_threads, true/*SBA*/, CHECK_NH);
