...
GCMemoryManager*MemoryManager::get_pgcScavenge_memory_manager(){
return(GCMemoryManager*)new PGCScavengeMemoryManager();
}

GCMemoryManager*MemoryManager::get_pgcFull_memory_manager(){
return(GCMemoryManager*)new PGCFullMemoryManager();
}

GCMemoryManager*MemoryManager::get_gpgc_newgc_memory_manager(){
return(GCMemoryManager*)new GPGCNewGCMemoryManager();
}

GCMemoryManager*MemoryManager::get_gpgc_oldgc_memory_manager(){
return(GCMemoryManager*)new GPGCOldGCMemoryManager();
}
...
objectRef mgr_ref=ALWAYS_UNPOISON_OBJECTREF((uint64_t)OrderAccess::load_ptr_acquire(&_memory_mgr_obj));
if(mgr_ref.is_null()){
    ...
    Handle mgr_name = java_lang_String::create_from_str(name(), true/* SBA*/, CHECK_0);
    ...
    instanceRef m = *(instanceRef*) result.get_value_addr();
Handle mgr(THREAD,m);
    StackBasedAllocation::ensure_in_heap((JavaThread*)THREAD,mgr,"MemoryManager::get_memory_manager_instance");

    {
      // Get lock before setting _memory_mgr_obj
      // since another thread may have created the instance
      MutexLockerAllowGC ml(Management_lock,(JavaThread*)THREAD);
...
mgr_ref=ALWAYS_UNPOISON_OBJECTREF((uint64_t)OrderAccess::load_ptr_acquire(&_memory_mgr_obj));
if(mgr_ref.not_null()){
         return lvb_instanceRef((instanceRef*)&mgr_ref).as_instanceOop();
      }
...
OrderAccess::release_store_ptr((volatile intptr_t*)&_memory_mgr_obj,ALWAYS_POISON_OBJECTREF(mgr_ref).raw_value());
    }
  }
...
void MemoryManager::oops_do(OopClosure* f) {
  f->do_oop((objectRef*) &_memory_mgr_obj);
}
