...
objectRef pool_ref=ALWAYS_UNPOISON_OBJECTREF((uint64_t)OrderAccess::load_ptr_acquire(&_memory_pool_obj));
if(pool_ref.is_null()){
    ...
    Handle pool_name = java_lang_String::create_from_str(_name, true/*SBA*/, CHECK_NULL);
    ...
    instanceRef p = *(instanceRef*) result.get_value_addr();
Handle pool(THREAD,p);
    StackBasedAllocation::ensure_in_heap((JavaThread*)THREAD,pool,"MemoryPool::get_memory_pool_instance");
...
pool_ref=ALWAYS_UNPOISON_OBJECTREF((uint64_t)OrderAccess::load_ptr_acquire(&_memory_pool_obj));
if(pool_ref.not_null()){
         return lvb_instanceRef((instanceRef*)&pool_ref).as_instanceOop();
      }

      // Get the address of the object we created via call_special.
      pool_ref = pool.as_ref();
...
OrderAccess::release_store_ptr((volatile intptr_t*)&_memory_pool_obj,ALWAYS_POISON_OBJECTREF(pool_ref).raw_value());
    }
  }
...
void MemoryPool::oops_do(OopClosure* f) {
  f->do_oop((objectRef*) &_memory_pool_obj);
...
MemoryUsage CodeCachePool::get_memory_usage(){
  size_t used      = used_in_bytes();
  size_t committed = CodeCache::max_capacity_in_bytes();
  size_t maxSize   = (available_for_allocation() ? max_size() : 0);

  return MemoryUsage(initial_size(), used, committed, maxSize);
}

GenPauselessHeapPool::GenPauselessHeapPool(GPGC_Generation* gen, 
                                           const char* name,
                                           PoolType type, 
                                           size_t max_size,
                                           bool support_usage_threshold) :
CollectedMemoryPool(name,type,gen->capacity(),max_size,
                      support_usage_threshold), _gen(gen) {
}

MemoryUsage GenPauselessHeapPool::get_memory_usage(){
  size_t maxSize   = (available_for_allocation() ? max_size() : 0);
  size_t used      = used_in_bytes();
size_t committed=_gen->capacity();
