...
volatile intptr_t ThreadService::_exiting_threads_count=0;
volatile intptr_t ThreadService::_exiting_daemon_threads_count=0;
...
void ThreadService::remove_thread(JavaThread* thread, bool daemon) {
  ...
  Atomic::dec_ptr(&_exiting_threads_count);
...
Atomic::dec_ptr(&_exiting_daemon_threads_count);
  }
}
...
Atomic::inc_ptr(&_exiting_threads_count);

  oop threadObj = jt->threadObj();
  if (threadObj != NULL && java_lang_Thread::is_daemon(threadObj)) {
Atomic::inc_ptr(&_exiting_daemon_threads_count);
  }
}
...
MutexLockerAllowGC ml(Management_lock,JavaThread::current());
...
objArrayOop r=oopFactory::new_objArray(ik(),num_threads,true/*SBA*/,CHECK_NH);
...
StackFrameInfo::StackFrameInfo(vframe vf, bool with_lock_info, JavaThread* thread) {
  POISON_AND_STORE_REF( &_method, methodRef(vf.method()) );
  _bci = vf.bci();
  _locked_monitors = NULL;
  if (with_lock_info) {
    ResourceMark rm;
    Unimplemented();
//    GrowableArray<MonitorInfo*>* list = vf.locked_monitors(thread);
//    int length = list->length();
//    if (length > 0) {
//      _locked_monitors = new (ResourceObj::C_HEAP) GrowableArray<objectRef>(length, true);
...
f->do_oop(&_method);
  if (_locked_monitors != NULL) {
    ...
    for (int i = 0; i < length; i++) {
f->do_oop(_locked_monitors->adr_at(i));
    }
  }
}
...
void StackFrameInfo::print_on(outputStream* st) const {
  ...
  for (int i = 0; i < len; i++) {
    objectRef o = _locked_monitors->at(i);
instanceKlass*ik=instanceKlass::cast(o.as_oop()->klass());
st->print_cr("\t- locked <"INTPTR_FORMAT"> (a %s)",o.raw_value(),ik->external_name());
  }
...
ThreadStackTrace::ThreadStackTrace(JavaThread* t, bool with_locked_monitors) {
  ...
  if (_with_locked_monitors) {
    _jni_locked_monitors = new (ResourceObj::C_HEAP) GrowableArray<objectRef>(INITIAL_ARRAY_SIZE, true);
  } else {
...
void ThreadStackTrace::dump_stack_at_safepoint(int maxDepth) {
  ...
  int count = 0;
  for (vframe vf = _thread->last_java_vframe(); !vf.done(); vf.next()) {
    add_stack_frame(vf, _thread);
    count++;
...
objArrayOop ste=oopFactory::new_objArray(ik(),_depth,true/*SBA*/,CHECK_NH);
  objArrayHandle backtrace(THREAD, ste);
  for (int j = 0; j < _depth; j++) {
    StackFrameInfo* frame = _frames->at(j);
    methodHandle mh(THREAD, frame->method());
    objectRef element = java_lang_StackTraceElement::create(mh, frame->bci(), CHECK_NH);
    objArrayOopDesc::ref_at_put(backtrace.as_ref(),j, element);
  }
  return backtrace;
}

void ThreadStackTrace::add_stack_frame(vframe vf, JavaThread* thread) {
  StackFrameInfo* frame = new StackFrameInfo(vf, _with_locked_monitors, thread);
...
f->do_oop(_jni_locked_monitors->adr_at(j));
  }
}
...
st->print_cr("\t- <"PTR_FORMAT"> (a %s)",(address)obj,ik->external_name());
  }
  st->cr();
}
...
void ThreadConcurrentLocks::oops_do(OopClosure* f) {
  ...
  for (int i = 0; i < length; i++) {
    f->do_oop((objectRef*) _owned_locks->adr_at(i));
  }
}
...
ThreadSnapshot::ThreadSnapshot(JavaThread* thread) {
  _thread = thread;
POISON_AND_STORE_REF(&_threadObj,thread->threadObj());
  _stack_trace = NULL;
...
_blocker_object=nullRef;
_blocker_object_owner=nullRef;

  _thread_status = java_lang_Thread::get_thread_status(
                   thread->threadObj());
  _is_ext_suspended = thread->is_being_ext_suspended();

  // not blocked but at safepoint?  must be in native
  _is_in_native = (thread != Thread::current() && !thread->is_hint_blocked()) ? true : false;
...
POISON_AND_STORE_REF(&_blocker_object,obj());
JavaThread*owner=obj->lock_owner();
...
POISON_AND_STORE_REF(&_blocker_object_owner,owner->threadObj());
      }
    }
  }
  ...
  if (JDK_Version::supports_thread_park_blocker() &&
    ...
    POISON_AND_STORE_REF(&_blocker_object, thread->current_park_blocker());
    if (_blocker_object.not_null() && thread->current_park_blocker()->is_a(SystemDictionary::abstract_ownable_synchronizer_klass())) {
      POISON_AND_STORE_REF(&_blocker_object_owner,
                           java_util_concurrent_locks_AbstractOwnableSynchronizer::get_owner_threadObj(thread->current_park_blocker()));
    }
  }
}
...
void DeadlockCycle::print_on(outputStream* st) const {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (waitingToLockMonitor != NULL) {
st->print("  waiting to lock monitor "PTR_FORMAT,waitingToLockMonitor);
      oop obj = (oop)waitingToLockMonitor->object();
      if (obj != NULL) {
st->print(" (object "PTR_FORMAT", a %s)",(address)obj,
...
st->print("  waiting for ownable synchronizer "PTR_FORMAT", (a %s)",
