...
static volatile intptr_t _exiting_threads_count;
static volatile intptr_t _exiting_daemon_threads_count;
...
public:
  ...
  static intptr_t   exiting_threads_count()        { return _exiting_threads_count; }
  static intptr_t   exiting_daemon_threads_count() { return _exiting_daemon_threads_count; }
...
private:
  ...
  jlong   _sleep_ticks;
  jlong   _sleep_count;
  objectRef _blocker_object;     
  objectRef _blocker_object_owner;
...
public:
  // Dummy snapshot 
ThreadSnapshot():_thread(NULL),_threadObj(nullRef),_thread_status(java_lang_Thread::NEW),_stack_trace(NULL),_concurrent_locks(NULL),_next(NULL),
_blocker_object(nullRef),_blocker_object_owner(nullRef){};
  ThreadSnapshot(JavaThread* thread);
  ~ThreadSnapshot();

  java_lang_Thread::ThreadStatus thread_status() { return _thread_status; }

  objectRef   threadObj() const           { return lvb_ref(&_threadObj); }
  ...
  objectRef   blocker_object()            { return lvb_ref(&_blocker_object); }
  objectRef   blocker_object_owner()      { return lvb_ref(&_blocker_object_owner); }
...
class ThreadStackTrace : public CHeapObj {
 private:
  ...
  GrowableArray<objectRef>*       _jni_locked_monitors;
 ...
 public:
  ...
  void            add_stack_frame(vframe vf, JavaThread* thread);
  void            dump_stack_at_safepoint(int max_depth);
  Handle          allocate_fill_stack_trace_element_array(TRAPS);
  void            oops_do(OopClosure* f);
  GrowableArray<objectRef>* jni_locked_monitors() { return _jni_locked_monitors; }
...
class StackFrameInfo : public CHeapObj {
 private:
  methodRef           _method;
  int                 _bci;
  GrowableArray<objectRef>* _locked_monitors; // list of object monitors locked by this frame

 public:

  StackFrameInfo(vframe vf, bool with_locked_monitors, JavaThread* thread);
  ~StackFrameInfo() {
    if (_locked_monitors != NULL) {
      delete _locked_monitors;
    }
  };
  methodOop method() const       { return lvb_methodRef((methodRef*)&_method).as_methodOop(); }
  int       bci()    const       { return _bci; }
  void      oops_do(OopClosure* f);

  int       num_locked_monitors()       { return (_locked_monitors != NULL ? _locked_monitors->length() : 0); }
  GrowableArray<objectRef>* locked_monitors() { return _locked_monitors; }
...
class JavaThreadStatusChanger : public StackObj {
 private:
  ...
  void save_old_state(JavaThread* java_thread) {
    _old_state=java_lang_Thread::NEW;
