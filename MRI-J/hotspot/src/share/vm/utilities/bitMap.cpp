...
BitMap::idx_t
BitMap::inverted_bit_mask_for_range(idx_t beg,idx_t end){
...
bool BitMap::is_same(BitMap other) const {
  assert(size() == other.size(), "must have same size");
  idx_t* dest_map = map();
  idx_t* other_map = other.map();
  idx_t size = size_in_words();
...
void BitMap::iterate(BitMapClosure*blk,idx_t leftOffset,idx_t rightOffset)const{
