...
class BitMap VALUE_OBJ_CLASS_SPEC {
 ...
 public:
  ...
  // Return a mask that sets all bits in a word not in the specified range.
  static idx_t inverted_bit_mask_for_range(idx_t beg, idx_t end);
 ...
 public:
  ...
  // Iteration support
void iterate(BitMapClosure*blk,idx_t leftIndex,idx_t rightIndex)const;
inline void iterate(BitMapClosure*blk)const{
  ...
  bool is_same(BitMap bits) const;
