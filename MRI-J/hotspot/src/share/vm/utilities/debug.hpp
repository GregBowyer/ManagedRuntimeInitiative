...
//#define USE_REPEATED_ASSERTS
  ...
  #define assert(p,msg)                                              \
    { for (int __i = 0; __i < AssertRepeat; __i++) {                 \
        if (!(p)) {                                                  \
if(report_assertion_failure(__FILE__,__LINE__,\
"assert("XSTR(p)",\""msg"\")")){\
            BREAKPOINT;                                              \
          }                                                          \
        }                                                            \
...
#endif
#define Untested(msg) { report_assertion_failure(__FILE__, __LINE__, "Untested " msg); BREAKPOINT; }
...
#define guarantee1(b,msg,x1)     { if (!(b)) fatal1(msg,x1); }
...
void pd_ps(frame f);
void pd_psx(intptr_t*sp, address pc, JavaThread *thread);


class Debug {
 public:
  static void find(intptr_t x, bool print_pc, outputStream* st);
  static void examine(char* command, address addr, outputStream* st);
  static void dump_stack(Thread* t, outputStream* st);
};
