...
void* ThreadLocalObj::operator new(size_t size, void *where) {
  return where;
}

void* ThreadLocalObj::operator new(size_t size, ttype::ThreadType thr_type) {
  guarantee(size < USER_THREAD_SPECIFIC_DATA_SIZE, "need to make more space for Thread or its subclasses");
  // We need a new thread. Ask the OS to allocate a stack for this thread and use that
  // zero => default
  address a = os::create_thread_stack(thr_type, 0);
  return (void *)a;
}

void ThreadLocalObj::operator delete(void*p){
  /* No-op */
};
...
void ThreadShadow::set_pending_exception(oop exception, const char* file, int line) {
  ...
  // Java threads only test the pending exception if the polling
  // advisory bit is set.
  if( _is_Complete_Java_thread ) 
    ((JavaThread*)this)->set_jvm_lock_poll_advisory();
}

void ThreadShadow::clear_pending_exception() {
  if (TraceClearedExceptions) {
    oop pending = pending_exception();
if(pending!=NULL){
      tty->print_cr("Thread::clear_pending_exception: cleared exception:");
pending->print(tty);
    }
  }
_pending_exception=nullRef;
  _exception_file    = NULL;
  _exception_line    = 0;
}

// Pending exceptions should be handled in the transition from the VM
// (where they are set) to Java code.  Java code should handle them
// right away.  Re-entering the VM with pending_exception set means
// that Java code failed to handle it when it got set.
bool ThreadShadow::unhandled_pending_exception() {
  ResourceMark rm;
  ResetNoHandleMark rnhm;
  HandleMark   hm;
  oop ex = pending_exception();
  clear_pending_exception();	// Avoid infinite recursion
ex->print(tty);
  set_pending_exception(ex,0,0);
  return false;			// Cause assert to fail
}
...
if(thread->is_VM_thread()||thread->is_Compiler_thread()){
    // We do not care what kind of exception we get for the vm-thread or a thread which
    // is compiling.  We just install a dummy exception object
    thread->set_pending_exception(Universe::vm_exception(), file, line);
    return true;
...
assert(h_exception.not_null(),"exception should not be NULL");

  // tracing (do this up front - so it works during boot strapping)
  if (TraceExceptions) {
    ttyLocker ttyl;
    ResourceMark rm;
tty->print_cr("Exception <%s>%s%s ("PTR_FORMAT" ) \n"
"thrown [%s, line %d]\nfor thread "PTR_FORMAT,
  ...
  // vm log
  Log::log4(NOTAG, Log::M_EXCEPTION | Log::L_LO, "throw_exception INTPTR_FORMAT", h_exception());
}
...
oop e=instanceKlass::cast(k)->allocate_instance(true/*in-stack SBA*/,CHECK);
...
Handle Exceptions::new_exception(Thread *thread, symbolHandle h_name, 
  ...
  if (!thread->has_pending_exception()) {    
    ...
    if (!thread->has_pending_exception()) {      
      ...
      bool in_stack = thread->is_Java_thread() && !((JavaThread*)thread)->root_Java_frame();
      h_exception = klass->allocate_instance_handle(in_stack,thread);
      if (!thread->has_pending_exception()) {      
...
Handle Exceptions::new_exception(Thread* thread, symbolHandle h_name, 
  ...
  if (message == NULL) {
    ...
    if (to_utf8_safe == safe_to_utf8) {
      ...
      bool in_stack = thread->is_Java_thread() && !((JavaThread*)thread)->root_Java_frame();
      msg = java_lang_String::create_from_str(message, in_stack, thread);
    } else {
