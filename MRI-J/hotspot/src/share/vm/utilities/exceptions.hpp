...
class ttype {                   // class just for namespace control
public:
  enum ThreadType {
    vm_thread,
    java_thread,
...
#if defined(AZUL)
#define THREADSHADOW_SUPER_CLASS ThreadLocalObj
// Base class for objects allocated into existing memory, such as Thread, JavaThread.
// Calling delete will result in fatal error.
// new can either be used with a placement address, and the object will live "inline" there,
// or with no specified address, the os will be asked to allocate a thread stack.
class ThreadLocalObj ALLOCATION_SUPER_CLASS_SPEC{
public:
  void* operator new(size_t size) { ShouldNotReachHere(); return (void*)1; }
  void* operator new(size_t size, void *where);
  void* operator new(size_t size, ttype::ThreadType thr_type);
  void  operator delete(void* p);
};

#else
  #define THREADSHADOW_SUPER_CLASS CHeapObj
#endif // defined(AZUL)

class ThreadShadow:public THREADSHADOW_SUPER_CLASS{
public:
objectRef _pending_exception;//overlays Thread::_pending_exception
  // --- LIBOS FIELDS --- DO NOT TOUCH OR REORDER ---
  u8 _libos_eva, _libos_epc;     // Crash-n-burn SIGSEGV fault address and error PC
  u4 _thread_create_millis;      // Thread-create time.  Here for all threads for ARTA
  // Bits used to control stack extensions.  i.e., these can change during a
  // tick/interrupt/signal which triggers a SEGV which extends the stack.
  u2 _jex_stk_ext_cnt;           // Number of stack extensions for the interpreter expression stack
  u2 _usr_stk_ext_cnt;           // Number of stack extensions for the user/C/C++ stack
  // --- TYPE-STABLE BITS: Always zero for all threads except for HotSpot managed threads
  u1 _is_Complete_Java_thread:1, // i.e., after construction, but before destruction
    _is_Complete_Thread:1;       // If the rest of the Thread* is initialized
  // for nio_protection_fault, contains access type code 
  u1 _libos_nio_sigsegv_access_type;  // 0: exec, 1: store, 2: load
  // --- LIBOS FIELDS --- DO NOT TOUCH OR REORDER ---
  u1 _libos01, _libos02, _libos03, _libos04, _libos05, _libos06;
protected:
  // --- Back to your usually expected bits....

  jint _please_self_suspend;     // see comments in JavaThread
 ...
 public:
  bool is_Complete_Java_thread() const           { return _is_Complete_Java_thread; }
  bool is_Complete_Thread() const                { return _is_Complete_Thread; }

  oop  pending_exception() const                 { assert0(_pending_exception.is_in_a_stack());
                                                   return _pending_exception.as_oop(); }
  objectRef pending_exception_ref() const        { assert0(_pending_exception.is_in_a_stack());
                                                   return _pending_exception; }
  bool has_pending_exception() const             { return _pending_exception.not_null(); }
  ...
  void set_pending_exception_ref(objectRef exception) { _pending_exception = exception; }

  // use CLEAR_PENDING_EXCEPTION whenever possible!
  void clear_pending_exception();

  bool unhandled_pending_exception();

  ThreadShadow() : _pending_exception(nullRef), _exception_file(NULL), 
		   _exception_line(0), _please_self_suspend(0) {}
};
...
#define PENDING_EXCEPTION_REF (((ThreadShadow*)THREAD)->pending_exception_ref())
