...
"thread %#llx sleeps after %d yields",
           (long long unsigned int)(Thread::current()), yield_count);
        }
        yield_count = 0;
...
gclog_or_tty->print_cr("CTQ: save "PTR_FORMAT,(void*)t);
  }
...
gclog_or_tty->print_cr("  CTQ: retrieve "PTR_FORMAT,(void*)result);
  }
  return result;
}
...
bool ChunkTaskQueueWithOverflow::retrieve_from_stealable_queue(
  ...
  if (TraceChunkTasksQueuing && Verbose) {
gclog_or_tty->print_cr("CTQ: retrieve_stealable "PTR_FORMAT,(void*)chunk_task);
  }
  return result;
}
