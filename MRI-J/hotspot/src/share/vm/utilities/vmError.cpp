...
if(_id!=(int)0xE0000000){
signame=os::exception_name(_id,signame_buf,sizeof(signame_buf));
}

  if (signame) {
    jio_snprintf(buf, buflen,
                 "%s (0x%x) at pc=" PTR_FORMAT ", pid=" INT32_FORMAT ", tid=" PTR_FORMAT, 
                 signame, _id, _pc,
    ...
    if (_filename != NULL && _lineno > 0) {
      ...
      jio_snprintf(buf, buflen,
"Internal Error at %s:%d, pid="INT32_FORMAT", tid="PTR_FORMAT" \nError: %s",
        p ? p + 1 : _filename, _lineno, 
        os::current_process_id(), os::current_thread_id(),
        _message ? _message : "");
    } else {
      jio_snprintf(buf, buflen,
"Internal Error (0x%x), pid="INT32_FORMAT", tid="PTR_FORMAT,
...
void VMError::report(staticBufferStream*st){
...
# define END }
  ...
  bool stack_trace_printed = false;
...
if(_id!=(int)0xE0000000){
os::exception_name(_id,buf,sizeof(buf));
       st->print("%s", buf);
...
Abstract_VM_Version::vm_info_string()
  ...
  STEP(60, "(printing problematic frame)")
     ...
     if (_id != (int)0xE0000000) {
       ...
       st->print("# ");
frame fr=os::fetch_frame_from_context(_pc);
...
if(_thread->is_Complete_Thread()){
  ...
  STEP(90, "(printing siginfo)" )
     ...
     if (_verbose && _id != (int)0xE0000000) {
  ...
  STEP(110, "(printing stack bounds)" )
     ...
     if (_verbose) {
       ...
       address stack_top = (address)_thread + thread_stack_size-1;
       size_t stack_size = thread_stack_size;

       address stack_bottom = stack_top - stack_size;
       st->print("[" PTR_FORMAT "," PTR_FORMAT "]", stack_bottom, stack_top);

       frame fr = _id != (int)0xE0000000 ? os::fetch_frame_from_context(_pc) : os::current_frame();

       if (fr.sp()) {
         st->print(",  sp=" PTR_FORMAT, fr.sp());
st->print(",  free space=%ldk",
...
frame fr=_id!=(int)0xE0000000?os::fetch_frame_from_context(_pc):os::current_frame();
       ...
       if (fr.pc()) {
st->print_cr("Native frames: (J Jn Jo=compiled Java code, j=interpreted, Vv=VM code, C=native code)");
  ...
  STEP(130, "(printing Java stack)" )
     ...
     if (_verbose && !stack_trace_printed && _thread && _thread->is_Complete_Java_thread()) {
       JavaThread* jt = (JavaThread*)_thread;
       if (!jt->root_Java_frame()) {
st->print_cr("Java frames: (J Jn Jo=compiled Java code, j=interpreted, Vv=VM code)");
  ...
  STEP(140, "(printing VM operation)" )
     ...
     if (_verbose && _thread && _thread->is_Complete_Thread() && _thread->is_VM_thread()) {
  ...
  STEP(150, "(printing current compile task)" )
     ...
     if (_verbose && _thread && _thread->is_Complete_Thread() && _thread->is_Compiler_thread()) {
        ResourceMark rm;
        CompilerThread* t = (CompilerThread*)_thread;
        const char* status_msg = t->status_msg();
        if (status_msg) {
          st->print_cr("Current CompileTask:");
st->print_cr(status_msg);
          st->cr();
        }
     }
...
if(show_message_box(buffer,sizeof(buffer))){
//User typed yes, was attached in the debugger (executed BREAKPOINT)
        // and returned to here consciously to continue execution. This is needed
        // to make forward progress after Untested.
first_error=NULL;
        return;
      }
...
"[thread "INTPTR_FORMAT" also had an error]",
                   mytid);
  ...
  if (!log_done) {
    ...
    if (!log.is_open()) {
      ...
      if ((ErrorFile != NULL) && (strlen(ErrorFile) > 0)) {
        bool copy_ok = 
