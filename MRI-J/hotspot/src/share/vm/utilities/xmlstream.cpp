...
if(method->has_bci2cpd_map()){
print(" has_bci2cpd_map");
    // Don't acquire the OsrList_lock, since its order is higher than tty_lock,
    // which has been acquired. Since we don't care the details of the code 
    // profile here, have a lock-less version of has_codeprofile should be good
    // enough.
    if( method->has_codeprofile() ) print(" and_profile");
  }
}
