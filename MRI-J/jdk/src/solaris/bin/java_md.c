...
typedef int (*launch_avm_t)(void* javamain, void* args, void* vmargs);
launch_avm_t launch_avm_fn = NULL;
...
CreateExecutionEnvironment(int *_argcp,
                           ...
                           char **original_argv) {
#ifdef AZ_LINUX
    /* No need to re-exec java on AZ_LINUX. Just fill in jrepath and jvmpath */
    char* execname = SetExecname(*_argvp);
...
#ifdef AZ_LINUX
    launch_avm_fn = (launch_avm_t) dlsym(libjvm, "LaunchAvm");
    if (launch_avm_fn == NULL)
      goto error;
#endif /* AZ_LINUX */
...
#ifdef AZ_LINUX
int
ContinueInNewThread(int (JNICALL *continuation)(void *), jlong stack_size, void * args, void * vmargs, int ret) {
    int rslt = (*launch_avm_fn)((void*) continuation, args, vmargs);
