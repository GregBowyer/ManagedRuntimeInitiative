...
#ifdef   AZ_PROXIED
/*
 * AZUL NOTE - Most of this implementation is proxied
 * New functions in 1.5 - ping4 and isReachable0 unimplemented
 */
#endif /* AZ_PROXIED */
...
#endif
#else  /* AZ_PROXIED */
#define HENT_ARRAY_SIZE 32                        /* the initial size of our hostent buffers */
#define BIG_HENT_ARRAY_SIZE 1024                  /* a jumbo-sized one */
#define HENT_BUF_SIZE 32                          /* the initial size of our hostent buffers */
#define BIG_HENT_BUF_SIZE 1024                	  /* a jumbo-sized one */
#endif /* AZ_PROXIED */
...
#else  /* AZ_PROXIED */
    if (proxy_gethostname(hostname, MAXHOSTNAMELEN) != PROXY_ERROR_NONE) {
    strcpy(hostname, "localhost");            /* Something went wrong, maybe networking not set up? */
#endif /* AZ_PROXIED */
    }
...
Java_java_net_Inet4AddressImpl_lookupAllHostAddr(JNIEnv *env, jobject this,
    ...
    int h_error=0;
#else  /* AZ_PROXIED */
    proxy_in_addr_t address[HENT_ARRAY_SIZE];
    uint32_t address_count = HENT_ARRAY_SIZE;     // Number of addresses returned; on input = available size
    char *tmp = NULL;                             // Temporary buffer in case the small one is not enough
    proxy_in_addr_t *addressP = NULL;             // Pointer to in address 
    proxy_error_t h_error = 0;
#endif /* AZ_PROXIED */
...
#else  /* AZ_PROXIED */
    addressP = address;                           // Use the smaller buffer first
#endif /* AZ_PROXIED */
...
#endif
#else  /* AZ_PROXIED */
    h_error = proxy_gethostbyname(hostname, addressP, &address_count)
#endif /* AZ_PROXIED */
...
#else  /* AZ_PROXIED */
    if (h_error == AZPR_ERANGE) {
        if ((tmp = (char *)malloc(BIG_HENT_ARRAY_SIZE * sizeof(proxy_in_addr_t))) != NULL) {
            addressP = (proxy_in_addr_t *)tmp;
            address_count = BIG_HENT_ARRAY_SIZE;
            h_error = proxy_gethostbyname(hostname, addressP, &address_count);
        } else {
    ...
    if (h_error == PROXY_ERROR_NONE) {
        unsigned int i = 0;

	ret = (*env)->NewObjectArray(env, address_count, ni_iacls, NULL);
	if (IS_NULL(ret)) {
	    /* we may have memory to free at the end of this */
	    goto cleanupAndReturn;
	}
        for (i = 0; i < address_count; i++) {
            ...
            if (addressP[i].family == PROXY_FAMILY_INET) {
                int len = PROXY_FAMILY_INET;
	  jobject iaObj = (*env)->NewObject(env, ni_ia4cls, ni_ia4ctrID);
	  if (IS_NULL(iaObj)) {
	    ret = NULL;
	    goto cleanupAndReturn;
...
#endif
#else  /* AZ_PROXIED */
    proxy_in_addr_t addr;
    (*env)->GetByteArrayRegion(env, addrArray, 0, 4, (jbyte *)addr.in_addr.data);
    addr.in_addr.ipv4 = htonl(addr.in_addr.ipv4);
    addr.family = htons(PROXY_FAMILY_INET);

    h_error = proxy_gethostbyaddr(&addr, buf, sizeof(buf));
#endif /* AZ_PROXIED */
    /* With the re-entrant system calls, it's possible that the buffer
     * we pass to it is not large enough to hold an exceptionally
     * large DNS entry.  This is signaled by errno->ERANGE.  We try once
...
#else  /* AZ_PROXIED */
    if (h_error == AZPR_ERANGE) {          /* } */
        if ((tmp = (char*)malloc(BIG_HENT_BUF_SIZE)) != NULL) {        /* } */
            hp = tmp;
            h_error = proxy_gethostbyaddr(&addr, tmp, BIG_HENT_BUF_SIZE);
#endif /* AZ_PROXIED */
        } else {
...
ping4(JNIEnv *env, jint fd, struct sockaddr_in* him, jint timeout,
    ...
    JNU_ThrowInternalError(env, "Unimplemented: ping4");
}
...
Java_java_net_Inet4AddressImpl_isReachable0(JNIEnv *env, jobject this,
					   ...
					   jbyteArray ifArray,
					   jint ttl) {
    JNU_ThrowInternalError(env, "Unimplemented: Java_java_net_Inet4AddressImpl_isReachable0");
}
#else /* !AZ_PROXIED */
