...
int getScopeID (struct sockaddr *him) {
    struct sockaddr_in6 *him6 = (struct sockaddr_in6 *)him;
    return him6->sin_scope_id;
}

int cmpScopeID (unsigned int scope, struct sockaddr *him) {
    struct sockaddr_in6 *him6 = (struct sockaddr_in6 *)him;
    return him6->sin_scope_id == scope;
}
#endif /* AZ_PROXIED */
...
{
    // AZUL FIXME: Stash this as a global named: IO_fd_fdID and add extern.
...
#else  /* AZ_PROXIED */
jint
IPv6_supported(int preferIPv4Stack) {
    jint ipv6_support;
    proxy_error_t rc = proxy_socketimpl_init(preferIPv4Stack, &ipv6_support);
    if (rc != PROXY_ERROR_NONE) {
	    fprintf(stderr, "java: IPv6_supported: proxy_socketimpl_init() returned error %d\n", rc);
	    exit(1);
    }
    return ipv6_support;
}
...
#else  /* AZ_PROXIED */
// Azul socket structures are IPv6-capable so we just create one of 
// those on our end.
void 
NET_AllocSockaddr(struct sockaddr **him, int *len) {
    *len = sizeof(struct sockaddr);
    *him = (struct sockaddr *)malloc(*len);
}
#endif /* AZ_PROXIED */
...
*/
#else  /* AZ_PROXIED */
// Translate from an InetAddress object to our socket address type.
// N.B. len is always 16 (bytes) == sizeof(struct sockaddr)
#endif /* AZ_PROXIED */
...
#ifdef    AZ_PROXIED
/*******************************************************************************/
/* If we ever build this proxied, the stuff below needs to happen on the proxy */
/*******************************************************************************/
#else  /* !AZ_PROXIED */
                    if (scope_id != 0) {
...
#endif /* AF_INET6 */
#else  /* AZ_PROXIED */
    // FIXME: Probably IPv6-implementation dependent
    // Should this be a mask in rather than assignment ?
    if (him->sa_family == AF_INET6) {
	      him->sin_flowinfo = htonl((trafficClass & 0xff) << 20);
    }
#endif /* AZ_PROXIED */
}
...
NET_Wait(JNIEnv *env, jint fd, jint flags, jint timeout)
{
    ...
    return timeout;
#else  /* AZ_PROXIED */
	JNU_ThrowInternalError(env, "Unimplemented: NET_Wait");
#endif /* AZ_PROXIED */
}
